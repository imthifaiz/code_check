...
<%
String title = "Bulk Kitting Dekitting with BOM";
%>
...
<div class="row">
  	<div class="col-12 col-sm-12"><INPUT Type=Checkbox  class="form-check-input" style="border:0;" name = "select" id="select" value="select" <%if(allChecked.equalsIgnoreCase("true")){%>checked <%}%>onclick="return checkAll(this.checked);">
...
function onAdd() {
	...
	document.form.action="/track/ProductionBomServlet?action=BULK_KITTING_WITHBOM";
...
function onGo(index) {
    ...
    $.ajax({type: "POST",url: urlStr, data: {ITEM:product,BATCH_0:pbatch,PARENTQTY:pqty,LOC:loc,action: "VIEW_KITBOM_DETAILS_BULK",PLANT:"<%=plant%>"},dataType: "json", success: callback });
...
function getTable(){
        return  '<TABLE class="table" id="tabledata" WIDTH="100%" align = "center">'+
        	    ...
        	    '<tr>'+
        	    '<th width="3%" style="display:none;">Select</th>'+
         		'<th width="2%">No</th>'+
         		'<th width="15%">Use Equivalent Item</th>'+
         		...
         		'<th width="4%">Kit Qty</th>'+
         		'<th width="4%">Child InvQty</th>'+
         		'<th width="8%">Equivalent InvQty</th>'+
         		'<th width="7%">Batch/Serial No</th>'+
         		'<th width="3%">Kitted Qty</th>'+
