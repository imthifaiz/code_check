...
<link rel="stylesheet" href="css/style.css">
 ...
 <script>
	...
	function popWinBatch(URL,dolnno) {
		var locId = document.getElementById("LOC_"+dolnno).value;
		if(locId=="" || locId.length==0 ) {
			alert("Enter Location!");
			document.getElementById("LOC_"+dolnno).focus();
			return false;
		}
		else{
...
//    String gino = _TblControlDAO.getNextOrder(plant,userId,"GINO");
   ...
   String contactname="",telno="",email="",add1="",add2="",add3="",add4="",country="",zip="",remarks="",REF = "",ISSUEDATE="";
...
<form class="form-horizontal" id="frmOutBoundOrderBulkIssue" name="form" method="post" action="/track/OrderIssuingServlet?">
      ...
      if(al.size()==0)
      {
       ...
       %>
        ...
        <TR bgcolor = "<%=bgcolor%>">
             ...
             <TD align="center" width="15%"> 
						<div class="input-group">
						<input class="form-control" type="text" name = "LOC_<%=dolnno%>" id = "LOC_<%=dolnno%>"  size = "6" maxlength = "10" value = "<%=loc%>"
					  	onkeypress="if((event.keyCode=='13') && ( this.value.length > 0)){validateLocation(this.value,<%=dolnno%>);}" MAXLENGTH=80>
					  	<span class="input-group-addon" onClick="javascript:popUpWin('../jsp/loc_list_MultiReceivewms.jsp?INDEX=<%=dolnno%>&LOC_<%=dolnno%>='+form.LOC_<%=dolnno%>.value);return false;"> 	
             	...
             	<a href="#" class="input-group-addon" data-toggle="tooltip" data-placement="top" title="Batch Details" onClick="javascript:popWinBatch('../jsp/OutBoundMultiPickingBatch.jsp?ITEMNO=<%=item%>&DOLNNO=<%=dolnno%>&LOC_0='+form.LOC_<%=dolnno%>.value+'&BATCH_0='+form.BATCH_<%=dolnno%>.value+'&BATCH_ID_0='+form.BATCH_ID_<%=dolnno%>.value+'&INDEX=0&TYPE=OBBULK&UOM=<%=(String)uom%>',<%=dolnno%>);return false;">
       ...
       <%}%>
        ...
        <%}else {%>
        <INPUT class="form-control datepicker" name="ISSUEDATE_0" id="ISSUEDATE_0" value="" type="TEXT" size="30" MAXLENGTH="80" readonly="readonly">
      <%}%>       
        </div>		
        </div>
 		<div class="form-inline">
