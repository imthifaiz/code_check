...
<html>
<head>
<title>Clearance Type List</title>
<meta charset="utf-8">
...
<h3 class="panel-title">Clearance Type List</h3> 
</div>
</div>
...
ClearanceUtil.setmLogger(mLogger);
  ...
  String sBGColor = "";
 try{
  List listQry = ClearanceUtil.getClearanceTypeList(clearance_type_id,plant,"");
  for (int i =0; i<listQry.size(); i++){
  Map map = (Map) listQry.get(i);
  sBGColor = ((i == 0) || (i % 2 == 0)) ? "#FFFFFF" : "#dddddd";
   String clearancetypeid        = strUtils.fString((String) map.get("CLEARANCETYPE"));
   String ctype             = strUtils.fString((String) map.get("TYPE"));
if(type.equalsIgnoreCase("clearancesummary")){
	%>
	   <TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			onClick="window.opener.form.CLEARANCE_TYPE_ID.value='<%=clearancetypeid%>';window.close();"><%=clearancetypeid%></a></td>
		<td align="left" class="main2">&nbsp;<%=ctype%></td>
...
%>
    <TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			onClick="window.opener.form1.CUSTOMER_TYPE_ID.value='<%=clearancetypeid%>';setCheckedValue( window.opener.form1.ACTIVE,'<%=ctype%>');window.close();"><%=clearancetypeid%></a></td>
		<td align="left" class="main2">&nbsp;<%=ctype%></td>
