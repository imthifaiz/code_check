...
function onGo(){
  document.form.action="ClockinoutTrackingMass.jsp?action=View";
  document.form.submit();
}
...
{   
  // if(item == "" || item == null) {alert("Please Select productID "); document.form.ITEM.focus(); return false; }
    var i=0;var ischeck="";var flag= false;
	 ...
	 if(flag)
	 {
		 document.form.action="/track/ClockInOutServlet?Submit=CheckIn_Out_Mass&STATUS="+buttonVal;
		 document.form.submit();
	 }
	 else
	 {
 ...
 try{              
        ...
        if(_strUtils.fString(PICKSTATUS).length() > 0)       ht.put("B.PICKSTATUS",PICKSTATUS);
         if(_strUtils.fString(HPNO).length() > 0)            ht.put("A.CONTACTNUM",HPNO);
           ht.put("b.trantype","Clockin");
       movQryList = movHisUtil.getAttendanceClockinList(ht,fdate,tdate,DIRTYPE,COMPANY,DESC);
	...
	try{
		String donoValues = (String)session.getAttribute("chkinoutval");
...
<FORM name="form" method="post" action="ClockinoutTrackingMass.jsp" >
...
<input type="hidden" name="PGaction" value="View">
 <input type="hidden" name="PRICE" value="">
  ...
  <TABLE border="0" width="100%" cellspacing="0" cellpadding="0" align="center" bgcolor="#dddddd">
    <TR>
      <TH BGCOLOR="#000066" COLSPAN="20"><font color="white">Clock In/Out Tracking Mass</font></TH>
    </TR>
  </TABLE>
   <br>
...
<%} %>
         ...
         <TR>        
          ...
          <TD><SELECT NAME ="ISSUESTATUS" size="1">
            <OPTION selected  value=""> </OPTION>
     		...
     		<OPTION   value='O' <%if(ISSUESTATUS.equalsIgnoreCase("O")){ %>selected<%} %>>CLOCK-IN </OPTION>
     		<OPTION   value='C' <%if(ISSUESTATUS.equalsIgnoreCase("C")){ %>selected<%} %>>CLOCK-OUT </OPTION>
          </SELECT></TD>
  ...
  <TABLE WIDTH="80%"  border="0" cellspacing="1" cellpadding ="2" align = "center">
    <TR BGCOLOR="#000066">
       ...
       <%
          ...
          for (int iCnt =0; iCnt<movQryList.size(); iCnt++){
               ...
               String appendstatus= (String)lineArr.get("lnstat");
               ...
               chkString = (String)lineArr.get("dono")+"AAABAAC"+(String)lineArr.get("item")+"AAABAAC"+appendstatus;
       %>
   ...
   <td>   <input type="button" value="Clock-In" onClick="onConfirm('OPEN');" > </td>
   
     <td>   <input type="button" value="Clock-Out" onClick="onConfirm('CLOSE');" > </td>
