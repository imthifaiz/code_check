...
function submitForm(actionvalue){
	 ...
	 var orderLNo;
	 var len = document.form.chkdEstNo.length; 
	 if(len == undefined) len = 1;  
	 for (var i = 0; i < len ; i++)
    {
		if(len == 1 && (!document.form.chkdEstNo.checked))
		{
			checkFound = false;
		}
		
		else if(len ==1 && document.form.chkdEstNo.checked)
	     {
	    	 checkFound = true;
	    	 orderLNo = document.form.chkdEstNo.value;
	    	if(!verifyIssuingQty(orderLNo))	
		    	  return false;
	     }
	
	     else {
		     if(document.form.chkdEstNo[i].checked){
		    	 checkFound = true;
		    	 orderLNo = document.form.chkdEstNo[i].value;
		    	 if(!verifyIssuingQty(orderLNo))	
			    	  return false;
		     }
	     }
...
function checkAll(isChk)
{
    ...
    if (document.form.chkdEstNo)
    {
        ...
        {      
              	if(len == 1){
              		document.form.chkdEstNo.checked = isChk;
              		 orderLNo = document.form.chkdEstNo.value;
              	}
              	else{
              		document.form.chkdEstNo[i].checked = isChk;
              		 orderLNo = document.form.chkdEstNo[i].value;
              	}
            	setIssuingQty(orderLNo,i);       	
...
function setIssuingQty(orderLNo,i){
	...
	else if(len !=1 && document.form.chkdEstNo[i].checked){
		if(document.form.fullIssue.checked)
		{
			setQty(orderLNo);
		}
...
function ongo()
{  
	var dono = document.getElementById("EST_DONO").value;
	document.form.action = "/track/deleveryorderservlet?Submit=View&DONO="+dono;
	document.form.submit();
...
String contactname = "", telno = "", email = "",currencyid="", add1 = "", add2 = "", add3 = "", add4 = "", country = "", zip = "", remarks = "", gst="";
String sCustName="",sContactName= "",sAddr1="",sAddr2="",sCity="",sCountry="",sZip="",sTelno="";
String sSaveEnb = "disabled",statusID="",EMP_NAME="",fullIssue = "",allChecked = "";*/
...
shippingid="",shippingcustomer="",orderdiscount="",shippingcost="",incoterms="",statusID="",
EMP_NAME="",fullIssue = "",allChecked = "",paymenttype="",deliverydateformat="",sTAXTREATMENT="",sSALES_LOC="";
...
List supplierattachlist= new ArrayList();
	...
	Map checkedEST = (Map) request.getSession().getAttribute("checkedEST");
...
<form class="form-horizontal" id="salesform" name="form" method="post"  action="/track/EstimateServlet?" onSubmit="return validatePO(document.form)">
   
        <INPUT type="Hidden" name="DIRTYPE" value="OUTBOUND">
        <input type="hidden" name="xlAction" value="">
		<input type="hidden" name="formtype" value="outbound">
  		<INPUT type = "hidden" name="ESTNO" value ="<%=estno%>">
...
<div class="form-group">
        ...
        <div class="col-sm-3">
        <div class="input-group">          
        <INPUT class="form-control" type = "TEXT" size="30"  MAXLENGTH=20 id="DELIVERYDATE" name="DELIVERYDATE" <%if(deliverydateformat.equals("1")) {%>readonly class="form-control datepicker"<%} else {%> class=form-control <%}%> value="<%=deliverydate%>">
      	</div>
    	</div>
    	<div class="form-inline">
...
<div class="form-group">
        ...
        <span class="input-group-addon"  onClick="javascript:popUpWin('../jsp/OrderType_list.jsp?ORDERTYPE='+form.ORDERTYPE.value+'&FORMTYPE=outbound');">
...
<div class="form-group">
<label class="control-label col-sm-2" for="Existing Outbound">Existing Sales:</label>
        <div class="col-sm-3">
        <div class="input-group">
        <INPUT class="form-control" name="EST_DONO" id="EST_DONO" type = "TEXT" value="" size="30"  MAXLENGTH=20>
        <span class="input-group-addon"  onClick="javascript:popUpWin('../jsp/estimate_do_list.jsp?ESTNO='+form.ESTNO.value);">
   		 	<a href="#" data-toggle="tooltip" data-placement="top" title="Existing Sales Details">
   		 	<i class="glyphicon glyphicon-log-in" style="font-size: 20px;"></i></a></span>
        </div>
 		</div>
 		<div class="form-inline">
 		<div class="col-sm-1">
 		<button type="button" class="Submit btn btn-default" onClick="return ongo()"><b>View</b></button>&nbsp;&nbsp;
 		</div>
 		</div>
 	  ...
 	  <div class="form-group">
		...
		<th width="10%">Sales Estimate Quantity</th>
		<th width="10%">Converted Quantity </th>
        ...
        <% 
      ArrayList al= _ESTUtil.listEstDetails(plant, estno);
      ...
      /*if(al.size()==0)
      {
       ...
       for(int i=0 ; i<al.size();i++)
       {
          ...
          if(checkedEST!=null && result.equalsIgnoreCase("catchrerror")){
              value = (String)checkedEST.get(estlnno);   
          ...
          String qtyIsValue =(String)m.get("qtyis");
          ...
          double dUnitPrice = Double.parseDouble(unitPricerd);
          unitPricerd = StrUtils.addZeroes(dUnitPrice, numberOfDecimal);
       ...
       <TR bgcolor = "<%=bgcolor%>">
              ...
              name="chkdEstNo"  value="<%=estlnno%>" <%if(value!=null){%>checked <%}%> onclick = "setIssuingQty(<%=estlnno%>,<%=i%>);"></font></TD>
              <TD align="center" width="10%"><%=(String)m.get("estlnno")%></TD>
              <TD align="center" width="13%"><%=(String)m.get("item")%></TD>
              <TD align="center" width="20%"><%=(String)desc%></TD>
              <TD align="center" width="12%"><%=unitPricerd%></TD>
              <TD align="center" id = "QtyOrdered_<%=estlnno%>"width="10%"><%=qtyOrValue%></TD>              
              <TD align="center" id = "QtyIssued_<%=estlnno%>" width="10%"><%=qtyIsValue%></TD>
              <TD align="center" width="10%">
						<input class="form-control" type="text" name = "issuingQty_<%=estlnno%>" id = "issuingQty_<%=estlnno%>" 
       ...
       <% }%>
      	...
      	<button type="button" class="Submit btn btn-default" onClick="if(validatePO(document.form)) {submitForm('Convert To Outbound Order');}"><b>Confirm</b></button>&nbsp;&nbsp;
...
function add_attachments(){
    ...
    $.ajax({
        ...
        //	window.location.reload();
        	document.form.action  = "/track/EstimateServlet?Submit=Convert To Outbound Order";
         document.form.submit();
        },
        error: function (data) {
