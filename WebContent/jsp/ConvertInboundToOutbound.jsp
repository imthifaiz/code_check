...
<%
String vend = "", deldate = "", jobNum = "", custName = "",ordertype="", custCode = "", personIncharge = "",
contactNum = "",remark1 = "", remark2 = "", address = "", address2 = "", address3 = "", collectionDate = "", 
collectionTime = "",contactname = "", telno = "", email = "",currencyid="", add1 = "", add2 = "", 
add3 = "", add4 = "", country = "", zip = "", remarks = "", gst="",
customertypedesc="",customerstatusdesc="",customerstatusid="",customertypeid="",statusID="",EMP_NAME="",deliverydate="",
shippingid="",shippingcustomer="",orderdiscount="",shippingcost="",incoterms="",paymenttype="",deliverydateformat="";
String sSaveEnb = "disabled",fullIssue = "",allChecked = "",sTAXTREATMENT="",sPURCHASE_LOC="";
...
CustomerBeanDAO customerBeanDAO = new CustomerBeanDAO();
	...
	if (action.equalsIgnoreCase("View")) {
	  ...
	  if(m.size()>0){
		...
		deliverydate=(String) m.get("deldate");
...
<form class="form-horizontal" id="salesform" name="form" method="post"  action="/track/deleveryorderservlet?" onSubmit="return validatePO(document.form)">
		...
		<input type="hidden" name="copytype" value="IBTOOB">
...
<div class="form-group">
        ...
        <div class="col-sm-3">
        <div class="input-group">          
       <INPUT type = "TEXT" size="30"  MAXLENGTH=20 id="DELIVERYDATE"  name="DELIVERYDATE" <%if(deliverydateformat.equals("1")) {%>readonly class="form-control datepicker"<%} else {%> class=form-control <%}%> value="<%=deliverydate%>">
       </div>
    	</div>
    	<div class="form-inline">
...
<!-- <label class="control-label col-sm-2" for="Order Status">Order Status:</label>
       ...
       <% 
      ArrayList al= _POUtil.listPODETDetailsforcopyfunc(plant, pono);
       ...
       if(al.size()>0)
     {
          ...
          //String price = _ItemMstDAO.getItemPrice(plant,item);
          String price = _ItemMstDAO.getLocalCurrency(plant,currencyid,item,uom);
          ...
          float unitPrice ="".equals(price) ? 0.0f :  Float.parseFloat(price);
          ...
          double dUnitPrice = Double.parseDouble(price);
          price = StrUtils.addZeroes(dUnitPrice, numberOfDecimal);
       ...
       %>
        ...
        <TR bgcolor = "<%=bgcolor%>">
             ...
             name="chkdDONO"  value="<%=lnno%>" <%if(value!=null){%>checked <%}%> onclick = "setIssuingQty(<%=lnno%>,<%=i%>);"></font></TD>
              ...
              <TD align="center" width="12%"><%=price%></TD>
              <TD align="center" id = "QtyOrdered_<%=lnno%>"width="10%"><%=qtyor%></TD>              
