...
// 	    if(phonenumber == "" || phonenumber == null) {alert("Please Enter Phone Number");document.form1.PHNO.focus(); return false; }
	   ...
	   document.form1.action  = "../jsp/CreateContacts.jsp?action=ADD";
...
function onClear(){
	...
	document.form1.HOWWEKNOW.value = "";
	document.form1.USER_NAME.value = "";
	document.form1.COUNTRY.value = "";
	document.form1.INDUSTRY.value = "";
	document.form1.WEBSITE.value = "";
	document.form1.FACEBOOK.value = "";
	document.form1.LINKED.value = "";
	document.form1.SALESPROBABILITY.value = "";
	}
</SCRIPT>
<%
...
String res        = "",action     =   "",sSAVE_RED,region="",sCountry="",USER_NAME="",cHdrId="";
String name ="",email="",companycontact="",job="",phnumber="",lifecycle="",leadstatus="",note="";
...
List contactattachlist= new ArrayList();
 ...
 if(action.equalsIgnoreCase("ADD")){
	 ...
	 List<Hashtable<String,String>> ContactAttachmentList = null;
		...
		contactAttachmentInfoList = new ArrayList<Hashtable<String,String>>();		//Hashtable<String,String> contactAttachment = null;
	 if(isMultipart)
		{
				...
				if (item.isFormField()) {
					...
					if (item.getFieldName()
						...
						name = strUtils.fString(item.getString());
							...
							.equalsIgnoreCase("ID")) {
						cHdrId = strUtils.fString(item.getString());
					}
					else if (item.getFieldName()
						...
						email = strUtils.fString(item.getString());
							...
							.equalsIgnoreCase("COMPANYCONTACT")) {
						companycontact = strUtils.fString(item.getString());
					}
					else if (item.getFieldName()
							.equalsIgnoreCase("JOB")) {
						job = strUtils.fString(item.getString());
					}
					else if (item.getFieldName()
							.equalsIgnoreCase("PHNO")) {
						phnumber = strUtils.fString(item.getString());
					}
					else if (item.getFieldName()
							.equalsIgnoreCase("MOBILENO")) {
						MOBILENO = strUtils.fString(item.getString());
					}
					else if (item.getFieldName()
							.equalsIgnoreCase("LIFECYCLE")) {
						lifecycle = strUtils.fString(item.getString());
					}
					else if (item.getFieldName()
							.equalsIgnoreCase("LEADSTATUS")) {
						leadstatus = strUtils.fString(item.getString());
					}
					else if (item.getFieldName()
							.equalsIgnoreCase("NOTE")) {
						note = strUtils.fString(item.getString());
					}
					else if (item.getFieldName()
							.equalsIgnoreCase("HOWWEKNOW")) {
						HOWWEKNOW = strUtils.fString(item.getString());
					}
					else if (item.getFieldName()
							.equalsIgnoreCase("USER_NAME")) {
						USER_NAME = strUtils.fString(item.getString());
					}
					else if (item.getFieldName()
						...
						INDUSTRY = strUtils.fString(item.getString());
					}
					else if (item.getFieldName()
						...
						LINKED = strUtils.fString(item.getString());
					}
					else if (item.getFieldName()
							.equalsIgnoreCase("SALESPROBABILITY")) {
						SALESPROBABILITY = strUtils.fString(item.getString());
					}
				}
				 if (!item.isFormField() && (item.getName().length() > 0)) {
					 if(item.getFieldName().equalsIgnoreCase("file")){
					String fileLocation = "C:/ATTACHMENTS/CONTACT" + "/"+ name;
					String filetempLocation = "C:/ATTACHMENTS/CONTACT" + "/temp" + "/"+ name;
					...
					contactAttachment.put("FilePath", fileLocation);
					contactAttachment.put("CRAT",DateUtils.getDateTime());
					contactAttachment.put("CRBY",sUserId);
    ...
    hdr.put(IDBConstants.PLANT,sPlant);
    hdr.put("NAME",name);
    //hdr.put("ID",cHdrId);
     if(!(plantmstutil.isExistsContactName(hdr))) // if the Item  exists already
     {
           hdr.put(IDBConstants.PLANT,sPlant);
           hdr.put("NAME",name);
           hdr.put("EMAIL",email); 
           hdr.put("COUNTRY",sCountry); 
           hdr.put("COMPANYCONTACT",companycontact); 
           hdr.put("JOB",job); 
           hdr.put("PHONENUMBER",phnumber); 
           hdr.put("LEADSTATUS",leadstatus); 
           hdr.put("MOBILENO",MOBILENO); 
           hdr.put("HOWWEKNOW",HOWWEKNOW); 
           hdr.put("USER_NAME",USER_NAME); 
           hdr.put("INDUSTRY",INDUSTRY); 
           hdr.put("WEBSITE",WEBSITE); 
           hdr.put("FACEBOOK",FACEBOOK); 
           hdr.put("LINKED",LINKED); 
           hdr.put("SALESPROBABILITY",SALESPROBABILITY); 
           hdr.put("UPBY",sUserId);   hdr.put("CRBY",sUserId);
           hdr.put("CRAT",dateutils.getDateTime());
           hdr.put("UPAT",dateutils.getDateTime());
           
           
//            hdr.put(IDBConstants.CREATED_AT,new DateUtils().getDateTime());
  			int contacthdrID = plantmstutil.insertContactHdr(hdr, sPlant);
  			if(contacthdrID > 0){
            ...
            if(!note.equals(""))
  		  {
  			htm.put(IDBConstants.REMARKS, name+","+note);
  		  }
  		  else
  		  {
  			htm.put(IDBConstants.REMARKS, name);
  		  }
          ...
          // det.put("USER_NAME",USER_NAME); 
           ...
           det.put("UPAT",dateutils.getDateTime());
 		   ...
 		   boolean updateFlag;
//            boolean ContactInserted = plantmstutil.insertContactHdr(hdr);
           boolean ContactDetInserted = plantmstutil.insertContactDet(det);
//         boolean  inserted = mdao.insertIntoMovHis(htm);

           if(ContactDetInserted) {
        	   sSAVE_RED="Contact Added Successfully";
           } else {
                     res = "<font class = "+IDBConstants.FAILED_COLOR +">Failed to add New Contact </font>";
           }
     }else{
            res = "<font class = "+IDBConstants.FAILED_COLOR +">Contact  Exists already. Try again</font>";
     }
...
<div class="box"> 
 <!-- Thanzith Modified on 28.02.2022 -->
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Create Contacts</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 28.02.2022 --> 

            <div class="box-header menu-drop">
	              <h1 style="font-size: 20px;" class="box-title"><%=title %></h1>
	              <h1 style="font-size: 18px; cursor: pointer;" class="box-title pull-right" onclick="window.location.href='../contact/summary'">
...
<form class="form-horizontal" id="contactform" name="form1" autocomplete="off" method="post" enctype="multipart/form-data">
<!-- <div class="col-sm-6"> -->
<div class="row">
   ...
   <div class="form-group">
      <label class="control-label col-form-label col-sm-2 required" for="Company Name">Company Name</label>
      	<div class="col-sm-4">
      		<div class="input-group">
      	  	 <input class="form-control" name="NAME" id="NAME" type = "TEXT" value="" size="30"  MAXLENGTH=100 placeholder="Max 100 characters">
    ...
    <div class="form-group">
      ...
      <div class="col-sm-4">
      <INPUT class="form-control" name="INDUSTRY" id="INDUSTRY" type="TEXT" value="" placeholder="Max 50 Characters" size="25" MAXLENGTH=50>
  		        </div>
    </div>
   
     <div class="form-group">
      <label class="control-label col-form-label col-sm-2"for="Contact">Contact Person</label>
      	<div class="col-sm-4">
      		<div class="input-group">
      	  	 <input class="form-control" name="COMPANYCONTACT" id="COMPANYCONTACT" type = "TEXT" value="" size="30"  MAXLENGTH=50 placeholder="Max 100 characters">
  			</div>
      	</div>
   </div>
   ...
   <div class="form-group">
      <label class="control-label col-form-label col-sm-2"for="Designation">Designation</label>
      	<div class="col-sm-4">
      		<div class="input-group">
      	  	 <input class="form-control" name="JOB" id="JOB" type = "TEXT" value="" size="30"  MAXLENGTH=50 placeholder="Max 50 characters">
  			</div>
      	</div>
   </div>
   ...
   <div class="form-group">
      <label class="control-label col-form-label col-sm-2"for="Phone Number">Telephone Number</label>
      	<div class="col-sm-4">
      		<div class="input-group">
      	  	 <input class="form-control" name="PHNO" id="PHNO" type = "TEXT" value="" size="30" onkeypress="return validateInput(event)" MAXLENGTH=20 placeholder="Max 20 characters">
  			</div>
      	</div>
   </div>
   <div class="form-group">
      <label class="control-label col-form-label col-sm-2"for="Mobile Number">Contact Number</label>
      	<div class="col-sm-4">
      		<div class="input-group">
      	  	 <input class="form-control" name="MOBILENO" id="MOBILENO" type = "TEXT" value="" size="30" onkeypress="return validateInput(event)"  MAXLENGTH=20 placeholder="Max 20 characters">
  			</div>
      	</div>
   </div>
   <div class="form-group">
      <label class="control-label col-form-label col-sm-2"for="How We Know">How We Know</label>
      	<div class="col-sm-4">
      		<div class="input-group">
      	  	 <input class="form-control" name="HOWWEKNOW" id="HOWWEKNOW" type = "TEXT" value="" size="30"  MAXLENGTH=50 placeholder="Max 100 characters">
  			</div>
      	</div>
   </div>
   <div class="form-group">
      <label class="control-label col-form-label col-sm-2 required"for="Email">Email</label>
      	<div class="col-sm-4">
      		<div class="input-group">
      	  	 <input class="form-control" name="EMAIL" id="EMAIL" type = "TEXT" value="" size="30"  MAXLENGTH=50 placeholder="Max 50 characters">
  			</div>
      	</div>
   </div>
  
   <div class="form-group">
      <label class="control-label col-form-label col-sm-2"for="Sales Consaltent">Sales Consultant</label>
      	<div class="col-sm-4">
      		<div class="input-group">
      	  	<INPUT name="USER_NAME"  id="USER_NAME" type="TEXT" value="<%=sUserId%>" size="20" MAXLENGTH="20" class="form-control" onchange="checkUser(this.value)" placeholder="Select Sales Consaltent">
 			<span class="select-icon" onclick="$(this).parent().find('input[name=\'USER_NAME\']').focus()"><i class="glyphicon glyphicon-menu-down"></i></span>
  			</div>
      	</div>
   </div>
   <div class="form-group">
      <label class="control-label col-form-label col-sm-2 required"for="LifeCycle Stage">LifeCycle Stage</label>
      	<div class="col-sm-4">
      		<div class="input-group">
      	  	 <input type="text" name="LIFECYCLE" id="LIFECYCLE" value="" class="ac-selected form-control" placeholder="LifeCycle Stage" >
   ...
   <div class="form-group">
      <label class="control-label col-form-label col-sm-2 required"for="Lead Status">Lead Status</label>
      	<div class="col-sm-4">
      		<div class="input-group">
      	  	 <input type="text" name="LEADSTATUS" id="LEADSTATUS" value="" class="ac-selected form-control" placeholder="Lead Status" >
   ...
   <div class="form-group">
      ...
      <div class="col-sm-4">
      <div class="input-group">
      	  	 <input type="text" name="SALESPROBABILITY" id="SALESPROBABILITY" value="" class="ac-selected form-control" placeholder="Sales Probability" >
				<span class="select-icon" onclick="$(this).parent().find('input[name=\'SALESPROBABILITY\']').focus()">
				<i class="glyphicon glyphicon-menu-down"></i>
				</span>
  			</div>
      <!-- <INPUT class="form-control" name="LINKEDIN" id="LINKEDIN"type="TEXT" value="" placeholder="Max 50 Characters" size="25" MAXLENGTH=50> -->
  	 </div>
    </div>
   ...
   <div class="bs-example">
      ...
      <div class="col-sm-4">
      <INPUT class="form-control" name="WEBSITE" id="WEBSITE" type="TEXT" value="" placeholder="Max 50 Characters" size="25" MAXLENGTH=50>
  		        </div>
    </div>
         <div class="form-group">
      <label class="control-label col-form-label col-sm-2" for="Facebook">Facebook</label>
      <div class="col-sm-4">
      <INPUT class="form-control" name="FACEBOOK" id="FACEBOOK"type="TEXT" value="" placeholder="Max 50 Characters" size="25" MAXLENGTH=50>
  		        </div>
    </div>
         <div class="form-group">
      <label class="control-label col-form-label col-sm-2" for="LinkedIn">LinkedIn</label>
      <div class="col-sm-4">
      <INPUT class="form-control" name="LINKED" id="LINKED"type="TEXT" value="" placeholder="Max 50 Characters" size="25" MAXLENGTH=50>
  		        </div>
    </div>
      ...
      <label class="control-label col-form-label col-sm-2" for="Note">Note</label>
      	<div class="col-sm-4">
      		<div class="input-group">
<!--       	  	 <input class="form-control" name="NOTE" id="NOTE" type = "TEXT" value="" size="30"  MAXLENGTH=500> -->
      	  	 <textarea rows="4" name="NOTE" id="NOTE" class="ember-text-area form-control ember-view" placeholder="Max 500 characters"  maxlength="500"></textarea>
  					</div>
     		 	</div>
   			</div>
        </div>
...
$(document).ready(function(){
    ...
    if(document.form1.SAVE_RED.value!="") {
	document.form1.action  = "../contact/summary?PGaction=View&result=Contacts Created Successfully";	   
    document.form1.submit();
	}
    else{
    	document.form1.action  = "../contact/summary?PGaction=View&result=Contacts Created Successfully";	   
   ...
   /*    function add_attachments(){
          ...
          $.ajax({
              ...
              //	window.location.reload();
              	document.form.action  = "../jsp/CreateContacts.jsp?action=ADD";
