...
<script>
	...
	function ExportReport(){
		document.form.action="/track/deleveryorderservlet?Submit=ExportExcelFOCSummary";
...
<FORM class="form-horizontal" name="form" method="post" action="FOCSummary.jsp">
...
<script>
		...
		function getParameters(){
			return {
				...
				"ACTION": "VIEW_FOC_SUMMARY","PLANT":"<%=PLANT%>",SRC:"VOID",LOGIN_USER:"<%=USERID%>"
			}
		}
		function onGo(){
			    	...
			    	tabletype = $('#table').DataTable({
					        ...
					        "dataSrc": function(data){
					        	...
					        	if(typeof data.POS[0].dono === 'undefined'){
						        		...
						        		for(var dataIndex = 0; dataIndex < data.POS.length; dataIndex ++){
						        			...
						        			data.POS[dataIndex]['qty']=parseFloat(data.POS[dataIndex]['qty']).toFixed(3);
				        ...
				        "drawCallback": function ( settings ) {
				            ...
				            api.column(groupColumn, {page:'current'} ).data().each( function ( group, i ) {
				                ...
				                groupTotalPickQty += parseFloat($(rows).eq( i ).find('td:eq(5)').html().replace(',', ''));
				                totalPickQty += parseFloat($(rows).eq( i ).find('td:eq(5)').html().replace(',', ''));
				            ...
				            if (groupEnd > 0 || rows.length == (currentRow + 1)){
				            	$(rows).eq( currentRow ).after(
				                        '<tr class="group"><td colspan=' + groupRowColSpan + '></td><td>Total</td><td class=" t-right">' + parseFloat(totalPickQty).toFixed(3) + '</td></tr>'
			                    );
			                	/* $(rows).eq( currentRow ).after(
				                        '<tr class="group"><td colspan=' + groupRowColSpan + '></td><td>Total</td><td>' + parseFloat(groupTotalPickQty).toFixed(3) + '</td></tr>'
			                    ); */
