...
<%
String title = "Order Receipt Summary With Cost";
%>
...
<%
	...
	String TO_ASSIGNEE="",LOANASSIGNEE="",ORDERTYPE,LOC="",PRD_DEPT_ID="",REASONCODE="",FILTER="",LOCALEXPENSES="";
	PGaction = StrUtils.fString(request.getParameter("PGaction"))
			.trim();
	String html = "", cntRec = "false",PRD_BRAND_ID = "",PRD_TYPE_ID="",PRD_CLS_ID="",LOC_TYPE_ID="",LOC_TYPE_ID2="",LOC_TYPE_ID3;
	
	boolean displaySummaryExport=false;
	if(systatus.equalsIgnoreCase("ACCOUNTING"))
	{
		displaySummaryExport = ub.isCheckValAcc("exportrecvsummrycost", plant,LOGIN_USER);
		displaySummaryExport = true;
	}
	if(systatus.equalsIgnoreCase("INVENTORY"))
	{
		displaySummaryExport = ub.isCheckValinv("exportrecvsummrycost", plant,LOGIN_USER);
		displaySummaryExport = true;
	}
...
<div class="container-fluid m-t-20">
	...
	<!-- Muruganantham Modified on 16.02.2022 -->
            <ol class="breadcrumb" style="background-color: rgb(255, 255, 255);">      	
                ...
                <li><label>Order Receipt Summary with Cost</label></li>                             
            </ol>             
    <!-- Muruganantham Modified on 16.02.2022 -->
...
<div class="box-body">
<FORM class="form-horizontal" name="form1" method="post" action="GoodsReceiptSummaryWithCost.jsp">
<input type="hidden" name="xlAction" value=""> <input
	type="hidden" name="PGaction" value="View"> 
...
<!-- 		  		<span class="select-search btn-danger input-group-addon " onClick="javascript:popUpWin('../jsp/miscreasoncode.jsp?ITEMNO='+form1.ITEM.value+'&FORM=form1');"><span class="glyphicon glyphicon-search" aria-hidden="true" style="left: -4px;"></span></span> -->
  		...
  		<div class="row" style="padding:3px">
	  		...
	  		<div class="col-sm-4 ac-box">  		
  			<input type="text" class="ac-selected form-control" id="grno" name="grno" value="<%=StrUtils.forHTMLTag(GRNO)%>"
  			placeholder="GRNO" >
      ...
      <!-- <button type="button" class="Submit btn btn-default" onClick="{backNavigation('reports.jsp','RTR');}"> <b>Back</b></button> -->
       	  ...
       	  <div class="form-group">
 				<div class="col-sm-4">
 	<input type="checkbox" class="form-check-input" name="LOCALEXPENSES" id="withlocalexpenses" value="<%=StrUtils.forHTMLTag(LOCALEXPENSES)%>" Onclick="checkval()" <%=LOCALEXPENSESCHK%> />&nbsp;View By Landed Cost&nbsp;			
 				</div>
 				</div>
 				<br>
   		     
	<INPUT type="Hidden" name="DIRTYPE" value="RECEIVEWITHCOST">
              	...
              	<table id="tableInventorySummary"
					...
					<thead>
		                <tr role="row">
		                	...
		                	<th style="font-size: smaller;">RECVED DATE</th>
		                	...
		                	<th style="font-size: smaller;">RECVE QTY</th>
  ...
  var ITEMNO, GRNO,ITEMDESC, FROM_DATE, TO_DATE, DIRTYPE, SUPPLIER, ORDERNO, JOBNO, ORDERTYPE, PRD_CLS_ID, LOC_TYPE_ID,LOC_TYPE_ID2,LOC_TYPE_ID3, PRD_BRAND_ID,
  PRD_TYPE_ID, TO_ASSIGNEE, LOANASSINEE, PRD_DEPT_ID,REASONCODE,BATCH,LOC,FILTER,LOCALEXPENSES, groupRowColSpan = 9;
function getParameters(){
	return {
		
		"ITEM": ITEMNO,"GRNO":GRNO,	"PRD_DESCRIP":ITEMDESC,
		"FDATE":FROM_DATE,"TDATE":TO_DATE,"DTYPE":DIRTYPE,"CNAME":SUPPLIER,"ORDERNO":ORDERNO,"JOBNO":JOBNO,"ORDERTYPE":ORDERTYPE,
		"PRD_BRAND_ID":PRD_BRAND_ID,"PRD_TYPE_ID":PRD_TYPE_ID,"PRD_DEPT_ID":PRD_DEPT_ID,"PRD_CLS_ID":PRD_CLS_ID,"LOC_TYPE_ID":LOC_TYPE_ID,"TOASSINEE":TO_ASSIGNEE,"LOC_TYPE_ID2":LOC_TYPE_ID2,
		"LOC_TYPE_ID3":LOC_TYPE_ID3,"LOANASSINEE":LOANASSINEE,"REASONCODE":REASONCODE,"BATCH":BATCH,"LOC":LOC,"FILTER":FILTER,"LOCALEXPENSES":LOCALEXPENSES,
				
		"ACTION": "VIEW_GOODS_RECIPT_SMRY_WITH_COST",
		"PLANT":"<%=plant%>"
	}
}  
  function onGo(){
   ...
   if(ALLOWCATCH_ADVANCE_SEARCH==='1'){
	  //storeInLocalStorage('GoodsReceiptSummaryWithCost_FROMDATE', FROM_DATE);
	//	storeInLocalStorage('GoodsReceiptSummaryWithCost_TODATE', TO_DATE);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_CUSTOMER', SUPPLIER);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_ORDERNO', $('#ORDERNO').val());
		storeInLocalStorage('GoodsReceiptSummaryWithCost_ITEM', ITEMNO);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_PRD_CLS_ID', PRD_CLS_ID);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_PRD_TYPE_ID', PRD_TYPE_ID);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_PRD_BRAND_ID', PRD_BRAND_ID);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_PRD_DEPT_ID', PRD_DEPT_ID);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_ORDERTYPE',ORDERTYPE);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_LOC', LOC);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_BATCH', BATCH);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_LOC_TYPE_ID',LOC_TYPE_ID);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_LOC_TYPE_ID2',LOC_TYPE_ID2);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_LOC_TYPE_ID3', LOC_TYPE_ID3);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_FILTER',FILTER);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_REASONCODE',REASONCODE);
		storeInLocalStorage('GoodsReceiptSummaryWithCost_GRNO', GRNO);
   }
...
<%-- 		        			data.items[dataIndex]['gstpercentage'] = parseFloat(data.items[dataIndex]['gstpercentage']).toFixedSpecial(<%=DbBean.NOOFDECIMALPTSFORCURRENCY%>); --%>
		        			/*data.items[dataIndex]['tax'] = parseFloat(data.items[dataIndex]['tax']).toFixedSpecial1(5);
		        			data.items[dataIndex]['recvCost'] = parseFloat(data.items[dataIndex]['recvCost']).toFixedSpecial1(<%=DbBean.NOOFDECIMALPTSFORCURRENCY%>);
		        			data.items[dataIndex]['recvCostwTax'] = parseFloat(data.items[dataIndex]['recvCostwTax']).toFixedSpecial1(<%=DbBean.NOOFDECIMALPTSFORCURRENCY%>);*/
		        			
		        			data.items[dataIndex]['ordqty'] = addZeroes(parseFloat(data.items[dataIndex]['ordqty']).toFixed(3));
		        			data.items[dataIndex]['recqty'] = addZeroes(parseFloat(data.items[dataIndex]['recqty']).toFixed(3));
		        			...
		        			data.items[dataIndex]['recvCost'] = addZeroes(parseFloat(data.items[dataIndex]['recvCost']).toFixed(<%=numberOfDecimal%>));
		        			data.items[dataIndex]['recvCostwTax'] = addZeroes(parseFloat(data.items[dataIndex]['recvCostwTax']).toFixed(<%=numberOfDecimal%>));
	        ...
	        "columns": [
			{"data": 'pono', "orderable": true}, /* 0 */
			{"data": 'grno', "orderable": true}, /* 1 */
			{"data": 'transactiondate', "orderable": true}, /* 2 */
			{"data": 'cname', "orderable": true}, /* 3 */
			{"data": 'item', "orderable": true}, /* 4 */
			{"data": 'itemdesc', "orderable": true}, /* 5 */
			{"data": 'loc', "orderable": true}, /* 6 */
			{"data": 'batch', "orderable": true}, /* 7 */
			{"data": 'expiredate', "orderable": true}, /* 8 */
			{"data": 'ordqty', "orderable": true}, /* 9 */
			{"data": 'recqty', "orderable": true}, /* 10 */
			{"data": 'uom', "orderable": true}, /* 11 */
			{"data": 'unitCost', "orderable": true}, /* 12 */
			{"data": 'unitCostConv', "orderable": true}, /* 13 */
			{"data": 'gstpercentage', "orderable": true}, /* 14 */
			{"data": 'exchangerate', "orderable": true}, /* 15 */
			{"data": 'recvCost', "orderable": true}, /* 16 */
			{"data": 'recvConvCost', "orderable": true}, /* 17 */
			{"data": 'tax', "orderable": true}, /* 18 */
			{"data": 'taxcon', "orderable": true}, /* 19 */
			{"data": 'recvCostwTax', "orderable": true}, /* 20  */
			{"data": 'recvCostConvwTax', "orderable": true}, /* 21 */
			{"data": 'users', "orderable": true}, /* 22 */
			{"data": 'remark', "orderable": true} /* 23 */
			],
			"columnDefs": [{"className": "t-right", "targets": [10, 16, 17,18,19,20,21]}],
	        ...
	        buttons: [
                ...
                columns: ':not(:eq('+groupColumn+')):not(:eq(9)):not(:eq(10)):not(:eq(14)):not(:eq(15)):not(:eq(16))'
            }
	        ],
	        "order": [],
			...
			"drawCallback": function ( settings ) {
	            ...
	            api.column(groupColumn, {page:'current'} ).data().each( function ( group, i ) {
	                if ( last !== group ) {
	                	if (i > 0) {
	                		$(rows).eq( i ).before(
			                        '<tr class="group"><td colspan="' + groupRowColSpan + '"></td><td>Total</td><td class="t-right">' + parseFloat(groupTotalQty).toFixed(3) + '</td><td></td><td></td><td></td><td></td><td></td><td class="t-right">' + parseFloat(groupTotalCost).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(groupTotalCost1).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(grouptax).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(grouptax1).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(groupTotalPrice).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(groupTotalPrice1).toFixed(<%=numberOfDecimal%>) + '</td><td></td><td></td></tr>'
			                    );
	                	}
	                    last = group;
	                    groupEnd = i;
	                ...
	                groupTotalQty += parseFloat($(rows).eq( i ).find('td:eq(10)').html().replace(',', '').replace('$', ''));
	                totalQty += parseFloat($(rows).eq( i ).find('td:eq(10)').html().replace(',', '').replace('$', ''));
	                groupTotalCost += parseFloat($(rows).eq( i ).find('td:eq(16)').html().replace(',', '').replace('$', ''));
	                totalCost += parseFloat($(rows).eq( i ).find('td:eq(16)').html().replace(',', '').replace('$', ''));

	                groupTotalCost1 += parseFloat($(rows).eq(i).find('td:eq(17)').html().replace(/[^0-9.]/g, ''));
	                totalCost1 += parseFloat($(rows).eq(i).find('td:eq(17)').html().replace(/[^0-9.]/g, ''));
	                
	                grouptax += parseFloat($(rows).eq( i ).find('td:eq(18)').html().replace(',', '').replace('$', ''));
	                totaltax += parseFloat($(rows).eq( i ).find('td:eq(18)').html().replace(',', '').replace('$', ''));
	                
	                grouptax1 += parseFloat($(rows).eq(i).find('td:eq(19)').html().replace(/[^0-9.]/g, ''));
	                totaltax1 += parseFloat($(rows).eq(i).find('td:eq(19)').html().replace(/[^0-9.]/g, ''));
	                	                
	                groupTotalPrice += parseFloat($(rows).eq( i ).find('td:eq(20)').html().replace(',', '').replace('$', ''));
	                totalPrice += parseFloat($(rows).eq( i ).find('td:eq(20)').html().replace(',', '').replace('$', ''));

	                groupTotalPrice1 += parseFloat($(rows).eq(i).find('td:eq(21)').html().replace(/[^0-9.]/g, ''));
	                totalPrice1 += parseFloat($(rows).eq(i).find('td:eq(21)').html().replace(/[^0-9.]/g, ''));
	                
	                currentRow = i;
	            } );
	            if (groupEnd > 0 || rows.length == (currentRow + 1)){
	            	$(rows).eq( currentRow ).after(
	                        '<tr class="group"><td colspan=' + groupRowColSpan + '></td><td>Grand Total</td><td class="t-right">' + parseFloat(totalQty).toFixed(3) + '</td><td></td><td></td><td></td><td></td><td></td><td class="t-right">' + parseFloat(totalCost).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(totalCost1).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(totaltax).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(totaltax1).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(totalPrice).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(totalPrice1).toFixed(<%=numberOfDecimal%>) + '</td><td></td><td></td></tr>'
                );
            	$(rows).eq( currentRow ).after(
            			'<tr class="group"><td colspan="' + groupRowColSpan + '"></td><td>Total</td><td class="t-right">' + parseFloat(groupTotalQty).toFixed(3) + '</td><td></td><td></td><td></td><td></td><td></td><td class="t-right">' + parseFloat(groupTotalCost).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(groupTotalCost1).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(grouptax).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(grouptax1).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(groupTotalPrice).toFixed(<%=numberOfDecimal%>) + '</td><td class="t-right">' + parseFloat(groupTotalPrice1).toFixed(<%=numberOfDecimal%>) + '</td><td></td><td></td></tr>'
                );
	        ...
	        "footerCallback": function(row, data, start, end, display) {
	            ...
	            // Update footer
	           /*  $(api.column(10).footer()).html(parseFloat(recvQty).toFixed(<%=numberOfDecimal%>));
	            $(api.column(14).footer()).html(parseFloat(recvCostVal).toFixed(<%=numberOfDecimal%>));
				...
				$(api.column(10).footer()).html(parseFloat(groupTotalQty).toFixed(<%=numberOfDecimal%>));
	            $(api.column(14).footer()).html(parseFloat(groupTotalCost).toFixed(<%=numberOfDecimal%>));
	            $(api.column(16).footer()).html(parseFloat(grouptax).toFixed(<%=numberOfDecimal%>));
	            $(api.column(17).footer()).html(parseFloat(groupTotalPrice1).toFixed(<%=numberOfDecimal%>));
	          }
		});
...
function getTable(){
 return  '<TABLE WIDTH="90%"  border="0" cellspacing="1" cellpadding = 2 align = "center">'+
         ...
         '<TH><font color="#ffffff" align="right"><b>Unit Cost</TH>'+
         '<TH><font color="#ffffff" align="left"><b>Tax%</TH>'+
         '<TH><font color="#ffffff" align="left"><b>Receive Cost</TH>'+
         '<TH><font color="#ffffff" align="left"><b>Tax</TH>'+
         '<TH><font color="#ffffff" align="right"><b>Total</TH>'+
         '<TH><font color="#ffffff" align="right"><b>User</TH>'+
...
// document.getElementById('VIEW_RESULT_HERE').innerHTML =  getTable()+'</TABLE>';
 ...
 $(document).ready(function(){
	 var ALLOWCATCH_ADVANCE_SEARCH = '<%=ALLOWCATCH_ADVANCE_SEARCH%>';
	    if(ALLOWCATCH_ADVANCE_SEARCH==='1'){
	 //getLocalStorageValue('GoodsReceiptSummaryWithCost_FROMDATE', $('#FROM_DATE').val() , 'FROM_DATE');
	 //getLocalStorageValue('GoodsReceiptSummaryWithCost_TODATE', '', 'TO_DATE');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_CUSTOMER', '', 'CUSTOMER');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_ORDERNO', '', 'ORDERNO');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_ITEM', '', 'ITEM');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_PRD_CLS_ID', '', 'PRD_CLS_ID');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_PRD_TYPE_ID', '', 'PRD_TYPE_ID');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_PRD_BRAND_ID', '', 'PRD_BRAND_ID');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_PRD_DEPT_ID', '', 'PRD_DEPT_ID');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_ORDERTYPE', '', 'ORDERTYPE');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_LOC', '', 'LOC');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_BATCH', '', 'BATCH');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_LOC_TYPE_ID', '', 'LOC_TYPE_ID');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_LOC_TYPE_ID2', '', 'LOC_TYPE_ID2');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_LOC_TYPE_ID3', '', 'LOC_TYPE_ID3');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_FILTER', '', 'FILTER');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_REASONCODE', '', 'REASONCODE');
	 getLocalStorageValue('GoodsReceiptSummaryWithCost_GRNO', '', 'grno');
	    }
 	 onGo();
    $('.Show').click(function() {
	    $('#target').show(500);
