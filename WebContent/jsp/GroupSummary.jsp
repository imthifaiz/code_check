...
<%
String title = "User Access Rights Group Summary";
%>
...
<script language="javascript">
 ...
 function onGo(){
	 document.form1.action="../useraccess/invsummary";
...
<%
	...
	if(systatus.equalsIgnoreCase("INVENTORY")) {
		displaySummaryNew = ub.isCheckValinv("newuaMaintAcct", plant,userName);
		displaySummaryLink = ub.isCheckValinv("summarylnkuaMaintAcct", plant,userName);
		displaySummaryEdit = ub.isCheckValinv("edituaMaintAcct", plant,userName);
	}
	...
	if (PGaction.equalsIgnoreCase("View")) {
		try {
			...
			invQryList = _userBean
					.getGroupListSummary(ht, PLANT, GROUP);
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>User Access Rights Group Summary</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 28.02.2022 --> 
            <div class="box-header menu-drop">
              ...
              <button type="button" class="btn btn-default" onClick="window.location.href='../useraccess/invnew'" style="background-color: rgb(214, 72, 48);color: rgb(255, 255, 255);" >+ New</button>&nbsp;
              <%}%>
              </div>
		</div>
...
<form class="form-horizontal" name="form1" method="post" action="../useraccess/invsummary">
<input type="hidden" name="xlAction" value="">
<input type="hidden" name="PGaction" value="View">
		
<div class="form-group">
<label class="control-label col-sm-4" for="Group">User Access Rights Group</label>
<div class="col-sm-4" >
<div class="input-group">
<INPUT name="GROUP"   id="GROUP" type = "TEXT" value="<%=GROUP%>" size="20" MAXLENGTH=20 class="form-control">
<span class="select-icon"  onclick="$(this).parent().find('input[name=\'GROUP\']').focus()"><i class="glyphicon glyphicon-menu-down"></i></span>
<!-- <span class="btn-danger input-group-addon" onClick="javascript:popUpWin('../jsp/group_list.jsp?GROUP='+form1.GROUP.value);">
...
<table id="table" class="table table-bordered table-hover dataTable no-footer "
   ...
   <thead style="background: #eaeafa;text-align: center">  
          <tr>  
            <th >S/N</th>  
            <th>User Access Rights Group Name</th>
            <th>Edit</th>       
          </tr>  
        </thead> 
	...
	%>
	  <TR>
		...
		<TD align="center"><a href="../useraccess/invdetail?action=View&LEVEL_NAME=<%=(String) lineArr.get("LEVEL_NAME")%>")%><%=(String) lineArr.get("LEVEL_NAME")%></a></TD>
		<%}else{%>
		<TD align="center"><%=(String) lineArr.get("LEVEL_NAME")%></TD>
		<% }%>
		<% if (displaySummaryEdit) { %>
		<%if(!lname.equalsIgnoreCase("DefaultGroup")){%>
		<TD align="center"><a href="../useraccess/invedit?action=View&LEVEL_NAME=<%=(String) lineArr.get("LEVEL_NAME")%>")%><i class="fa fa-pencil-square-o"></i></a></TD>
		<%}else{ %>
		<TD></TD>
		<%} %>
		<% }else{ %>
