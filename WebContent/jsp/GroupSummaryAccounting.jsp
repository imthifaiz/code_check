...
<%
String title = "Accounting User Access Rights Summary";
%>
...
function onGo(){
	document.form1.action="../useraccess/summary";
...
<%
	...
	if(systatus.equalsIgnoreCase("ACCOUNTING")) {
		displaySummaryNew = ub.isCheckValAcc("newuaMaintAcct", plant,userName);
		displaySummaryLink = ub.isCheckValAcc("summarylnkuaMaintAcct", plant,userName);
		displaySummaryEdit = ub.isCheckValAcc("edituaMaintAcct", plant,userName);
	}	
	...
	if (PGaction.equalsIgnoreCase("View")) {
		try {
			...
			invQryList = _userBean
					.getGroupListSummaryAccounting(ht, PLANT, GROUP);
	 ...
	 <div class="box"> 
	  <!-- Thanzith Modified on 23.02.2022 -->
            <ul class="breadcrumb backpageul">      	
                ...
                <li><label>Accounting User Access Rights Summary</label></li>                                   
            </ul>             
    <!-- Thanzith Modified on 23.02.2022 -->
            <div class="box-header menu-drop">
              ...
              <button type="button" class="btn btn-default" onclick="window.location.href='../useraccess/new'"  style="background-color: rgb(214, 72, 48);color: rgb(255, 255, 255);" >+ New</button>&nbsp;
              <% } %>
              </div>
		</div>
...
<form class="form-horizontal" name="form1" method="post" action="../useraccess/summary">
<input type="hidden" name="xlAction" value="">
<input type="hidden" name="PGaction" value="View">
		
<div class="form-group">
<label class="control-label col-sm-4" for="Group">Accounting User Access Rights</label>
...
<!-- <span class="btn-danger input-group-addon" onClick="javascript:popUpWin('../jsp/group_list_accounting.jsp?GROUP='+form1.GROUP.value);"> -->
...
<table id="table" class="table table-bordered table-hover dataTable no-footer "
   ...
   <thead style="background: #eaeafa;text-align: center">  
          <tr>  
            <th >S/N</th>  
            <th>Accounting User Access Rights Name</th>
            <th>Edit</th>       
          </tr>  
        </thead> 
	...
	%>
	  <TR>
		...
		<TD align="center"><a href="../useraccess/detail?action=View&LEVEL_NAME=<%=(String) lineArr.get("LEVEL_NAME")%>")%><%=(String) lineArr.get("LEVEL_NAME")%></a></TD>
		<%}else{%>
		<TD align="center"><%=(String) lineArr.get("LEVEL_NAME")%></TD>
		<% } %>
		<% if (displaySummaryEdit) { %>
		<%if(!lname.equalsIgnoreCase("DefaultGroup")){%>
		<TD align="center"><a href="../useraccess/edit?action=View&LEVEL_NAME=<%=(String) lineArr.get("LEVEL_NAME")%>")%><i class="fa fa-pencil-square-o"></i></a></TD>
		<%}%>
		<% }else{ %>
