...
String res        = "",action     =   "",pitem="",pbatch="",citem="",cbatch="",qty="",EMP_ID="",EMP_NAME="",EMP_LNAME="",LOC_ID="",
...
<!--  <table  border="0" width="100%" cellspacing="1" cellpadding="2"  bgcolor="" align="center">
 ...
 <div class="box-header menu-drop">
              <h2>
              <small>Employee/Location/Remarks/Reason Code</small>
              </h2>
		</div>
      ...
      <div class="col-sm-2">
         <div class="input-group">    
    		<input class="form-control" type="TEXT" size="20" MAXLENGTH=50 name="ITEM" id="ITEM"	value="<%=pitem%>"	onkeypress="if((event.keyCode=='13') && ( document.form.ITEM.value.length > 0)){validateProduct();}">
   		 	<span class="input-group-addon"  onClick="javascript:popUpWin('KittingPitemList.jsp?ITEM='+form.ITEM.value+'&TYPE=KITCHECK');">
...
<div class=form-inline>
   <label class="control-label col-sm-3" for="Qty">Qty:</label>
      <div class="col-sm-3">    
       <input class="form-control" name="PARENTQTY" type="TEXT" id="PARENTQTY" value="1" size="4" MAXLENGTH=50>       
       </div>
           </div>
       </div>
      ...
      <div class="col-sm-3">
         <div class="input-group">    
    		<input class="form-control" type="TEXT" size="20" MAXLENGTH=50 name="CITEM" id="CITEM" value="<%=citem%>" onkeypress="if((event.keyCode=='13') && ( document.form.CITEM.value.length > 0)){ValidateChildProduct();}">
   		 	<span class="input-group-addon" onClick="javascript:popUpWin('KittingchilditemList.jsp?PITEM='+form.ITEM.value+'&CITEM='+form.CITEM.value+'&TYPE=KITCHECK');">
      ...
      <div class="col-sm-3">
         <div class="input-group">    
    		<input class="form-control" name="BATCH_1" id="BATCH_1" type="TEXT" value="<%=cbatch%>" size="20" MAXLENGTH=40
   		 	...
   		 	<span class="input-group-addon" onClick="javascript:popUpWin('batch_list_kitting.jsp?PITEM='+form.ITEM.value+'&PBATCH='+form.BATCH_0.value+'&CITEM='+form.CITEM.value+'&BATCH='+form.BATCH_1.value+'&TYPE=KITCHECK');">
   		 	<a href="#" data-toggle="tooltip" data-placement="top" title="Batch/Serial Details">
   		 	<i class="glyphicon glyphicon-log-in" style="font-size: 20px;"></i></a></span>
  		</div>        
       </div>
...
<div class="form-inline">
 ...
 <div class="form-group">
  ...
  <div class="form-group">        
     ...
     <button type="button" class="Submit btn btn-default" onClick="window.location.href='inPremises.jsp'"><b>Back</b></button>&nbsp;&nbsp;
      <button type="button" class="Submit btn btn-default" value="Check" onClick="onAdd()"><b>Check</b></button>&nbsp;&nbsp;
 ...
 <table border="0" cellspacing="0" cellpadding="0"  WIDTH="60%"  align = "center">
  ...
  <div class="form-group">       
     <div class="col-sm-12" align="center">
      <button type="button" class="Submit btn btn-default" value="Uncheck" onClick="onDelete()"><b>Uncheck</b></button>&nbsp;&nbsp;
       </div>
          </div> 
...
function validateProduct() {
	...
	$.ajax( {
		...
		ITEM : productId,
		PLANT : "<%=plant%>",
		action : "VALIDATE_KITCHECK_PARENTITEM"
					},
					dataType : "json",
					success : function(data) {
						if (data.status == "100") {
...
function ValidateChildProduct() {
	...
	var childitem = document.getElementById("CITEM").value;
			...
			if (batch == "" || batch.length == 0) {
				...
				$.ajax( {
					...
					data : {
						...
						BATCH : batch,
						PLANT : "<%=plant%>",
						action : "VALIDATE_KITCHECK_BATCH"
						},
						dataType : "json",
						success : function(data) {
							...
							if (data.status == "100") {
								var resultVal = data.result;
								document.getElementById("BATCH"+"_"+index).value = resultVal.BATCH;
...
function onAdd() {
	...
	var scanbatch = trim((document.getElementById("BATCH_1").value).toUpperCase());
	var scanqty = parseInt(document.getElementById("QTY").value);
	for ( var i = 1; i < len; i++) {
		
		var item = (document.getElementById("citem_" + i).innerText).toUpperCase();
		var item = trim(item);
		var batch = (document.getElementById("batch_" + i).innerText).toUpperCase();
		var batch = trim(batch);
		var kitqty = parseInt(document.getElementById("qty_" + i).innerText) ;
		var chkqty = parseInt(document.getElementById("chkqty_" + i).innerText) ;
		
		var balqty = parseInt(scanqty)+parseInt(chkqty);	
			
		if(scanitem == item && scanbatch==batch){
			if(balqty>kitqty)
			{
				alert("Scan Qty Exceeds Kit Qty for the Product Scanned.");
    ...
    $.ajax({type: "POST",url: urlStr, data: {EMP_ID:emp,LOC_ID:loc,LOC_TYPE_ID:loctype,REMARKS:remarks,TYPE:"KITCHECK",REASONCODE:rsncode,ITEM:pitem,BATCH_0:pbatch,CITEM:citem,BATCH_1:cbatch,QTY:qty,action: "ADD_KITCHECK",PLANT:"<%=plant%>"},dataType: "json", success: callback });
    document.form.CITEM.value = "";
...
function onGo(index) {
    ...
    // Call the method of JQuery Ajax provided
    $.ajax({type: "POST",url: urlStr, data: {ITEM:product,LOC_ID:loc,BATCH_0:pbatch,TYPE:"KITCHECK",action: "VIEW_KITTING_DETAILS",PLANT:"<%=plant%>"},dataType: "json", success: callback });
...
function onDelete()
{
	 ...
	 for (var i = 0; i < len ; i++)
   {
	 ...
	 var chkmsg = confirm("Are you sure you would like to do kit uncheck?");
		if (chkmsg) {
	  	  document.form.action="/track/ProductionBomServlet?action=DO_KITUNCHECK";
...
function callback(data){
	...
	if(!errorBoo){
        ...
        $.each(data.kitting, function(i,item){
                   
        	outPutdata = outPutdata+item.KITTINGDATA;
                    	ii++;
...
function getTable(){
        return '<TABLE class="table" id="tabledata">'+
               '<thead style="background:#eaeafa">'+
        	   '<tr>'+
        	   '<th width="5%">Select</th>'+
        		'<th width="5%">No</th>'+
        		'<th width="11%">Child Prod ID</th>'+
        		'<th width="15%">Child Prod Desc</th>'+
        		'<th width="15%">Child Prod Detail Desc</th>'+
        		'<th width="12%">Child Batch/Serial No</th>'+
        		'<th width="5%">KIT Qty</th>'+
        		'<th width="12%">Check Batch/Serial No</th>'+
        		'<th width="5%">Qty</th>'+
