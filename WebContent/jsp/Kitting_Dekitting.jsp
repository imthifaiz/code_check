...
<%
String title = "Kitting Dekitting";
%>
...
<div class="form-inline">
  <label class="control-label col-sm-3" for="Batch/Serial Number">Batch/Serial Number:</label>
      <div class="col-sm-3">
         <div class="input-group">    
    		<input class="form-control" name="BATCH_0" id="BATCH_0" type="TEXT" value="<%=pbatch%>" size="20" MAXLENGTH=40 onkeypress="if((event.keyCode=='13') && ( document.form.BATCH_1.value.length > 0)){validateBatch(1);}">
 ...
 <div class=form-inline>
   <div class="col-sm-offset-3 col-sm-3">
    <button type="button" class="Submit btn btn-default" onclick="generateBatch(0);"  value="Generate Batch" name="actionBatch"><b>Generate Batch</b></button>&nbsp;&nbsp;
       </div>
          </div>
       </div>
...
function onAdd() {
    ...
    $.ajax({type: "POST",url: urlStr, data: {EMP_ID:emp,LOC_ID:loc,LOC_TYPE_ID:loctype,REMARKS:remarks,REASONCODE:rsncode,ITEM:pitem,BATCH_0:pbatch,CITEM:citem,BATCH_1:cbatch,QTY:qty,PARENTQTY:pqty,action: "ADD_KITTING",PLANT:"<%=plant%>"},dataType: "json", success: callback });
    document.form.CITEM.value = "";
...
function onGo(index) {
    ...
    // Call the method of JQuery Ajax provided
    $.ajax({type: "POST",url: urlStr, data: {ITEM:product,LOC_ID:loc,BATCH_0:pbatch,action: "VIEW_KITTING_DETAILS",PLANT:"<%=plant%>"},dataType: "json", success: callback });
...
function getTable(){
        return '<TABLE class="table" id="tabledata" WIDTH="100%" align = "center">'+
        	    ...
        	    '<th width="5%">Select</th>'+
         		'<th width="5%">No</th>'+
         		'<th width="13%">Child Prod ID</th>'+
         		'<th width="20%">Child Prod Desc</th>'+
         		'<th width="20%">Child Prod Detail Desc</th>'+
