...
function onPrint()
{
   ...
   var formData = $('form#LabelPrintProductForm').serialize();
	$.ajax({
      ...
      type: 'post',
      url: '/track/LabelPrintServlet?action=PrintLocation', 
      dataType:'html',
      data:  formData,
      async:    false,
...
function onGo(){
   ...
   if(DIRTYPE != null     && DIRTYPE != "") { flag = true;}
  document.form1 .action="LabelPrintLoc.jsp";
  document.form1 .submit();
}
...
<title>Label Print (Location)</title>
</head>
...
<%
	...
	String  fieldDesc="",errorDesc="",expiredate="",item="",batch="",REASONCODE="",chkString ="",MODULETYPE ="";
	...
	String PLANT 	= "",loc= "",locdesc= "",loctype = "",PrintStatus="";
	String action   = su.fString(request.getParameter("action")).trim();
	String REFNO="",PRINTTYPE="",LOC="",LOC_TYPE_ID ="",ACTIVE="",PRINTSTATUS="",RECS_PER_PAGE="";
	...
	LOC_TYPE_ID     = _strUtils.fString(request.getParameter("LOC_TP_ID"));
  	...
  	try{
       ...
       // int start =  Integer.parseInt(currentPage);
        ...
        //locQryList= itemUtil.queryItemMstForSearchCriteriaNew(ITEM,ITEMDESC ,PRD_CLS_ID,PRD_TYPE_ID,PRD_BRAND_ID,plant,extcond,start,end);
         ...
         locQryList=      _LocUtil.getLocDetails(LOC,PLANT," ",ht);
...
<input type="hidden" name="PGaction" value="View">
 ...
 <TABLE border="0" width="100%" cellspacing="0" cellpadding="0" align="center" bgcolor="#dddddd">
    <TR>
      <TH BGCOLOR="#000066" COLSPAN="20"><font color="white">Label Print (Location)</font></TH>
    </TR>
  </TABLE>
  <br>
   <Center>
  ...
  <TABLE border="0" width="70%" height = "12%" cellspacing="0" cellpadding="0" align="center" bgcolor="#dddddd">
   
        <TR align="Center">
        <TH ALIGN="right" width="15%">Location Type Desc or ID:</TH>
         <TD width="20%" align="center"><INPUT name="LOC_TP_ID" type = "TEXT" value="<%=LOC_TYPE_ID%>" size="20"  MAXLENGTH=20>
          <a href="#" onClick="javascript:popUpWin('LocTypeList.jsp?LOC_TYPE_ID='+form1.LOC_TP_ID.value+'&TYPE=locsummary');"><img src="images/populate.gif" border="0"></a>
          </TD>
          <TH ALIGN="right" width="15%">Location Desc or ID: </TH>
         <TD width="20%" align="center"><INPUT name="LOC" type = "TEXT" value="<%=LOC%>" size="20"  MAXLENGTH=20>
          <a href="#" onClick="javascript:popUpWin('loc_list_inventory.jsp?LOC='+form1.LOC.value);">
           <img src="images/populate.gif" border="0"/>
          </TD>
          <TD ALIGN="center" width="10%"> 
          <input type="button" value="View"   align="left" onClick="javascript:return onGo();">&nbsp;&nbsp;&nbsp;&nbsp; <!-- <input type='button' value='Export Master Data'  onclick="ExportReport();" /> -->  </TD>
        </TR>
...
<table BORDER = "1" WIDTH = "92%" align="center" bgcolor="#dddddd" >
  ...
  <TABLE WIDTH="95%"  border="0" cellspacing="1" cellpadding = 2 align = "center">
    ...
    <TR BGCOLOR="#000066">
       ...
       <%
		  ...
		  <%}%>
			...
			<%	
			 for (int iCnt =0; iCnt<locQryList.size(); iCnt++){
				    int iIndex = iCnt + 1;
				     ...
				     locdesc= (String)lineArr.get("LOCDESC");
				     loctype= (String)lineArr.get("LOC_TYPE_ID");  
		            chkString  =loc+",,,"+StrUtils.replaceCharacters2Send(locdesc)+",,,"+StrUtils.replaceCharacters2Send(loctype); 
		             String lineno=String.valueOf(iIndex)+"_"+loc;
			...
			<TD align="center" width="13%"><%=loc%></TD>
			<input type="hidden" name = "LOC_<%=lineno%>" id = "LOC_<%=lineno%>"   value = <%=loc%> ></input>
			 
			 
			<TD align="center" width="15%"><%=locdesc%></TD>
            <input type="hidden" name = "LOCDESC_<%=lineno%>" id = "LOCDESC_<%=lineno%>"  value = <%=locdesc%> ></input>
			
			
			
			<TD align="center" width="10%"><%=loctype%></TD>
			<input type="hidden" name = "LOCTYPE_<%=lineno%>" id = "LOCTYPE_<%=lineno%>"  value = <%=loctype%> ></input>
   ...
   <INPUT type="Hidden" name="DIRTYPE" value="LOCATION">
