...
var pono=document.form.DONO.value;
   ...
   var noofcheckbox = document.form.chkdDoNo.length;
    ...
    if(form.chkdDoNo.length == undefined)
    {
            ...
            {
               alert("Please Select Product For Pick/Issue");
               return false;
            }
    ...
    }else
    {
             ...
             for (i = 0; i < noofcheckbox; i++ )
              ...
              document.form.action ="/track/OrderReceivingServlet?";
...
<form class="form-horizontal" name="form" method="post"action="/track/LoanOrderPickingServlet?">
 ...
 <h2><small><%=fieldDesc%></small></h2>
      ...
      onkeypress="if((event.keyCode=='13') && ( document.form.DONO.value.length > 0)){loadLoanOrderDetails();}" />
  		...
  		<div class="col-sm-1 ">   
  		<button type="button" class="Submit btn btn-default" onClick="viewLoanOrders();"><b>View</b></button>
  		</div>
  		</div>  
 		</div>
        ...
        <div class="col-sm-4">
        <INPUT type="TEXT" size="30" class="form-control" MAXLENGTH="20" readonly name="JOB_NUM" value="<%=jobNum%>">
       </div>
 		</div>
 		 
        <div class="form-group">
        <label class="control-label col-sm-4" for="from Location">Order Date:</label>
        <div class="col-sm-4">
        <INPUT type="TEXT" class="form-control" readonly 	size="30" MAXLENGTH="10" name="DELDATE" value="<%=deldate%>" />
        </div>
        ...
        <div class="col-sm-4">
       <INPUT type="TEXT" size="30" class="form-control"	MAXLENGTH="20" readonly name="FRLOC" value="<%=frLoc%>" />
        </div>
    	<!-- <div class="form-inline">
        ...
        <!-- </div>
 		...
 		<div class="form-group">
        <label class="control-label col-sm-4" for="Order Date">To Location:</label>
        <div class="col-sm-4">
        <INPUT type="TEXT" size="30" MAXLENGTH="20"	class="form-control" readonly name="TOLOC" value="<%=toLoc%>" />
        </div>
    	<!-- <div class="form-inline">
        ...
        <!-- </div>
         ...
         <tr>
       	...
       	<%
						if (pono.length() > 0) {
							ArrayList al = _loanUtil.loadLoanOrderDetailstoPickorRecv(
									plant, pono, "P");
							if (al.size() == 0) {
								AFLAG = "";
							}
							if (al.size() > 0) {
								...
								for (int i = 0; i < al.size(); i++) {
									...
									chkString = ordno + "," + ordlnno + "," + item + ","
											+ StrUtils.replaceCharacters2Send(desc) + "," + qtyor + "," + qtyis + ","
											+ qtyrc + "," + sUserId + "," + loc + ","
											+ loc1 + "," + custname + "," + uom+ "," + UOMQTY;
					...
					<TR bgcolor="<%=bgcolor%>">
						...
						<TD width="10%" align="center"><font color="black"><%=(String) m.get("ordlnno")%></font></TD>
					...
					<TR>
						<TD align="center" colspan="9">Data's Not Found For Loan
						Order Picking</TD>
					</TR>
					<%
						}
        ...
        <INPUT type="Hidden" name="AFLAG" value="<%=AFLAG%>">
      	...
      	<button type="button" class="Submit btn btn-default" onClick="if(onPicking(document.form)){submitForm();}"><b>Pick/Issue</b></button>&nbsp;&nbsp;
...
function viewLoanOrders(){
			document.form.action="/track/LoanOrderPickingServlet?action=View";
			//document.form.action.value ="View";
            document.form.submit();
		}
function submitForm(){
	document.form.action="/track/LoanOrderPickingServlet?action=Pick/Issue";
    document.form.submit();
}
