...
<%
String title = "Rental Order Summary By Price";
%>
...
var subWin = null;
  ...
  function ExportReport()
  {
    ...
    document.form1.action="/track/ReportServlet?action=InvExportExcelLoanOrderSummaryByPrice";
...
function onView(){
    ...
    /*  if(flag == "false"){ alert("Please Enter Order No"); return false;} */
     ...
     document.form1.action="LoanOrderSummaryByPrice.jsp";
     document.form1.submit();
}
...
<div class="box-body">
    <FORM class="form-horizontal" name="form1" method="post" action="LoanOrderSummaryByPrice.jsp">
 ...
 <div class="form-group">
  ...
  <th style="background-color: #eaeafa; color:#333; text-align: left;">Customer Name</th>
    ...
    <th style="background-color: #eaeafa; color:#333; text-align: left;">UOM</th>
        <th style="background-color: #eaeafa; color:#333; text-align: left;">Rental Price</th>
        <th style="background-color: #eaeafa; color:#333; text-align: left;">Order Price</th>
        <th style="background-color: #eaeafa; color:#333; text-align: left;">Tax%</th>
        <th style="background-color: #eaeafa; color:#333; text-align: left;">Tax</th>
        <th style="background-color: #eaeafa; color:#333; text-align: left;">Total</th>
   ...
   <%
          ...
          for (int iCnt =0; iCnt<QryList.size(); iCnt++){
            ...
            String bgcolor = ((iCnt == 0) || (iCnt % 2 == 0)) ? "#FFFFFF"  : "#dddddd";
             ...
             rowData[rowData.length] = '<%=StrUtils.formatNum((String)lineArr.get("PRODGST"))%>';
  	...
  	<%  if (pickQty>0){btnDisabled="";}%>
  	 $(document).ready(function(){
  		 ...
  		 for(var rowIndex = 0; rowIndex < tableData.length; rowIndex ++){
  			 tableData[rowIndex][8] = parseFloat(tableData[rowIndex][8]).toFixedSpecial1(3);
  			 tableData[rowIndex][9] = parseFloat(tableData[rowIndex][9]).toFixedSpecial1(3);
  			 tableData[rowIndex][10] = parseFloat(tableData[rowIndex][10]).toFixedSpecial1(3);
  			 tableData[rowIndex][12] = parseFloat(tableData[rowIndex][12]).toFixedSpecial1(<%=com.track.gates.DbBean.NOOFDECIMALPTSFORCURRENCY%>);
		            ...
		            var totalPrice = 0;
  		            ...
  		            api.column(groupColumn, {page:'current'} ).data().each( function ( group, i ) {
  		                if ( last !== group ) {
  		                	if (i > 0) {
  		                		$(rows).eq( i ).before(
  				                        '<tr class="group"><td colspan="' + groupRowColSpan + '"></td><td>Total</td><td>' + parseFloat(groupTotalPickQty).toFixed(5).replace(/\.?0+$/, '') + '</td><td>' + parseFloat(groupTotalIssueQty).toFixed(5).replace(/\.?0+$/, '') + '</td><td></td><td>' + parseFloat(groupTotalIssuePriceQty).toFixed(5).replace(/\.?0+$/, '') + '</td><td>' + parseFloat(groupTotalPrice).toFixed(5).replace(/\.?0+$/, '') + '</td><td></td><td></td><td></td></tr>'
  				                    );
  		                	}
  		                    last = group;
  		                    groupEnd = i;    
  		                ...
  		                groupTotalPickQty += parseFloat($(rows).eq( i ).find('td:eq(12)').html().replace(',', '').replace('$', ''));
  		                totalPickQty += parseFloat($(rows).eq( i ).find('td:eq(12)').html().replace(',', '').replace('$', ''));
  		                groupTotalIssueQty += parseFloat($(rows).eq( i ).find('td:eq(13)').html().replace(',', '').replace('$', ''));
  		                totalIssueQty += parseFloat($(rows).eq( i ).find('td:eq(13)').html().replace(',', '').replace('$', ''));
  		              ...
  		              groupTotalPrice += parseFloat($(rows).eq( i ).find('td:eq(16)').html().replace(',', '').replace('$', ''));
  		            totalPrice += parseFloat($(rows).eq( i ).find('td:eq(16)').html().replace(',', '').replace('$', ''));
  		            ...
  		            if (groupEnd > 0 || rows.length == (currentRow + 1)){
  		            	$(rows).eq( currentRow ).after(
  		                        '<tr class="group"><td colspan=' + groupRowColSpan + '></td><td>Grand Total</td><td>' + parseFloat(totalPickQty).toFixed(5).replace(/\.?0+$/, '') + '</td><td>' + parseFloat(totalIssueQty).toFixed(5).replace(/\.?0+$/, '') + '</td><td></td><td>' + parseFloat(totalIssuePriceQty).toFixed(5).replace(/\.?0+$/, '') + '</td><td>' + parseFloat(totalPrice).toFixed(5).replace(/\.?0+$/, '') + '</td><td></td><td></td><td></td></tr>'
  	                   );
  	               	$(rows).eq( currentRow ).after(
  		                        '<tr class="group"><td colspan=' + groupRowColSpan + '></td><td>Total</td><td>' + parseFloat(groupTotalPickQty).toFixed(5).replace(/\.?0+$/, '') + '</td><td>' + parseFloat(groupTotalIssueQty).toFixed(5).replace(/\.?0+$/, '') + '</td><td></td><td>' + parseFloat(groupTotalIssuePriceQty).toFixed(5).replace(/\.?0+$/, '') + '</td><td>' + parseFloat(groupTotalPrice).toFixed(5).replace(/\.?0+$/, '') + '</td><td></td><td></td><td></td></tr>'
  	                   );
