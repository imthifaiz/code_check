...
<SCRIPT LANGUAGE="JavaScript">
  ...
  function popUpWin(URL) {
    subWin = window.open(URL, 'MultiStockMove', 'toolbar=0,scrollbars=yes,location=0,statusbar=0,menubar=0,dependant=1,resizable=1,width=700,height=400,left = 200,top = 184');
  }
  function validatePO(form)//form
  {
    ...
    else if(frmRoot.TOLOC.value==frmRoot.FROM_LOC.value)
    {
       ...
       {
           if(i==checkcnt){
       document.form.action.value="/track/LocationTransferServlet?action=MultiLocationTransfer";
...
<FORM name="form" method="post" action="/track/LocationTransferServlet?action=MultiLocationTransfer"  >
  <br>
  <CENTER>
...
<br>
    ...
    <table align="center" border="0" CELLSPACING=0 width="100%" border="0" id="MULTI_ADD_PRODUCT" >
       ...
       <a href="#" onClick="javascript:popUpWin('batch_list_locationtransfer.jsp?FROM_LOC='+form.FROM_LOC.value+'&ITEMNO='+form.ITEMNO_0.value+'&BATCH'+form.BATCH_0.value+'&INDEX=0');">
         ...
         <TD width = "15%"> <INPUT name="QTY_0" type = "TEXT" id="QTY_0" value="<%=QTY%>" size="5" onkeypress="if((event.keyCode=='13') && ( this.value.length > 0)){validateQuantity(this.value,0);}" MAXLENGTH=80 >&nbsp;
         <INPUT name="UOM_0" type = "TEXT" id="UOM_0" value="<%=UOM %>" size="10"  MAXLENGTH=80 ></TD>
         
         <td width = "15%"><INPUT name="REFDET_0" type="TEXT"  id="REFDET_0" value="<%=REFDET%>"
...
function addRow() {
	...
	var availableQtyCell = row.insertCell(3);
	...
	var reasonCell = row.insertCell(6);
	...
	var invQtyCell = row.insertCell(7);
