...
<%
String title = "Sales Estimate Order Summary With Price";
%>
...
function onGo(){
  ...
  document.form1.action="../salesestimate/salesestordersumwithprice";
  document.form1.submit();
}
function storeUserPreferences(){
    storeInLocalStorage('OrderSummaryEstimate_FROMDATE', $('#FROM_DATE').val());
	storeInLocalStorage('OrderSummaryEstimate_TODATE', $('#TO_DATE').val());
	storeInLocalStorage('OrderSummaryEstimate_CUSTOMER', $('#CUSTOMER').val());
	storeInLocalStorage('OrderSummaryEstimate_ORDERNO', $('#ORDERNO').val());
	storeInLocalStorage('OrderSummaryEstimate_JOBNO', $('#JOBNO').val());
	storeInLocalStorage('OrderSummaryEstimate_ITEM', $('#ITEM').val());
	storeInLocalStorage('OrderSummaryEstimate_PRD_CLS_ID', $('#PRD_CLS_ID').val());
	storeInLocalStorage('OrderSummaryEstimate_PRD_TYPE_ID', $('#PRD_TYPE_ID').val());
	storeInLocalStorage('OrderSummaryEstimate_PRD_BRAND_ID', $('#PRD_BRAND_ID').val());
	storeInLocalStorage('OrderSummaryEstimate_PRD_DEPT_ID', $('#PRD_DEPT_ID').val());
	storeInLocalStorage('OrderSummaryEstimate_STATUS',$('#STATUS').val());
	storeInLocalStorage('OrderSummaryEstimate_EMP_NAME',$('#EMP_NAME').val());
}
var postatus =   [{
    "year": "Send Later",
    "value": "Send Later",
...
<div class="container-fluid m-t-20">
	...
	<ol class="breadcrumb backpageul">      	
                ...
                <li><label>Sales Estimate Order Summary With Price</label></li>                                   
            </ol> 
...
<div class="box-body">
<FORM class="form-horizontal" name="form1" method="post" action="OrderSummaryEstimate.jsp" >
...
<%--        rowData[rowData.length] = '<a href="/track/EstimateServlet?ESTNO=<%=(String)lineArr.get("estno")%>&Submit=View&RFLAG=3"><font color="blue"><%=(String)lineArr.get("estno")%></font></a>'; --%>
  ...
  $(document).ready(function(){
	   ...
	   /*if (document.form1.FROM_DATE.value == ''){
		   getLocalStorageValue('OrderSummaryEstimate_FROMDATE', '','FROM_DATE');
     }
	   if (document.form1.TO_DATE.value == ''){
		   getLocalStorageValue('OrderSummaryEstimate_TODATE', '','TO_DATE');
     } 
	   if (document.form1.CUSTOMER.value == ''){
			getLocalStorageValue('OrderSummaryEstimate_CUSTOMER', '','CUSTOMER');
     } 	
	   if (document.form1.ORDERNO.value == ''){
		   getLocalStorageValue('OrderSummaryEstimate_ORDERNO','','ORDERNO');
     } 
	   if (document.form1.JOBNO.value == ''){
		   getLocalStorageValue('OrderSummaryEstimate_JOBNO','', 'JOBNO');
     } 
	   if (document.form1.ITEM.value == ''){
		   getLocalStorageValue('OrderSummaryEstimate_ITEM','', 'ITEM');
     } 	
	   if (document.form1.PRD_CLS_ID.value == ''){
		   getLocalStorageValue('OrderSummaryEstimate_PRD_CLS_ID','', 'PRD_CLS_ID');
     } 	
	   if (document.form1.PRD_TYPE_ID.value == ''){
		   getLocalStorageValue('OrderSummaryEstimate_PRD_TYPE_ID','', 'PRD_TYPE_ID');
     }	
	   if (document.form1.PRD_BRAND_ID.value == ''){
		   getLocalStorageValue('OrderSummaryEstimate_PRD_BRAND_ID','', 'PRD_BRAND_ID');
     }	
	   if (document.form1.STATUS.value == ''){
			getLocalStorageValue('OrderSummaryEstimate_STATUS','','STATUS');
		}	
	   if (document.form1.EMP_NAME.value == ''){
		   getLocalStorageValue('OrderSummaryEstimate_EMP_NAME','','EMP_NAME');
		}
	   storeUserPreferences();*/
 	        ...
 	        buttons: [
 	        	{
	                ...
	                buttons: [
	  	            ...
	  	            {
	  	            	extend: 'colvis',
	  	            	columns: ':not(:eq('+groupColumn+')):not(:eq(2)):not(:eq(3)):not(:eq(5)):not(:eq(7)):not(:eq(8))'
	                  }
	  	        ],
		        ...
		        "drawCallback": function ( settings ) {
	  	        	this.attr('width', '100%');
			            ...
			            api.column(groupColumn, {page:'current'} ).data().each( function ( group, i ) {
			                ...
			                totalPickQty += parseFloat($(rows).eq( i ).find('td:eq(7)').html().replace(',', ''));
			            ...
			            if (groupEnd > 0 || rows.length == (currentRow + 1)){
			            	$(rows).eq( currentRow ).after(
			                        '<tr class="group"><td colspan=' + groupRowColSpan + '></td><td>Total</td><td>' + parseFloat(totalPickQty).toFixed(3) + '</td><td></td><td></td><td>' + parseFloat(totalIssueQty).toFixed(3) + '</td><td>' + parseFloat(totalReceiveQty).toFixed(3) + '</td><td></td><td></td><td></td><td></td><td></td><td></td></tr>'
		                    );
		                }
			        },
			        "footerCallback": function(row, data, start, end, display) {
	    	            ...
	    	            // Update footer
	    	            $(api.column(7).footer()).html(parseFloat(totalOrdVal).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>));
	    	            $(api.column(10).footer()).html(parseFloat(totalPicVal).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>));
	    	            $(api.column(11).footer()).html(parseFloat(totalIseVal).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>));
	    	          } 
	  		  });	 
	   });
