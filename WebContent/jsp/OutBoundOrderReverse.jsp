...
<!-- <title>Pick Reversal By Outbound Order</title>
...
function popUpWin(URL) {
 subWin = window.open(URL, 'OutBoundsOrderReverse', 'toolbar=0,scrollbars=yes,location=0,statusbar=0,menubar=0,dependant=1,resizable=1,width=800,height=400,left = 200,top = 184');
}
...
function onIssue(form){
    ...
    if(form.chkdDoNo.length == undefined)
    {
            ...
            {
               alert("Please Select Product For Picking Reverse");
               return false;
            }
...
pdb.setmLogger(mLogger);
   ...
   if(action.equalsIgnoreCase("View")){
      ...
      {
        fieldDesc="Details not found for Sales reverse:"+ pono;
...
<!-- Muruganantham Modified on 16.02.2022 -->
            <ol class="breadcrumb backpageul">      	
                ...
                <li>Pick Return By Sales Order</li>                                   
            </ol>             
    <!-- Muruganantham Modified on 16.02.2022 -->
 ...
 <center>
   <h2><small><%=fieldDesc%></small></h2>
       ...
       onkeypress="if((event.keyCode=='13') && ( document.form.DONO.value.length > 0)){loadOutboundOrderDetails();}"/>
   		 	<span class="input-group-addon"  onClick="javascript:popUpWin('../jsp/do_list_doreverse.jsp?DONO='+form.DONO.value);">
        ...
        <div class="col-sm-4">
        <INPUT type = "TEXT" size="30"  MAXLENGTH="20"  class = "form-control" readonly name="PERSON_INCHARGE" value="<%=su.forHTMLTag(personIncharge)%>">
        </div>
 		</div>
        ...
        <!-- </div>
         ...
         <th width="7%">PICK QTY </th>
           <th width="7%">ISSUED QTY </th>
         <th width="7%"><%=IDBConstants.UOM_LABEL%> </th>
         <th width="5%">STATUS </th>      
         </tr>
       	 </thead>
       	 <tbody style="font-size: 15px;">
       	 <% 
      ArrayList al= _DOUtil.listOutGoingReverseDODET(plant,pono);
      if(al.size()==0)
      {
  	   AFLAG="";
      }
       ...
       for(int i=0 ; i<al.size();i++)
       {
          ...
          String qtyPick=  _ShipHisDAO.getPickQty(plant,dono,dolnno,item,loc,batch);
          String qtyis=  _ShipHisDAO.getIssueQty(plant,dono,dolnno,item,loc,batch);
          String qtyreverse=  _ShipHisDAO.getReverseQty(plant,dono,dolnno,item,loc,batch);
		  ...
		  double dblqtypick =  Double.parseDouble(qtyPick) + Double.parseDouble(qtyreverse);
		  double dblqtyissue = Double.parseDouble(qtyis) + Double.parseDouble(qtyreverse);
		  ...
		  strqtyissue=Double.toString(dblqtyissue);
		  strpickedqty=Double.toString(dblqtypick);
		
		  	 
		 		  
		  double balanceqty=dblqtypick-dblqtyissue;
		  
		  if(balanceqty<0)
		  {
		     balanceqty=0;
		  }
		  String strbalqty=Double.toString(balanceqty);
			  
		  chkString  =dono+","+dolnno+","+item+","+su.replaceCharacters2Send(desc)+","+qtyor+","+strpickedqty+","+qtyis+","+sUserId+","+loc+","+batch+","+su.replaceCharacters2Send(custname)+","+strbalqty+","+uom+","+UOMQTY;
         if(!strqtyissue.equals(strpickedqty)  ){
      ...
      %>
        <TR bgcolor = "<%=bgcolor%>">
              ...
              <TD align="left" width="10%"><%=(String)m.get("loc1")%></TD>
              <TD align="left" width="10%"><%=(String)m.get("batch")%></TD>
              
              <TD align="center" width="15%"><%=StrUtils.formatNum((String)m.get("qtyor"))%></TD>
              <TD align="center" width="7%"><%=StrUtils.formatNum((String)strpickedqty)%></TD>
                <TD align="center" width="7%"><%=StrUtils.formatNum((String)strqtyissue)%></TD>
       ...
       <%}}}else {%>
             ...
             <TR> <TD align="center" colspan="11"> Data's Not Found For Reversing</TD></TR>
       <%}%>
         </tbody>
       </TABLE>
...
function submitForm(){
	document.form.action="/track/OrderIssuingServlet?action=Reverse";
    document.form.submit();
}
