...
<%
	...
	boolean displaySummaryNew=false,displaySummaryExport=false,displaySummaryEdit=false,displaySummaryPdf=false,displaySummaryPayroll=false,displaySummaryDelete=false;
	if(systatus.equalsIgnoreCase("PAYROLL")){
		displaySummaryExport = ub.isCheckValPay("pexportpayroll", plant,username);
		displaySummaryNew = ub.isCheckValPay("pnewpayroll", plant,username);
		displaySummaryEdit = ub.isCheckValPay("peditpayroll", plant,username);
		displaySummaryPdf = ub.isCheckValPay("payrollprint", plant,username);
		displaySummaryPayroll = ub.isCheckValPay("pdetailpayroll", plant,username);
		}
...
<div class="container-fluid m-t-20">
	...
	<!-- Muruganantham Modified on 16.02.2022 -->
            <ul class="breadcrumb backpageul">      	
                ...
                <li><label>Payroll Summary</label></li>                                   
            </ul>             
    <!-- Muruganantham Modified on 16.02.2022 -->
			...
			<div class=" pull-right">
				...
				<% if (displaySummaryNew) { %>	
					<button type="button" class="btn btn-default"
						onClick="window.location.href='../payroll/new'"
						        	...
						        	if(typeof data.PAYROLL[0].ID === 'undefined'){
						        		return [];
						        			...
						        			for(var dataIndex = 0; dataIndex < data.PAYROLL.length; dataIndex ++){
						        				...
						        				<% if (displaySummaryPayroll) { %>	
						        				data.PAYROLL[dataIndex]['PAYROLL'] = '<a href="../payroll/detail?ID=' +lineno+'">'+payno+'</a>';
						        				<% } else { %>	
						        				data.PAYROLL[dataIndex]['PAYROLL'] = '<a style="pointer-events: none; cursor: default; color: darkgrey;" href="#">'+payno+'</a>';
						        				<% } %>
						        				if(data.PAYROLL[dataIndex]['STATUS']=='PAID'||data.PAYROLL[dataIndex]['STATUS']=='Paid'){
							        				data.PAYROLL[dataIndex]['STATUS'] = '<span style="color:rgb(2,151,69);text-transform:uppercase;">'+data.PAYROLL[dataIndex]['STATUS']+'</span>';
						        				}else{
								        			data.PAYROLL[dataIndex]['STATUS'] = '<span style="color:rgb(38,141,221);text-transform:uppercase;">'+data.PAYROLL[dataIndex]['STATUS']+'</span>';
						        				}
						        			}
						        		return data.PAYROLL;
						        	}
