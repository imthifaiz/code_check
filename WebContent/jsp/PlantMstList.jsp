	...
	<%
		...
		String plant = "", plantdesc = "",rcbno="", startdate = "", noofcatalogs="",expirydate = "", sdate = "", edate = "", actexpirydate = "", name = "", desgination = "", telno = "", hpno = "", email = "", add1 = "", add2 = "", add3 = "", add4 = "", remarks = "", fax = "", county = "", zip = "",
                sales="",sales_percent="",sales_fr_dollars="",sales_fr_cents="",enquiry_fr_dollars="",enquiry_fr_cents="",currencyCode="",noofsignatures="",state="",ENABLE_INVENTORY="",ENABLE_ACCOUNTING="",istaxreg="",reportbasis="",
                		DECIMALPRECISION="",IMAGEPATH="",
                		FISCALYEAR="",PAYROLLYEAR="",REGION="",WEBSITE="",FACEBOOK="",TWITTER="",LINKEDIN="",SKYPE="",ENABLEPAYROLL="0",FYEAR="",PYEAR="",sCountryCode="",PLANTID="";
		try {
	...
	<TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
           ...
           window.opener.form1.NOOFSIGNATURES.value='<%=noofsignatures%>';  
	        ...
	        window.opener.form1.EDIT_STATE.value='<%=state%>';
           ...
           window.opener.form1.ENABLE_PAYROLL.checked=(window.opener.form1.ENABLE_PAYROLL && '1' == '<%=ENABLEPAYROLL%>');           
           window.opener.form1.decimal_precision.value='<%=DECIMALPRECISION%>';
	       window.opener.form1.IMAGE_UPLOAD.value='<%=IMAGEPATH%>';
	       window.opener.form1.WEBSITE.value='<%=WEBSITE%>';
	       window.opener.form1.SKYPE.value='<%=SKYPE%>';
	       window.opener.form1.FACEBOOK.value='<%=FACEBOOK%>';
	       window.opener.form1.TWITTER.value='<%=TWITTER%>';
	       window.opener.form1.LINKEDIN.value='<%=LINKEDIN%>';
	       window.opener.form1.EDIT_REGION.value='<%=REGION%>';
	       window.opener.form1.EDIT_COUNTRY.value='<%=sCountryCode%>';
	       window.opener.form1.FISCALYEAR.value='<%=FISCALYEAR%>';
	       window.opener.form1.PAYROLLYEAR.value='<%=PAYROLLYEAR%>';
	       window.opener.form1.ID.value='<%=PLANTID%>'; --%>
