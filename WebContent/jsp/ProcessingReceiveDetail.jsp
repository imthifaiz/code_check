...
<%
//String title = "Processing Receive Detail";
String title = "De-Kitting Detail";
%>
...
String res        = "",action     =   "",pitem="",pitemdesc ="",pbatch="",LOC_ID="",pqty="1",DESC="",GRNO="",PCOST="0",HDR_ID="0",PUOMQTY="1",Wastage="0",WastageCost="0",ISProcessing="0",
...
try {
	...
	String sql = "select ID,GRNO,PARENT_PRODUCT,(select ITEMDESC from [" + plant +"_ITEMMST] where ITEM = PARENT_PRODUCT)ITEMDESC,PARENT_PRODUCT_LOC,PARENT_PRODUCT_BATCH,PARENT_PRODUCT_QTY,PARENT_COST,ORDDATE,PARENTUOM,ISNULL(WASTAGE,0) WASTAGE,ISNULL(WASTAGECOST,0) WASTAGECOST,ISNULL(ISSAUTOPROCESSING,0) ISSAUTOPROCESSING from " + plant +"_PROCESSING_RECEIVEHDR A WHERE A.PLANT='"+ plant+"' AND A.GRNO='"+GRNO+"'";
	...
	if (movQryList.size() > 0) {
		for (int iCnt =0; iCnt<movQryList.size(); iCnt++){
            ...
            ISProcessing = StrUtils.fString((String)lineArr.get("ISSAUTOPROCESSING"));
            ...
            pqty =  StrUtils.addZeroes(curqty, "3");
            Wastage =  StrUtils.addZeroes(Double.parseDouble((String)lineArr.get("WASTAGE")), "3");
            WastageCost =  StrUtils.addZeroes(Double.parseDouble((String)lineArr.get("WASTAGECOST")), numberOfDecimal);
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>De-Kitting Detail</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 28.02.2022 --> 
...
<label class="radio-inline">
				      	<input type="radio" name="Processing" type = "radio"  value="1"  id="Auto" <%if(ISProcessing.equals("1")) {%>checked <%}%>  disabled>Auto 
				    	</label>
				    	<label class="radio-inline">
				      	<input type="radio" name="Processing" type = "radio" value="0"  id = "Manual" <%if(ISProcessing.equals("0")) {%>checked <%}%> disabled >Manual 
				     	</label>
  		</div>
     ...
     <div class="col-sm-2">
    	...
    	<input type="text" name="PARENTUOM" id="PARENTUOM" class="form-control" placeholder="Parent UOM" readonly value="<%=PUOM%>">
  		...
  		<div class="form-group">
					...
					<div class="col-sm-2">
					<div class="input-group">
						<input class="form-control" name="WASTAGE" id="WASTAGE" type="TEXT" readonly value="<%=StrUtils.addZeroes(Double.parseDouble(Wastage), "3")%>" size="20" MAXLENGTH=40>
					</div>
					</div>
					<div class="col-sm-1">
					<div class="input-group">
						<input class="form-control" name="WASTAGECOST" id="WASTAGECOST" type="TEXT" readonly  value="<%=StrUtils.addZeroes(Double.parseDouble(WastageCost), numberOfDecimal)%>" size="20" MAXLENGTH=40>
					</div>
					</div>
					<div class="col-sm-1">
					<div class="input-group">
...
function onGo() {
    ...
    var urlStr = "/track/ProcessingReceive/VIEW_PROCESSED_DETAIL";
...
function getTable(){
        return '<TABLE class="table table-bordred table-striped" id="tabledata" WIDTH="100%" align = "center">'+
        	   ...
        	   '<tr >'+
        		...
        		'<th width="11%">Product Description</th>'+
        		'<th width="11%">Location</th>'+
        		'<th width="8%">UOM</th>'+
        		'<th width="8%">Batch</th>'+
        		'<th width="5%">Tran.Quantity</th>'+
        		'<th width="9%">Average UnitCost</th>'+
        		'<th width="9%">Total Average UnitCost</th>'+
        		'</tr>'+
         		'</thead>';
