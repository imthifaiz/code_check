...
<%
	...
	String title = "Product Receive Detail";
	...
	boolean displaySummaryEdit=false;
	if(systatus.equalsIgnoreCase("ACCOUNTING")) {
		displaySummaryEdit = ub.isCheckValAcc("goodsreceiptbyproductreceive", plant,username);
	}
	if(systatus.equalsIgnoreCase("INVENTORY")) {
		displaySummaryEdit = ub.isCheckValinv("goodsreceiptbyproductreceive", plant,username);
	}
	...
	ItemMstDAO itemMstDAOs = new ItemMstDAO();
	...
	String invoiceHdrId = StrUtils.fString(request.getParameter("PRODUCT_HDR"));
	...
	List invoiceHdrList =  itemMstDAOs.getConvProductReceiveHdrById(ht);
	Map productHdr=(Map)invoiceHdrList.get(0);
	ht.put("RECEIVEHDRID", invoiceHdrId);
	//List invoiceDetList =  invoiceUtil.getInvoiceDetByHdrId(ht);
	List invoiceDetList =  itemMstDAOs.getConvProductReceiveDetByHdrId(ht);
	...
	{
		response.sendRedirect("../productreceive/detail?result=No Data Found");
	}
	...
	String adjustment = (String) productHdr.get("ADJUSTMENT");
...
// 	String custName = custMstDAO.getCustName(plant, (String) productHdr.get("CUSTNO"));
	
	String orddisctype = (String) productHdr.get("ORDERDISCOUNTTYPE");
	if(orddisctype.length() == 0){
		orddisctype = "%";
	}
	String disctype = (String) productHdr.get("DISCOUNT_TYPE");
	int isodtax = Integer.valueOf((String) productHdr.get("ISORDERDISCOUNTTAX"));
	int isdtax = Integer.valueOf((String) productHdr.get("ISDISCOUNTTAX"));
	int isshiptax = Integer.valueOf((String) productHdr.get("ISSHIPPINGTAX"));
	double ceqt = Double.valueOf((String) productHdr.get("CURRENCYUSEQT"));
	int taxid = Integer.valueOf((String) productHdr.get("TAXID"));
	String totalTax = (String) productHdr.get("TAXAMOUNT");
	...
	String orderdiscount = (String) productHdr.get("ORDER_DISCOUNT");
	...
	String discounttype = (String) productHdr.get("DISCOUNT_TYPE");
	String discount = "0";
	String discountpercent = (String) productHdr.get("DISCOUNT");
	if(discounttype.equalsIgnoreCase("%")){
		discount = (String) productHdr.get("DISCOUNT");
	...
	curency = (String) productHdr.get("DISPLAY");
	
   VIEW_ORDERTYPE=(String)productHdr.get("ORDERTYPE");
	...
	if( ((String)productHdr.get("RECEIVE_STATUS")).equalsIgnoreCase("Paid")){
		showCreditBtn = "style='display:none;'";
		showPaymentBtn = "style='display:none;'";
	}
	
	if( ((String)productHdr.get("RECEIVE_STATUS")).equalsIgnoreCase("Draft")){
		showCreditBtn = "style='display:none;'";
		showPaymentBtn = "style='display:none;'";
	}
	
	if( ((String)productHdr.get("RECEIVE_STATUS")).equalsIgnoreCase("CANCELLED")){
		showCreditBtn = "style='display:none;'";
		showPaymentBtn = "style='display:none;'";
	}
	
	String shipingcost = (String) productHdr.get("SHIPPINGCOST");
...
ArrayList arrCust = new CustUtil().getCustomerDetails((String) productHdr.get("CustCode"),plant);
	...
	String shipId = (String) productHdr.get("SHIPPINGID");
	...
	int transportid = Integer.valueOf((String)productHdr.get("TRANSPORTID"));
	if(transportid > 0){
  ...
  <%} %>
	...
	<!-- Muruganantham Modified on 16.02.2022 -->
            <ul class="breadcrumb backpageul">      	
                ...
                <li><a href="../productreceive/summary"><span class="underline-on-hover">Product Receive Summary</span> </a></li>                
                <li><label>Product Receive Detail</label></li>                                   
            </ul>             
		<div class="box-header menu-drop">
			...
			<%if(displaySummaryEdit){ %>
				<%if("OPEN".equalsIgnoreCase((String) productHdr.get("RECEIVE_STATUS")) || "PARTIALLY PROCESSED".equalsIgnoreCase((String) productHdr.get("RECEIVE_STATUS"))){ %>
						<button type="button" class="btn btn-success" data-toggle="dropdown" 						
						onClick="{processPurchaseReceipt()}">
						Product Receipt</button>
						<form method="POST" action="../OrderReceivingServlet?action=ViewMultiple&type=BulkRecieve" id="frmPONO" style="display: none;">
							<input type="hidden" name="ICRNO" value="<%=(String) productHdr.get("ICRNO")%>" />
							<input type="hidden" name="ICRNOHDR" value="<%=(String) productHdr.get("ID")%>" />
							<input type="hidden" name="RFLAG" value="prdreceive" />
							<input type="hidden" name="ENCRYPT_FLAG" value="1" />
						</form>
						<script>
							function processPurchaseReceipt(){
								$('#frmPONO').submit();
							}
						</script>
				<%} } %>
			...
			<div class="btn-group" role="group">
				...
				<!-- changes by imthi based on URL -->
					<h1 style="font-size: 18px; cursor: pointer; vertical-align:middle;" class="box-title pull-right"
						onclick="window.location.href='../productreceive/summary'">
						<i class="glyphicon glyphicon-remove"></i>
					</h1>
			</div>
		</div>
		<input type="hidden" id="dnotes" name="dnotes" value="<%=productHdr.get("NOTE")%>" >
		...
		<div id="ember1308" class="ribbon text-ellipsis tooltip-container ember-view">
			<div class="ribbon-inner ribbon-draft"><%=productHdr.get("RECEIVE_STATUS")%></div>
		</div>
		<%
			}
		%>
...
<%=Email.trim()%> <%=EMAIL.trim()%>
			...
			<div class="col-xs-6 text-right">
				<h1 id="headerpage">PRODUCT RECEIVE</h1>
<%-- 			<%if(checkordertype == 1){%> --%>
...
<%-- 				 <%} %> --%>
				...
				<figure class="pull-right text-center">
					...
					<figcaption># <%=productHdr.get("ICRNO")%></figcaption>
				</figure>
				<br style="clear:both"> 
			...
			<div class="col-xs-5 text-right">
				<table id="table1" class="table pull-right">
					<tbody>
						<tr>
							<td>Receive Date:</td>
							<td><%=productHdr.get("RECEIVE_DATE")%></td>
						</tr>
						<%if(printEmployee.equals("1") && ((String)productHdr.get("EMP_NAME")).length() > 0){%>
						<tr>
							<td><%=Employee%></td>
							<td><%=productHdr.get("EMP_NAME")%></td>
						</tr>
						<%} %>
						<tr>
							<td><%=PreparedBy%></td>
						...
						<tr>
							<td><%=Terms%></td>
							<td><%=productHdr.get("PAYMENT_TERMS")%></td>
						</tr>
					</tbody>
				</table>
			</div>
				...
				<table id="table2" class="table">
					<thead>
						<tr>
							...
							<td class="text-center">Recived Qty</td>
					...
					for(int i =0; i<invoiceDetList.size(); i++) {   
				  		...
				  		String qty="",qtyrc="", cost="", amount="", percentage="", tax="";
			  			...
			  			double rqty ="".equals(qtyrc) ? 0.0d :  Double.parseDouble(qtyrc);
			  			qtyrc = Numbers.toMillionFormat(rqty, "3");
...
<%-- 							<td class="text-center"><%=ITEM %></td> --%>
						...
						<tr>
          					<td></td>
          					<td colspan="7">&emsp;&emsp;&emsp;<%=m.get("ITEMDESC") %></td>
						</tr>
...
// 				  	}
				...
				<table id="table3" class="table" style="margin-bottom: 0px;">
					<tbody style="text-align:right;">
						<tr>
							<td class="text-left">Qty Total</td>
							<td class="text-left" style="position: relative;left: 35px;"><%=Numbers.toMillionFormat(dTotalQty, "3")%></td>
							<td class="text-left" style="position: relative;right: 10px;"><%=Numbers.toMillionFormat(rTotalQty, "3")%></td>
							<td></td>
							<td></td>
							<td><%=SubTotal%></td>
					...
					<%
						...
						<tr style="font-size:14px;">
							<td colspan="6"><b><%=Total%> (<%=productHdr.get("CURRENCYID") %>)</b></td>
							<td><b><%=Numbers.toMillionFormat(totalAmount, Integer.valueOf(numberOfDecimal))  %></b></td>
						</tr>
						<%if(!basecurrency.equalsIgnoreCase((String)productHdr.get("CURRENCYID"))) {%><!--  Author: Azees  Create date: August 04,2021  Description:  Total of Local Currency -->
						<tr style="display: none;font-size:14px;" id="showtotalcur">
						...
						<tr style="display: none;">
							<td colspan="6"><%=PaymentMade%> (<%=productHdr.get("CURRENCYID") %>)</td>
							<td style="color:red;">(-) <%=Numbers.toMillionFormat(Spaymentmade, Integer.valueOf(numberOfDecimal)) %></td>
						</tr>
						<%if(!basecurrency.equalsIgnoreCase((String)productHdr.get("CURRENCYID"))) {%>
						<tr id="showtotalPay" style="display: none;">
							<td colspan="6"><%=PaymentMade%> (<%=basecurrency%>)</td>
							<td style="color:red;">(-) <%=Numbers.toMillionFormat(paymentMadeloc, numberOfDecimal) %></td>
						</tr>
						<%}%>
						<tr class="grey-bg" style="font-size:14px;display: none;">
							<td colspan="6"><b><%=BalanceDue %> (<%=productHdr.get("CURRENCYID") %>)</b></td>
							<td><b><%=Numbers.toMillionFormat(balduest, Integer.valueOf(numberOfDecimal)) %></b></td>
						</tr>
						<%if(!basecurrency.equalsIgnoreCase((String)productHdr.get("CURRENCYID"))) {%>
						<tr class="grey-bg" style="display: none;font-size:14px;" id="showtotalBal">
				...
				<table id="footerTable" class="table">		
					<tr>
						<td colspan="7"><%=remark3%> <%=(String) productHdr.get("NOTE")%></td>
					</tr>	
					<tr>
						<td colspan="7"></td>
					</tr>		
		...
		<div id="journal_detail_box" class="box" style="display: none;">
						...
						<div class="p-3 border border-secondary">
							<div id="ember1147" class="ember-view">
								<div class="clearfix pb-3">
									<small class="text-muted">Amount is displayed in your invoice currency</small>&nbsp;<span class="badge text-semibold badge-success"><%=productHdr.get("CURRENCYID")%></span>
									<!---->
								</div>
								<p class="font-large">
									<b> <!---->
      ...
      <div class="modal-content">
		...
		<script>
			$(document).ready(function(){
				...
				document.getElementById('headerpage').innerHTML="PRODUCT RECEIVE";
				setTimeout(function() {
				    $('.alert').fadeOut('fast');
				}, 2000);
			  ...
			  $("#bill-cancel").click(function() {
				  var invoiceHdrId = "<%=productHdr.get("ID")%>";
				  var invoice = "<%=productHdr.get("ICRNO")%>";
				  var status = "<%=productHdr.get("RECEIVE_STATUS")%>";
				  var taxstatus = "<%=productHdr.get("TAX_STATUS")%>";
				  var result="";
				  if(status.toLowerCase() == "open" || status.toLowerCase() == "draft" ) {
						if(taxstatus.toLowerCase() == "tax generated"){
							...
							window.location.href = "../productreceive/detail?INVOICE_HDR="+invoiceHdrId+"&resultnew="+ result;
						}else {
			  ...
			  $("#cfmcancel").click(function(){
				   window.location.href = "/track/InvoiceServlet?ACTION=convertToCancel&INVID=<%=productHdr.get("ID")%>&INVOICE=<%=productHdr.get("ICRNO")%>&STATUS=<%=productHdr.get("RECEIVE_STATUS")%>&TAXSTATUS=<%=productHdr.get("TAX_STATUS")%>";
				});
			 ...
			 $("#cfmdelete").click(function(){
				 window.location.href = "/track/InvoiceServlet?ACTION=deleteinvoice&INVID=<%=productHdr.get("ID")%>&INVOICE=<%=productHdr.get("ICRNO")%>&STATUS=<%=productHdr.get("RECEIVE_STATUS")%>&TAXSTATUS=<%=productHdr.get("TAX_STATUS")%>";
				});
			});
			JsBarcode("#barCode", "<%=productHdr.get("ICRNO")%>", {format: "CODE128",displayValue: false});
			function convertToDraft(){
				window.location.href = "/track/InvoiceServlet?ACTION=convertToDraft&INVID=<%=productHdr.get("ID")%>";
			}
			...
			function generatePdf(dataUrl){
				...
				<%}%>
				<%}%>
<%-- 				doc.text('<%=Terms%> <%=productHdr.get("PAYMENT_TERMS")%>', 16, rY+=6); --%>
				/* **** */
				<%if(printwithtransportmode == 1){ %>
				...
				/* Top Right */
				doc.setFontSize(27);
				doc.text('PRODUCT RECEIVE', 195, 19, {align:'right'});
				...
				doc.setFontSize(10);
				//doc.setFontStyle("bold");
				doc.text('# <%=productHdr.get("ICRNO")%>', 180, 37, {align:'right'});
...
// 				finalY = doc.previousAutoTable.finalY;
			...
			function generateKitchen() {
				...
				if(print=='0'){
					setTimeout(function(){ 
						window.open('/track/deleveryorderservlet?Submit=Print Invoice With Price&INVOICE=<%=productHdr.get("ICRNO")%>&PRINT=defafult','_blank') 
					}, 0);
				}else if(print=='1'){
					setTimeout(function(){ 
						window.open('/track/deleveryorderservlet?Submit=Print Invoice With Price&INVOICE=<%=productHdr.get("ICRNO")%>&PRINT=defafult','_blank') 
					...
					setTimeout(function(){ 
						window.open('/track/deleveryorderservlet?Submit=Print Invoice With Price&INVOICE=<%=productHdr.get("ICRNO")%>&PRINT=customer','_blank') 
					...
					setTimeout(function(){ 
						window.open('/track/deleveryorderservlet?Submit=Print Invoice With Price&INVOICE=<%=productHdr.get("ICRNO")%>&PRINT=collection','_blank') 
					}, 6000);// it open in second tab window with 8 sec of opening defalut page
				}
				//end
				
<%-- 				window.open(
						"/track/deleveryorderservlet?Submit=Print Invoice With Price&INVOICE=<%=productHdr.get("ICRNO")%>",
...
<%-- 			function recordpayment()
			{
				...
				if(credit > 0){
					...
					window.location.href="../productreceive/recordpayment?invnum=<%=(String) productHdr.get("ICRNO")%>&DONO=<%=(String) productHdr.get("DONO")%>&CUST_CODE=<%=(String) productHdr.get("CUSTNO")%>&EMPNO=<%=(String) productHdr.get("EMPNO")%>&TRANID=<%=(String) productHdr.get("ID")%>&cmd=Edit&type=REGULAR&CUST_NAME=<%=custName%>&AMT=<%=balamtrp%>&INVOICEID=<%=invoiceHdrId%>";
				}
...
<%-- 		var orderbal = <%=orderbalance%> --%>
	...
	function applyCredit(){
		...
		//location.reload();
		window.location.href = "../productreceive/detail?INVOICE_HDR="+<%=productHdr.get("ID")%>;
	}
...
<%-- 			var amcheck = <%=InvPaymentdao.getcreditamoutusingorderno(plant, (String) productHdr.get("DONO")) %>; --%>
// 			if(amcheck <= 0){
