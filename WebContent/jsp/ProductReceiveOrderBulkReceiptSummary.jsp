...
<%
String title = "Goods Receipt By Product Receive ";
%>
...
String pono     = StrUtils.fString(request.getParameter("ICRNO"));
...
function onReceive(form){
	var pono=document.form.ICRNO.value;
			...
			$.ajax( {
				...
				success : function(data) {
					if (data.status == "100") {
						...
						document.form.GRNO.value= resultV;
						//alert("ok");
						document.form.action ="/track/ItemReturnServlet?ACTION=BulkReceive&createBill=Y";
						document.form.submit();
					} else {
						alert("GRNO "+document.form.GRNO.value+" already converted as Bill. Please process with the new GRNO.");
						document.form.GRNO.value = "";
...
function generatePdfMail(dataUrl,attachName){	
	...
	formData.append("GRNO_PONO", $("#ICRNO").val());
	progressBar();
...
<div class="container-fluid m-t-20">
	 ...
	 <!-- Muruganantham Modified on 16.02.2022 -->
             <ol class="breadcrumb backpageul" >      	
                  ...
                  <li><a href="../productreceive/detail?&PRODUCT_HDR=<%=hdrid %>"><span class="underline-on-hover">Product Receive Detail</span> </a></li>
                  <%} %>
                  <li>Goods Receipt By Product Receive</l>                                    
             </ol>   
     <!-- Muruganantham Modified on 16.02.2022 --> 
            <div class="box-header menu-drop">
              <h1 style="font-size: 20px;" class="box-title"><%=title %></h1>
                      <h1 style="font-size: 18px; cursor: pointer;" class="box-title pull-right" onclick="window.location.href='../productreceive/detail?&PRODUCT_HDR=<%=hdrid %>'">
  ...
  <form class="form-horizontal" name="form" id="frmInboundOrderBulkReceipt" method="post"  action="/track/OrderReceivingByPOServlet?action=BulkReceive">
      ...
      <INPUT class="form-control" type="TEXT" size="30" MAXLENGTH=20 name="ICRNO" id="ICRNO"	value="<%=pono%>"> 
      							 <span class="select-icon" onclick="$(this).parent().find('input[name=\'ICRNO\']').focus()"><i class="glyphicon glyphicon-menu-down"></i></span>
<!-- 	  onkeypress="if((event.keyCode=='13') && ( document.form.PONO.value.length > 0)){loadInbundOrderDetails();}"> -->
      ...
      <div class="form-group">
      <label class="control-label col-sm-4" for="Supplier Name">Customer Name:</label>
      <div class="col-sm-4">          
      ...
      <INPUT type="hidden" name="ICRNOHDR" value="<%=hdrid%>"> 
	  ...
	  <tbody style="font-size: 15px;">
	  					<% 
	  					if(mp.size()>0){
       ArrayList al= _poUtil.listInboundSummaryProductReceiveDET(pono,plant);
...
<!--       </div> -->
      ...
      <div class="dropup">
		...
		<button type="button" class="Submit btn btn-success" id="btnRedeive" onclick="onReceiveAndBill(document.form);" href="#" >Receive</button>&nbsp;
...
$(document).ready(function(){
	...
	},
	{
	  display: 'PONO',  
	  source: function (query, process,asyncProcess) {
		var urlStr = "/track/productreceive";
		$.ajax( {
		type : "POST",
		url : urlStr,
		async : true,
	  ...
	  empty: [
		...
		if($(this).val() == ""){
			document.form.ICRNO.value = "";
			document.getElementById("ICRNO").value = "";
		}
