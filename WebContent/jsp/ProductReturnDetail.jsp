...
<%
	...
	String title = "Product Return Detail";
	...
	List invoiceHdrList =  itemMstDAOs.getConvProductHdrById(ht);
	Map productHdr=(Map)invoiceHdrList.get(0);
	ht.put("PRODUCTHDRID", invoiceHdrId);
	//List invoiceDetList =  invoiceUtil.getInvoiceDetByHdrId(ht);
	List invoiceDetList =  itemMstDAOs.getConvProductDetByHdrId(ht);
	...
	{
		response.sendRedirect("../productreturn/detail?result=No Data Found");
	}
...
// 	String custName = custMstDAO.getCustName(plant, (String) productHdr.get("CUSTNO"));
   ...
   VIEW_ORDERTYPE=(String)productHdr.get("ORDERTYPE");
	...
	if( ((String)productHdr.get("RETURN_STATUS")).equalsIgnoreCase("Paid")){
		showCreditBtn = "style='display:none;'";
		showPaymentBtn = "style='display:none;'";
	}
	
	if( ((String)productHdr.get("RETURN_STATUS")).equalsIgnoreCase("Draft")){
		showCreditBtn = "style='display:none;'";
		showPaymentBtn = "style='display:none;'";
	}
	
	if( ((String)productHdr.get("RETURN_STATUS")).equalsIgnoreCase("CANCELLED")){
		showCreditBtn = "style='display:none;'";
		showPaymentBtn = "style='display:none;'";
	}
...
// 	ArrayList arrCust = new CustUtil().getCustomerDetails((String) productHdr.get("CUSTNO"),plant);
	ArrayList SupplyDetails = new CustomerBeanDAO().getVendorDetailsForIdrno(plant, (String) productHdr.get("IDRNO"));
// 	ArrayList arrCust = new CustUtil().getVendorDetails((String) productHdr.get("VENDNO"),plant);
  ...
  <%} %>
	...
	<!-- Muruganantham Modified on 16.02.2022 -->
            <ul class="breadcrumb backpageul">      	
                ...
                <li><a href="../productreturn/summary"><span class="underline-on-hover">Product Return Summary</span> </a></li>                
                <li><label>Product Return Detail</label></li>                                   
            </ul>             
		...
		<div id="ember1308" class="ribbon text-ellipsis tooltip-container ember-view">
			<div class="ribbon-inner ribbon-draft"><%=productHdr.get("RETURN_STATUS")%></div>
		</div>
		<%
			}
		%>
...
<%=Email.trim()%> <%=EMAIL.trim()%>
			...
			<div class="col-xs-6 text-right">
				<h1 id="headerpage">PRODUCT RETURN</h1>
<%-- 			<%if(checkordertype == 1){%> --%>
...
<%-- 				 <%} %> --%>
				...
				<figure class="pull-right text-center">
					...
					<figcaption># <%=productHdr.get("IDRNO")%></figcaption>
				</figure>
				<br style="clear:both"> 
		...
		<div class="row">
			...
			<span style="font-size: 24px;"><b><%=InvoiceOrderToHeader%></b></span>	<br>	
				<span style="font-size: 14px;">
					...
					<span>UENNO : <%=SupplyDetails.get(25)%></span>	<br>			
			...
			<div class="col-xs-5 text-right">
				<table id="table1" class="table pull-right">
					<tbody>
						<tr>
							<td>Return Date:</td>
							<td><%=productHdr.get("RETURN_DATE")%></td>
						</tr>
      ...
      <div class="modal-content">
		...
		<script>
			$(document).ready(function(){
				...
				document.getElementById('headerpage').innerHTML="PRODUCT RETURN";
				setTimeout(function() {
				    $('.alert').fadeOut('fast');
				}, 2000);
			  ...
			  $("#bill-cancel").click(function() {
				  ...
				  var invoice = "<%=productHdr.get("IDRNO")%>";
				  var status = "<%=productHdr.get("RETURN_STATUS")%>";
				  ...
				  if(status.toLowerCase() == "open" || status.toLowerCase() == "draft" ) {
						if(taxstatus.toLowerCase() == "tax generated"){
							...
							window.location.href = "../productreturn/detail?INVOICE_HDR="+invoiceHdrId+"&resultnew="+ result;
						}else {
			  ...
			  $("#cfmcancel").click(function(){
				   window.location.href = "/track/InvoiceServlet?ACTION=convertToCancel&INVID=<%=productHdr.get("ID")%>&INVOICE=<%=productHdr.get("IDRNO")%>&STATUS=<%=productHdr.get("RETURN_STATUS")%>&TAXSTATUS=<%=productHdr.get("TAX_STATUS")%>";
				});
			 ...
			 $("#cfmdelete").click(function(){
				 window.location.href = "/track/InvoiceServlet?ACTION=deleteinvoice&INVID=<%=productHdr.get("ID")%>&INVOICE=<%=productHdr.get("IDRNO")%>&STATUS=<%=productHdr.get("RETURN_STATUS")%>&TAXSTATUS=<%=productHdr.get("TAX_STATUS")%>";
				});
			});
			JsBarcode("#barCode", "<%=productHdr.get("IDRNO")%>", {format: "CODE128",displayValue: false});
...
// 				<br>
				...
				doc.text('<%=SupplyDetails.get(1)%>', 16, rY+=4);
				doc.text('UENNO : <%=SupplyDetails.get(25)%>', 16, rY+=4);
				doc.text('<%=rcbno%> <%=SupplyDetails.get(24)%>', 16, rY+=4);				
				doc.text('<%=SupplyDetails.get(2)%> <%=SupplyDetails.get(3)%>', 16, rY+=4);				
				doc.text('<%=SupplyDetails.get(4)%> <%=SupplyDetails.get(15)%>', 16, rY+=4);
				doc.text('<%=SupplyDetails.get(22)%>  <%=SupplyDetails.get(5)%> <%=SupplyDetails.get(6)%> ', 16, rY+=4);				
				doc.text('<%=Email%> <%=SupplyDetails.get(12)%>', 16, rY+=4);				
				doc.text('Remarks: <%=SupplyDetails.get(15)%>', 16, rY+=4);
...
<%-- 				doc.text('<%=Terms%> <%=productHdr.get("PAYMENT_TERMS")%>', 16, rY+=6); --%>
				...
				/* Top Right */
				doc.setFontSize(27);
				doc.text('PRODUCT RETURN', 195, 19, {align:'right'});
				...
				doc.setFontSize(10);
				//doc.setFontStyle("bold");
				doc.text('# <%=productHdr.get("IDRNO")%>', 180, 37, {align:'right'});
...
// 				finalY = doc.previousAutoTable.finalY;
			...
			function generateKitchen() {
				...
				if(print=='0'){
					setTimeout(function(){ 
						window.open('/track/deleveryorderservlet?Submit=Print Invoice With Price&INVOICE=<%=productHdr.get("IDRNO")%>&PRINT=defafult','_blank') 
					}, 0);
				}else if(print=='1'){
					setTimeout(function(){ 
						window.open('/track/deleveryorderservlet?Submit=Print Invoice With Price&INVOICE=<%=productHdr.get("IDRNO")%>&PRINT=defafult','_blank') 
					...
					setTimeout(function(){ 
						window.open('/track/deleveryorderservlet?Submit=Print Invoice With Price&INVOICE=<%=productHdr.get("IDRNO")%>&PRINT=customer','_blank') 
					...
					setTimeout(function(){ 
						window.open('/track/deleveryorderservlet?Submit=Print Invoice With Price&INVOICE=<%=productHdr.get("IDRNO")%>&PRINT=collection','_blank') 
					}, 6000);// it open in second tab window with 8 sec of opening defalut page
				}
				//end
				
<%-- 				window.open(
						"/track/deleveryorderservlet?Submit=Print Invoice With Price&INVOICE=<%=productHdr.get("IDRNO")%>",
...
<%-- 			function recordpayment()
			{
				...
				if(credit > 0){
					...
					window.location.href="../productreturn/recordpayment?invnum=<%=(String) productHdr.get("IDRNO")%>&DONO=<%=(String) productHdr.get("DONO")%>&CUST_CODE=<%=(String) productHdr.get("CUSTNO")%>&EMPNO=<%=(String) productHdr.get("EMPNO")%>&TRANID=<%=(String) productHdr.get("ID")%>&cmd=Edit&type=REGULAR&CUST_NAME=<%=custName%>&AMT=<%=balamtrp%>&INVOICEID=<%=invoiceHdrId%>";
				}
...
<%-- 		var orderbal = <%=orderbalance%> --%>
	...
	function applyCredit(){
		...
		//location.reload();
		window.location.href = "../productreturn/detail?INVOICE_HDR="+<%=productHdr.get("ID")%>;
	}
