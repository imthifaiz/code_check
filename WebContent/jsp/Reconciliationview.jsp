...
<%
	...
	String title = "Bank Reconciliation Detail";
...
<script src="../jsp/js/Reconciliationview.js"></script>
<style>
.extraInfo {
...
<div class="container-fluid m-t-20">
	...
	<div class="box">
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Bank Reconciliation Detail</label></li>                                   
            </ul>
		<div class="box-header menu-drop">
			<h1 style="font-size: 20px;" class="box-title"><%=title%></h1>
			<div class="box-title pull-right">
              <div class="btn-group" role="group">
			<button type="button" class="btn btn-danger" id="bounceid" onclick="recpopupcfm()" style="margin-right: 10px;">Bounce</button>
			&nbsp;
			&nbsp;
			&nbsp;
			&nbsp;
			...
			<div class="col-sm-12" style="margin-top: 1%;">
					...
					<div class="col-sm-4">
      					<div class="input-group">    
				  		    ...
				  		    <span class="input-group-btn"></span>
    						<input class="form-control" maxlength="50" size="45" type="TEXT" onchange="chgbkbal();" value="<%=rechdr.getBANKCLOSEBALANCE()%>" name="BANKBALANCE" style="width: 238px;" readonly>
   		 				</div>
   					</div>
			</div>
	      ...
	      <div class="modal-body">
	        <div class="row">
			   ...
			   <div class="col-lg-10" style="padding-left: 2px">
			      <p> Are you sure about bounce reconciled data?</p>
			      
			      <div class="alert-actions btn-toolbar">
			         <button class="btn btn-primary ember-view" onclick="bouncedata();" style="background:red;">
			        	Yes 
			         </button>
		        ...
		        success: function (data) {
		        	...
		        	if(rstatus === 0){
		        		...
		        		$.each(recListedit,function(i,r){
		        			...
		        			body += '<tr>';
		        			body += '<td class="text-left">';
		        			body += '<input class="form-check-input" type="Checkbox" onclick="chkd(this)" name="CHKD" value="" checked disabled>';
	        		...
	        		//$('#cbookpending').text(parseFloat(json.RDEPOSIT).toFixed(numberOfDecimal)); --14_09_24 by azees (undeposit zero issue)
	        		var cbookpending=(parseFloat(json.LEDGERCLOSINGGBAL)-(parseFloat(tdebit)-parseFloat(tcredit)))-parseFloat(json.BANKOPENINGBAL);
	        		$('#cbookpending').text(parseFloat(cbookpending).toFixed(numberOfDecimal));
...
function generatePdfPrint(dataUrl){
	...
	var cdeposits = $("input[name ='CDEPOSITS']").val();
	var cpayments = $("input[name ='CPAYMENTS']").val();
	...
	doc.text(badjbal, 170, rY, {align:'right'});
	doc.text(bledgerbal, 195, rY, {align:'right'});
	...
	doc.text(bledgerbal, 170, rY, {align:'right'});

	doc.text('Reconcillation Difference', 20, rY+=6);
	doc.text(bankdiffbal, 170, rY, {align:'right'});
...
function bouncedata(){		
		...
		$.ajax( {
	        ...
	        success: function (data) {
	        	...
	        	console.log(json); 
	        	if(json.STATUS == "OK"){
	        		window.location.href = '../accountant/bankreconcilationsummary?result=Reconciliation bounce successfully';
	        	}
	        	if(json.STATUS == "NOT OK"){
	        		window.location.href = '../accountant/bankreconcilationview?ID=<%=rid%>';
	        	}
	        },
	        cache: false
		});
...
function checkbouncedata(){		
	var urlStr = "/track/JournalServlet";
	$.ajax( {
		type : "GET",
		url : urlStr,
		data: {
			"action":"CHECK_BOUNCE_STATUS",
			"rid":<%=rid%>,
		}, 
        success: function (data) {
 		 	var json = $.parseJSON(data);
        	console.log(json); 
        	if(json.STATUS == "OK"){
        		 document.getElementById('bounceid').style.visibility = 'visible';
        	}
        	if(json.STATUS == "NOT OK"){
        		 document.getElementById('bounceid').style.visibility = 'hidden';
        	}
        },
        cache: false
	});
