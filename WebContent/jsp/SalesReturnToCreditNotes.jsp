...
DoHdr doheader = doHdrDAO.getDoHdrById(plant,ORDERNO);
...
String invoiceHdrId = StrUtils.fString(request.getParameter("HDRID")).trim();
...
String ccnnumber="";
String payTerms="",shippingid="",shippingcust="";
int pid = 0 ;
/* com.track.dao.TblControlDAO _TblControlDAO=new   com.track.dao.TblControlDAO();
_TblControlDAO.setmLogger(mLogger); 
String ccnnumber = _TblControlDAO.getNextOrderminvlaue(plant,username,"CUSTOMER_CREDIT_NOTE");
 */
...
if(invoice.length()>0){
	...
	shippingid = (String)doheader.getSHIPPINGID();
	shippingcust = (String)doheader.getSHIPPINGCUSTOMER();
	payTerms= (String)doheader.getPAYMENT_TERMS();
	pid = Integer.valueOf(doheader.getPROJECTID());
}
...
String CURRENCYUSEQT="0",DISPLAY="",DISPLAYID="";
	...
	String Custno ="",tax_type="";
	String sAddr4 = "",sCountry="",sZip="",sDesgination="",sTelNo="",sHpNo="",sEmail="",sFax="",sState="",sWorkphone="";
	String CustName="",Address="",Address2="",Address3="",SHIPCONTACTNAME="",SHIPDESGINATION="",SHIPADDR1="",SHIPADDR2="",SHIPADDR3 ="",
		   SHIPADDR4 = "",SHIPSTATE="",SHIPCOUNTRY="",SHIPZIP="",SHIPHPNO="",SHIPWORKPHONE="",SHIPEMAIL="";
...
// 	Map invoiceHdr=(Map)invoiceHdrList.get(0);
	...
	{
		Map mcut=(Map)invoiceHdrList.get(0);
		Custno = (String) mcut.get("CUSTNO");
		tax_type = (String) mcut.get("TAX_TYPE");
		SHIPCONTACTNAME = (String) mcut.get("SHIPCONTACTNAME");
		SHIPDESGINATION = (String) mcut.get("SHIPDESGINATION");
		SHIPWORKPHONE = (String) mcut.get("SHIPWORKPHONE");
		SHIPHPNO = (String) mcut.get("SHIPHPNO");
		SHIPEMAIL = (String) mcut.get("SHIPEMAIL");
		SHIPCOUNTRY = (String) mcut.get("SHIPCOUNTRY");
		SHIPADDR1 = (String) mcut.get("SHIPADDR1");
		SHIPADDR2 = (String) mcut.get("SHIPADDR2");
		SHIPADDR3 = (String) mcut.get("SHIPADDR3");
		SHIPADDR4 = (String) mcut.get("SHIPADDR4");
		SHIPSTATE = (String) mcut.get("SHIPSTATE");
		SHIPZIP = (String) mcut.get("SHIPZIP");
	}
	
	if(invoiceHdrId.equalsIgnoreCase("")){
		 Custno = (String)doheader.getCustCode(); 
	}else{
		 Custno = Custno; 
	}
	
	ArrayList arrCust =  custUtil.getCustomerDetails(Custno, plant);
	CustName = (String) arrCust.get(1);
	Address = (String) arrCust.get(2);
	Address2 = (String) arrCust.get(3);
	Address3 = (String) arrCust.get(4);
...
// 	STATE_CODE = (String) arrCust.get(65);
// 	SHIPSTATE_CODE = (String) arrCust.get(65);
...
<script src="../jsp/js/SalesReturnToCreditNote.js"></script>
<style>
 .sideiconspan {
	color: #23527c;
	position: absolute;
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                <li><a href="../home"><span class="underline-on-hover">Dashboard</span></a></li>	
                 ...
                 <li><a href="../salesreturn/detail?SORETURN=<%=sorno %>&DONO=<%=ORDERNO %>&GINO=<%=gino %>&INVOICE=<%=invoice %>"><span class="underline-on-hover">Sales Order Return Detail</span></a></li>	
                <li><label>New Credit Note</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 28.02.2022 --> 
	 <div class="box-header menu-drop">
              <h1 style="font-size: 20px;" class="box-title"><%=title %></h1>
              <h1 style="font-size: 18px;cursor:pointer;" class="box-title pull-right" onclick="window.location.href='../salesreturn/detail?SORETURN=<%=sorno %>&DONO=<%=ORDERNO %>&GINO=<%=gino %>&INVOICE=<%=invoice %>'">
              <i class="glyphicon glyphicon-remove"></i>
              </h1>
		</div>
<div class="container-fluid">
<input type="number" id="numberOfDecimal" name="numberOfDecimal" style="display:none;" value=<%=numberOfDecimal%>>
	<form id="creditnoteForm" class="form-horizontal" name="form1" action="/track/CustomerCreditNoteServlet?Submit=Save"  method="post" enctype="multipart/form-data" onsubmit="return validateCreditnote()">
...
<%-- 	<input type="hidden" name="SHIPPINGID" value="<%=doheader.getSHIPPINGID()%>"> --%>
	...
	<div class="form-group customer-section">
			...
			<div class="col-sm-6 ac-box">
				...
				<input type="text" class="ac-selected  form-control typeahead" id="CUSTOMER" placeholder="Select a customer" name="CUSTOMER" value="<%=CUSTOMER%>" readonly onclick="removetaxtrestl()">
				<!-- </div> -->
        ...
        <%
		   ...
		   ArrayList ccList1 =  _MasterUtiln.getTaxTreatmentList("",plant,"");
			...
			<INPUT type="hidden" name="soreturn" value="<%=sorno%>" /> 
			<INPUT type="hidden" name="soreturnstatus" value="SORETURN" /> 
			<INPUT type="hidden" name="invselect" value="" />
...
<!-- 								</span> -->
				...
				<div class="form-group shipCustomer-section" id="shipbilladd">
					...
					<div class="col-sm-3" style="line-height: 7px; font-size: 13px;">
						...
						<%-- <p><%=doheader.getCustName() %></p>
						<p><%=doheader.getAddress() %> <%=doheader.getAddress2() %></p>
						<p><%=doheader.getAddress3() %>  <%=sAddr4%></p> --%>
						<p><%=CustName%></p>
					...
					<div class="col-sm-3" style="line-height: 7px; font-size: 13px;">
						...
						<p><%=doheader.getSHIPCONTACTNAME() %></p>
						<p><%=doheader.getSHIPDESGINATION()%></p>
						<p><%=doheader.getSHIPADDR1() %> <%=doheader.getSHIPADDR2() %></p>
						<p><%=doheader.getSHIPADDR3() %> <%=doheader.getSHIPADDR4() %></p>
						<p><%=doheader.getSHIPSTATE() %></p>
						<p><%=doheader.getSHIPCOUNTRY() %> <%=doheader.getSHIPZIP() %></p>
						<p><%=doheader.getSHIPHPNO() %></p>
						<p><%=doheader.getSHIPWORKPHONE() %></p>
						<p><%=doheader.getSHIPEMAIL()%></p>
						<%}else{%>
					...
					<div class="col-sm-4 ac-box">
						<div class="input-group">
								<input type="text" class="ac-selected  form-control typeahead" 
									...
									name="project"  value="<%=projectname%>" readonly>
...
<!-- 								</span>  -->
		...
		<div class="form-group invoice-section">
			...
			<div class="col-sm-4 ac-box">
			<input type="text" class="ac-selected  form-control typeahead" id="invoice" name="invoice" value="<%=invoice%>" readonly>	
			</div>
		</div>
		<div class="form-group invoice-section">
			<label class="control-label col-form-label col-sm-2">GINO</label>
			<div class="col-sm-4 ac-box">
			<input type="text" class="ac-selected  form-control typeahead" id="gino" name="gino" value="<%=gino%>" readonly>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-4">
				<!-- <input type="text" class="form-control" id="reference" maxlength="300" name="reference"> -->
						...
						<input type="text" class="ac-selected form-control typeahead" maxlength="300" id="reference" name="reference" value="<%=ORDERNO%>">
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-4">
			<div class="input-group">
				<input type="text" class="form-control" id="creditnote" name="creditnote" value="<%=ccnnumber%>" onchange="checkorderno(this.value)" onkeypress="return blockSpecialCharOrderNo(event)">
		...
		<div class="form-group">
			...
			<div class="col-sm-4">
				<input type="text" class="form-control datepicker" id="creditnote_date" name="creditnote_date" value="<%=curDate%>">
			</div>
											<div class="col-sm-6 no-padding">
       				<label class="control-label col-form-label col-sm-5">Payment Terms</label>
					<div class="col-sm-6 ac-box">
       				<input type="text" class=" ac-selected form-control" id="payment_terms" name="payment_terms" value="<%=payTerms%>" placeholder="Select Payment Terms" READONLY>
		...
		<div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
			   		 	<input type="text" class="form-control" id="GST" name="GST" onchange="changingtaxpercentage()" value="0" readonly>
		...
		<div class="row" style="margin:0px;">
			...
			<tbody>
			  <tr>
				...
				<td class="item-cost text-right"><input name="cost" type="text" class="form-control text-right" value="0.00" onchange="calculateAmount(this)"></td>
							...
							<div class=" col-lg-12 col-sm-3 col-12">
								<div class="input-group my-group" style="width:120px;"> 
									...
									<%--<textarea rows="1" maxlength="3" style="resize: none;padding: 5px;" class="discountPicker form-control item_discounttypeSearch" name="item_discounttype" onchange="calculateAmount(this)"></textarea>--%>
									 <select name="item_discounttype" class="discountPicker form-control" onchange="calculateAmount(this)">
		...
		<div class="row">
			<div class="col-sm-6">
				<!-- <a class="add-line" style="text-decoration:none;cursor: pointer;" onclick="addRow(document.getElementById('invoice').value)">+ Add another line</a> -->
			</div>
				...
				<div class="total-row"> 
					<div class="total-label">
						<div class="row">
							...
							<div class=" col-lg-6 col-sm-6 col-6">
								<div class="input-group my-group"> 
									<input class="form-control text-right" type="text" id="discountblock" name="discount" onchange="calculateTotal()">
									<!-- <span class="input-group-addon">%</span>	-->
				...
				<div class="total-row">
					<div class="badge-editable total-label">
						<div class="row">
							...
							<div class="col-lg-6 col-sm-6 col-6"> 
								<input class="form-control text-right ember-view" type="text" name="adjustment" value="0.0" onchange="calculateTotal()" onkeypress="return isNumberKey(event,this,4)"> 
							</div>  
						</div>
					</div> 
		...
		<%if(invoiceHdrId.equalsIgnoreCase("")){ %>
		<input type = "hidden" name="SHIPCONTACTNAME" value="<%=doheader.getSHIPCONTACTNAME()%>">
		<input type = "hidden" name="SHIPDESGINATION" value="<%=doheader.getSHIPDESGINATION()%>">
		<input type = "hidden" name="SHIPADDR1" value="<%=doheader.getSHIPADDR1()%>">
		<input type = "hidden" name="SHIPADDR2" value="<%=doheader.getSHIPADDR2()%>">
		<input type = "hidden" name="SHIPADDR3" value="<%=doheader.getSHIPADDR3()%>">
		<input type = "hidden" name="SHIPADDR4" value="<%=doheader.getSHIPADDR4()%>">
		<input type = "hidden" name="SHIPSTATE" value="<%=doheader.getSHIPSTATE()%>">
		<input type = "hidden" name="SHIPZIP" value="<%=doheader.getSHIPZIP()%>">
		<input type = "hidden" name="SHIPWORKPHONE" value="<%=doheader.getSHIPWORKPHONE()%>">
		<input type = "hidden" name="SHIPCOUNTRY" value="<%=doheader.getSHIPCOUNTRY()%>">
		<input type = "hidden" name="SHIPHPNO" value="<%=doheader.getSHIPHPNO()%>">
		<input type = "hidden" name="SHIPEMAIL" value="<%=doheader.getSHIPEMAIL()%>">
		<%}else{ %>
...
{	
	...
	function onAdd(){
	 ...
	 /* document.form.action  = "/track/CreateCustomerServlet?action=ADD&reurl=createCreditNote";
	 document.form.submit(); */
	 var datasend = $('#formCustomer').serialize();
	   
	  
	   var urlStr = "/track/CreateCustomerServlet?action=JADD&reurl=createCreditNote";
		$.ajax( {
		type : "POST",
		url : urlStr,
		async : true,
	...
	function onIDGen()
	{
	/*  document.form.action  = "/track/CreateCustomerServlet?action=Auto-ID&reurl=createCreditNote";
	 document.form.submit(); */
