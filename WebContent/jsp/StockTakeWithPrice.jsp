...
<%
String title = "Stock Take With Cost Difference";
%>
...
//document.getElementById('VIEW_RESULT_HERE').innerHTML = '';
  subWin = window.open(URL, 'StockTakeWithCost', 'toolbar=0,scrollbars=yes,location=0,statusbar=0,menubar=0,dependant=1,resizable=1,width=800,height=400,left = 200,top = 184');
}



function onGo(){
document.form1.action="../inhouse/stocktakewithprice";
...
String fieldDesc="";
String USERID ="",PLANT="",LOC ="",  ITEM = "", BATCH="",PRD_CLS_ID="",PRD_TYPE_ID="",PRD_DESCRIP="", QTY ="",PRD_BRAND_ID= "",FROM_DATE="",TO_DATE="";
...
<div class="container-fluid m-t-20">
	<div class="box">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Stock Take With Cost Difference</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 --> 
...
<div class="box-body">
<FORM class="form-horizontal" name="form1" method="post" action="../inhouse/stocktakewithprice">
...
<!--    	  <button type="button" class="Submit btn btn-default" onClick="window.location.href='../home'"><b>Back</b></button> -->
    	 ...
    	 <input type="hidden" name="isWithCost" value="true">
...
<!--               	<table id="tableInventorySummary" class="table table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="tableInventorySummary_info"> -->
					<thead>
		                <tr role="row">
		                	...
		                	<th style="font-size: smaller;">STOCK TAKE COST</th>
		                	<th style="font-size: smaller;">INVENTORY QUANTITY</th>
		                	<th style="font-size: smaller;">INVENTORY COST</th>
		                	<th style="font-size: smaller;">PC/PCS/EA QUANTITY</th>
		                	<th style="font-size: smaller;">PC/PCS/EA COST</th>
		                	<th style="font-size: smaller;">INVENTORY QUANTITY DIFF</th>
		                	<th style="font-size: smaller;">INVENTORY COST DIFF</th>
		                	<th style="font-size: smaller;">PC/PCS/EA QUANTITY DIFF</th>
		                	<th style="font-size: smaller;">PC/PCS/EA COST DIFF</th>
...
function getParameters(){
	return { 
		...
		"ACTION": "VIEW_STOCK_TAKE_SUMMARY_WITHCOST",
		"PLANT":"<%=PLANT%>"
	}
}  
 	...
 	tableInventorySummary.ajax.url( urlStr ).load();
	    ...
	    tableInventorySummary = $('#tableInventorySummary').DataTable({
		        ...
		        "dataSrc": function(data){
		        	if(typeof data.items[0].ITEM === 'undefined'){
		        		return [];
		        			...
		        			data.items[dataIndex]['totalAvalableCost'] = parseFloat(data.items[dataIndex]['totalAvalableCost']).toFixedSpecial(<%=DbBean.NOOFDECIMALPTSFORCURRENCY%>);
		        			data.items[dataIndex]['totalDiffentetCost'] = parseFloat(data.items[dataIndex]['totalDiffentetCost']).toFixedSpecial(<%=DbBean.NOOFDECIMALPTSFORCURRENCY%>);
		        			data.items[dataIndex]['totalInventeryCost'] = parseFloat(data.items[dataIndex]['totalInventeryCost']).toFixedSpecial(<%=DbBean.NOOFDECIMALPTSFORCURRENCY%>);
		        			data.items[dataIndex]['totalPcsCost'] = parseFloat(data.items[dataIndex]['totalPcsCost']).toFixedSpecial(<%=DbBean.NOOFDECIMALPTSFORCURRENCY%>);
		        			data.items[dataIndex]['totalpcsDiffentetCost'] = parseFloat(data.items[dataIndex]['totalpcsDiffentetCost']).toFixedSpecial(<%=DbBean.NOOFDECIMALPTSFORCURRENCY%>);*/
		        			
		        			data.items[dataIndex]['UNITCOST'] = parseFloat(data.items[dataIndex]['UNITCOST']).toFixed(<%=numberOfDecimal%>);
		        			data.items[dataIndex]['totalAvalableCost'] = parseFloat(data.items[dataIndex]['totalAvalableCost']).toFixed(<%=numberOfDecimal%>);
		        			data.items[dataIndex]['totalDiffentetCost'] = parseFloat(data.items[dataIndex]['totalDiffentetCost']).toFixed(<%=numberOfDecimal%>);
		        			data.items[dataIndex]['totalInventeryCost'] = parseFloat(data.items[dataIndex]['totalInventeryCost']).toFixed(<%=numberOfDecimal%>);
		        			data.items[dataIndex]['totalPcsCost'] = parseFloat(data.items[dataIndex]['totalPcsCost']).toFixed(<%=numberOfDecimal%>);
		        			data.items[dataIndex]['totalpcsDiffentetCost'] = parseFloat(data.items[dataIndex]['totalpcsDiffentetCost']).toFixed(<%=numberOfDecimal%>);
		        		}
	        ...
	        "columns": [
 			...
 			{"data": 'totalAvalableCost', "orderable": true},
 			{"data": 'QTY', "orderable": true},
 			{"data": 'totalInventeryCost', "orderable": true},
 			{"data": 'PCSQTY', "orderable": true},
 			{"data": 'totalPcsCost', "orderable": true},
 			{"data": 'strDiffQty', "orderable": true},
 			{"data": 'totalDiffentetCost', "orderable": true},
 			{"data": 'strpcsDiffQty', "orderable": true},
 			{"data": 'totalpcsDiffentetCost', "orderable": true},
...
function getTable(){
            return '<TABLE WIDTH="90%"  border="0" cellspacing="1" cellpadding = 2 align = "center">'+
                    ...
                    '<TH><font color="#ffffff" align="Right" ><b>StockTake Qty</TH>'+
                    '<TH><font color="#ffffff" align="Right" ><b>StockTake Cost</TH>'+
                    '<TH><font color="#ffffff" align="Right" ><b>Inventory Quantity</TH>'+
                    '<TH><font color="#ffffff" align="Right" ><b>Inventory Cost</TH>'+
                    '<TH><font color="#ffffff" align="Right" ><b>PC/PCS/EA Quantity</TH>'+
                    '<TH><font color="#ffffff" align="Right" ><b>PC/PCS/EA Cost</TH>'+
                    '<TH><font color="#ffffff" align="Right" ><b>Quantity Diff</TH>'+
                    '<TH><font color="#ffffff" align="Right" ><b>Cost Diff</TH>'+
                    '<TH><font color="#ffffff" align="Right" ><b>PC/PCS/EA Quantity Diff</TH>'+
                    '<TH><font color="#ffffff" align="Right" ><b>PC/PCS/EA Cost Diff</TH>'+
