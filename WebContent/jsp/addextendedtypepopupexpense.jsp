<div id="create_extended_modalmain" class="modal fade" role="dialog">
			...
			<input type="hidden" id="acc_type_extended_id" name="acc_type_extended_id">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-success" id="create_user"
						onclick="create_extendend_account_main()">Save and Close</button>
...
$(document).ready(function() {
	$("#create_extended_modalmain #ealert").hide();
	exendloadAccountmain();
	function exendloadAccountmain()
	{
		$.ajax({
			type : "POST",
			...
			success : function(dataitem) {
				...
				$("#create_extended_modalmain #acc_type_extended").select2({
	...
	function selectExtAccTypemain(id)
	{
		$("#create_extended_modalmain #acc_type_extended").val(id).trigger('change');
		//$('#acc_type').val(id);
	}
	$("#create_extended_modalmain").on('show.bs.modal', function(){
		$("#create_extended_modalmain #ealert").hide();
		$('#create_extended_modalmain #ealert').html("Account type already exist.");
		$("#create_extended_modalmain #acc_det_type_extended").val("");
		var acctype=$('#create_account_modal #acc_type').val();
		//AccDetailType(acctype);
		console.log("Extend type:"+acctype);
		selectExtAccTypemain(acctype);
	});
});

function create_extendend_account_main()
{
	var extaccid=$("#create_extended_modalmain #acc_type_extended").val();
	var extdettype=$("#create_extended_modalmain #acc_det_type_extended").val();
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(count>0)
				{
					$("#create_extended_modalmain #ealert").show();
		    		$('#create_extended_modalmain #ealert').html("Account type already exist.");
					return false;
				}
			else
				...
				$.ajax({
					...
					success : function(data) {
						if (data.STATUS == "FAIL") {		                               
							...
							$('#create_extended_modalmain').modal('toggle');
							var acctype=$('#create_account_modal #acc_type').val();
							AccDetailType(acctype);
							//createDetail(data.EXTENDEDID,data.EXTENDEDTYPE);
							$("#create_account_modal #acc_det_type").typeahead('val', data.EXTENDEDTYPE);
							$("#create_account_modal #acc_det_type_id").val(data.EXTENDEDID);
						}
					},
					error : function(data) {
