...
ArrayList listQry = itemUtil.getAlternateBrandProductList(sItem,plant,sItemDesc);
String item="",itemdesc="",productbrand="";
try

    {
     ...
     //	sBGColor = ((i == 0) || (i % 2 == 0)) ? "#FFFFFF" : "#dddddd";
     	...
     	if(type.equalsIgnoreCase("MAINITEM")){
	     	item   =  strUtils.fString((String)m.get("ITEM"));
	        itemdesc    =  _ItemMstDAO.getItemDesc(plant,item);
	        productbrand=strUtils.fString((String)m.get("PRD_BRAND_ID"));
     	}
     	else
     	{
     		item   =  strUtils.fString((String)m.get("ALTERNATE_ITEM_NAME"));
	        itemdesc    =  _ItemMstDAO.getItemDesc(plant,item);
	        productbrand=strUtils.fString((String)m.get("ALTERNATE_ITEM_BRAND_ID"));
     	}
        if(type.equalsIgnoreCase("MAINITEM")){
%>
...
<TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			onClick="window.opener.form.ITEM.value='<%=item%>';window.opener.form.DESC.value='<%=itemdesc%>';window.opener.form.PRD_BRAND_ID.value='<%=productbrand%>';window.close();"><%=item%></a></td>
...
<%   }else{%>
		<TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			onClick="window.opener.form.ALTERNATEBRANDITEM.value='<%=item%>';window.opener.form.ALTERNATEBRANDDESC.value='<%=itemdesc%>';window.opener.form.ALTERNATE_PRD_BRAND_ID.value='<%=productbrand%>';window.close();"><%=item%></a></td>
			<td align="left" class="main2">&nbsp;<%=itemdesc%></td>
		<td align="left" class="main2">&nbsp;<%=productbrand%></td>
