...
<%
	...
	int paymentid = Integer.valueOf((String) expenselist.get("PAYMENTID"));
	double debitnoteamt = Double.valueOf((String) expenselist.get("DEBITNOTEAMOUNT"));
	String debitrmks = (String) expenselist.get("DEBITNOTEREMARK");
	String dbdate = "";
	String dbamt = "0.0";
	if(paymentid > 0){
		dbdate = billPaymentDAO.getpaydebitdate(paymentid, plant);
	 	dbamt = StrUtils.addZeroes(Double.valueOf(billPaymentDAO.getpayamtbalance(paymentid, plant)), numberOfDecimal);
	}
	...
	<div class="box">
	 ...
	 <ul class="breadcrumb backpageul">    
                <li class="underline-on-hover"><a href="../home">Dashboard</a></li>	
                 <li><a href="../expenses/apsummary"><span class="underline-on-hover">Expenses Summary</span></a></li> 
		...
		<div class="box-header menu-drop">
			...
			<%if(expjnledit.equals("1")){
				...
				<%if(paymentid == 0){ %>
				<div class="btn-group" role="group">
					<button type="button pull-right" id="applydebitnote" class="btn btn-success ">Apply Debit Note</button>
				</div>
				<%}else{ %>
				<div class="btn-group" role="group">
					<button type="button pull-right" id="editapplydebitnote" class="btn btn-success ">View Debit Note</button>
				</div>
				<%} %>
				&nbsp;
				<div class="btn-group" role="group">
				 <% if (displaySummaryEdit) { %>
					<button type="button" class="btn btn-default"
					 ...
					 <a href="../expenses/apnew?cmd=Edit&TRANID=<%=ExpenseHDRid%>">
				...
				<% if (displayConvertToInvoice) { %>
					<%if(expenselist.get("STATUS").equals("UNBILLED")){ %>
						<a href="../expenses/apconverttoinvoice?cmd=insert&TRANID=<%=ExpenseHDRid%>"><button type="button pull-right" class="btn btn-success ">Convert to Invoice</button></a>
					<%}%>
					<%}%>
				</div>
				&nbsp;
				...
				<div class="btn-group" role="group">
					  ...
					  <button type="button" class="btn btn-default" data-toggle="dropdown" >More <span class="caret"></span></button>
					   ...
					   <ul class="dropdown-menu" style="min-width: 0px;">
					    <li id="bill-copy"><a href="../expenses/apnew?cmd=Copy&TRANID=<%=ExpenseHDRid%>">Copy</a></li>
				...
				<h1 style="font-size: 18px; cursor: pointer; vertical-align:middle;" class="box-title pull-right"
					onclick="window.location.href='../expenses/apsummary'">
				...
				<table id="table2" class="table" style="table-layout: fixed;">
					...
					for(int i =0; i<expenselisthdrdet.size(); i++) {  
						Map m=(Map)expenselisthdrdet.get(i);
				  		expacc=(String)m.get("EXPENSES_ACCOUNT");
			...
			<div class="col-xs-12">
				<table id="table3" class="table text-right">
					<tbody>
						...
						<%if(paymentid > 0){ %>
						<tr>
							<td><b>Debit Note Amount (<%=curency%>)</b></td>
							<td><%=debitnoteamt%></td>
						</tr>
						<%} %>>
					</tbody>
	...
	<div id="deleteexpense" class="modal fade" role="dialog">
	  <div class="modal-dialog modal-sm">	
		...
		<div class="row">
			...
			<div id="applydebitnotepopup" class="modal fade" role="dialog">
				<div class="modal-dialog modal-lg box-body">
					<form id="journalForm" class="form-horizontal" name="form1"
						action="/track/ExpensesServlet?ACTION=AP_DEBIT_NOTE" method="post">
						<!-- Modal content-->
						<div class="modal-content">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal">&times;</button>
								<h3 class="modal-title">Debit Note</h3>
							</div>
							<div class="modal-body">
								<input type="text" name="username" value=<%=username%> hidden>
								<input type="text" name="plant" value="<%=plant%>" hidden>
								<input type="text" name="currency" value="<%=basecurrency%>" hidden>
								<input type="text" name="vendno" value="<%=expenselist.get("VENDNO")%>" hidden> 
								<input id="expacc" name="expacc" value="<%=expacc%>" hidden> 
								<div class="row form-group">
									<label class="col-form-modal-label col-sm-offset-3 col-sm-2">Expense
										Amount</label>
									<div class="col-sm-4">
										<input class="form-control" name="expamount" id="expamount"
											type="text" value="<%=totalAmount%>" disabled> <input
											name="eid" id="eid" type="text" value="<%=ExpenseHDRid%>"
											hidden>
									</div>
								</div>
								<div class="row form-group">
									<label class="col-form-modal-label col-sm-offset-3 col-sm-2">Expense
										Date</label>
									<div class="col-sm-4">
										<input class="form-control" name="expdate" id="expdate"
											type="text" value="<%=expenselist.get("EXPENSES_DATE")%>"
											disabled>
									</div>
								</div>
								<div class="row form-group">
									<label class="col-form-modal-label col-sm-offset-3 col-sm-2 required">Debit Note Date</label>
									<div class="col-sm-4">
										<input class="form-control datepicker" name="debitnotedate" id="debitnotedate"
											type="text" >
									</div>
								</div>
								<div class="row form-group">
									<label class="col-form-modal-label col-sm-offset-3 col-sm-2 required">Debit Note Amount</label>
									<div class="col-sm-4">
										<input class="form-control" name="debitnoteamount" id="debitnoteamount" type="text" onkeypress="return isNumberKey(event,this,4)" required>
									</div>
								</div>
								<div class="row form-group">
									<label class="col-form-modal-label col-sm-offset-3 col-sm-2">Debit Note Remarks</label>
									<div class="col-sm-4">
								<textarea rows="2" name="expdebitremarks"
...
<!-- onclick="()"  -->
								...
								<div class="row form-group">
									...
									<div class="col-sm-3">
										<div class="alert-actions btn-toolbar">
											<button type="submit" class="btn btn-primary ember-view" style="background: green;">Submit</button>
											<button type="button" class="btn btn-primary ember-view" data-dismiss="modal" style="background: red;">Cancel</button>
										</div>
									</div>
									<div class="col-sm-5"></div>
								</div>
			...
			<div id="editapplydebitnotepopup" class="modal fade" role="dialog">
									<div class="modal-dialog modal-lg box-body">
										<form id="ejournalForm" class="form-horizontal" name="eform1"
											action="/track/ExpensesServlet?ACTION=AP_DEBIT_NOTE"
											method="post">
											<!-- Modal content-->
											<div class="modal-content">
												<div class="modal-header">
													<button type="button" class="close" data-dismiss="modal">&times;</button>
													<h3 class="modal-title">View Debit Note</h3>
												</div>
												<div class="modal-body">
													<input type="text" name="eusername" value=<%=username%>
														hidden> <input type="text" name="eplant"
														value="<%=plant%>" hidden> <input type="text"
														name="ecurrency" value="<%=basecurrency%>" hidden>
													<input type="text" name="evendno"
														value="<%=expenselist.get("VENDNO")%>" hidden> <input
														type="text" name="eexpacc" value="<%=expacc%>" hidden>
													<input type="text" name="epayid" value="<%=paymentid%>"
														hidden> <input type="text" name="epaybalanceamt"
														value="<%=dbamt%>" hidden>
													<div class="row form-group">
														<label class="col-form-modal-label col-sm-offset-3 col-sm-2">Expense
															Amount</label>
														<div class="col-sm-4">
															<input class="form-control" name="eexpamount"
																id="eexpamount" type="text" value="<%=totalAmount%>"
																disabled> <input name="eeid" id="eeid"
																type="text" value="<%=ExpenseHDRid%>" hidden>
														</div>
													</div>
													<div class="row form-group">
														<label class="col-form-modal-label col-sm-offset-3 col-sm-2">Expense
															Date</label>
														<div class="col-sm-4">
															<input class="form-control" name="eexpdate" id="eexpdate"
																type="text"
																value="<%=expenselist.get("EXPENSES_DATE")%>" disabled>
														</div>
													</div>
													<div class="row form-group">
														<label class="col-form-modal-label col-sm-offset-3 col-sm-2 required">Debit
															Note Date</label>
														<div class="col-sm-4">
															<input class="form-control"
																name="edebitnotedate" id="edebitnotedate"
																value="<%=dbdate%>" type="text" disabled>
														</div>
													</div>
													<div class="row form-group">
														<label class="col-form-modal-label col-sm-offset-3 col-sm-2 required">Debit
															Note Amount</label>
														<div class="col-sm-4">
															<input class="form-control" name="edebitnoteamount"
																id="edebitnoteamount" type="text"
																value="<%=debitnoteamt%>"
																onkeypress="return isNumberKey(event,this,4)" disabled>
														</div>
													</div>
													<div class="row form-group">
														<label class="col-form-modal-label col-sm-offset-3 col-sm-2">Debit
															Note Remarks</label>
														<div class="col-sm-4">
															<textarea rows="2" name="eexpdebitremarks" disabled
																class="ember-text-area form-control ember-view"
																maxlength="950"><%=debitrmks%></textarea>
														</div>
													</div>
													...
													<!-- <div class="row form-group">
														...
														<div class="col-sm-3">
															<div class="alert-actions btn-toolbar">
																<button type="button"  onclick="addeditdebitnote()" class="btn btn-primary ember-view"
...
/* 
			...
			$("#applydebitnote").click(function() {  
				$('#applydebitnotepopup').modal('show');
			 });
			
			$("#editapplydebitnote").click(function() {  
				$('#editapplydebitnotepopup').modal('show');
			 });
