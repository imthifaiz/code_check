...
<%
	String title = "Balance Sheet Account Transactions";
	...
	String accountcode = coaDAO.GetAccountCodeByName(trialBalanceDetails.get(0).getACCOUNT(), plant);
...
<div class="container-fluid m-t-20">
	...
	<div class="box">
		...
		<div class="box-header menu-drop">
			 <h1 style="font-size: 18px;cursor:pointer;" class="box-title pull-right" onclick="window.location.href='../businessoverview/balance'">
			...
			<div class="row">
				...
				<div class="col-sm-4" style="text-align:center">
					...
					<div style="font-size:18px"><%=accountcode+" "+trialBalanceDetails.get(0).getACCOUNT()%></div>
		...
		<div class="box-body">
			 ...
			 <table class="table table-striped" id="balancesheetledgers">
    <thead>
      <tr>
        <th>DATE</th>
        <th>ACCOUNT CODE</th>
    ...
    <tbody>
     <tr>
    	...
    	<%for(TrialBalanceDetails trialBalDet:trialBalanceDetails) 
    	{
    		...
    		String actcode = coaDAO.GetAccountCodeByName(trialBalDet.getACCOUNT(), plant);
    	%>
      <tr>
    ...
    <%
        ...
        <td><%=n.format(Double.parseDouble(String.format("%."+numberOfDecimal+"f", trialBalDet.getDEBIT()))) %></td>
        <td><%=n.format(Double.parseDouble(String.format("%."+numberOfDecimal+"f", trialBalDet.getCREDIT()))) %></td>
        <%Double amount=trialBalDet.getDEBIT()-trialBalDet.getCREDIT();
        if(amount>0){ %>
        <td><%=n.format(Double.parseDouble(String.format("%."+numberOfDecimal+"f", amount)))%>Dr</td>
        <%}else{%>
         <td><%=n.format(Double.parseDouble(String.format("%."+numberOfDecimal+"f", Math.abs(amount)))) %>Cr</td>
        <%} %>
      </tr>
      <%} %>
      <tr>
    	 ...
    	 <td></td>
    	 <td></td>
    	 <td><%=n.format(Double.parseDouble(String.format("%."+numberOfDecimal+"f", totalDebit))) %></td>
    	 <td><%=n.format(Double.parseDouble(String.format("%."+numberOfDecimal+"f", totalCredit))) %></td>
    	 <td></td>
    	 </tr>
      <tr>
      <% totalClosingBalance=(totalDebit-totalCredit);%>
        ...
        <td><%=curency%> <%=n.format(Double.parseDouble(String.format("%."+numberOfDecimal+"f", totalClosingBalance)))%></td>
        <%}else{%>
        <td></td>
        <%} %>
        <% if(totalClosingBalance<0){ %>
         <td><%=curency%> <%=n.format(Double.parseDouble(String.format("%."+numberOfDecimal+"f", Math.abs(totalClosingBalance)))) %></td>
        <%}else{%>
        <td></td>
        <%} %>
    	 <td></td>
...
{     
	...
	var accname = "<%=trialBalanceDetails.get(0).getACCOUNT()%>";
...
function generatePdf(dataUrl){	
	...
	if (typeof doc.putTotalPages === 'function') {
	  ...
	  doc.save('Balancesheetdetails.pdf')
}
...
function generatePdfMail(dataUrl,attachName){
	...
	doc.setFontSize(12);
	doc.text('<%=trialBalanceDetails.get(0).getACCOUNT()%>',cwidth,50,'center');
