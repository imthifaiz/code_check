...
if(systatus.equalsIgnoreCase("ACCOUNTING")) {
	displaySummaryNew = ub.isCheckValAcc("newbill", plant,username);
	displaySummaryLink = ub.isCheckValAcc("summarylnkbill", plant,username);
	displaySummaryExport = ub.isCheckValAcc("exportbill", plant,username);
}
	 ...
	 <div class="box"> 
	  <!-- Muruganantham Modified on 16.02.2022 -->
            <ul class="breadcrumb backpageul" >      	
                ...
                <li><label>Bill Summary</label></li>                                   
            </ul>             
    <!-- Muruganantham Modified on 16.02.2022 -->
		<div class="box-header menu-drop">
              ...
              <div class="box-title pull-right">
               <div class="btn-group" role="group">
             	<button type="button" class="btn btn-default" onClick="window.location.href='../bill/import'">Import Bill</button>
			      &nbsp;
				</div>
               <% if (displaySummaryNew) { %>
              <button type="button" class="btn btn-default" onClick="window.location.href='../bill/new'" style="background-color: rgb(214, 72, 48);color: rgb(255, 255, 255);" >+ New</button>&nbsp;
               <% } %>
...
<!-- 				<div class="input-group">  -->
				 <input type="text" class="ac-selected form-control" id="vendnames" placeholder="SUPPLIER" name="vendname" >				
...
<!-- 				</div> -->
      ...
      <div class="ShowSingle">
              ...
              <table id="tableInventorySummary" class="table table-bordred table-striped">                   
                   ...
                   <tr><th style="font-size: smaller;">DATE</th>
                    ...
                    <th style="font-size: smaller;">GRNO</th>
                     ...
                     <th style="font-size: smaller;">AVAILABLE CREDIT (<%=curency%>)</th></tr>
                   </thead>
		...
		var numberOfDecimal = "<%=numberOfDecimal%>";
		 var FROM_DATE,TO_DATE,USER,ORDERNO,ITEM,BILL,GRNO,REFERENCE,STATUS,ORDERTYPE, groupRowColSpan = 6;
		function getParameters(){
			return {
				"FDATE":FROM_DATE,"TDATE":TO_DATE,"BILL":BILL,"GRNO":GRNO,"ORDERTYPE":ORDERTYPE,
				"CNAME":USER,"ORDERNO":ORDERNO,"ITEM":ITEM,"REFERENCE":REFERENCE,"STATUS":STATUS,
				"ACTION": "VIEW_BILL_SUMMARY_VIEW",
				"PLANT":"<%=plant%>"
			}
		}  
		  function onGo(){
		   var flag    = "false";
		    ...
		    ORDERTYPE	= document.form.ORDERTYPE.value;
		   ...
		   if(USER != null    && USER != "") { flag = true;}
			...
			/*  storeInLocalStorage('billSummary_FROMDATE', FROM_DATE);
				storeInLocalStorage('billSummary_TODATE', TO_DATE);
				storeInLocalStorage('billSummary_SUPPLIER', USER);
				storeInLocalStorage('billSummary_PONO', ORDERNO);
				storeInLocalStorage('billSummary_ITEM', ITEM);
				storeInLocalStorage('billSummary_GRNO', GRNO);
				storeInLocalStorage('billSummary_BILL', BILL);
				storeInLocalStorage('billSummary_REFERNCE', REFERENCE);
				storeInLocalStorage('billSummary_STATUS', STATUS); */
				        ...
				        "dataSrc": function(data){
				        	if(typeof data.items[0].jobnum === 'undefined'){
				        		...
				        		for(var dataIndex = 0; dataIndex < data.items.length; dataIndex ++){
				        			...
				        			data.items[dataIndex]['availablecredit'] = parseFloat(data.items[dataIndex]['availablecredit']).toFixed(numberOfDecimal).replace(/\d(?=(\d{3})+\.)/g, "$&,");
				        			
				        			 <% if (displaySummaryLink) { %>
				        			data.items[dataIndex]['bill'] = '<a href="../bill/detail?BILL_HDR=' +data.items[dataIndex]['id']+'">'+data.items[dataIndex]['bill']+'</a>';
...
<script>
 $(document).ready(function(){
	 if (document.form.ORDERTYPE.value == ''){
		   getLocalStorageValue('viewBillSummary','', 'ORDERTYPE_SRH');
		 }
	 /* getLocalStorageValue('billSummary_FROMDATE',  $('#FROM_DATE').val(), 'FROM_DATE');
	 getLocalStorageValue('billSummary_TODATE', '', 'TO_DATE');
	 getLocalStorageValue('billSummary_SUPPLIER', '', 'vendname');
	 getLocalStorageValue('billSummary_PONO', '', 'pono');
	 getLocalStorageValue('billSummary_ITEM', '', 'item');
	 getLocalStorageValue('billSummary_GRNO', '', 'grno');
	 getLocalStorageValue('billSummary_BILL', '', 'bill');
	 getLocalStorageValue('billSummary_REFERNCE', '', 'REFERENCE');
	 getLocalStorageValue('billSummary_STATUS', '', 'status'); */
 onGo();
 $('.Show').click(function() {
	    $('#target').show(500);
	    $('.ShowSingle').hide(0);
 ...
 function storeUserPreferences(){
		storeInLocalStorage('viewBillSummary', $('#ORDERTYPE_SRH').val());
}
 </script>
