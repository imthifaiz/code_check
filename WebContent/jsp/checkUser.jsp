    ...
    PlantMstUtil _PlantMstUtil =new PlantMstUtil();
   	...
   	if(arrSize==0)
   	{
   		...
   		if (!"track".equalsIgnoreCase((String)lineArr.get("plant"))){
   	   		...
   	   		boolean isAccountinEnabled = _PlantMstUtil.isAccountingModuleEnabled((String)lineArr.get("plant"));
   	   		boolean isPayrollEnabled = _PlantMstUtil.isPayrollModuleEnabled((String)lineArr.get("plant"));
   	   		if (!isInventoryEnabled && !isAccountinEnabled && !isPayrollEnabled){
   	   			statusCode = 111;
   	   		}
   		}
   	}
   	...
   	}else if (arrSize>1)
   	{
	    ...
	    //DateUtils _dateUtils = new DateUtils(company);
...
//            String plant = ub.getDepartment(user_id);
          ...
          session.setAttribute("VALID_USER", "");
          session.setAttribute("LOGIN_USER", user_id);
          session.setAttribute("IDS", pwd);//select multi company fix-azees 21.02.22
...
<%
             ...
             {
        	 boolean isInventoryEnabled = _PlantMstUtil.isInventoryModuleEnabled(company);
    	   	 boolean isAccountinEnabled = _PlantMstUtil.isAccountingModuleEnabled(company);
    	   	 boolean isPayrollEnabled = _PlantMstUtil.isPayrollModuleEnabled(company);
        	 String rootURI = HttpUtils.getRootURI(request);
        	 if(isInventoryEnabled && !isAccountinEnabled && !isPayrollEnabled)
           	 response.sendRedirect(rootURI+"/DashboardServlet?ACTION=LOGIN_TRANSFER&LOGPAGE=INVENTORY");
        	 else if(!isInventoryEnabled && isAccountinEnabled && !isPayrollEnabled)
           	 response.sendRedirect(rootURI+"/DashboardServlet?ACTION=LOGIN_TRANSFER&LOGPAGE=ACCOUNTING");
        	 else if(!isInventoryEnabled && !isAccountinEnabled && isPayrollEnabled)
           	 response.sendRedirect(rootURI+"/DashboardServlet?ACTION=LOGIN_TRANSFER&LOGPAGE=PAYROLL");
        	 else
   	...
   	try{
        ...
        case 111: result = "Module is not enabled for the company " + company; break;
        }
