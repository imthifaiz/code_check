...
<%
	...
	String NOOFORDER=StrUtils.fString((String) session.getAttribute("NOOFORDER"));
	String SupValidNumber="",OrdValidNumber="";
...
// 	DateUtils _dateUtils = new DateUtils();
	...
	String isgrn=StrUtils.fString(request.getParameter("isgrn"));
		String gbill=StrUtils.fString(request.getParameter("gbill"));
// 		String Urlred="billSummary.jsp";
		String Urlred="../bill/summary";
		if(isgrn.equalsIgnoreCase("1"))
		{
			Urlred="../goods/detail?action=View&PONO="+pono+"&GRNO="+grno+"&VEND_NAME="+vendName+"&VENDNO="+vendno+"";
		}
	String shimentid = "0";
	String instatus = "", origin = "manual", deductInv = "1";
	...
	if (action.equalsIgnoreCase("view")) {
		...
		if(StrUtils.fString(shipmentcode).length() > 0){
			  movQryList = _MasterUtil.getExpenseDetailusingponoanddnol(plant, pono, shipmentcode);
		        if(!movQryList.isEmpty()){
		        Map shipmMap = (Map) movQryList.get(0);
				shimentid	= (String) shipmMap.get("ID");
		        }
		}else{
			Hashtable ht = new Hashtable();
	        if(StrUtils.fString(pono).length() > 0)       ht.put("PONO",pono);
	        movQryList = _MasterUtil.getShipmentRefForBill(plant,pono,"");	
	        /* if(!movQryList.isEmpty()){
	        Map shipmMap = (Map) movQryList.get(0);
			shipmentcode = (String) shipmMap.get("SHIPMENT_CODE");
			shimentid	= (String) shipmMap.get("ID");
	        } */
		}
		origin = "sales";
	...
	String gstexp = sb.getGST("STANDARD RATE",plant);
	gstexp=StrUtils.addZeroes(Double.parseDouble(gstexp), "3");
...
<div class="container-fluid m-t-20">
	<div class="tab">
		<button class="tablinks active" id="inventory"
			onclick="openTab(event, 'inventory')">Inventory</button>
		<button class="tablinks" onclick="openTab(event, 'noninventory')">Non
			Inventory</button>
	</div>
       ...
       <!-- Muruganantham Modified on 16.02.2022 -->
			...
			<div class="container-fluid">
				...
				<form id="createBillForm" class="form-horizontal" name="form" autocomplete="off"
					...
					<input type="text" name="vendno" id="vendno" value="<%=vendno%>"
						hidden> <INPUT type="hidden" name="curency"
						value="<%=curency%>"> <INPUT type="hidden" name="action"
						value="<%=action%>"> <INPUT type="hidden"
						name="STATE_PREFIX" value="<%=plantstatecode%>" /> <INPUT
						type="hidden" name="PO_STATE_PREFIX" value="" /> <INPUT
						type="hidden" name="TAXTREATMENT_VALUE" value="<%=sTAXTREATMENT%>">
						<input type="hidden" name="TRANSPORTID" value="">
					<INPUT type="hidden" name="isgrn" value="<%=isgrn%>"> <INPUT
						type="hidden" name="taxpercentage" value="0"> <INPUT
						type="hidden" name="shiptaxstatus" value="0"> <INPUT
						type="hidden" name="CURRENCYID" value="<%=DISPLAYID%>"> <INPUT
						type="hidden" name="ORIGIN" value="<%=origin%>"> <INPUT
						type="hidden" name="DEDUCT_INV" value="<%=deductInv%>"> <input
						type="hidden" name="PROJECTID" value=""> <input
						type="hidden" name="SHIPPINGID" value=""> <input
					...
					<input type="hidden" name="ptaxtype" value=""> <input
				...
				<INPUT type="hidden" name="ISPRODUCTMAXQTY"  value="<%=isproductmaxqty%>">   <!-- imthi -->
				 <input name="COMP_INDUSTRY" type="hidden" value="<%=COMP_INDUSTRY%>">  <!-- imti added condition based on product popup -->
						...
						<input type="hidden" name="CURRENCYUSEQTOLD" value="<%=CURRENCYUSEQT%>"> <input
						type="hidden" name="aorderdiscount" value="0"> <INPUT
						type="hidden" name="ValidNumber" value="<%=OrdValidNumber%>">
						...
						<div class="col-sm-6 ac-box">
							<!-- <div class="input-group"> -->
								<input type="text" class="ac-selected  form-control typeahead"
									onclick="removetaxtrestl()" id="vendname"
									placeholder="Select a Supplier" name="vendname" onchange="checksupplier(this.value)"
									value="<%=vendName%>"> <span class="select-icon"
									onclick="$(this).parent().find('input[name=\'vendname\']').focus()">
									<i class="glyphicon glyphicon-menu-down" id="ibtnpop"></i>
								</span> 
<!-- 								<span class="btn-danger input-group-addon" id="btnpop" -->
<!-- 									onclick="javascript:popUpWin('vendor_list.jsp?AUTO_SUGG=Y&CUST_NAME='+form.vendno.value);"> -->
<!-- 									<span class="glyphicon glyphicon-search" aria-hidden="true"></span> -->
<!-- 								</span> -->
							<!-- </div> -->
							<SELECT class="form-control taxdropdown" data-toggle="dropdown"
								style="border: none;" onchange="OnChkTaxChange(this.value)"
								data-placement="right" id="nTAXTREATMENT" name="nTAXTREATMENT"
			...
			for(int i=0 ; i<ccList1.size();i++)
			 ...
			 %>
					...
					<div class="form-group shipCustomer-section">
						<label class="control-label col-form-label col-sm-2">Shipping
							Address:</label>
						<div class="col-sm-6 ac-box">
							<input type="text" class="form-control typeahead"
								id="SHIPPINGCUSTOMER" name="SHIPPINGCUSTOMER" onchange="checkcustomer(this.value)" value="">
					...
					<div class="form-group shipCustomer-section">
						...
						<div class="col-sm-4 ac-box">
							<input type="text" class="form-control typeahead" id="project"
								placeholder="Select a project" name="project" onchange="checkproject(this.value)" value="">
							<span class="select-icon"
				...
				<input type="hidden" name="project" value="">
					...
					%>
						<div class="col-sm-4">
							<input type="text" class="form-control pono" id="pono"
								name="pono" value="<%=pono%>"> <span class="select-icon"
					...
					%>
						<div class="col-sm-4">
							<input type="text" class="form-control pono" name="pono"
								value="<%=pono%>" readonly>
						</div>
						<%
						}
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							<div class="input-group">
								<input type="text" class="form-control" id="grno" name="grno"
									value="<%=grno%>" readonly> <span
									class="input-group-addon" id="invElsh"
									onClick="javascript:onNewGRN();return false;"> <a
									href="#" data-toggle="tooltip" data-placement="top"
									title="Auto-Generate"> <i class="glyphicon glyphicon-edit"
										style="font-size: 15px;"></i></a></span>
							</div>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-form-label col-sm-2">Shipment
							Reference</label>
						<div class="col-sm-4">
							<input type="text" class="form-control" id="shipRef"
								name="shipRef" value="<%=shipmentcode%>"> <span
								class="select-icon"
								onclick="$(this).parent().find('input[name=\'shipRef\']').focus()">
								<i class="glyphicon glyphicon-menu-down"></i>
							</span>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							<input type="text" class="form-control" id="refNum" name="refNum" 
								value="<%=refnum%>" maxlength="50">
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-form-label col-sm-2 required">Bill
							Date</label>
						<div class="col-sm-4">
							<input type="text" class="form-control datepicker" id="bill_date"
								value="<%=curDate%>" name="bill_date" READONLY>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-4">
							<input type="text" class="form-control datepicker"
								placeholder="Select due date" id="due_date" name="due_date"
								READONLY>
						</div>
						...
						<div class="col-sm-6">														
							...
							name="payment_terms" placeholder="Select Payment Terms">
							<span class="select-icon"
						...
						<div class="col-sm-6">														
							<input type="text" class="form-control" id="payment_terms"
								name="payment_terms" onchange="checkpaymentterms(this.value)" value="" placeholder="Select Payment Terms">
							<span class="select-icon"
								onclick="$(this).parent().find('input[name=\'payment_terms\']').focus()"><i
								class="glyphicon glyphicon-menu-down"></i></span>
						</div>
					...
					<div class="col-sm-4 ac-box">
						<input type="text" class="ac-selected form-control typeahead"
							id="EMP_NAME" name="EMP_NAME" onchange="checkemployeess(this.value)" value="<%=empname%>" placeholder="Select a employee">
					...
					<div class="form-group">
						<label class="control-label col-form-label col-sm-2"
							for="Purchase Location">Purchase Location:</label>
						<div class="col-sm-4">
							<input type="text" class="ac-selected form-control typeahead"
					...
					<div class="form-group">
						...
						<div class="col-sm-4 ac-box">
							<input type="text" class="ac-selected form-control typeahead"
								id="CURRENCY" name="CURRENCY" onchange="checkcurrency(this.value)" placeholder="Select a Currency"
								value="<%=DISPLAY%>"> <span class="select-icon"
					...
					<div class="col-sm-4">
			   		 	<%if(Tpono.length() > 0){ %>
						<input type="text" class="form-control" id="GST" name="GST" onchange="changingtaxpercentage()" onkeypress="return isNumberKey(event,this,4)" value="<%=gst%>" readonly>
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							<input type="text" class="form-control" id="Purchasetax"
								name="Purchasetax" onchange="checktax(this.value)" placeholder="Select a Tax"> <span
								class="select-icon"
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							<div id="CHK1">
								<input type="checkbox" class="check" id="REVERSECHARGE"
									name="REVERSECHARGE" <%if(sREVERSECHARGE.equals("1")) {%>
									checked <%}%> /><b>&nbsp;This transaction is applicable for
									reverse charge </b> <br/> <input type="checkbox" class="check"
									id="GOODSIMPORT" name="GOODSIMPORT"
									<%if(sGOODSIMPORT.equals("1")) {%> checked <%}%> /><b>&nbsp;This
									transaction is applicable for goods import &nbsp; &nbsp; </b>
							</div>
						</div>
					</div>
					<hr />
							...
							<tbody>
								<tr>
									<td class="item-img text-center">
										...
										type="hidden" name="convcost" value="0.00"> 
									...
									<input type="hidden" name="minsp" value="">
										...
										<input type="hidden" name="tax" class="taxSearch" value=""><input
										type="hidden" name="basecost" value="0.00"> <!-- 				  <input type="hidden" name="lnno" value="1"> -->
									</td>
									<td class="bill-item"><input type="text" name="item" onchange="checkitems(this.value,this)" 
										class="form-control itemSearch" style="width: 92%"
									...
									<td class="bill-item"><input type="text" value="Inventory Asset"
										name="account_name" class="form-control accountSearch"
										placeholder="Select Account"></td>
									<td class="bill-item invEl"><input type="text" name="uom" onchange="checkprduom(this.value,this)"
										class="form-control uomSearch" placeholder="UOM"></td>
									<td class="bill-item invEl"><input type="text" name="loc" onchange="checkprdloc(this.value,this)" 
										class="form-control locSearch" placeholder="Location"></td>
									<td class="bill-item invEl"><div class="input-group">
											<input type="text" name="batch"
												class="form-control batchSearch" value="NOBATCH"
												placeholder="Batch"> <span class="input-group-addon"
												onclick="javascript:generateBatch(this);return false;"
												id="actionBatch" name="actionBatch"> <a href="#"
											...
											<div class=" col-lg-12 col-sm-3 col-12">
												<div class="input-group my-group" style="width: 120px;">
													<input name="item_discount" type="text"
														...
														onchange="calculateAmount(this)" onkeypress="return isNumberKey(event,this,4)"> <select
														name="item_discounttype" id="item_discounttype"
...
<!-- 									<td class="item-tax"> -->
<!-- 										<input type="hidden" name="tax_type"> <input -->
<!-- 										type="text" name="tax" class="form-control taxSearch" -->
...
<!-- 									</td> -->
									<td class="item-amount text-right grey-bg"><input
										...
										value="0.00" readonly="readonly" tabindex="-1"> <input
										name="landedCost" type="text" value="0.0" hidden></td>
								</tr>
							</tbody>
						</table>
					</div>
...
<!-- 								onclick="addRow()">+ Add another line</a> -->
								<a href="#" onclick="addRow(event);"<%if(isgrn.equalsIgnoreCase("1")){ %> style="display: none" <% } %>>
									...
									<div class="row">
										...
										<div class="col-lg-1 col-sm-1 col-1">
											<input type="checkbox" class="form-check-input isodisctax"
												name="isodisctax" Onclick="isodisctaxing(this)">
										</div>
										<div class=" col-lg-6 col-sm-6 col-6">
											<div class="input-group my-group">
												<input class="form-control text-right oddiscount"
													id="oddiscount" type="text" value="<%=zerovalper%>"
													readonly name="orderdiscount" onchange="calculateTotal()" onkeypress="return isNumberKey(event,this,4)">
												<!-- <span class="input-group-addon">%</span> -->
												<select class="discountPicker form-control"
													name="oddiscount_type" id="oddiscount_type" disabled="true">
													<option value="%">%</option>
												</select>
											</div>
										</div>
							...
							<div class="total-row">
								<div class="total-label">
									<div class="row">
										...
										<div class=" col-lg-6 col-sm-6 col-6">
											<div class="input-group my-group">
												<input class="form-control text-right dediscount"
													type="text" value="<%=zeroval %>" name="discount"
													onchange="calculateTotal()" onkeypress="return isNumberKey(event,this,4)">
												<!-- <span class="input-group-addon">%</span>	-->
												<select class="discountPicker form-control"
									...
									<div class="row">
										<div class="col-lg-5 col-sm-5 col-5">
											<input type="text" class="form-control"
												name="adjustmentLabel" maxlength="50" value="Adjustment">
										</div>
										<div class="col-lg-6 col-sm-6 col-6">
											<input
												class="form-control text-right ember-view deadjustment"
												value="<%=zeroval%>" type="text" name="adjustment"
												onchange="calculateTotal()" onkeypress="return isNumberKey(event,this,4)">
										</div>
									</div>
								</div>
						...
						if(pono.equals("") || pono.equals(null)){
							...
							<div class="expense-section">
								...
								<div class="accountAddBtn footer addexphide"
									style="margin-top: 0px;" id="addexpensemodel">
									<a href="#">Add Additional Expenses</a>
								</div>
								<br> <br>
							</div>
							<%} %>  
						...
						<div class="col-sm-6 notes-sec">
							...
							<div>
								<textarea rows="2" name="note"
									class="ember-text-area form-control ember-view" maxlength="950"></textarea>
							</div>
						</div>
					</div>
					<input id="sub_total" name="sub_total" value="" hidden> <input
						...
						name="bill_status" value="Save" hidden> <input
						name="oShippingcost" value="" hidden> <input
						name="taxTotal" value="0.0" hidden> <input
						name="totalAdditionalExpenses" value="0.0" hidden>
			...
			<INPUT type="hidden" id="TaxByLabelOrderManagement"
				name="taxbylabelordermanagement"
				value=<%=taxbylabelordermanagement%>> <INPUT type="hidden"
				id="expencetax" name="expencetax" value=<%=gstexp%>>
			...
			<jsp:include page="../jsp/NewAdditionalExpenses.jsp" flush="true" />
			<%-- <%@include file="NewChartOfAccountAdd.jsp"%> --%>
			<jsp:include page="../jsp/NewChartOfAccountpopup.jsp" flush="true" />
			...
			<input type="hidden" name="pronumberOfDecimal"
				id="pronumberOfDecimal" value="<%=numberOfDecimal%>">
			<!-- Modal -->
				 ...
				 <input name="ORDER_TYPE_MODAL" type="hidden" value="BILL">
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							...
							<INPUT type="hidden" name="CUST_CODE1" value=""> <INPUT
								type="hidden" name="COUNTRY" value=""> <INPUT
								type="hidden" name="COUNTRY_REG" id="COUNTRY_REG" value="<%=region%>"> <INPUT
								type="hidden" name="ValidNumber" value="<%=SupValidNumber%>">
								...
								<div class="form-group">
									...
									<div class="col-sm-4 ac-box">
										...
										<INPUT name="BANKNAME" type="TEXT" value="" id="BANKNAME"
											class="form-control" onchange="checksupplierbank(this.value)" placeholder="BANKNAME"> <span
											class="select-icon"
...
function create_accountcoa() {
	...
	if ($('#create_account_modalcoa #acc_code').val() == "") {
		alert("please fill account code");
		$('#create_account_modalcoa #acc_code').focus();
		return false;
	}
	...
	if(document.create_formcoa.acc_is_sub.checked)
	{
		if ($('#create_account_modalcoa #acc_subAcct').val() == "") {
			...
			{
			 //var parType=$( "#acc_type option:selected" ).text();
			 var parType=$('#create_account_modalcoa #acc_det_type').val();
			 subType=subType.trim();
	...
	/* if ($('#create_account_modalcoa #acc_balance').val() != "") {
		if (parseFloat($('#create_account_modalcoa #acc_balance').val()) != 0) {
		if ($('#create_account_modalcoa #acc_balanceDate').val() == "") {
			if ($('#create_account_modalcoa #coamainid').val() == "1" || $('#create_account_modalcoa #coamainid').val() == "3") {
				alert("Please Fill Balance Date");
			}else if($('#create_account_modalcoa #coamainid').val() == "2"){
	...
	var accName=$('#create_account_modalcoa #acc_name').val();
	$.ajax({
		type : 'post',
...
<%-- <%@include file="newTransportModeModal.jsp"%> <!-- imti for add transport -->
<%@include file="newGstDetailModal.jsp"%>
 ...
 <jsp:include page="newPaymentModeModal.jsp" flush="true"></jsp:include><!-- added imthi for payment mode -->
			...
			<jsp:include page="newShipmentModal.jsp" flush="true"></jsp:include>
