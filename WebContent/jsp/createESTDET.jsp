...
<%
String title = "Sales Estimate Order- Add Products";
%>
...
<script language="javascript">
  ...
  function popUpWinForDiscount(URL) {
    subWinForDiscount = window.open(URL, 'Calculate', 'toolbar=0,scrollbars=no,location=0,statusbar=0,menubar=0,dependant=1,resizable=1,width=800,height=400,left = 200,top = 184');
  }
</script>
<script language="javascript">
...
<form class="form-horizontal" name="form" method="post" action="/track/EstimateServlet?Submit=Add Product" onSubmit="return validateForm()">
     ...
     <div class="col-sm-6">  <!-- column is used for grid function -->
      ...
      <div class="col-sm-6">
      <INPUT type = "TEXT" class="form-control" size="20"  MAXLENGTH=20 name="ESTNO"   value="<%=estno%>" READONLY>
	  </div>
      </div>
      ...
      <div class="col-sm-6">          
      <INPUT  class="form-control" name="CUST_NAME" type="TEXT" value="<%=su.forHTMLTag(custName)%>" size="30" MAXLENGTH=100 readonly>
      ...
      <div class="col-sm-6">          
      <INPUT type = "TEXT"  class="form-control" size="20"  MAXLENGTH=4 name="ESTLNNO" value="<%=tpdb.getEstNextLineNo(estno,plant)%>" READONLY>
      </div>
      </div>
      ...
      <span class="input-group-addon" 
   	  onClick="javascript:popUpWin('../jsp/item_list_do.jsp?ITEM='+form.ITEM.value);"> 	
      ...
      <span class="input-group-addon" 
   	  onClick="javascript:popUpWin('../jsp/item_list_do.jsp?ITEM='+form.ITEM.value+'&DESC='+form.DESC.value);"> 	
      <a href="#" data-toggle="tooltip" data-placement="top" title="Product Details">
  	  <i class="glyphicon glyphicon-log-in" style="font-size: 20px;"></i></a></span>
      </div>
  	  </div>
      ...
      <div class="col-sm-6"> 
      <div class="input-group">
      <INPUT class="form-control"  type = "TEXT" size="30"  MAXLENGTH="10" name="QTY" onchange="validToThreeDecimal(this.value)" onkeypress="if((event.keyCode=='13') && ( this.value.length > 0)){validateForm();}">
	 ...
	 <div class="col-sm-6">
      ...
      <TH id="TH2REMARKS_0"><div class="input-group"><INPUT class="form-control" name="PRDREMARKS_0" type="TEXT" id="PRDREMARKS_0" value="<%=PRDREMARKS%>"	size="88" MAXLENGTH="100"> 
			         <a href="#" class="input-group-addon" data-toggle="tooltip" data-placement="top" Title="Remarks Details" onClick="javascript:popUpWin('../jsp/remarks_list.jsp?REMARKS='+form.PRDREMARKS_0.value+'&TYPE=MULTI'+'&INDEX=0');">
     ...
     <a href="#" onclick="deleteRow('MULTI_IB_REMARKS');" data-toggle="tooltip" data-placement="top" Title="Remove Last Remarks">
      ...
      <button type="button" class="Submit btn btn-default" name="Submit" 
       onclick="window.location.href='/track/EstimateServlet?ESTNO=<%=estno%>&RFLAG=<%=RFLAG%>&Submit=View';">
      <b>Back</b></button>&nbsp;&nbsp;
...
function validateProduct() {
	var productId = document.form.ITEM.value;
	 var estno = document.form.ESTNO.value;
     var uprice,mprice;
				...
				success : function(data) {
					...
					if (data.status == "100") {
						...
						if(resultVal.ConvertedUnitCost == null || resultVal.ConvertedUnitCost == undefined || resultVal.ConvertedUnitCost == 0 ){
							document.form.LISTPRICE.value = parseFloat("0.00000").toFixed(<%=numberOfDecimal%>);
						}else{
							document.form.LISTPRICE.value = parseFloat(resultVal.ConvertedUnitCost).toFixed(<%=numberOfDecimal%>);
...
function addRow() {
	...
	remarksCellText = remarksCellText + "<a class=\"input-group-addon\" href=\"#\" onClick=\"javascript:popUpWin('../jsp/remarks_list.jsp?REMARKS='+form.PRDREMARKS"+'_'+rowCount+".value+'&TYPE=MULTI'+'&INDEX="+rowCount+"');\"><i class=\"glyphicon glyphicon-log-in\" style=\"font-size: 20px;\"></i></a></div>";
...
function CheckPriceVal(uom) {
	var productId = document.form.ITEM.value;
	var dono = document.form.ESTNO.value;
