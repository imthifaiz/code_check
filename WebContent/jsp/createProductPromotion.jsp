...
<%
	String title = "Create Product Promotion";
...
<div class="container-fluid m-t-20">
	<div class="box">
             <ul class="breadcrumb backpageul">      	
                  ...
                  <li><label>Create Product Promotion</label></li>                                   
             </ul>   
		...
		<div class="container-fluid">
			<form class="form-horizontal" id="productpromotionForm" name="cpoform" action="../productpromotion/save"  method="post"  enctype="multipart/form-data">
				...
				<div class="col-sm-4">
			    	...
			    	<label class="radio-inline">
			      		<input type="radio" name="BYQTY" id="BYQTY" value=2 onclick="SelectVal(event, 'bydis')">By Discount</label>
			    </div>
				</div>
				...
				<label class="control-label col-form-label col-sm-2" for="Limit Of Usage">Limit Of Usage:</label>
        			<div class="col-sm-4">
        				<INPUT class="form-control" type="TEXT" size="20" MAXLENGTH="100" name="DLimitUsage" value="0.000" id="DLimitUsage" onkeypress="return isNumberKey(event,this,4)">
       				</div>
				</div>
					...
					<table class="table table-bordered line-item-table po-table">
						...
						<tbody>
							<tr>
								...
								<td class="bill-desc BV">
									<input type="text" name="promotionvalue" onchange="calculateAmount(this)" onkeypress="return isNumberKey(event,this,4)" value="0.000" class="form-control">
								</td>
								<td class="bill LU">
									<input type="text" name="LIMIT_USAGE" onkeypress="return isNumberKey(event,this,4)" value="0.000" class="form-control">
								</td>
							</tr>
						</tbody>
					</table>
...
<input type="text" id="PageName" style="display: none;" value="productpromotion">
	...
	$('[data-toggle="tooltip"]').tooltip(); 
	   ...
	   $("#PROMOTION_DESCS").typeahead({
     	...
     	},
     	{
     	  name: 'promotion',
     	  display: 'value',  
     	  source: substringMatcher(promotion),
     	  limit: 9999,
     	  templates: {
     	  empty: [
...
//      		return '<div onclick="document.form.PRMDESC.value = \''+data.value+'\'"><p> '+ data.value + '</p></div>';
     		return '<div onclick="setPromotion(\''+data.value+'\')"><p>' + data.value + '</p></div>';
     		}
     	  }
...
// 	document.cpoform.PRMDESC.value= value;
	
	 if(value == "Buy X qty Get Y Qty Free"){
		  $(".BXGDESC").hide();
		  $(".XAYB").hide();
		  $(".XAB").hide();
	  }
	
 	 if(value == "Buy X qty Get Disc"){
  		  $(".XAYB").hide();
		  $(".XAB").hide();
	  }
	
	 if(value == "Buy X qty on Product A Get Y Qty Free on Product B"){
		  $(".BXGDESC").hide();
		  $(".XAB").hide();
	  }
	
	 if(value == "Buy X qty on Product A Get Disc on Product B"){
		  $(".BXGDESC").hide();
	  }
...
var promotion =   [{
    "year": "Buy X qty Get Y Qty Free",
    "value": "Buy X qty Get Y Qty Free",
    "tokens": [
      "Buy X qty Get Y Qty Free"
    ]
  },
	  	{
    "year": "Buy X qty Get Disc",
    "value": "Buy X qty Get Disc",
    "tokens": [
      "Buy X qty Get Disc"
    ]
  },
	  	{
    "year": "Buy X qty on Product A Get Y Qty Free on Product B",
    "value": "Buy X qty on Product A Get Y Qty Free on Product B",
    "tokens": [
      "Buy X qty on Product A Get Y Qty Free on Product B"
    ]
  },
  {
    "year": "Buy X qty on Product A Get Disc on Product B",
    "value": "Buy X qty on Product A Get Disc on Product B",
    "tokens": [
      "Buy X qty on Product A Get Disc on Product B"
	    ]
	  }];
