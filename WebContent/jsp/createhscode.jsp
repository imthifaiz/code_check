...
<%
String title = "Create HS Code Master";
%>
...
function popUpWin(URL) {
 		...
 		subWin = window.open(URL, 'HSCODE', 'toolbar=0,scrollbars=yes,location=0,statusbar=0,menubar=0,dependant=1,resizable=1,width=800,height=400,left = 200,top = 184');
...
String res= "",action="",hscode="",fieldDesc="",allChecked="";
...
if(fieldDesc.equals("HSCODE Added Successfully")){
	response.sendRedirect("edithscode.jsp");
}
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><a href="../hscode/edit"><span class="underline-on-hover">Edit HS Code</span></a></li>                       
                <li><label>Create HS Code Master</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->     
            <div class="box-header menu-drop">
              ...
              <h1 style="font-size: 18px; cursor: pointer;"
				...
				onclick="window.location.href='../hscode/edit'">
...
<label class="control-label col-sm-2" for="HSCODE">HS Code</label>
<INPUT type="hidden" name="RFLAG" value="1">
<div class="col-sm-4">
<input name="HSCODE" id="HSCODE" type="TEXT" value="<%=hscode%>" onkeypress="return blockSpecialChar(event)"
...
<button type="button" class="btn btn-success" value="Add HS Code" onClick="onAdd()">Add To HS Code</button>&nbsp;&nbsp;
...
function onAdd() {
    ...
    $.ajax({type: "POST",url: urlStr, data: {HSCODE:hscode,action: "ADD_HSCODE",PLANT:"<%=plant%>"},dataType: "json", success: callback });
    document.form.HSCODE.value = "";
...
function onGo(index) {
    ...
    // Call the method of JQuery Ajax provided
    $.ajax({type: "POST",url: urlStr, data: {PLANT:"<%=plant%>",action: "VIEW_HSCODE_DETAILS",PLANT:"<%=plant%>"},dataType: "json", success: callback });
...
function callback(data){
	...
	if(!errorBoo){
        ...
        $.each(data.hscodemaster, function(i,item){
                   
        	outPutdata = outPutdata+item.HSCODEMASTERDATA;
                    	ii++;
    ...
    if(errorMsg=="HSCODE Added Successfully")
 	{
 	document.form.action  = "../hscode/edit?result=HSCODE Added Successfully";
