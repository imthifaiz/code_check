...
<%
String title = "Credit Note Summary";
...
if(systatus.equalsIgnoreCase("ACCOUNTING")) {
	displaySummaryNew = ub.isCheckValAcc("newCreditNote", plant,username);
	displaySummaryLink = ub.isCheckValAcc("summarylnkCreditNote", plant,username);
	displaySummaryExport = ub.isCheckValAcc("exportCreditNote", plant,username);
}
...
<div class="container-fluid m-t-20">
	 ...
	 <!-- Muruganantham Modified on 16.02.2022 -->
            <ul class="breadcrumb backpageul">      	
                ...
                <li><label>Credit Note Summary</label></li>                                   
            </ul>             
    <!-- Muruganantham Modified on 16.02.2022 -->
            <div class="box-header menu-drop">
              ...
              <button type="button" class="btn btn-default" onClick="window.location.href='../creditnote/new'" style="background-color: rgb(214, 72, 48);color: rgb(255, 255, 255);" >+ New</button>&nbsp;              
                <%}%>
              </div>
              </h1>
		</div>
		...
		<input type="text" name="cmd" hidden>
		<input type="text" name="TranId" hidden>
		<input type="text" name="curency" hidden>
		<input type="text" name="CUST_CODE" hidden>
		<input type="text" name="STATE_PREFIX" hidden>
		<input type="text" name="invselect" hidden>
		<input type="text" name="soreturn" hidden>
		<input type="text" name="creditnote" hidden>
...
<!-- 				<span class="btn-danger input-group-addon" onClick="javascript:popUpWin('customer_list_issue_summary.jsp?CUSTOMER='+form1.CUSTOMER.value);"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></span>	 -->
      ...
      <div class="ShowSingle">
              ...
              <table id="tableInventorySummary" class="table table-bordred table-striped">                   
                   ...
                   <th style="font-size: smaller;">ID</th>
                    <th style="font-size: smaller;">CREDIT NOTE#</th>
                     ...
                     <th style="font-size: smaller;">BALANCE (<%=curency%>)</th>
                   </thead>
              </table> 
              <!-- </font> -->    
		</div>
		...
		var tableInventorySummary;
		 var FROM_DATE,TO_DATE,USER,ORDERNO,CREDITNOTE,INVOICE,STATUS,REFERENCE, groupRowColSpan = 6;
		function getParameters(){
			return {
				...
				"STATUS":STATUS,
				"REFERENCE":REFERENCE,
				"ACTION": "VIEW_CREDIT_NOTE_SUMMARY_VIEW",
				"PLANT":"<%=plant%>"
			}
		}  
		  function onGo(){
		    ...
		    if (tableInventorySummary){
			    ...
			    tableInventorySummary = $('#tableInventorySummary').DataTable({
				        ...
				        "dataSrc": function(data){
				        	if(typeof data.items[0].creditnote === 'undefined'){
				        		...
				        		for(var dataIndex = 0; dataIndex < data.items.length; dataIndex ++){
				        			//data.items[dataIndex]['convamount'] = parseFloat(data.items[dataIndex]['convamount']).toFixed(numberOfDecimal).replace(/\d(?=(\d{3})+\.)/g, "$&,");
				        			...
				        			data.items[dataIndex]['creditnote'] = '<a href="../creditnote/detail?custcreditid=' +data.items[dataIndex]['custcreditid']+'">'+data.items[dataIndex]['creditnote']+'</a>';
				        			  <% }else{ %>
			        ...
			        "columns": [
		    			...
		    			{"data": 'custcreditid', "orderable": true},
