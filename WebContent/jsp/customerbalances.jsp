...
<%
String title = "Customer Balances";
...
<div class="container-fluid m-t-20">
	...
	<!-- Thanzith Modified on 26.02.2022 -->
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Customer Balances</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 26.02.2022 --> 
			      ...
			      <div class="ShowSingle"></div>
					...
					<div class="row">
						<div class="col-sm-12">
							...
							<table id="tableCustomerBalance" class="table table-bordred table-striped"> 
								<thead>
									<tr><th style="font-size: smaller;">CUSTOMER NAME</th>
									<th style="font-size: smaller;">INVOICE BALANCE</th>
									<th style="font-size: smaller;">AVAILABLE CREDITS(<%=baseCurrency%>)</th>
...
var FROM_DATE,TO_DATE,USER,ORDERNO,ITEM,INVOICENO,STATUS, groupRowColSpan = 6;

function getParameters(){
	return {
		"FDATE":FROM_DATE,"TDATE":TO_DATE,
		"CNAME":USER,					
		"ACTION": "VIEW_CUSTOMER_BALANCES",
		"PLANT":"<%=plant%>"
	}
}

function storeUserPreferences(){
	storeInLocalStorage('customerbalances_FROM_DATE', $('#FROM_DATE').val());
	storeInLocalStorage('customerbalances_TO_DATE', $('#TO_DATE').val());
	storeInLocalStorage('customerbalances_CUSTOMER', $('#CUSTOMER').val());
	storeInLocalStorage('customerbalances_CUST_CODE', $('#CUST_CODE').val());
}
...
function onGo(){
   ...
   if(ALLOWCATCH_ADVANCE_SEARCH==='1'){
	...
	if (tableSummary){
		...
		tableSummary = $('#tableCustomerBalance').DataTable({
		"processing": true,
		...
		"ajax": {
			...
			"dataSrc": function(data){
	        	if(typeof data.items[0].cname === 'undefined'){
	        		return [];
	        	}else {	
	        		return data.items;
	        	}
	    ...
	    "columns": [
			...
			{"data": 'invoicebalance', "orderable": true},
			{"data": 'availablecredit', "orderable": true},
		...
		buttons: [
            ...
            {
                extend: 'colvis',
                columns: ':not(:eq(1)):not(:eq(2)):not(:last)'
            }
        ],
	        "drawCallback": function ( settings ) {
	            ...
	            api.column(groupColumn, {page:'current'} ).data().each( function ( group, i ) {
	            	...
	            	var invoicebalance = $(rows).eq( i ).find('td:nth-last-child(3)').html();
	            	invoicebalance = invoicebalance.replace(/[^0-9.]/g, "");
	            	
	            	var availableCredits = $(rows).eq( i ).find('td:nth-last-child(2)').html();
	            	availableCredits = availableCredits.replace(/[^0-9.]/g, "");
	            	
	            	var balance = $(rows).eq( i ).find('td:last').html();
	            	balance = balance.replace(/[^0-9.]/g, "");	            	
	            	
	            	totalInvoicelBalance += parseFloat(invoicebalance);
	            	totalCredits += parseFloat(availableCredits);
	            	totalBalance += parseFloat(balance);
	            });
	            if (rows.length == (currentRow + 1)){
	            	$(rows).eq( currentRow ).after(
							'<tr class="group"><td>Total</td>'
							+'<td class="t-right">'+ addZeroes(parseFloat(totalInvoicelBalance).toFixed(<%=numberOfDecimal%>)) + '</td>'
							+'<td class="t-right">'+ addZeroes(parseFloat(totalCredits).toFixed(<%=numberOfDecimal%>)) + '</td>'
							+'<td class="t-right">'+ addZeroes(parseFloat(totalBalance).toFixed(<%=numberOfDecimal%>)) + '</td></tr>'
<%-- 							+'<td class="t-right">' +'<%=baseCurrency%>'+ addZeroes(parseFloat(totalInvoicelBalance).toFixed(<%=numberOfDecimal%>)) + '</td>'
							+'<td class="t-right">' +'<%=baseCurrency%>'+ addZeroes(parseFloat(totalCredits).toFixed(<%=numberOfDecimal%>)) + '</td>'
	        ...
	        "footerCallback": function(row, data, start, end, display) {
	            ...
	            $(api.column(1).footer()).html(parseFloat(totalInvoicelBalance).toFixed(<%=numberOfDecimal%>));
	            $(api.column(2).footer()).html(parseFloat(totalCredits).toFixed(<%=numberOfDecimal%>));
	            $(api.column(3).footer()).html(parseFloat(totalBalance).toFixed(<%=numberOfDecimal%>));
	          }
		});
		 $("#tableCustomerBalance_filter.dataTables_filter").append($("#searchType")); $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
...
$('#tableCustomerBalance').on('column-visibility.dt', function(e, settings, column, state ){
	if (!state){
		groupRowColSpan = parseInt(groupRowColSpan) - 1;
	}else{
		groupRowColSpan = parseInt(groupRowColSpan) + 1;
	}
	$('#tableCustomerBalance tr.group td:nth-child(1)').attr('colspan', groupRowColSpan);
	$('#tableCustomerBalance').attr('width', '100%');
});

function ExportReport()
{
  document.form1.action="/track/InvoiceServlet?ACTION=ExportExcelCustomerbalances";
...
function generateExpenseDetail(data){
	...
	table1body += "<th>Invoice balance</th>";
	table1body += "<th>Available Credits</th>";
	...
	if(data.items != ""){	
		...
		$(data.items).each(function(){
			...
			table1body += "<td>"+this.invoicebalance+"</td>";
			table1body += "<td>"+this.availablecredit+"</td>";
			table1body += "<td>"+this.balance+"</td>";
			table1body += "</tr>";
			totalInvoicelBalance += parseFloat(this.invoicebalance.replace(/[^0-9.]/g, ""));
			totalCredits += parseFloat(this.availablecredit.replace(/[^0-9.]/g, ""));
			totalBalance += parseFloat(this.balance.replace(/[^0-9.]/g, ""));
        	row += 1;
		});
		
		table1body +='<tr><tfoot><td>Total</td>' 
			+'<td>'+'<%=baseCurrency%>'+ addZeroes(parseFloat(totalInvoicelBalance).toFixed(<%=numberOfDecimal%>)) + '</td>'
			+'<td>'+'<%=baseCurrency%>'+ addZeroes(parseFloat(totalCredits).toFixed(<%=numberOfDecimal%>)) + '</td>'
	...
	doc.save('customer_balances.pdf');
}

$(document).ready(function(){
	var ALLOWCATCH_ADVANCE_SEARCH = '<%=ALLOWCATCH_ADVANCE_SEARCH%>';
    if(ALLOWCATCH_ADVANCE_SEARCH==='1'){
	getLocalStorageValue('customerbalances_FROM_DATE',document.form1.FROM_DATE.value , 'FROM_DATE');
	getLocalStorageValue('customerbalances_TO_DATE','', 'TO_DATE');
	getLocalStorageValue('customerbalances_CUSTOMER','', 'CUSTOMER');
	getLocalStorageValue('customerbalances_CUST_CODE','', 'CUST_CODE');	
    }
 	onGo();
 	$('.Show').click(function() {
	    $('#target').show(500);
