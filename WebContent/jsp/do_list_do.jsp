...
<table id="myTable" class="table">
   ...
   // ItemUtil itemUtil = new ItemUtil();
    ...
    System.out.println("ordertype.ddfdf.."+orderType+" "+CashCust);
   ...
   try{
     ...
     extCond =extCond+ " AND ORDERTYPE ='MOBILE ORDER' ";
        ...
        extCond =extCond+ " AND ORDERTYPE ='MOBILE REGISTRATION' ";
       }
     
     ArrayList listQry = new ArrayList();
     if(orderType.equalsIgnoreCase("DNPL")){
    	 if(dono.length()>0) extCond=" AND plant='"+plant+"' and POSTRANID like '"+dono+"%'";
    	 extCond=extCond+" and status in('C')"; 
      }
     else
     {
     ...
     if("YES".equalsIgnoreCase(SalesPickIssue)){
    	 extCond = extCond+ " and ((ORDER_STATUS = 'OPEN' OR ORDER_STATUS='PARTIALLY PROCESSED') OR (STATUS = 'O' and ORDER_STATUS='PROCESSED'))";
     }
     if(CashCust.equalsIgnoreCase("1")){
    	 POSUtil posUtil = new POSUtil();    	 
    	 extCond=extCond+"and POSTRANID like'T%' order by POSTRANID desc";
    	 listQry = posUtil.getPosHdrDetails("POSTRANID as dono,custName,custCode,jobNum,status,TRANDT as collectiondate",ht,extCond);	 
     }
     else{
         ...
         listQry = _DOUtil.getDoHdrDetails("dono,custName,custCode,jobNum,status,collectiondate",ht,extCond);
     }
