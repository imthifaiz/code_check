...
<%
String title = "Edit Sales Estimate Order Email Message";
%>
...
<script>
	...
	function onAdd() {
		...
		document.form1.action = "../editsales/salesestimateemailconfig";
...
<%
		...
		try{
                ...
                if (!isUpdated) {
                        ...
                        res = "<font class = " + IConstants.SUCCESS_COLOR+ ">Email Configuration for Sales Estimate Order edited successfully</font>";
                }
	...
	try{
         Map<String, String> m= mailUtil.getEmailMsgDetails(plant,IConstants.ESTIMATE_ORDER);
...
<div class="container-fluid m-t-20">
	<div class="tab">
		...
		<button class="tablinks" onclick="openTab(event, 'uponConvertToSalesOrder')">Upon Convert To Sales Order</button>
	</div>
	<div id="inventory" class="tabcontent active" style="display: block;">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Edit Sales Estimate Order Email Message</label></li>                                   
            </ul> 
        <!-- Thanzith Modified on 23.02.2022 -->  
 ...
 <div class="box-body">
  ...
  <form class="form-horizontal" name="form1" method="post" action="editEstimateOrderEmailMsg.jsp">
        <INPUT type="hidden" name="cmd" value="" />
        
        <INPUT name="OrderType" id="OrderType" type="hidden" value="<%=IConstants.ESTIMATE_ORDER%>">
	...
	<div class="col-sm-12 col-md-4">
      	...
      	<ul id="ulUponCreation">
      		<li>{ORDER_NO}</li>
      		<li>{CUSTOMER_NAME}</li>
      	</ul>
      	<ul id="ulUponCSO" style="display:none;">
      		<li>{ORDER_NO}</li>
      		<li>{EST_ORDER_NO}</li>
      		<li>{CUSTOMER_NAME}</li>
      	</ul>
  ...
  <%--   <div id="ccDetails"
   ...
   <div class="form-group" id="div_send_attachment">
      <label class="control-label  col-form-label col-sm-2" for="Web Link">Send Attachment</label>
      <div class="col-sm-4">          
       <input type="radio" name="send_attachment" id="send_attachment_1" value="estimate" /> <label for="send_attachment_1">Estimate</label><br/>
       <input type="radio" name="send_attachment" value="none" /> None<br/>
      </div>
    </div>
        <INPUT type="hidden" name="BillType" value="INVENTORY"> 
    <div class="form-group"> 
    <b id="noteBold" style="display:none;">Note : Attachment will be sent based on the Sales Order Email Configuration</b><br/>       
     <div class="col-sm-offset-4 col-sm-8">     
...
function openTab(evt, ref) {
	 ...
	 /*  tabcontent = document.getElementsByClassName("tabcontent");
	  ...
	  if(ref == "uponCreation"){
		  $('label[for=send_attachment_1]').html('Estimate');
		  $('#send_attachment_1').val('estimate');
		  $('#OrderType').val('<%=IConstants.ESTIMATE_ORDER%>');
		  $('#ulUponCreation').css('display', 'block');
		  $('#ulUponCSO').css('display', 'none');
		  $('#noteBold').css('display', 'none');
		  $('#div_send_attachment').css('display', 'block');
	  }else{
		 $('#div_send_attachment').css('display', 'none');
		 $('label[for=send_attachment_1]').html('Estimate');
		 $('#send_attachment_1').val('estimate');
		 $('#OrderType').val('<%=IConstants.ESTIMATE_ORDER_UCSO%>');
		 $('#ulUponCreation').css('display', 'none');
		 $('#ulUponCSO').css('display', 'block');
		 $('#noteBold').css('display', 'block');
		 $('#div_send_attachment').css('display', 'none');
	  }
	  $('#result').css('display', 'none');
	  $.ajax({
			type : 'get',
			url : "/track/email_config/detail_" + (ref == "uponCreation" ? $('#send_attachment_1').val() : ($('#send_attachment_1').val() + "_ucso")),
			dataType: "json",
