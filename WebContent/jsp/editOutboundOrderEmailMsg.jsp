...
<%
String title = "Edit Sales Order Email Message";
%>
...
<script>
	...
	function onClear()
	{
		...
		document.form1.printGINO.checked = false;
	...
	function onAdd() {
		...
		if ($('#send_attachment_1').val() == 'do' && document.form1.send_attachment.value != 'do' && document.form1.send_attachment.value != 'both' && document.form1.printGINO.checked){
			alert("Can not use Print With GINO option with Invoice and None");
			return false;
		}
		...
		document.form1.action = "../editsales/salesorderemailconfig";
...
<%
		...
		String res = "";		
		String action = "";
		String subject  = "",  OrderNo  = "",webLink ="",CC_CHECK="",strChecked="",isAutoEmail = "",sendAttachment = "",printGINO="";;
		...
		try{
                ...
                printGINO =  StrUtils.fString(request.getParameter("printGINO")).equalsIgnoreCase("Y") ? "Y" : "N";
                if ("Y".equals(printGINO)){
                    sendAttachment += "withgino";
                }
                ...
                if (!isUpdated) {
                        ...
                        res = "<font class = " + IConstants.SUCCESS_COLOR+ ">Email Configuration for Sales Order edited successfully</font>";
                }
	...
	try{
         Map<String, String> m= mailUtil.getEmailMsgDetails(plant,IConstants.SALES_ORDER);
         ...
         if(!m.isEmpty()){
              ...
              printGINO = "gino".equalsIgnoreCase(sendAttachment) ? "Y" : "N";
...
<div class="container-fluid m-t-20">
	<div class="tab">
		...
		<button class="tablinks" onclick="openTab(event, 'afterPickAndIssue')">After Pick &amp; Issue</button>
	</div>
	<div id="inventory" class="tabcontent active" style="display: block;">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Edit Sales Order Email Message</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->  
 ...
 <div class="box-body">
  ...
  <form class="form-horizontal" name="form1" method="post" action="editOutboundOrderEmailMsg.jsp">
        <INPUT type="hidden" name="cmd" value="" />
        <INPUT name="OrderType" id="OrderType" type="hidden" value="<%=IConstants.SALES_ORDER%>">
    ...
    <div class="form-group">
      ...
      <div class="col-sm-12 col-md-4 paraminfo_api" style="display: none;">
      	Allowed Parameters in subject : 
      	<ul>
      		<li>{ORDER_NO}</li>
      		<li>{ORDER_NO_2}</li>
  ...
  <div class="col-sm-4">
    ...
    <label class="radio-inline">
     ...
     <div class="form-group">
      ...
      <div class="col-sm-4">          
        <INPUT class="form-control" name="OrderNo" type="TEXT" value="<<Order No>&gt;"	size="50" MAXLENGTH=30 readonly>
      </div>
    </div>
    <div class="form-group">
...
<%--     <div id="ccDetails"
    ...
    <div class="form-group">
      <label class="control-label col-form-label col-sm-2" for="Subject">Subject</label>
      <div class="col-sm-4">          
        <INPUT class="form-control"  name="ccsubject" type="TEXT" value="<%=subject%>" size="50" MAXLENGTH=50>
      </div>
    </div>
    <div class="form-group">
      <label class="control-label col-form-label col-sm-2" for="Body">Body 1</label>
      <div class="col-sm-4">          
        <textarea class="form-control" name="ccBody1" id="ccBody1" cols="50" rows="2" onKeyDown="limitText(this.form.ccBody1,100);"><%=ccBody1.toString()%></textarea>
      </div>
    </div>
    <div class="form-group">
      <label class="control-label col-form-label col-sm-2" for="Order Number">Order Number</label>
      <div class="col-sm-4">          
        <INPUT class="form-control" name="ccOrderNo" type="TEXT" value="<<Order No>&gt;" size="50" MAXLENGTH=30 readonly>
      </div>
    </div>
    <div class="form-group">
      <label class="control-label col-form-label col-sm-2" for="Body 2">Body 2</label>
  ...
  <div class="form-group">
	  ...
	  <div class="pull-left">         
       			<input type="radio" name="send_attachment" id="send_attachment_1" value="picking_list" /> <label for="send_attachment_1">Picking List</label><br/>
       </div>
       <div id="printwithgino" style="margin-left:50px; display:none;">
     				<input type="checkbox" class="form-check-input" id="printGINO" name="printGINO" value="Y" <%if(printGINO.equalsIgnoreCase("Y")){%>checked <%}%> />&nbsp;Print with GINO
  		</div>
  		</div>
       <input type="radio" name="send_attachment" value="invoice" onclick="{uncheckGINO()}"  /> Invoice<br/>
       <input type="radio" name="send_attachment" value="both" /> Both<br/>
       <input type="radio" name="send_attachment" value="none" onclick="{uncheckGINO()}"/> None<br/>
      </div>
    </div>
...
function openTab(evt, ref) {
	 ...
	 /*  tabcontent = document.getElementsByClassName("tabcontent");
	  ...
	  if(ref == "uponCreation"){
		  $('label[for=send_attachment_1]').html('Picking List');
		  $('#send_attachment_1').val('picking_list');
		  $('#OrderType').val('<%=IConstants.SALES_ORDER%>');
		  $('#printwithgino').css('display', 'none');
		  $('.paraminfo_uc').css('display', 'block');
		  $('.paraminfo_api').css('display', 'none');
	  }else{
		 $('label[for=send_attachment_1]').html('DO');
		 $('#send_attachment_1').val('do');
		 $('#OrderType').val('<%=IConstants.SALES_ORDER_API%>');
		 $('#printwithgino').css('display', 'block');
		 $('.paraminfo_uc').css('display', 'none');
		 $('.paraminfo_api').css('display', 'block');
	  }
	  ...
	  $.ajax({
			...
			success : function(data) {
				if (data.ERROR_CODE == '100'){
					...
					if (data['<%=IDBConstants.SEND_ATTACHMENT%>'].indexOf('gino') != -1){
						document.form1.printGINO.checked = true;
					}else{
						document.form1.printGINO.checked = false;
					}
					if (data['<%=IDBConstants.SEND_ATTACHMENT%>'].indexOf('do') != -1){
						data['<%=IDBConstants.SEND_ATTACHMENT%>'] = 'do';
...
function uncheckGINO(){
	if (typeof document.form1.printGINO != 'undefined'){
		document.form1.printGINO.checked = false;
	}
}
</script>
