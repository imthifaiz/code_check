...
<%
	...
	//Validate no.of Customers -- Azees 15.11.2020
		...
		ArrayList arrCust =  custUtil.getCustomerDetails(doHdr.getCustCode(), plant);
		...
		PltApprovalMatrixDAO pltApproval = new PltApprovalMatrixDAO();//Sales Approve 08.22 - Azees
		 boolean approvalcheck = pltApproval.CheckApprovalByUser(plant, "SALES", "UPDATE", username);
		
		String CUST_ADDONCOST="",CUST_ADDONCOSTTYPE="";
		 CustUtil custUtils = new CustUtil();
         ArrayList arrCust1 = custUtils.getCustomerListStartsWithNameActive(doHdr.getCustName(),plant);
         if (arrCust1.size() > 0) {
             for(int i =0; i<arrCust1.size(); i++) {
                 Map arrCustLine = (Map)arrCust1.get(i);
                 CUST_ADDONCOST = (String)arrCustLine.get("CUST_ADDONCOST");
                 CUST_ADDONCOSTTYPE = (String)arrCustLine.get("ADDONCOSTTYPE");
             }
         }
...
<script src="../jsp/js/editSalesOrder.js"></script>
<center>
	<h2>
	...
	<!-- Thanzith Modified on 28.02.2022 -->
	 <ul class="breadcrumb backpageul">    
                <li><a href="../home"><span class="underline-on-hover">Dashboard</span></a></li>	
                 ...
                 <li><a href="../salesorder/detail?dono=<%=doHdr.getDONO()%>"><span class="underline-on-hover">Sales Order Detail</span></a></li>	
                <li><label>Edit Sales Order</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 28.02.2022 --> 
		<div class="box-header menu-drop">
			...
			<div class=" pull-right">
				...
				&nbsp;
				<h1 style="font-size: 18px; cursor: pointer;" class="box-title pull-right" onclick="window.location.href='../salesorder/detail?dono=<%=doHdr.getDONO()%>'">
		...
		<div class="container-fluid">
			<form class="form-horizontal" id="salesOrderForm" name="form1" autocomplete="off" method="post" action="../salesorder/edit" enctype="multipart/form-data">
			...
			<input type="hidden" name="CURRENCYUSEQTOLD" value="<%=doHdr.getCURRENCYUSEQT()%>">
			   ...
			   <input type = "hidden" name="custordersataus" value="<%=doHdr.getAPP_CUST_ORDER_STATUS()%>">
...
<!-- 				</div> -->
						...
						<div class="col-sm-6 ac-box">
							<input type="text" class="form-control datepicker" id="DELDATE" name="DELDATE" value="<%=doHdr.getDELDATE()%>" readonly>
						</div>
					    </div>
						</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<div class="input-group">
							<input type="text" class="form-control" id="JOB_NUM" name="JOB_NUM" value="<%=doHdr.getJobNum() %>"  <%=setreadonly%>
...
<%-- 								name="DELDATE" value="<%=doHdr.getDELDATE()%>" readonly> --%>
<!-- 						</div> -->
<!-- 					</div> -->
						...
						<div class="col-sm-6">
							<div class="input-group" style="display: none;">
								<input type="text" class="form-control" id="COLLECTION_TIME"
									name="COLLECTION_TIME" value="<%=doHdr.getCollectionTime()%>" readonly>
				   		 	</div>
						</div>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-4 ac-box">
						<input type="text" class="ac-selected form-control" id="CURRENCY" name="CURRENCY" value="<%=DISPLAY%>" readonly>
					</div>
               ...
               <!--  Ends  -->
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<div class="input-group">
							<input type="text" class="form-control" id="GST" name="GST" onchange="changetaxforgst()" onkeypress="return isNumberKey(event,this,4)"  value="<%=doHdr.getOUTBOUND_GST()%>">
...
<!-- 								<th colspan=2 style="width:15%">Delivery Date</th> -->
						...
						%>
							<tr id="">
								...
								<td class="bill-item">
									<input type="text" name="item" class="form-control itemSearch" onclick="loaddropdata(this)" onchange="checkitems(this.value,this)" style="width:87%"
								...
								<td>
									...
									<input type="text" name="unitcost" class="form-control text-right" value="<%=unitCost%>" readonly tabindex="-1">
								</td>
								...
								%>
								<input type="text" name="addonshow" class="form-control text-right" value="<%=addoncost%>" readonly tabindex="-1">
								<%}else{
...
<%-- 								<td>
				...
				<div class="row">
					...
					<%if(doHdr.getORDER_STATUS().equalsIgnoreCase("Draft")){%>
					    ...
					    <button type="button" class="btn btn-success" id="btnSalesDraft" >Save as Draft</button> --%>
					    <%
							if (approvalcheck || doHdr.getAPP_CUST_ORDER_STATUS().equalsIgnoreCase("CREATE REJECTED") || doHdr.getAPP_CUST_ORDER_STATUS().equalsIgnoreCase("EDIT REJECTED")) {
							%>
...
<%@include file="../jsp/newProductModal.jsp" %> <!-- resvi -->
...
function advPayment()
{
	...
	if(dono == ""){
		...
		window.location.href = "../salesorder/advancepayment?type=ADVANCE&DONO="+dono+"&CUST_CODE="+form1.CUST_CODE1.value+"&CUST_NAME="+form1.CUST_NAME.value;
	}
	}
