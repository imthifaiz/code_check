...
<%
	String title = "Edit Consignment Order Email Message";
%>
<%@include file="sessionCheck.jsp"%>
<jsp:include page="header2.jsp" flush="true">
	<jsp:param name="title" value="<%=title%>" />
	<jsp:param name="mainmenu" value="<%=IConstants.SETTINGS%>" />
	<jsp:param name="submenu"
		value="<%=IConstants.PRINTOUT_CONFIGURATION%>" />
</jsp:include>
<link rel="stylesheet" href="../jsp/css/typeahead.css">
<link rel="stylesheet" href="../jsp/css/accounting.css">
<link rel="stylesheet" type="text/css"
	href="${context}/jsp/dist/css/bootstrap3-wysihtml5.min.css">
...
<script>
	...
	function onAdd() {
		...
		document.form1.action = "../editconsignment/consignorderemail";
...
<%
	...
	String action = "";
	String subject = "", OrderNo = "", webLink = "", CC_CHECK = "", strChecked = "", isAutoEmail = "",
			sendAttachment = "";
	...
	if (action.equalsIgnoreCase("UPDATE_EMAIL_MSG")) {
		try {
			...
			//autoEmail = strUtils.fString(request.getParameter("autoEmail"));
			...
			if (!isUpdated) {
				...
				res = "<font class = " + IConstants.SUCCESS_COLOR
						+ ">Email Message for Consignment Order edited successfully</font>";
			}
		} catch (Exception e) {
	...
	try {
		Map<String, String> m = mailUtil.getEmailMsgDetails(plant, IConstants.CONSIGNMENT_ORDER);
...
<div class="container-fluid m-t-20">
	<div class="tab">
		...
		<button class="tablinks" onclick="openTab(event, 'afterPickAndIssue')">After
			Pick &amp; Issue</button>
	</div>
	<div id="inventory" class="tabcontent active" style="display: block;">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Edit Consignment Order Email Message</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->  
			...
			<div class="box-body">
				...
				<form class="form-horizontal" name="form1" method="post"
					action="editTransferOrderEmailMsg.jsp">
					<INPUT type="hidden" name="cmd" value="" /> <INPUT
						name="OrderType" id="OrderType" type="hidden"
						value="<%=IConstants.CONSIGNMENT_ORDER%>">
					<div class="form-group">
						<label class="control-label col-form-label col-sm-2" for="From">From</label>
						<div class="col-sm-4">
							<INPUT class="form-control" name="emailFrom" id="EMAIL_FROM"
								type="TEXT" value="<%=emailFrom%>" size="50" MAXLENGTH=50>
						</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							<INPUT class="form-control" name="subject" id="SUBJECT"
								type="TEXT" value="<%=subject%>" size="50" MAXLENGTH=100>
						</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							<textarea class="form-control" name="Body1" id="BODY1" cols="50"
								rows="15" onKeyDown="limitText(this.form.Body1,200);"><%=Body1.toString()%></textarea>
						</div>
					...
					<div id="target" style="display: none">
						<div class="form-group">
							...
							<div class="col-sm-4">
								<INPUT class="form-control" name="emailTo" type="TEXT"
									value="<%=emailTo%>" size="50" MAXLENGTH=50>
							</div>
						</div>
						<div class="form-group">
							<label class="control-label col-form-label col-sm-2"
								for="Order Number">Order Number:</label>
							<div class="col-sm-4">
								<INPUT class="form-control" name="OrderNo" type="TEXT" value=""
									size="50" MAXLENGTH=30 readonly>
							</div>
						</div>
						<div class="form-group">
							<label class="control-label col-form-label col-sm-2" for="Body 2">Body
								2</label>
							<div class="col-sm-4">
								<textarea class="form-control" name="Body2" id="Body2" cols="50"
									rows="2" onKeyDown="limitText(this.form.Body2,100);"><%=Body2.toString()%></textarea>
							</div>
						</div>
						<div class="form-group">
							<label class="control-label col-form-label col-sm-2"
								for="Web Link">Web Link</label>
							<div class="col-sm-4">
								<INPUT class="form-control" name="webLink" type="TEXT"
									value="<%=webLink%>" size="50" MAXLENGTH=50>
							</div>
						</div>
						<div class="form-group">
							<div class="col-sm-offset-4 col-sm-8">
								<label class="checkbox-inline"> <INPUT type="checkbox"
									name="CC_CHECK" VALUE="N" <%=strChecked%>
									onclick="fnchecked(this.checked);">Copy To(cc)
								</label>
							</div>
						</div>
						<div class="form-group">
							<div class="col-form-label col-sm-2">
								<a href="#" class="Show" style="font-size: 15px; display: none">Show
									CC Details</a> <a href="#" class="Hide"
									style="font-size: 15px; display: none">Hide CC Details</a>
							</div>
						</div>
						<%--  <div id="ccDetails"
	...
	style="display: none;" <%}%>> --%>
						<%-- <div class="form-inline">
							<div class="col-sm-4">
								<label class="radio-inline"> <input type="radio"
									name="ISAUTOEMAIL" id="turnOff" value="N"
									<%if (isAutoEmail.equalsIgnoreCase("N")) {%> checked <%}%>>Turn
									Off Auto Email
								</label> <label class="radio-inline"> <input type="radio"
									name="ISAUTOEMAIL" id="turnOn" value="Y"
									<%if (isAutoEmail.equalsIgnoreCase("Y")) {%> checked <%}%>>Turn
									On Auto Email
								</label>
							</div>
						</div> --%>
						<div class="form-group">
							...
							<div class="col-sm-4">
								<INPUT class="form-control" name="ccemailTo" type="TEXT"
									value="<%=ccemailTo%>" size="50" MAXLENGTH=30>
							</div>
						</div>
						<div class="form-group">
							<label class="control-label col-form-label col-sm-2"
							...
							<div class="col-sm-4">
								<INPUT class="form-control" name="ccsubject" type="TEXT"
									value="<%=ccsubject%>" size="50" MAXLENGTH=50>
							</div>
						</div>
						<div class="form-group">
							<label class="control-label col-form-label col-sm-2" for="Body">Body
								1</label>
							<div class="col-sm-12">
								<textarea class="form-control" name="ccBody1" id="ccBody1"
									cols="50" rows="2"
									onKeyDown="limitText(this.form.ccBody1,100);"><%=ccBody1.toString()%></textarea>
							</div>
							...
							<div class="form-group">
								<label class="control-label col-form-label col-sm-2"
									for="Order Number">Order Number</label>
								<div class="col-sm-4">
									<INPUT class="form-control" name="ccOrderNo" type="hidden"
										value="" size="50" MAXLENGTH=30 readonly>
								</div>
							</div>
							<div class="form-group">
								<label class="control-label col-form-label col-sm-2"
									for="Body 2" style="display: none">Body 2</label>
								<div class="col-sm-4">
									<textarea class="form-control" name="Body2" id="Body2"
										cols="50" rows="2" onKeyDown="limitText(this.form.Body2,100);"><%=Body2.toString()%></textarea>
								</div>
							</div>
							<div class="form-group">
								<label class="control-label col-form-label col-sm-2"
									for="Web Link" style="display: none">Web Link</label>
								<div class="col-sm-4">
									<INPUT class="form-control" name="ccwebLink" type="hidden"
										value="<%=ccwebLink%>" size="50" MAXLENGTH=50>
								</div>
							</div>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label  col-form-label col-sm-2"
							for="Web Link">Send Attachment</label>
						<div class="col-sm-4">
							<input type="radio" name="send_attachment" id="send_attachment_1"
								value="consignment" /> <label for="send_attachment_1">CO</label><br />
							<input type="radio" name="send_attachment" value="invoice" />
							Invoice<br /> <input type="radio" name="send_attachment"
								value="both" /> Both<br /> <input type="radio"
								name="send_attachment" value="none" /> None<br />
						</div>
					</div>
...
<script type="text/javascript"
	src="${context}/jsp/dist/js/bootstrap3-wysihtml5.all.min.js"></script>
<script>
...
function openTab(evt, ref) {
	 ...
	 /*  tabcontent = document.getElementsByClassName("tabcontent");
	  ...
	  if(ref == "uponCreation"){
		  $('label[for=send_attachment_1]').html('CO');
		  $('#send_attachment_1').val('consignment');
		  $('#OrderType').val('<%=IConstants.CONSIGNMENT_ORDER%>');
	  }else{
		 $('label[for=send_attachment_1]').html('CO');
		 $('#send_attachment_1').val('consignment');
		 $('#OrderType').val('<%=IConstants.CONSIGNMENT_ORDER_API%>');
	  }
	  $('#result').css('display', 'none');
	  $.ajax({
			type : 'get',
			url : "/track/email_config/detail_" + (ref == "uponCreation" ? $('#send_attachment_1').val() : ($('#send_attachment_1').val() + "_api")),
			dataType: "json",
