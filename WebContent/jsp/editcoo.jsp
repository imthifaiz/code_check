...
function popUpWin(URL) {
 		...
 		subWin = window.open(URL, 'COO', 'toolbar=0,scrollbars=yes,location=0,statusbar=0,menubar=0,dependant=1,resizable=1,width=800,height=400,left = 200,top = 184');
...
String res= "",action="",coo="",fieldDesc="",allChecked="";
...
{
displaySummaryDelete = ub.isCheckValAcc("removecoos", plant,USERID);
displaySummaryNew = ub.isCheckValAcc("newcoos", plant,USERID);

}
if(systatus.equalsIgnoreCase("INVENTORY"))
{
displaySummaryDelete = ub.isCheckValinv("removecoos", plant,USERID);
displaySummaryNew = ub.isCheckValinv("newcoos", plant,USERID);
}
	 ...
	 <ul class="breadcrumb backpageul">    
        ...
        <!-- Thanzith Modified on 23.02.2022 -->     
            <div class="box-header menu-drop">
              ...
              <button type="button" class="btn btn-default" onclick="window.location.href='../coo/new'" style="background-color: rgb(214, 72, 48);color: rgb(255, 255, 255);" >+ New</button>&nbsp;
              <%} %>
              </div>
		</div>
...
<button type="button" class="Submit btn btn-default" value="Remove From COO" onClick="onDelete()">Remove From COO</button>&nbsp;&nbsp;
<%} %>
	</div>
	 </div>
 ...
 function checkAll(isChk)
{
  ...
  $.ajax({type: "POST",url: urlStr, data: {PLANT:"<%=plant%>",action: "VIEW_COO_DETAILS_EDIT",PLANT:"<%=plant%>"},dataType: "json", success: callback });
...
function onDelete()
{
	 ...
	 for (var i = 0; i < len ; i++)
   {
	 ...
	 var chkmsg = confirm("Are you sure you would like to delete?");
		if (chkmsg) {
	  	  document.form.action="/track/MasterServlet?action=DELETE_COO";
...
function callback(data){
	...
	if(!errorBoo){
        ...
        $.each(data.coomaster, function(i,item){
                   
        	outPutdata = outPutdata+item.COOMASTERDATA;
                    	ii++;
