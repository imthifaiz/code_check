...
<%
String title = "Edit Footer Details";
%>
...
{
displaySummaryDelete = ub.isCheckValAcc("removeorderfooter", plant,USERID);
displaySummaryNew = ub.isCheckValAcc("neworderfooter", plant,USERID);

}
if(systatus.equalsIgnoreCase("INVENTORY"))
{
	displaySummaryDelete = ub.isCheckValinv("removeorderfooter", plant,USERID);
displaySummaryNew = ub.isCheckValinv("neworderfooter", plant,USERID);
}
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Edit Footer Details</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->      
            <div class="box-header menu-drop">
              ...
              <button type="button" class="btn btn-default" onclick="window.location.href='../footer/new'" style="background-color: rgb(214, 72, 48);color: rgb(255, 255, 255);" >+ New</button>&nbsp;
              <%} %>
              </div>
		</div>
...
<INPUT Type=Checkbox class="form-check-input"  style="border:0;" name = "select" value="select" <%if(allChecked.equalsIgnoreCase("true")){%>checked <%}%>onclick="return checkAll(this.checked);">
 ...
 <div id="VIEW_RESULT_HERE"></div>
  ...
  <div class="form-group">        
	 ...
	 <button type="button" class="Submit btn btn-default" value="Remove From Footer" onClick="onDelete()">Remove From Footer</button>&nbsp;&nbsp;
	 <%} %>
	</div>
	</div>
...
function onGo(index) {
    ...
    $.ajax({type: "POST",url: urlStr, data: {PLANT:"<%=plant%>",action: "VIEW_FOOTER_DETAILS_EDIT",PLANT:"<%=plant%>"},dataType: "json", success: callback });
...
function onDelete()
{
	 ...
	 for (var i = 0; i < len ; i++)
   {
	 ...
	 var chkmsg = confirm("Are you sure you would like to delete?");
		if (chkmsg) {
	  	  document.form.action="/track/MasterServlet?action=DELETE_FOOTER";
