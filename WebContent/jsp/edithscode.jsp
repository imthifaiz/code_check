...
{
displaySummaryDelete = ub.isCheckValAcc("removehscodes", plant,USERID);
displaySummaryNew = ub.isCheckValAcc("newhscodes", plant,USERID);

}
if(systatus.equalsIgnoreCase("INVENTORY"))
{
	displaySummaryDelete = ub.isCheckValinv("removehscodes", plant,USERID);
displaySummaryNew = ub.isCheckValinv("newhscodes", plant,USERID);
}
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Edit HS Code</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->     
            <div class="box-header menu-drop">
              ...
              <button type="button" class="btn btn-default" onclick="window.location.href='../hscode/new'"  style="background-color: rgb(214, 72, 48);color: rgb(255, 255, 255);" >+ New</button>&nbsp;
               <%} %>
              </div>
		</div>
...
<button type="button" class="Submit btn btn-default" value="Remove From HS Code" onClick="onDelete()">Remove From HS Code</button>&nbsp;&nbsp;
	 <%} %>
	</div>
	 </div>
 ...
 function checkAll(isChk)
{
  ...
  $.ajax({type: "POST",url: urlStr, data: {PLANT:"<%=plant%>",action: "VIEW_HSCODE_DETAILS_EDIT",PLANT:"<%=plant%>"},dataType: "json", success: callback });
...
function onDelete()
{
	 ...
	 for (var i = 0; i < len ; i++)
   {
	 ...
	 var chkmsg = confirm("Are you sure you would like to delete?");
		if (chkmsg) {
	  	  document.form.action="/track/MasterServlet?action=DELETE_HSCODE";
