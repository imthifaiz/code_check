...
<%
String title = "Edit INCOTERM Details";
%>
...
{
displaySummaryDelete = ub.isCheckValAcc("removeorderincoterms", plant,USERID);
displaySummaryNew = ub.isCheckValAcc("neworderincoterms", plant,USERID);

}
if(systatus.equalsIgnoreCase("INVENTORY"))
{
	displaySummaryDelete = ub.isCheckValinv("removeorderincoterms", plant,USERID);
displaySummaryNew = ub.isCheckValinv("neworderincoterms", plant,USERID);
}
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Edit INCOTERM Details</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->     
            <div class="box-header menu-drop">
              ...
              <button type="button" class="btn btn-default" onclick="window.location.href='../incoterms/new'" style="background-color: rgb(214, 72, 48);color: rgb(255, 255, 255);" >+ New</button>&nbsp;
              <%} %>
              </div>
		</div>
...
<button type="button" class="Submit btn btn-default" value="Remove From INCOTERMS" onClick="onDelete()">Remove From INCOTERMS</button>&nbsp;&nbsp;
<%} %>	
	</div>
	 </div> 
	 </form>
...
function onGo(index) {
    ...
    $.ajax({type: "POST",url: urlStr, data: {PLANT:"<%=plant%>",action: "VIEW_INCOTERMS_DETAILS_EDIT",PLANT:"<%=plant%>"},dataType: "json", success: callback });
...
function onDelete()
{
	 ...
	 for (var i = 0; i < len ; i++)
   {
	 ...
	 var chkmsg = confirm("Are you sure you would like to delete?");
		if (chkmsg) {
	  	  document.form.action="/track/MasterServlet?action=DELETE_INCOTERMS";
