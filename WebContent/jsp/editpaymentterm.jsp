...
<%
String title = "Edit Payment Terms";
%>
...
function popUpWin(URL) {
 		...
 		subWin = window.open(URL, 'PaymentTerm', 'toolbar=0,scrollbars=yes,location=0,statusbar=0,menubar=0,dependant=1,resizable=1,width=800,height=400,left = 200,top = 184');
...
String res= "",action="",paymentterm="",fieldDesc="",allChecked="";
...
{
displaySummaryDelete = ub.isCheckValAcc("Paytermremove", plant,USERID);
displaySummaryNew = ub.isCheckValAcc("Paytermnew", plant,USERID);

}
if(systatus.equalsIgnoreCase("INVENTORY"))
{
	displaySummaryDelete = ub.isCheckValinv("Paytermremove", plant,USERID);
displaySummaryNew = ub.isCheckValinv("Paytermnew", plant,USERID);
}
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Edit Payment Terms</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->     
            <div class="box-header menu-drop">
              ...
              <button type="button" class="btn btn-default"onclick="window.location.href='../Payterm/new'" style="background-color: rgb(214, 72, 48);color: rgb(255, 255, 255);" >+ New</button>&nbsp;
               <%} %> 
              </div>
		</div>
...
<button type="button" class="Submit btn btn-default" value="Remove From Payment Term" onClick="onDelete()">Remove From Payment Terms</button>&nbsp;&nbsp;
  <%} %> 
	</div>
	 </div>
 ...
 function checkAll(isChk)
{
  ...
  $.ajax({type: "POST",url: urlStr, data: {PLANT:"<%=plant%>",action: "VIEW_PAYMENTTERM_DETAILS_EDIT",PLANT:"<%=plant%>"},dataType: "json", success: callback });
...
function onDelete()
{
	 ...
	 for (var i = 0; i < len ; i++)
   {
	 ...
	 var chkmsg = confirm("Are you sure you would like to delete?");
		if (chkmsg) {
	  	  document.form.action="/track/MasterServlet?action=DELETE_PAYMENTTERM";
