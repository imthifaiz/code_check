...
ht.put("ID", TRANID);
ht.put("PLANT", plant);
List pdcHdrList =  billDao.getpdcpaymentById(ht);
if(pdcHdrList.size()>0)
{}
else
{
	response.sendRedirect("pdcpaymentSummary.jsp?result=No Data Found");
}
Map pdcHdr=(Map)pdcHdrList.get(0);
String PAYMENTID= (String) pdcHdr.get("PAYMENTID");
String PAYMENT_DATE= (String) pdcHdr.get("PAYMENT_DATE");
		...
		<div class="container-fluid">
			...
			<form class="form-horizontal" name="form" method="post" action="">
				<input type="text" name="plant" value="<%=plant%>" hidden> <input
					...
					type="text" name="vendno" value="" hidden> <input
			...
			<div class="col-sm-4">
				<input type="text" class="form-control" id="PAYMENT_DATE" name="PAYMENT_DATE" value="<%=PAYMENT_DATE%>" readonly>
					...
					<div class="form-group">
			<label class="control-label col-form-label col-sm-2">SUPPLIER</label>
			<div class="col-sm-4">
				<div class="input-group"> 
				<input type="text" class="form-control" name="vendname" value="<%=vendName%>" readonly>	
				</div>				
			</div>
		</div>
...
$("#cfmcancel").click(function(){
	document.form.action ="/track/BillPaymentServlet?Submit=PDA_PAYMENTCANCEL&TRANID=<%=pdcHdr.get("ID")%>";
	document.form.submit();
	});

function onProcess(form){
	document.form.action ="/track/BillPaymentServlet?Submit=PDA_PAYMENTEDIT";
	document.form.submit();
}
</script>
