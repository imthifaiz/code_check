...
<%
String title = "Edit Remarks Details";
%>
...
String res= "",action="",remarks="",fieldDesc="",allChecked="";
...
{
displaySummaryDelete = ub.isCheckValAcc("removeorderremarks", plant,USERID);
displaySummaryNew = ub.isCheckValAcc("neworderremarks", plant,USERID);

}
if(systatus.equalsIgnoreCase("INVENTORY"))
{
	displaySummaryDelete = ub.isCheckValinv("removeorderremarks", plant,USERID);
displaySummaryNew = ub.isCheckValinv("neworderremarks", plant,USERID);
}
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Edit Remarks Details</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->     
            <div class="box-header menu-drop">
              ...
              <button type="button" class="btn btn-default" onclick="window.location.href='../remarks/new'" style="background-color: rgb(214, 72, 48);color: rgb(255, 255, 255);" >+ New</button>&nbsp;
               <%} %>
              </div>
		</div>
...
<!-- <button type="button" class="Submit btn btn-default" onClick="window.location.href='../home'">Back</b></button>&nbsp;&nbsp; -->
...
<button type="button" class="Submit btn btn-default" value="Remove From Remarks" onClick="onDelete()">Remove From Remarks</button>&nbsp;&nbsp;
 <%} %>	
	</div>
	 </div>
 ...
 function checkAll(isChk)
{
  ...
  $.ajax({type: "POST",url: urlStr, data: {PLANT:"<%=plant%>",action: "VIEW_REMARKS_DETAILS_EDIT",PLANT:"<%=plant%>"},dataType: "json", success: callback });
...
function onDelete()
{
	 ...
	 for (var i = 0; i < len ; i++)
   {
	 ...
	 var chkmsg = confirm("Are you sure you would like to delete?");
		if (chkmsg) {
	  	  document.form.action="/track/MasterServlet?action=DELETE_REMARKS";
...
function callback(data){
	...
	if(!errorBoo){
        ...
        $.each(data.remarksmaster, function(i,item){
                   
        	outPutdata = outPutdata+item.REMARKSMASTERDATA;
                    	ii++;
