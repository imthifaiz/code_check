...
mLogger.setLoggerConstans(loggerDetailsHasMap);
    ...
    String sCustName = strUtils.fString(request.getParameter("EMP_NAME"));
	String type = strUtils.fString(request.getParameter("TYPE"));
	String form = strUtils.fString(request.getParameter("FORM"));
    custUtil.setmLogger(mLogger);
    
    String sBGColor = "";
   try{
    ArrayList arrCust = custUtil.getEmployeeListStartsWithName(sCustName,plant);
    for(int i =0; i<arrCust.size(); i++) {
        sBGColor = ((i == 0) || (i % 2 == 0)) ? "#FFFFFF" : "#dddddd";
        Map m=(Map)arrCust.get(i);
        String sCustCode     = (String)m.get("EMPNO");
        String sCustName1     = strUtils.replaceCharacters2Send((String)m.get("FNAME"));
        String sCustNameLast    = strUtils.replaceCharacters2Send((String)m.get("LNAME"));
        String isactive    = strUtils.removeQuotes((String)m.get("IsActive"));

if(type.equalsIgnoreCase("MOVETOWIP")){%>

<TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			onClick="window.opener.form.EMPNO.value='<%=sCustCode%>';window.opener.form.WONO.focus();
                     window.close();"><%=sCustCode%></a></td>
		<td align="left" class="main2">&nbsp;<%=strUtils.replaceCharacters2Recv(sCustName1)%></td>
		<%-- <td align="left" class="main2">&nbsp;<%=strUtils.replaceCharacters2Recv(sCustNameLast)%></td> --%>
   ...
   <TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			 onClick="window.opener.<%=form%>.EMP_NAME.value='<%=sCustCode%>';
                      window.close();"><%=sCustCode%></a></td>
		<td align="left" class="main2">&nbsp;<%=strUtils.replaceCharacters2Recv(sCustName1)%></td>
		<%-- <td align="left" class="main2">&nbsp;<%=strUtils.replaceCharacters2Recv(sCustNameLast)%></td> --%>
		<td align="left" class="main2">&nbsp;<%=isactive%></td>
   ...
   <TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			 onClick="window.opener.<%=form%>.EMP_ID.value='<%=sCustCode%>';
                      window.opener.<%=form%>.EMP_NAME.value='<%=strUtils.insertEscp(sCustName1)%>';
                      window.opener.<%=form%>.EMP_LNAME.value='<%=sCustNameLast%>';
                      window.close();"><%=sCustCode%></a></td>
		<td align="left" class="main2">&nbsp;<%=strUtils.replaceCharacters2Recv(sCustName1)%></td>
		<%-- <td align="left" class="main2">&nbsp;<%=strUtils.replaceCharacters2Recv(sCustNameLast)%></td> --%>
		<td align="left" class="main2">&nbsp;<%=isactive%></td>
   ...
   <TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			 onClick="window.opener.form.CUST_CODE.value='<%=sCustCode%>';
                      ...
                      <%-- window.opener.form.L_CUST_NAME.value='<%=sCustNameLast%>'; --%>
