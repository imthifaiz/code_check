...
<%
String title = "Sales Estimate Order Details Summary By Price";
%>
...
function ExportReport()
{
  ...
  document.form1.action="/track/EstimateServlet?Submit=ExportExcelEstimateOrderSummary";
...
function onGo(){
  ...
  document.form1.action="../salesestimate/salesestorderdetailbyprice";
  document.form1.submit();
}


function storeUserPreferences(){
	   storeInLocalStorage('estSumyr_issueWprice_FROMDATE', $('#FROM_DATE').val());
		storeInLocalStorage('estSumyr_issueWprice_TODATE', $('#TO_DATE').val());
		storeInLocalStorage('estSumyr_issueWprice_CUSTOMER', $('#CUSTOMER').val());
		storeInLocalStorage('estSumyr_issueWprice_ORDERNO', $('#ORDERNO').val());
		storeInLocalStorage('estSumyr_issueWprice_JOBNO', $('#JOBNO').val());
		storeInLocalStorage('estSumyr_issueWprice_ITEM', $('#ITEM').val());
		storeInLocalStorage('estSumyr_issueWprice_PRD_CLS_ID', $('#PRD_CLS_ID').val());
		storeInLocalStorage('estSumyr_issueWprice_PRD_TYPE_ID', $('#PRD_TYPE_ID').val());
		storeInLocalStorage('estSumyr_issueWprice_PRD_BRAND_ID', $('#PRD_BRAND_ID').val());
		storeInLocalStorage('convertedestimatesummarynopirce_PRD_DEPT_ID', $('#PRD_DEPT_ID').val());
		storeInLocalStorage('estSumyr_issueWprice_STATUS',$('#STATUS').val());
		storeInLocalStorage('estSumyr_issueWprice_EMP_NAME',$('#EMP_NAME').val());
}
...
{
	displaySummaryExport = ub.isCheckValAcc("exportsalesestimatepricereports", plant,LOGIN_USER);
	displaySummaryExport = true;
}
if(systatus.equalsIgnoreCase("INVENTORY"))
{
	displaySummaryExport = ub.isCheckValinv("exportsalesestimatepricereports", plant,LOGIN_USER);
	displaySummaryExport = true;
}
...
<div class="container-fluid m-t-20">
	...
	<!-- Muruganantham Modified on 16.02.2022 -->
            <ol class="breadcrumb backpageul" >      	
                ...
                <li><label>Sales Estimate Order Details Summary By Price</label></li>                                   
            </ol>             
    <!-- Muruganantham Modified on 16.02.2022 -->
...
<div class="box-body">
<FORM class="form-horizontal" name="form1" method="post" action="estSumyr_issueWprice.jsp" >
...
<!--   		<span class="select-search btn-danger input-group-addon " onClick="javascript:popUpWin('../jsp/employee_list.jsp?EMP_NAME='+form1.EMP_NAME.value+'&TYPE=ESTIMATE&FORM=form1');"><span class="glyphicon glyphicon-search" aria-hidden="true" style="left: -4px;"></span></span> -->
  ...
  <div id="tableIssueSummary_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
        ...
        <th style="font-size: smaller;">S/N</TH>
         ...
         <th style="font-size: smaller;">Converted Date</TH>
  ...
  var tableData = [];
       ...
       rowData[rowData.length] = '<%=new java.math.BigDecimal(unitPriceValue).toPlainString()%>';
       ...
       $(document).ready(function(){
    	   ...
    	   /*if (document.form1.FROM_DATE.value == ''){
    		   getLocalStorageValue('estSumyr_issueWprice_FROMDATE', '','FROM_DATE');
          }
    	   if (document.form1.TO_DATE.value == ''){
    		   getLocalStorageValue('estSumyr_issueWprice_TODATE', '','TO_DATE');
          } 
    	   if (document.form1.CUSTOMER.value == ''){
    			getLocalStorageValue('estSumyr_issueWprice_CUSTOMER', '','CUSTOMER');
          } 	
    	   if (document.form1.ORDERNO.value == ''){
    		   getLocalStorageValue('estSumyr_issueWprice_ORDERNO','','ORDERNO');
          } 
    	   if (document.form1.JOBNO.value == ''){
    		   getLocalStorageValue('estSumyr_issueWprice_JOBNO','', 'JOBNO');
          } 
    	   if (document.form1.ITEM.value == ''){
    		   getLocalStorageValue('estSumyr_issueWprice_ITEM','', 'ITEM');
          } 	
    	   if (document.form1.PRD_CLS_ID.value == ''){
    		   getLocalStorageValue('estSumyr_issueWprice_PRD_CLS_ID','', 'PRD_CLS_ID');
          } 	
    	   if (document.form1.PRD_TYPE_ID.value == ''){
    		   getLocalStorageValue('estSumyr_issueWprice_PRD_TYPE_ID','', 'PRD_TYPE_ID');
          }	
    	   if (document.form1.PRD_BRAND_ID.value == ''){
    		   getLocalStorageValue('estSumyr_issueWprice_PRD_BRAND_ID','', 'PRD_BRAND_ID');
          }	
    	   if (document.form1.STATUS.value == ''){
    			getLocalStorageValue('estSumyr_issueWprice_STATUS','','STATUS');
    		}	
    	   if (document.form1.EMP_NAME.value == ''){
    		   getLocalStorageValue('estSumyr_issueWprice_EMP_NAME','','EMP_NAME');
    		}
    	   storeUserPreferences();*/     	  
...
//     		   "lengthMenu": [[100, 500, 1000, 2500], [100, 500, 1000, "All"]],
    		        ...
    		        buttons: [
    		            ...
    		            {
    	                   extend: 'colvis',
    	                   columns: ':not(:eq(8)):not(:eq(11)):not(:eq(12))'
    	               }
    		        ], 
    		   	       "order": [],
    			        drawCallback: function( settings ) {
				            ...
				            api.column(groupColumn, {page:'current'} ).data().each( function ( group, i ) {
				                ...
				                totalcost += parseFloat($(rows).eq( i ).find('td:eq(8)').html().replace(',', ''));
				                totalIssueQty += parseFloat($(rows).eq( i ).find('td:eq(11)').html().replace(',', ''));
				                totalReceiveQty += parseFloat($(rows).eq( i ).find('td:eq(12)').html().replace(',', ''));
				            ...
				            if (groupEnd > 0 || rows.length == (currentRow + 1)){
				            	$(rows).eq( currentRow ).after(
				                        '<tr class="group"><td colspan=' + groupRowColSpan + '></td><td>Total</td><td>' + parseFloat(totalcost).toFixed(3) + '</td><td></td><td></td><td>' + parseFloat(totalIssueQty).toFixed(3) + '</td><td>' + parseFloat(totalReceiveQty).toFixed(3) + '</td><td></td><td></td><td></td><td></td><td></td></tr>'
			                    );
			                }
				        },
				        "footerCallback": function(row, data, start, end, display) {
		    	            ...
		    	            // Update footer
		    	            $(api.column(8).footer()).html(parseFloat(total).toFixed(<%=numberOfDecimal%>));
		    	            $(api.column(11).footer()).html(parseFloat(totalOrdVal).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>));
		    	            $(api.column(12).footer()).html(parseFloat(totalPicVal).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>));
		    	          }
    			  });	 
    	});       
