...
<div class="container-fluid m-t-20">
	...
	<!-- Thanzith Modified on 28.02.2022 -->
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Expense By Customer</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 28.02.2022 --> 
			      ...
			      <div class="ShowSingle"></div>
					...
					<div class="row">
						<div class="col-sm-12">
							...
							<table id="tableExpenseByCustomer" class="table table-bordred table-striped"> 
								<thead>
...
var FROM_DATE,TO_DATE,CUSTOMER;

function getParameters(){
	return {
		"FDATE":FROM_DATE,"TDATE":TO_DATE,
		"CUSTOMER":CUSTOMER,
		"ACTION": "VIEW_EXPENSES_BY_CUSTOMER",
		"PLANT":"<%=plant%>"
	}
}

function storeUserPreferences(){
	storeInLocalStorage('expenseByCustomer_FROM_DATE', $('#FROM_DATE').val());
	storeInLocalStorage('expenseByCustomer_TO_DATE', $('#TO_DATE').val());
	storeInLocalStorage('expenseByCustomer_CUSTOMER', $('#CUSTOMER').val());
}
...
function onGo(){
	...
	if (tableExpensesSummary){
		...
		tableExpensesSummary = $('#tableExpenseByCustomer').DataTable({
		"processing": true,
			...
			"dataSrc": function(data){
				if(typeof data.items[0].cname === 'undefined'){
					...
					for(var dataIndex = 0; dataIndex < data.items.length; dataIndex ++){
						...
						<%-- data.items[dataIndex]['amount'] = data.items[dataIndex]['basecurrency'] + addZeroes(parseFloat(data.items[dataIndex]['amount']).toFixed(<%=numberOfDecimal%>));
						data.items[dataIndex]['amountwithtax'] = data.items[dataIndex]['basecurrency'] + addZeroes(parseFloat(data.items[dataIndex]['amountwithtax']).toFixed(<%=numberOfDecimal%>)); --%>
					}
		       		return data.items;
		       	}
	        }
		...
		buttons: [
            ...
            {
                extend: 'colvis',
                columns: ':not(:eq('+groupColumn+')):not(:eq(2)):not(:last)'
            }
        ],
		 ...
		 $("#tableExpenseByCustomer_filter.dataTables_filter").append($("#searchType")); $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
...
$('#tableExpenseByCustomer').on('column-visibility.dt', function(e, settings, column, state ){
	if (!state){
		groupRowColSpan = parseInt(groupRowColSpan) - 1;
	}else{
		groupRowColSpan = parseInt(groupRowColSpan) + 1;
	}
	$('#tableExpenseByCustomer tr.group td:nth-child(1)').attr('colspan', groupRowColSpan);
	$('#tableExpenseByCustomer').attr('width', '100%');
});

function ExportReport()
{
  document.form1.action="/track/ExpensesServlet?ACTION=ExportExcelExpenseByCustomer";
...
function generateExpenseDetail(data){
	...
	doc.save('expenses_by_customer.pdf');
}

$(document).ready(function(){
	var ALLOWCATCH_ADVANCE_SEARCH = '<%=ALLOWCATCH_ADVANCE_SEARCH%>';
    if(ALLOWCATCH_ADVANCE_SEARCH==='1'){
		//getLocalStorageValue('expenseByCustomer_FROM_DATE',document.form1.FROM_DATE.value, 'FROM_DATE');
		//getLocalStorageValue('expenseByCustomer_TO_DATE','', 'TO_DATE');
		getLocalStorageValue('expenseByCustomer_CUSTOMER','', 'CUSTOMER');
    }
 	onGo();
 	$('.Show').click(function() {
	    $('#target').show(500);
