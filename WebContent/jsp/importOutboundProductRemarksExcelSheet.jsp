...
<%
String title = "Import Sales Order Product Remarks";
%>
...
{
  var frmRoot=document.form1;
  frmRoot.action = "/track/OutboundOrderImportServlet?action=confirmOutboundProdRemarksCountSheet" ;
  frmRoot.submit();
  return true;
}
 ...
 var frmRoot=document.form1;
  ...
  else if(frmRoot.SheetName.value=="" || frmRoot.SheetName.value.length==0)
	{
      ...
      if(con) {
          ...
          frmRoot.action = "/track/OutboundOrderImportServlet?action=importOutboundProdRemarksCountSheet" + "&ImportFile=" + file +"&SheetName=" + sheet;
          frmRoot.submit();
          return true;
   }else
   {
...
function onClear()
{
	document.form1.action  = "importOutboundProductRemarksExcelSheet.jsp";
	document.form1.submit();
}
...
if(action.equalsIgnoreCase("new")){
   ...
   session.removeAttribute("IMP_OUTBOUNDPRODREMARKSRESULT");
   session.removeAttribute("OUTBOUNDPRODREMARKSRESULT");
   ImportFile="";
   SheetName="";
   action="";
 ...
 fieldDesc=StrUtils.fString((String)session.getAttribute("OUTBOUNDPRODREMARKSRESULT"));

 if(null == session.getAttribute("IMP_OUTBOUNDPRODREMARKSRESULT"))
    {
      alRes = new ArrayList();
      buttonStat= "disabled";
   } else{
    
      alRes = (ArrayList)session.getAttribute("IMP_OUTBOUNDPRODREMARKSRESULT");
      if(alRes.size()==0)
      buttonStat= "disabled";
  }
...
%>
	...
	<!-- Muruganantham Modified on 16.02.2022 -->
            <ol class="breadcrumb backpageul">      	
                ...
                <li><label>Import Sales Order Product Remarks</label></li>                                   
            </ol>             
    <!-- Muruganantham Modified on 16.02.2022 -->
 ...
 <input type="hidden"  name="isDataPresent" value="true" ></input>
    <input type="hidden" value="" name="download"></input>
     ...
     <div class="form-group" style="margin-bottom: 3%;">
     	<div class="col-sm-3">  
     		<button type="button" class="btn btn-success" style="margin-left: 14%;" onclick="javascript:return onDownloadOutboundProdRemarksTemplate();">Download Sales Product RemarksTemplate</button>
     	</div> 
     </div>
<!--      ENDS -->
    <br>
  ...
  <TABLE class="table" WIDTH="90%"  border="0" cellspacing="1" cellpadding = 2 align = "center">
   <thead>
	 <tr>
        ...
        <th style="font-size: smaller;" width="3%"><font align="left">DONO</font></th> 
		<th style="font-size: smaller;" width="5%"><font align="left">Dolnno</font></th>
     ...
     <tbody>
       ...
       %>
           <TR>
               ...
               <TD align="left" width="10%"><%=StrUtils.forHTMLTag((String)lineArr.get(IConstants.OUT_DONO))%></TD>
               <TD align="left" width="12%"><%=StrUtils.forHTMLTag((String)lineArr.get(IConstants.OUT_DOLNNO))%></TD>
               <TD align="left" width="5%"><%=StrUtils.forHTMLTag((String)lineArr.get(IConstants.OUT_ITEM))%></TD>
