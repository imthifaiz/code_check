...
<%
String title = "Edit Invoice Receipt";
...
String custbank=custMstDAO.getCustBank(plant, custno);
String cmd="EDIT";
...
for(InvPaymentDetail invoicePaymentDetail:invoicePaymentDetails)
	...
	if(invoicePaymentDetail.getTYPE().equalsIgnoreCase("REGULAR"))
	{
		...
		ht.put("PLANT", plant);
		rebillno=String.valueOf(invoicePaymentDetail.getINVOICEHDRID());
...
String amountReceived = String.valueOf(invoicePaymentHdr.getAMOUNTRECEIVED());
	...
	CURRENCYID  = String.valueOf(invoicePaymentHdr.getCURRENCYID());
	DISPLAY  = String.valueOf(invoicePaymentHdr.getCURRENCYID());
	CURRENCYUSEQT = String.valueOf(invoicePaymentHdr.getCURRENCYUSEQT());
	conv_amount_Curr=String.valueOf(invoicePaymentHdr.getCONV_AMOUNTRECEIVED());
	conv_invoiceAmount=StrUtils.addZeroes((Float.parseFloat(invoicePaymentHdr.getCONV_AMOUNTRECEIVED())), numberOfDecimal);
...
double usedamount = invoicePaymentHdr.getAMOUNTRECEIVED() - invpaymentdao.getbalacedueusingrechdr(plant, String.valueOf(paymentheaderid));
String odpaystatus ="0";
double orderbalpay = 0.0;
String reccheck ="0";
...
if(invoicedetreccheck.size() > 0){
	 for(InvPaymentDetail invoicePaymentDetail:invoicedetreccheck)  {
		 if(!invoicePaymentDetail.getDONO().equalsIgnoreCase("0")){
			 odpaystatus = "1";
			 String ORDERNO = invoicePaymentDetail.getDONO();
			 ...
			 orderbalpay = (dTotalCost + Double.parseDouble(amountReceived)) - (orderadvanceamt + paidamountfororder);
	...
	for(InvPaymentDetail invoicePaymentDetail:invoicePaymentDetails)
		...
		if(invoiceHdrListin.size()>0)
		{
			...
			String totalAmount = (String)invoiceHdrin.get("TOTAL_AMOUNT");
			double baldue=Double.valueOf(totalAmount)-invoicePaymentDAO.getbalacedue(plant, String.valueOf(invoicePaymentDetail.getINVOICEHDRID()));
			balanceDue = baldue + Double.parseDouble(amountReceived);
		}
...
<script src="../jsp/js/InvoiceEditPayment.js"></script>
<style>
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Edit Invoice Receipt</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->
...
<input type="number" id="numberOfDecimal" style="display:none;" value=<%=numberOfDecimal%>>
	...
	<div class="form-group customer-section">
			...
			<div class="col-sm-4 ac-box">
				...
				<input type="text" class="ac-selected  form-control typeahead" id="CUSTOMER" placeholder="Select a customer" readonly name="customer" value="<%=custname%>" required>
		...
		<div class="form-group">
			...
			<div class="col-sm-4">
				<input type="text" class="form-control" id="amount_Curr" name="amount_Curr" value="<%=conv_invoiceAmount%>" required <%=payreadonly%> onchange="calculatebasecurrency(this)">
			</div>
		</div>
		<div class="form-group">
			<label class="control-label col-form-label col-sm-2 required">Amount&nbsp;(<%=curency%>)</label>
			<div class="col-sm-4">
				<input type="text" class="form-control" id="amount" name="amount" onchange="amountchanged(this)" value="<%=amountReceived%>" required <%=pay_currreadonly%>>
			</div>
		</div>
		...
		<div class="form-group">
			...
			<label class="control-label col-form-label col-sm-2 required">Mode of Payment</label>
			<div class="col-sm-4">
		...
		<div class="row">
			<h1 style="font-size: 20px;" class="box-title">Unpaid invoices</h1>
			<div class="col-xs-12">
				<table id="table2" class="table">
					<thead>
						<tr>
							...
							<td>Invoice Balance (<%=curency%>)</td>
							<td>Payment Amount (<%=curency%>)</td>	
							<td>Apply Credit</td>				
					...
					<tbody>
						...
						for(int i =0; i<invoiceHeaderList.size(); i++) {   
			  			...
			  			/* Paymentamount = String.valueOf(invDet.getAMOUNT()); */
			  			Paymentamount = String.valueOf( m.get("Amountpaid"));
			  			...
			  			Invoiceamount = (String) m.get("TOTAL_AMOUNT");
			  			...
			  			String cov_Invoiceamount= StrUtils.addZeroes((Float.parseFloat(Invoiceamount)*Float.parseFloat((String) m.get("CURRENCYUSEQT"))), numberOfDecimal);
			  			String paymentamountid=String.valueOf(invDet.getID());
			  		
			  			dtotalamount=dtotalamount+dPaymentamount;
			  			totalamount=StrUtils.addZeroes(dtotalamount, numberOfDecimal);
			  			
			  			double advancebalace = invpaymentdao.getcreditamout(plant, (String) m.get("CUSTNO"));
						double orderbalance  = invpaymentdao.getcreditamoutusingorderno(plant, (String) m.get("DONO"));
						double ovallbalace = advancebalace + orderbalance;
						double balance = invpaymentdao.getbalacedue(plant, (String) m.get("ID"));
						
						double tbalance = Double.parseDouble(Invoiceamount) - balance;
						String invbal = StrUtils.addZeroes(tbalance, numberOfDecimal);
			  		%>
				  		<tr>
							
							<td class="text-center"><%=m.get("INVOICE") %></td>
							<td class="text-center"><%=m.get("DONO") %></td>
							<td class="text-center"><%=m.get("INVOICE_DATE")%></td>
							<td class="text-center"><%=(String) m.get("CURRENCYID")%><%=cov_Invoiceamount%></td>
							<td class="text-center"><%=(String) m.get("CURRENCYUSEQT")%></td>
							<td class="text-center"><%=Invoiceamount%></td>
							<td class="text-center"><%=invbal%></td>
							<%-- <td class="text-center"><%=curency%><input type="text"  readonly class="paymentamountclass" style="border:1px solid #eee" id="<%=paymentamountid %>" name="<%=paymentamountid %>" onchange="invoicepaymentchange(this,<%=Invoiceamount %>)" value="<%=Paymentamount%>"></td> --%>
							<td class="text-center"><%=Paymentamount%></td>
							<%if(tbalance > 0){ %>
								<%if(ovallbalace > 0){ %>
					        		<td class="text-center"><button type="button" class="btn btn-default acredits" onClick="aCredit('<%=m.get("ID")%>','<%=m.get("DONO") %>','<%=m.get("CUSTNO") %>','<%=m.get("INVOICE") %>','<%=invbal%>','<%=m.get("CURRENCYID") %>','<%=m.get("CURRENCYUSEQT") %>')"  data-toggle="modal" data-target="#applycredit">Apply Credits</button></td>
					        	<%}else{ %>
				  	...
				  	<%}
						totalamount = StrUtils.addZeroes(Double.parseDouble(totalamount), numberOfDecimal);
				  	%>
				  	
				  	<%

						for(int i =0; i<invoiceHeaderListcredit.size(); i++) {   
				  		Map m=(Map)invoiceHeaderListcredit.get(i);
				  		String Paymentamount="",Invoiceamount;				  		
				  		InvPaymentDetail invDet=invoicePaymentDetcredit.get((String) m.get("INVOICE"));
			  			/* Paymentamount = String.valueOf(invDet.getAMOUNT()); */
			  			Paymentamount = String.valueOf( m.get("Amountpaid"));
			  			double dPaymentamount ="".equals(Paymentamount) ? 0.0d :  Double.parseDouble(Paymentamount);
			  			Paymentamount = StrUtils.addZeroes(dPaymentamount, numberOfDecimal);
			...
			<div class="col-xs-12">
				<div class="offset-lg-7 col-lg-5 clearfix">
					<div class="row">
						...
						<p class="col-lg-4 text-right" id="totalInvoiceAmount"><%=totalamount %></p>
					</div>
				</div>
			</div>
		</div>		
			...
			<div class="col-xs-12">
				<div class="extraInfo offset-lg-7 col-lg-5 clearfix">
					<div class="row">
						...
						<p class="col-lg-4 text-right" id="amountreceived"><%=amountReceived%></p>
					</div>
					<div class="row">
						<p class="col-lg-8 text-right">Amount used for payments :</p>
						<p class="col-lg-4 text-right" id="amountufp"><%=totalamount%></p>
					</div>
					<%
					double amountrefounded=invoicePaymentHdr.getAMOUNTREFUNDED();
					String srefoudedamounr = String.valueOf(amountrefounded);
					double drefoudedamounr ="".equals(srefoudedamounr) ? 0.0d :  Double.parseDouble(srefoudedamounr);
					srefoudedamounr = StrUtils.addZeroes(drefoudedamounr, numberOfDecimal);
					
					%>
					<div class="row">
						<p class="col-lg-8 text-right">Amount Refunded :</span>
						<p class="col-lg-4 text-right" id="amountrefunded"><%=srefoudedamounr%></p>
					</div>
					<% 
					double amountrec=invoicePaymentHdr.getAMOUNTRECEIVED();
					/* double amountused=invoicePaymentHdr.getAMOUNTUFP(); */
					double amountused=Double.parseDouble(totalamount);
					double amountexcess=(amountrec-amountused);
					String excessAmountStr = String.valueOf(amountexcess);
					double damountexcess ="".equals(excessAmountStr) ? 0.0d :  Double.parseDouble(excessAmountStr);
					excessAmountStr = StrUtils.addZeroes(damountexcess, numberOfDecimal);
							%>
					<div class="row">
						<p class="col-lg-8 text-right">Amount in excess:</p>
						<p class="col-lg-4 text-right" id="amountexcess"><%=excessAmountStr %></p>
					</div>
				</div>
			</div>
		</div>
...
function amountchanged(amountnode)
{
	...
	{ 
		...
		$("input[name ='amount_paid']").val(parseFloat(initailAmount).toFixed(numberOfDecimal));
