...
mLogger.setLoggerConstans(loggerDetailsHasMap);
   ...
   sItemDesc = " and replace(ITEMDESC,' ','') like '%"+ new StrUtils().InsertQuotes(sItemDesc.replaceAll(" ","")) + "%' ";
   }

	
    List listQry = itemUtil.qryItemMstCond(sItem,plant,sItemDesc);
    for(int i =0; i<listQry.size(); i++) {
     ...
     String stkqty    = (String)vecItem.get(8);
      ...
      String prd_cls_id    = (String)vecItem.get(10);    
         ...
         String price = StrUtils.fString((String)vecItem.get(12));  
         String cost = StrUtils.fString((String)vecItem.get(13));
         String discount = StrUtils.fString((String)vecItem.get(14));
...
%>
	<TR bgcolor="<%=sBGColor%>">
      ...
      window.opener.form.ITEM_DESC.value='<%=strUtils.insertEscp(sDesc)%>';
      window.opener.form.ARTIST.value='<%=sArtist%>';
       window.opener.form.UOM.value='<%=sUom%>';
      window.opener.form.TITLE.value='<%=sTitle%>';
      window.opener.form.MEDIUM.value='<%=strUtils.insertEscp(sMedium)%>';
      window.opener.form.ITEM_CONDITION.value='<%=sCondition%>';
      window.opener.form.REMARKS.value='<%=strUtils.insertEscp(sRemarks)%>';
      window.opener.form.STKQTY.value='<%=stkqty%>';
      window.opener.form.PRD_CLS_ID.value='<%=prd_cls_id%>';
       window.opener.form.PRICE.value='<%=price%>';
       window.opener.form.COST.value='<%=cost%>';
