...
<SCRIPT LANGUAGE="JavaScript">
	...
	function onNew(){
	   ...
	   document.form.action  = "item_list_order.jsp?action=NEW&PRD_CLS_ID="+prdcls;
	...
	document.form.DYNAMIC_SUPPLIERDISCOUNT_SIZE.value = document
			...
			document.form.action = "item_list_order.jsp?action=ADD";
			document.form.submit();
		}
	...
	function onViewMapItem() {
		...
		document.form.action = "item_list_order.jsp?action=VIEWMAPITEM";
		document.form.submit();
	}
	      ...
	      else
	      {
	      ...
	      } catch(Exception e)
	         {
	    	  mLogger.exception(true,
						"ERROR IN JSP PAGE - item_list_order.jsp ", e);
	         }
...
<input type="hidden" value="hide" name="search_criteria_status" id="search_criteria_status" />
		...
		<label class="control-label col-sm-2" style="padding-top: 5px;">Used as Non Stock Product:</label>
 		<div class="col-sm-2">
      	...
      	<input type="hidden" name="NONSTOCKFLAG" value="N"  id="NotNonStock" checked="checked" onclick="DisplayNonStkType();"><!-- NO
    	</label> -->
    	<!-- <label class="radio-inline"> -->
      	<input type="hidden" name="NONSTOCKFLAG" value="Y"  id = "NonStock" onclick="DisplayNonStkType();"><!-- YES
    	</label> -->
     	<!-- </div>
		</div> -->
      	</div>
...
String sCond = " AND ISNULL(NONSTKFLAG,'')<>'Y'";
...
<TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			onClick=" window.opener.form.ITEM.value='<%=item%>';window.opener.form.DESC.value='<%=itemdesc%>';
      	             ;window.opener.form.UOM.value='<%=uom%>';window.opener.validateProduct();window.close();"><%=item%></a></td>
