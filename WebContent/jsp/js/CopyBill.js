...
//				document.getElementById('nTAXTREATMENT').innerHTML="";
	...
	$("#btnBillDraft").click(function(){
		...
		$("#sub_total").val($("#subTotal").html());
		$("#total_amount").val($("#total").html());
		document.form.DEDUCT_INV.value="0";
		calculateLandedCost();
		$("#createBillForm").submit();
	});
	$("#btnBillOpen").click(function(){
...
function addRow(event){
	...
	body += '<img alt="" src="../jsp/dist/img/NO_IMG.png" style="width: 100%;"><input type="hidden" name="basecost" value="0.00"><input type="hidden" name="lnno" value="1">';
	...
	body += '<button type="button" style="position: absolute;margin-left: -18px;z-index: 2;vertical-align: middle;font-size: 20px;opacity: 0.5;height: 34px;" onclick="changeitem(this)"><i class="glyphicon glyphicon-menu-down" style="font-size: 8px;"></i></button>';
...
function productCallbackwithall(productData){
	if(productData.STATUS="SUCCESS"){
		...
		body += '<input type="hidden" name="outgoingqty" value="0.000">';
		body += '<input type="hidden" name="tax_type" class="taxSearch" value="'+taxdisplay+'">';
...
function calculateLandedCost(){
	...
	if(isNaN(additionalExpenses)){
		additionalExpenses = parseFloat("0");
	}
...
function loadBillTable(orders){
	...
	var ITEMDESC = escapeHtml(data.ITEMDESC);
		...
		body += '<td class="item-cost text-right"><input type="text" name="cost" class="form-control text-right" value="'+data.UNITCOST+'" readonly="readonly" onkeypress="return isNumberKey(event,this,4)"></td>';
		body += '<td class="item-discount text-right">';
		body += '<div class="row">';							
		body += '<div class=" col-lg-12 col-sm-3 col-12">';
		body += '<div class="input-group my-group" style="width:120px;">';
		body += '<input name="item_discount" type="text" class="form-control text-right item_discount" value="'+data.ITEM_DISCOUNT+'" onchange="calculateAmount(this)">';
