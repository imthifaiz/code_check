...
$(document).ready(function(){
	...
	var POESTNO = document.cpoform.PONOOLD.value;
	getEditPODetails(POESTNO);
//	getSupplierDetails();
	...
	$("#btnSalesOpenEmail").click(function(){
		...
		if(isValid){
			...
			$.ajax({
				type : "POST",
				url : "../purchaseorderestimate/copy",
				data : data,
				dataType : "json",
				contentType: false,
				processData: false,
...
//					}
	...
	$("#btnSaveRemarks").click(function(){
		...
		$.ajax({
			type : "POST",
			url : "../purchaseorderestimate/addRemarks",
			data : data,
			dataType : "json",
			success : function(data) {
				if(data.ERROR_CODE == 100)
	...
	$("input[name=QTY]").focusout(function(){
		...
		var plno = $(this).closest('tr').find("input[name=POESTLNNO]").val();
		alert(value);
		alert(rcqry);
		alert(plno);
		if(plno != "0"){
...
function checkqty(obj){
	...
	var plno = $(obj).closest('tr').find("input[name=POESTLNNO]").val();
	if(plno != "0"){
...
function addRow(event){
	...
	var body="";
	body += '<tr>';
	body += '<td><input type="Checkbox" style="border:0;background=#dddddd"	name="chkdPOESTNO" value="">';
	body += '</td>';
	...
	body += '<input type="hidden" name="POESTLNNO" value="0">';
...
function checkorderno(orderno){	
		var urlStr = "../purchaseorderestimate/CheckOrderno";
		$.ajax( {
			type : "GET",
			url : urlStr,
			async : true,
			data : {
				...
				success : function(data) {
					if (data.status == "100") {
						...
						document.getElementById("POESTNO").focus();
						document.getElementById("POESTNO").value="";
						return false;	
					} 
					else 
						return true;
...
function validateOrderNo(){
	var orderno = $("input[name=POESTNO]").val();
	if(orderno != ""){	
		var urlStr = "../purchaseorderestimate/CheckOrderno";
		$.ajax( {
			type : "GET",
	...
	if(precost == "1"){
		...
		$.ajax({
			...
			data : {
				...
				UOM:uom,
				ROWS:"2",
				Submit : "GET_PREVIOUS_ESTIMATE_ORDER_DETAILS"
			},
			dataType : "json",
			success : function(data) {
				if(data.orders[0].POESTNO != undefined){
				var result = "";
				$.each(data.orders, function( key, value ) {
					result += "<tr>";
					result += "<td>"+value.POESTNO+"</td>";
...
function CheckPriceVal(obj, uom) {
	...
	var POESTNO = $("input[name=DONO]").val();
	var desc = "";
...
function showRemarksDetails(obj) {
	...
	var POESTNO = $("input[name=POESTNO]").val();
	...
	if(POESTNO == ''){
		...
		$.ajax({
			type : "GET",
			url : "../purchaseorderestimate/getPurchaseOrderRemarks",
			data : {
				ITEM: item,
				POESTNO: POESTNO,
				POLNO: lnno
			...
			success : function(data) {
				...
				body += '<input type="hidden" name="r_POESTNO" value="'+POESTNO+'">';
...
function setLineNo(){
	...
	$(".po-table tbody tr td:first-child").each(function() {
		...
		$(this).find('input[name=chkdPOESTNO]').val(i);
		i++;
	});
}
...
function validatePurchaseOrder(){
	...
	$(".po-table tbody tr").each(function() {
		if($(this).find('input[name=chkdPOESTNO]').is(':checked')){
			checkedFlag = true;
...
function getEditPODetails(POESTNO){
	var plant = document.cpoform.plant.value;
	$.ajax({
		type : "POST",
		url : "../purchaseorderestimate/LoadEditDetails?POESTNO="+POESTNO,
		async : true,
		data : {
			PLANT : plant,
		},
...
function loadPOTable(orders){
	...
	$.each(orders, function( key, data) {
			...
			body += '<tr>';
			body += '<td>';
			body += '<input type="Checkbox" style="border:0;background=#dddddd"	name="chkdPOESTNO" value="'+j+'">';
			body += '</td>';
			body += '<td class="item-img text-center">';
			...
			body += '<input type="hidden" name="POESTLNNO" value="'+data.LNNO+'">';
...
{
	 var urlStrAttach = "../purchaseorderestimate/downloadAttachmentById?attachid="+id;
...
{
	var urlStrAttach = "../purchaseorderestimate/removeAttachmentById?removeid="+id;
	$.ajax( {
		type : "POST",
		url : urlStrAttach,
		success : function(data) {
...
{
	var len = document.cpoform.chkdPOESTNO.length;
 	var orderLNo;
 	if(len == undefined) len = 1;
	if (document.cpoform.chkdPOESTNO){
        for (var i = 0; i < len ; i++){
			if(len == 1){
				document.cpoform.chkdPOESTNO.checked = isChk;
			}
			else{
				document.cpoform.chkdPOESTNO[i].checked = isChk;
			}
        }
    }
}
...
function generatePdfMail(dataUrl,attachName){
	...
	formData.append("POESTNO", $("#POESTNO").val());
	progressBar();
