...
//				document.getElementById('nTAXTREATMENT').innerHTML="";
			...
			if($('select[name ="nTAXTREATMENT"]').val() =="GCC VAT Registered"||$('select[name ="nTAXTREATMENT"]').val()=="GCC NON VAT Registered"||$('select[name ="nTAXTREATMENT"]').val()=="NON GCC")
			{
				...
				getvendname(selection.TAXTREATMENT,selection.VENDO,selection.PAYMENTTYPE,selection.TRANSPORTNAME,selection.TRANSPORTID,selection.PAYMENT_TERMS,selection.PAY_TERMS,selection.NAME,selection.TELNO,selection.EMAIL,selection.ADD1,selection.ADD2,selection.ADD3,selection.ADD4,selection.REMARKS,selection.COUNTRY,selection.ZIP,selection.CURRENCYID,selection.CURRENCY,selection.CURRENCYUSEQT)
				$('#shipbilladd').empty();
...
//				<span data-toggle="modal" data-target="#shipaddr">Change</span> //old
	...
	$("#btnSalesOpenEmail").click(function(){
		...
		if(isValid){
			...
			$.ajax({
				type : "POST",
				url : "../purchaseorderestimate/save",
				data : data,
				dataType : "json",
				contentType: false,
				processData: false,
...
function addUserRow() {
    ...
    body += '</span>';
	body += '</td>';
	body += '<td class="text-center grey-bg" style="position:relative;" style="right: -95px;">';
