...
$(document).ready(function(){
	...
	var POMULTIESTNO = document.cpoform.POMULTIESTNO.value;
...
if(document.cpoform.nTAXTREATMENT.value =="GCC VAT Registered"||document.cpoform.nTAXTREATMENT.value=="GCC NON VAT Registered"||document.cpoform.nTAXTREATMENT.value=="NON GCC")
	...
	$(document).on('click','#autoGen',function(){
		$.ajax({
			type: "GET",
			url: "../purchaseestimate/Auto-Generate",
			dataType: "json",
			beforeSend: function(){
				showLoader();
			},
			success: function(data) {
				$("#POMULTIESTNO").val(data.POMULTIESTNO);
...
function addSuggestionToTable(){
	...
	},
	{
	  ...
	  source: function (query, process,asyncProcess) {
		...
		if (document.getElementById("isProductAssignedSupplier").checked == true) {
			action = "GET_PRODUCT_ASSIGNED_SUPPLIER";
		}
		var urlStr = "/track/MasterServlet";
		$.ajax( {
		type : "POST",
	  ...
	  empty: [
	    ...
	    suggestion: function(data) {
			if (document.getElementById("isProductAssignedSupplier").checked == true) {
				return '<div><p class="item-suggestion">Name: ' + data.VNAME + '</p><br/><p class="item-suggestion">Code: ' + data.VENDO + '</p><br/><p class="item-suggestion">Contact Name: ' + data.NAME + '</p></div>';
			}else{
...
function checkAllExp(isChk){
var expdate = document.getElementById("EXPIREDATE").value;
	$('.po-table tbody tr').each(function () {
		if (document.getElementById("selectesp").checked == true) {
			document.getElementById("selectesp").value = "1";
			$(this).find('input[name=EXPIREDATE]').val(expdate);
		}else {
			document.getElementById("selectesp").value = "0";
			$(this).find('input[name=EXPIREDATE]').val("");
			document.getElementById("EXPIREDATE").value = expdate;
		}
	});
} 

function checkItemSupplier(isChk){
...
function addRow(){
	...
	var expdate = $("input[name=EXPIREDATE]").val();
	var body="";
	body += '<tr>';
	...
	body += '<input type="text" name="item" class="form-control itemSearch" style="width:87%" onchange="checkitems(this.value,this)"  placeholder="Type or click to select an item.">';
	...
	body += '<td class="bill-item">';
	body += '<input type="text" name="vendname"  class="form-control vendsearch" onchange="checksupplier(this.value,this)" style="width:90%" placeholder="Supplier">';
	body += '</td>';
	...
	body += '<input type="text" name="EXPIREDATE" class="form-control datepicker" value="'+expdate+'" READONLY>';
	body += '</td>';
	}else{
	body += '<td class="item-amount grey-bg">';
...
function CheckPriceVal(obj, uom) {
    ...
    var currencyUSEQT = $(obj).closest('tr').find("input[name=CURRENCYUSEQT]").val();
	   ...
	   var urlStr = "/track/CreateCustomerServlet?action=JADD&reurl=createInvoice";
		...
		success : function(data) {
			if(document.cpoform.custModal.value == "cust"){
				...
				$("input[name ='CURRENCY']").val(data.customer[0].CURRENCY);
...
function productCallbackpotable(productData){
	if(productData.STATUS="SUCCESS"){
		...
		var taxdisplay = $("input[name=ptaxdisplay]").val();
		var taxdisplay = $("input[name=ptaxdisplay]").val();
		var tds = $('.po-table tbody').children('tr').length;
		if (document.getElementById("selectesp").checked == true) {
		expdate = document.getElementById("EXPIREDATE").value;
		}
		$last.remove();
		var curency = $("input[name=BASECURRENCYID]").val();
		...
		if(productData.incomingIBDiscount=='' || productData.incomingIBDiscount=='0' ||productData.incomingIBDiscount=='0.00'||productData.incomingIBDiscount==undefined)
		{
		body += '<td class="item-cost text-right"><input type="text" name="unitprice" class="form-control text-right" value="'+parseFloat(productData.ConvertedUnitCost).toFixed(numberOfDecimal)+'" onchange="calculateAmount(this)" onkeypress="return isNumberKey(event,this,4)"></td>';
		body += '<td class=\"table-icon\"><a href="#" class="fa fa-info-circle" onclick="getPreviousPurchaseOrderDetails(this)"></a></td>';
		} else { 
		body += '<td class="item-cost text-right"><input type="text" name="unitprice" class="form-control text-right" value="'+parseFloat(productData.incomingIBDiscount).toFixed(numberOfDecimal)+'" onchange="calculateAmount(this)" onkeypress="return isNumberKey(event,this,4)"></td>';
		body += '<td class=\"table-icon\" style="width:3%"><a href="#" class="fa fa-info-circle" onclick="getPreviousPurchaseOrderDetails(this)"></a></td>';
		}		
		/*body += '<td class="item-discount text-right">';
		body += '<div class="row">';							
		body += '<div class=" col-lg-12 col-sm-3 col-12">';
		...
		body += '<td class="item-amount grey-bg">';
		if (tds == 1) {
		body += '<input type="text" name="EXPIREDATE" id="EXPIREDATE" class="form-control datepicker" value="'+expdate+'" READONLY>';
		} else {
		body += '<input type="text" name="EXPIREDATE" class="form-control datepicker" value="'+expdate+'" READONLY>';
		}
		body += '</td>';
		...
		body += '<td class="item-amount grey-bg">';
		if (tds == 1) {
		body += '<input type="text" name="EXPIREDATE" id="EXPIREDATE" class="form-control datepicker" READONLY>';
		} else {
		body += '<input type="text" name="EXPIREDATE" class="form-control datepicker" READONLY>';
		}
		body += '</td>';
...
function loadPOTable(orders){
	...
	$.each(orders, function( key, data) {
			...
			body += '<td class="bill-item">';
			body += '<input type="text" name="vendname" onchange="checksupplier(this.value,this)" value="'+data.VNAME+'" class="form-control vendsearch" style="width:90%" placeholder="Supplier Name/ID">';
			body += '</td>';
		...
		//	body += '</td>';
			...
			body += '<td class="item-amount grey-bg">';
			if(data.LNNO==1) {
			body += '<input type="text" name="EXPIREDATE" id="EXPIREDATE" class="form-control datepicker" READONLY value="'+data.EXPIREDATE+'">';
			} else {
					...
					if (data.status == "100") {
						...
						else
						{
							...
							else
							{
								$(obj).closest('tr').find("input[name=discounttype]").val("");
								$(obj).closest('tr').find("input[name=customerdiscount]").val(parseFloat(resultVal.incomingIBDiscount).toFixed(numberOfDecimal));
								price = parseFloat(resultVal.ConvertedUnitCost-(resultVal.incomingIBDiscount*Curramt));
								//price = parseFloat(resultVal.incomingIBDiscount);
							}
							var calAmount = parseFloat(price).toFixed(numberOfDecimal);
							$(obj).closest('tr').find("input[name=unitprice]").val(calAmount);
