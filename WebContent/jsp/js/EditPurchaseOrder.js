...
$(document).ready(function(){
	...
	var pono = document.cpoform.PONO.value;
	getEditPODetails(pono);
...
//					}
	...
	$("#btnPurchaseOpen").click(function(){
		$('input[name ="draftstatus"]').val('1');
		var isValid = validatePurchaseOrder();
		if(isValid){
			$( "#purchaseOrderForm" ).submit();
			...
			data : {
				...
				success : function(data) {
					if (data.status == "99") {
						...
						document.getElementById("BANKNAMES").focus();
						$("#BANKNAMES").typeahead('val', '');
						return false;	
					} 
					else 
						return true;
				...
				success : function(data) {
					...
					if (data.status == "100") {
						...
						if(resultVal.ConvertedUnitCost == null || resultVal.ConvertedUnitCost == undefined || resultVal.ConvertedUnitCost == 0){
							...
							$(obj).closest('tr').find("input[name=unitprice]").val(parseFloat(covcalAmount).toFixed(numberOfDecimal));
						...
						else
						{
							...
							//var covcalAmount = parseFloat($("input[name ='CURRENCYUSEQT']").val()) * parseFloat(price);
...
function setLineNo(){
	...
	$(".po-table tbody tr td:first-child").each(function() {
		...
		$(this).find('input[name=prlnno]').val(i);
		i++;
	});
}
...
//			$('select[name ="CURRENCY"]').val(data.supplier[0].CURRENCY);
...
function productCallbackpotable(productData){
	if(productData.STATUS="SUCCESS"){
		...
		body += '</td>';
		body += '<td class="bill-item">';
		body += '<input type="text" name="item" class="form-control itemSearch" style="width:87%;display: inline-block;" value="'+productData.ITEM+'" placeholder="Type or click to select an item." onchange="calculateAmount(this)">';
		body += '<button type="button" style="position: absolute;margin-left: -18px;z-index: 2;vertical-align: middle;font-size: 20px;opacity: 0.5;" onclick="changeitem(this)"><i class="glyphicon glyphicon-menu-down" style="font-size: 8px;"></i></button>';
		...
		body += '<input type="text" name="PRODUCTDELIVERYDATE" class="form-control datepicker" READONLY tabindex="-1">';
		body += '</td>';
		body += '<td class="item-cost text-right"><input type="text" name="unitprice" class="form-control text-right" value="'+parseFloat(productData.UNITPRICE).toFixed(numberOfDecimal)+'" onchange="calculateAmount(this)" onkeypress="return isNumberKey(event,this,4)"></td>';
		body += '<td class=\"table-icon\"><a href="#" class="fa fa-info-circle" onclick="getPreviousPurchaseOrderDetails(this)"></a></td>';
		body += '<td class="item-discount text-right">';
		body += '<div class="row">';							
		body += '<div class=" col-lg-12 col-sm-3 col-12">';
...
//		body += '</td>';
		...
		removeSuggestionToTable();
		addSuggestionToTable();
		loadItemData($tbody.find('tr:last'), productData.ITEM,productData.CATLOGPATH,"Inventory Asset",productData.UNITPRICE,productData.PURCHASEUOM);
	}
	if($("input[name=DEDUCT_INV]").val() == "0"){
		$(".invEl").hide();
	}
...
function validatePurchaseOrder(){
	...
	var CURRENCYUSEQT = $("input[name=CURRENCYUSEQT]").val(); //RESVI
	var issupinvno = $("input[name=ISJOB_NUM]").val();
	var supinvno = $("input[name=JOB_NUM]").val();
	...
	var stats = $('input[name ="orderstatus"]').val();
...
//	var currencyfoc = document.cpoform.CURRENCY.focus();
	...
	if(issupinvno == "1"){
		if(supinvno == ""){
			alert("Please select a Supplier Invoice No.");
			document.getElementById("JOB_NUM").focus();
			return false;
		}
		
		if(suptamt == ""){
			alert("Please Enter Supplier Invoice Amount");
		...
		if(parseFloat(valisuptamt) == parseFloat(suptamt)){	
			...
			alert("Supplier Invoice Amount And Purchase Order Amount Does't Match'");
			document.getElementById("SUPPLIER_TAMOUNT").focus();
			return false;
		}
...
function getEditPODetails(pono){
	var plant = document.cpoform.plant.value;
	$.ajax({
		type : "POST",
		url : "../purchaseorder/LoadEditDetails?PONO="+pono,
		async : true,
		data : {
			PLANT : plant,
		},
...
function loadPOTable(orders){
	...
	$.each(orders, function( key, data) {
			...
			body += '<input type="hidden" name="tax_type" class="taxSearch" value="'+data.TAXTYPE+'">';
			...
			body += '<input type="text" name="item" class="form-control itemSearch" value="'+data.ITEM+'" style="width:87%;display: inline-block;" placeholder="Type or click to select an item.">';
			body += '<button type="button" style="position: absolute;margin-left: -18px;z-index: 2;vertical-align: middle;font-size: 20px;opacity: 0.5;" onclick="changeitem(this)"><i class="glyphicon glyphicon-menu-down" style="font-size: 8px;"></i></button>';
			body += '<input class="form-control"  name="ITEMDES" value="'+ITEMDESC+'" readonly style="height: 23px;background-color: #fff;" readonly>';
			body += '</td>';
			body += '<td>';
			body += '<input type="text" name="account_name" class="form-control accountSearch" value="'+data.ACCOUNTNAME+'" placeholder="Account">';
			body += '</td>';
			body += '<td class="bill-item">';
			body += '<input type="text" name="UOM" class="form-control uomSearch" value="'+data.UOM+'" placeholder="UOM">';
			body += '</td>';
			...
			body += '<div class=" col-lg-12 col-sm-3 col-12">';
			body += '<div class="input-group my-group" style="width:120px;">';
			body += '<input name="item_discount" type="text" class="form-control text-right" value="'+data.ITEMDISCOUNT+'" onchange="calculateAmount(this)" onkeypress="return isNumberKey(event,this,4)">';
			body += '<select name="item_discounttype" class="discountPicker form-control" onchange="calculateAmount(this)">';
			...
			body += '</td>';
//			body += '<td class="item-tax">';
//			body += '<input type="hidden" value="'+data.TAXTYPE+'" name="tax">';
//			body += '<input type="text" name="tax_type" class="form-control taxSearch" value="'+data.TAXTYPE+'" placeholder="Select a Tax" readonly>';
//			body += '</td>';
			body += '<td class="item-amount text-right grey-bg">';
	...
	$("input[name=SUPPLIER_TAMOUNT]").val($("input[name=total_amount]").val());
...
function checksino(){
	var scode = $("input[name=vendno]").val();
	var supinvno = $("input[name=JOB_NUM]").val();
	var pono = $("input[name=PONO]").val();
	var SUPPLIERINVNO = $("input[name=SUPPLIERINVNO]").val();
	if(supinvno == ""){
	
	}else{
		var urlStr = "../purchaseorder/SUPPLIER_INVOICE_CHECK_EDIT";
		$.ajax( {
			type : "POST",
			url : urlStr,
			async : true,
			data : {
				...
				success : function(data) {
					if (data.status == "99") {
						alert("Supplier Invoice Number Already Exists");
						$("input[name=JOB_NUM]").val(SUPPLIERINVNO);
					}
				}
			});
	}
...
var numberOfDecimal = $("#numberOfDecimal").val();
	...
	$.ajax( {
		...
		data : {
			...
			success : function(data) {
				...
				var purchaseuom = data.items.PURCHASEUOM;
	...
	$(obj).closest('tr').find("input[name=unitprice]").val(parseFloat(cost));
	//$(obj).closest('tr').find("input[name=UOM]").val(salesuom);
	$(obj).closest('tr').find("input[name=UOM]").typeahead('val', purchaseuom);
	$(obj).closest('tr').find('input[name = "account_name"]').val(account);
	var numberOfDecimal = $("input[name=numberOfDecimal]").val();
	var currencyID = $("input[name=CURRENCYID]").val();
	var custCode = $("input[name=CUST_CODE]").val();
...
/* Product GST Auto Suggestion */
	$('#'+rowid+' .gstSearch').typeahead({
		  hint: true,
		  minLength:0,  
		  searchOnFocus: true
		},
		{
		  ...
		  source: function (query, process,asyncProcess) {
			...
			data : {
				PLANT : plant,
				ACTION : "GET_PRD_GST_DATA",
				TYPE : "GST",
				QUERY : query
			},
			dataType : "json",
			success : function(data) {
				return asyncProcess(data.PRD_GST);
			}
				});
		},
		  limit: 9999,
		  ...
		  empty: [
			...
			suggestion: function(data) {
				return '<div ><p class="item-suggestion">'+data.PERCENTAGE+'</p></div>';
			}    
		  }
		}).on('typeahead:render',function(event,selection){
			var menuElement = $(this).parent().find(".tt-menu");
			...
			$(this).closest('tr').find('input[name="CGST"]').val(selection.CGST);
			$(this).closest('tr').find('input[name="SGST"]').val(selection.SGST);
			checkGst(this);
		}).on('typeahead:close',function(){
			var element = $(this).parent().parent().find('.select-icon').find('.glyphicon');
			element.toggleClass("glyphicon-menu-up",false);
...
//		}).on('typeahead:change',function(event,selection){
//			if($(this).val() == ""){
//				$(this).closest('tr').find('input[name="TAXGST"]').val("0.00");
//			}		
		});
		
		/* Product CESS Auto Suggestion */
	$('#'+rowid+' .cessSearch').typeahead({
		  hint: true,
		  minLength:0,  
		  searchOnFocus: true
		},
...
function addRow2(event){
	...
	body += '<td class="bill-item">';
	//body += '<input type="text" name="item" class="form-control itemSearch" placeholder="Type or click to select an item." onchange="calculateAmount(this)">';
	body += '<input type="text" name="item" class="form-control itemSearch" onchange="checkitems(this.value,this)" style="width:87%;display: inline-block;" placeholder="Type or click to select an item.">';
	body += '<button type="button" style="position: absolute;margin-left: -18px;z-index: 2;vertical-align: middle;font-size: 20px;opacity: 0.5;" onclick="changeitem(this)"><i class="glyphicon glyphicon-menu-down" style="font-size: 8px;"></i></button>';
	body += '<input class="form-control"  name="ITEMDES" readonly style="height: 23px;background-color: #fff;" readonly>';
	body += '</td>';
	body += '<td>';
