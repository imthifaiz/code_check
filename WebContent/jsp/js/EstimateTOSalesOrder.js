...
//			}
	...
	/*  Author: Azees  Create date: June 28,2021  Description: Typeahead Changes */
	//transport
	$('#transport').typeahead({
		  hint: true,
		  minLength:0,  
		        ...
		        success: function (data) {
		        	if(data.ERROR_CODE == 100){
						...
						$('.wysihtml5-sandbox').contents().find('.wysihtml5-editor').html(
									$('#template_body').val()
									.replace(/\{EST_ORDER_NO\}/, $('#ESTNO_FOR_DONO').val())
									.replace(/\{ORDER_NO\}/, $('#DONO').val())
...
function addRow(){
	...
	body += '</td>';
	body += '<td>';
	body += '<input type="text" name="addonshow" class="form-control text-right" value="'+szero+'" readonly>';
...
function setCustomerData(TAXTREATMENT,TRANSPORTNAME,TRANSPORTID,CUSTNO,CNAME,NAME,TELNO,EMAIL,ADD1,ADD2,ADD3,ADD4,REMARKS,COUNTRY,ZIP,CURRENCY,CURRENCYID,CURRENCYUSEQT,PAYMENTTYPE,CUST_ADDONCOST,CUST_ADDONCOSTTYPE){
		...
		if($(this).find('input[name=chkdDONO]').is(':checked')){
			...
			var costamt = issueQty;
			  if (costamt.indexOf('.') == -1) costamt += ".";
				...
				var issuedQty = $(this).find('input[name=QTYIS]').val();
...
function calculateAmount(obj){
	...
	var issuedQty = parseFloat($(obj).closest('tr').find('input[name=QTYIS]').val()).toFixed(3);
	var cost = parseFloat($(obj).closest('tr').find('input[name=unitprice]').val()).toFixed(numberOfDecimal);
	var itemDiscount = parseFloat($(obj).closest('tr').find('input[name=item_discount]').val()).toFixed(numberOfDecimal);
...
//	cost = parseFloat(parseFloat(cust_addoncost)+parseFloat(CUST_ADDONCOST_DET)).toFixed(numberOfDecimal);
	...
	if(qty >(orderedQty - issuedQty))
	{
		alert("Entered quantity exceeded above Ordered Qty");
		var balqty = parseFloat(orderedQty - issuedQty).toFixed(3);
		$(obj).closest('tr').find('input[name=QTY]').val(balqty);
		qty=balqty;
	}
...
function calculateTotal(){
	...
	if(ptaxstatus == "0"){
	 ...
	 var CURRENCYUSEQT = $('input[name ="CURRENCYUSEQT"]').val();//Author: Azees  Create date: July 20,2021  Description:  Total of Local Currency
