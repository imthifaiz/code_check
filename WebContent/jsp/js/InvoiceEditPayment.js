...
$(document).ready(function(){
	...
	$(".payment-table tbody").on('click','.payment-action',function(){
	 ...
	 $('#invoiceEditPaymentForm').submit(function(event) {
		var urlStr = "/track/InvoicePayment?CMD=EDIT";
        	...
        	if(pdcList.length > 0){
        		...
        		$.each(pdcList, function( key, data ) {
        			...
        			if(data.STATUS == "PROCESSED"){
            			...
            			body += '</td>';
            			body += '<td class="text-center">';
            			body += '<input class="form-control text-left datepicker" type="text" name="cheque-date" placeholder="Enter Cheque Date" value="'+data.CHEQUE_DATE+'">';
            			body += '</td>';
            			body += '<td class="text-center grey-bg" style="position:relative;">';
            			body += '<span class="glyphicon glyphicon-remove-circle payment-action" aria-hidden="true"></span>';
...
function addRow(){
	...
	body += '</td>';
	body += '<td class="text-center">';
	body += '<input class="form-control text-left datepicker" type="text" name="cheque-date" placeholder="Enter Cheque Date">';
	body += '</td>';
	body += '<td class="text-center grey-bg" style="position:relative;">';
	body += '<span class="glyphicon glyphicon-remove-circle payment-action" aria-hidden="true"></span>';
