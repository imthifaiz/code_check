...
//		setCURRENCYUSEQT(CURRENCYUSEQTCost);*/  /*This Line Command For Some Issue By Resvi */
	});
...
function checkorderno(orderno){	
		var urlStr = "../purchaseestimate/CheckOrderno";
		$.ajax( {
			type : "GET",
			url : urlStr,
			async : true,
			data : {
				...
				success : function(data) {
					if (data.status == "100") {
						...
						document.getElementById("POMULTIESTNO").focus();
						document.getElementById("POMULTIESTNO").value="";
						return false;	
					} 
					else 
						return true;
...
function validateOrderNo(){
	var orderno = $("input[name=POMULTIESTNO]").val();
	if(orderno != ""){	
		var urlStr = "../purchaseestimate/CheckOrderno";
		$.ajax( {
			type : "GET",
...
function loadVendData(obj,SName,TAXTREATMENT,sCURRENCY_ID,currencyID,Curramt){
	...
	//setCurrencyid(obj,sCURRENCY_ID,currencyID,Curramt);
...
function loadItemData(obj, productId, catalogPath, account, cost, purchaseuom,vendno ,vendname,TAXTREATMENT,CURRENCY,CURRENCYID,CURRENCYUSEQT){
	...
	//loadVendData(obj, vendno, TAXTREATMENT,CURRENCY,CURRENCYID,CURRENCYUSEQT);
...
function CheckPriceVal(obj, uom) {
    ...
    var numberOfDecimal = $("input[name=numberOfDecimal]").val();
    //var currencyID = $("input[name=BASECURRENCYID]").val();
    var currencyID = $(obj).closest('tr').find("input[name=CURRENCYID]").val();
...
function productCallbackpotable(productData){
	...
	//	body += '<input type="text" name="tax_type" class="form-control taxSearch" placeholder="Select a Tax" value="'+taxdisplay+'" readonly>';
	//	body += '</td>';
