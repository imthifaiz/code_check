...
//				<span data-toggle="modal" data-target="#shipaddr">Change</span> //old
	...
	$("input[name=QTY]").mouseenter(function(){		
		...
		content += "<p class='text-left'>Incomming Quantity: "+$(this).val()+"</p>";
		...
		$(this).closest('tr').find("input[name=QTY]").tooltip({title: content, html: true, placement: "top"});
	});
...
//function productCallbackpotable(data){
...
function checksino(){
	...
	if(supinvno == ""){
		...
		var urlStr = "../purchaseorder/SUPPLIER_INVOICE_CHECK";
		$.ajax( {
			type : "POST",
			url : urlStr,
			async : true,
...
//	var currencyfoc = document.cpoform.CURRENCY.focus();
	var msg = "";
	var isItemValid = true, isAccValid = true, isUnitPriceValid = true,isUnitPriceValid = true,issinoValid = true;
	...
	if(CURRENCYUSEQT == ""){
		...
		document.getElementById("CURRENCYUSEQT").focus();
		return false;
	}
	...
	/*if(!issinoValid){
		alert("Supplier Invoice Already Exists");
		$("input[name=JOB_NUM]").val("");
		$("input[name=JOB_NUM]").val().focus();
		return false;	
	}*/
...
function calculateAmount(obj){
	...
	if(parseFloat(itemDiscountval) > parseFloat("0")){
...
function calculateTotal(){
	...
	var odiscounttype= $("input[name='discount_type']").val();
	if(odiscounttype == "%"){
...
function setCURRENCYUSEQT(CURRENCYUSEQTCost){
	...
	$('tr').each(function () {
		...
		if(isNaN(parseFloat($(this).find('input[name=unitprice]').val())))
		cost=0.00;
		else {
		if((parseFloat($(this).find('input[name=unitprice]').val()))>0)
...
function addRow2(event){
	...
	body += '</td>';
	body += '<td class="bill-item">';
	//body += '<input type="text" name="item" class="form-control itemSearch" placeholder="Type or click to select an item." onclick="loaddropdata(this)" onchange="calculateAmount(this)">';
