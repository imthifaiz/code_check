...
//			}
			...
			if($(this).val() == ""){
				...
				addr += '<p>'+selection.COUNTRY+' '+selection.SHIPZIP+'</p>';
...
//			menuElement.after( '<div class="taxAddBtn footer"  data-toggle="modal" data-target="#gstModal"><a href="#"> + New Tax</a></div>');
	...
	$(document).on("focusout","input[name ='CURRENCYUSEQT']",function(){
		...
		if(!Number.isNaN(CURRENCYUSEQTCost))//  Author: Azees  Create date: July 20,2021  Description:  Exchange Rate Issue
...
function addRow(){
	...
	body += '<input type="text" name="item" style="width:87%" class="form-control onchange="checkitems(this.value,this)" itemSearch" placeholder="Type or click to select an item." onchange="calculateAmount(this)">';
	...
	body += '</td>';
	body += '<td>';
	body += '<input type="text" name="unitcost" class="form-control text-right" value="'+szero+'" readonly>';
	body += '</td>';
	body += '<td>';
...
function setCustomerData(TAXTREATMENT,TRANSPORTNAME,TRANSPORTID,CUSTNO,CNAME,NAME,TELNO,EMAIL,ADD1,ADD2,ADD3,ADD4,REMARKS,COUNTRY,ZIP,CURRENCY,CURRENCYID,CURRENCYUSEQT,PAYMENTTYPE,SHOWSALESBYPURCHASECOST,CUST_ADDONCOST,CUST_ADDONCOSTTYPE){
	...
	$(".so-table tbody tr").each(function() {
	//var catalogPath = $(this).find('td:nth-child(2)').find('img').attr();
	//var catalogPath = '';
	...
	setCurrencyid(CURRENCYID,CURRENCYUSEQT);
}
...
function setCurrencyid(CURRENCYID,CURRENCYUSEQT){
	...
	document.getElementById('lbltotal').innerHTML = "Total ("+CURRENCYID+")"; //  Author: Azees  Create date: July 20,2021  Description:  Total of Local Currency
...
function loadItemData(obj, productId, catalogPath, account, price, salesuom,cost){
	...
	var cust_addoncosttype = $("input[name=CUST_ADDONCOSTTYPE]").val();
	var addcustprice="";
	$(obj).closest('tr').find("td:nth-child(2)").find('img').attr("src",catalogPath);
...
function CheckPriceVal(obj, uom) {
	var cust_addoncost = parseFloat($("input[name=CUST_ADDONCOST]").val()).toFixed(3);
	var cust_addoncosttype = $("input[name=CUST_ADDONCOSTTYPE]").val();
	var productId = $(obj).closest('tr').find("td:nth-child(3)").find('input[name=item]').val();
...
//	var currencyfoc = document.form1.CURRENCY.focus();
	var currencyuseqt = $("input[name=CURRENCYUSEQT]").val();//  Author: Azees  Create date: July 20,2021  Description:  Exchange Rate Issue
	var msg = "";
	...
	$(".so-table tbody tr").each(function() {
		if($(this).find('input[name=chkdDONO]').is(':checked')){
			checkedFlag = true;
...
function showRemarksDetails(obj) {
	...
	var dono = $("input[name=DONO]").val();
	var olddono = $("input[name=COPYDONO]").val();
	   ...
	   var urlStr = "/track/CreateCustomerServlet?action=JADD&reurl=createInvoice";
		...
		success : function(data) {
			...
			if(document.form1.custModal.value == "cust"){
				...
				$("#CURRENCY").typeahead('val', data.customer[0].CURRENCY);
