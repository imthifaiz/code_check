		  ...
		  empty: [
		   ...
		   suggestion: function(data) {
			...
			if($(this).val() == ""){
				...
				setCustomerData(selection.TAXTREATMENT,selection.TRANSPORTNAME,selection.TRANSPORTID,selection.CUSTNO,selection.CNAME,selection.NAME,selection.TELNO,selection.EMAIL,selection.ADDR1,selection.ADDR2,selection.ADDR3,selection.ADDR4,selection.REMARKS,selection.COUNTRY,selection.ZIP,selection.CURRENCY,selection.CURRENCYID,selection.CURRENCYUSEQT,selection.PAYMENTTYPE);
				$('#shipbilladd').empty();
...
function validateOrderNo(){
	var orderno = $("input[name=ESTNO]").val();
	if(orderno != ""){	
		var urlStr = "../salesestimate/CheckOrderno";
		$.ajax( {
			type : "GET",
...
function renderTaxDetails(){
	...
	$.each(taxList, function( key, data ) {
		...
		if(data.value > 0 || taxtype=="ZERO RATE" && zerotype>0){
			...
			html+='<div class="total-amount taxAmount" align="right">'+parseFloat(parseFloat(data.value)).toFixed(numberOfDecimal)+'</div>';
...
function productCallback(productData){
	if(productData.STATUS="SUCCESS"){
		...
		body += '<input type="text" name="PRODUCTDELIVERYDATE" class="form-control datepicker" READONLY tabindex="-1">';
		body += '</td>';
		body += "<td class=\"item-cost text-right\"><input type=\"text\" name=\"unitprice\" class=\"form-control text-right\" value="+cost+" onchange=\"calculateAmount(this)\"onkeypress=\"return isNumberKey(event,this,4)\"></td>";
		body += '<td class=\"table-icon\"><a href="#" class="fa fa-info-circle" onclick="getPreviousPurchaseOrderDetails(this)"></a></td>';
		body += '<td class="item-discount text-right">';
		body += '<div class="row">';							
		body += '<div class=" col-lg-12 col-sm-3 col-12">';
