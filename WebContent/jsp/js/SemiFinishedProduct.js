    ...
    $('#ITEM').typeahead({
		...
		},
		{
		  ...
		  empty: [
			...
			suggestion: function(data) {
				return '<div onclick="loadItemData(this,\''+ data.PURCHASEUOM+'\',\''+ data.COST+'\',\''+ data.INCPRICE+'\',\''+ data.CPPI+'\',\''+ data.PRD_DEPT_ID+'\',\''+ data.ITEMDESC+'\')"><p class="item-suggestion">'+data.ITEM+'</p><p class="item-suggestion pull-right">Stock On Hand</p><br/><p class="item-suggestion">'+data.ITEMDESC+'</p><p class="item-suggestion pull-right">'+data.INVQTY+' </p></div>';
			}
		  }
		}).on('typeahead:open',function(event,selection){
			var element = $(this).parent().parent().find('.select-icon').find('.glyphicon');
...
function loadItemData(obj,PURCHASEUOM,cost,INCPRICE,CPPI,PRD_DEPT_ID,ITEMDESC)
{
	var numberOfDecimal = $("#numberOfDecimal").val();
	document.form.PARENTUOM.value = PURCHASEUOM;
	document.form.LOC_ID.value = PRD_DEPT_ID;
	document.form.INCPRICE.value = INCPRICE;
	document.form.CPPI.value = CPPI;
