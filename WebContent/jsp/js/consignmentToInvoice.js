...
$(document).ready(function(){
    ...
    const currencyuseqt = CURRENCYUSEQT.value;
    getCurrencyid(currency,currency,currencyuseqt);
		...
		},
		{
		  ...
		  empty: [
		    ...
		    suggestion: function(data) {
		    	return '<p onclick="getcustname(\''+data.TAXTREATMENT+'\',\''+data.TRANSPORTNAME+'\',\''+data.TRANSPORTID+'\',\''+data.CUSTNO+'\' ,\''+data.CURRENCY+'\',\''+data.CURRENCYID+'\',\''+data.CURRENCYUSEQT+'\')">' + data.CNAME + '</p>';
			}
		  }
...
//				addr += '<p>'+selection.ADDR4+'</p>';
				addr += '<p>'+selection.COUNTRY+' '+selection.STATE+' '+selection.ZIP+'</p>';
...
*/	
	...
	$("input[name=qty]").mouseenter(function(){
		...
		//content += "<p class='text-left'>Picked Quantity: "+$(this).closest('tr').find("input[name=piqty]").val()+"</p>";
		//content += "<p class='text-left'>Issued Quantity: "+$(this).closest('tr').find("input[name=isqty]").val()+"</p>";
		$(this).tooltip({title: content, html: true, placement: "top"}); 
	});
...
//	});
	...
	$("input[name=cost]").mouseenter(function(){
		...
		$.ajax({
			...
			url : "../consignment/addinvoiceRemarks",
			data : data,
			dataType : "json",
			success : function(data) {
	...
	$('#SHIPPINGCUSTOMER').typeahead({
		...
		},
		{
		  ...
		  empty: [
		    ...
		    suggestion: function(data) {
		    	return '<p onclick="document.form1.SHIPPINGID.value =\''+data.CUSTNO+'\'">' + data.CNAME + '</p>';
			}
		  }
		}).on('typeahead:render',function(event,selection){
			var menuElement = $(this).parent().find(".tt-menu");
			...
			top+="px";
			$('.shipCustomerAddBtn').remove();  
			$('.shipCustomer-section .tt-menu').after( '<div class="shipCustomerAddBtn footer"  data-toggle="modal" data-target="#customerModal" onclick="document.form1.custModal.value=\'shipcust\'"><a href="#"> + New Cutomer</a></div>');
			$(".shipCustomerAddBtn").width(menuElement.width());
			$(".shipCustomerAddBtn").css({ "top": top,"padding":"3px 20px" });			
			if($(this).parent().find(".tt-menu").css('display') != "block")
				menuElement.next().hide();
...
function showRemarksDetails(obj) {
	...
	var tono = $("input[name=JOB_NUM]").val();
	var item = $(obj).closest('tr').find("input[name=item]").val();

	if(item != ''){
		$.ajax({
			type : "GET",
			url : "../consignment/getInvoiceOrderRemarks",
			data : {
				ITEM: item,
				INVOICE: invoice,
				TONO: tono,
...
function getCurrencyid(CURRENCY,CURRENCYID,CURRENCYUSEQT){
	...
	//if (typeof document.form1.BASECURRENCYID != 'undefined'){
		var basecurrency = document.form1.curency.value; //Resvi
	...
	$(this).find('input[name=cost]').val(cost);
		...
		if(replacePreviousInvoiceCost=="3"){
			...
			//aodcost = parseFloat(aodcost).toFixed(numberOfDecimal);
				if(aodType == "BYPERCENTAGE"){
					aodcost = (parseFloat($(this).find('input[name=aod]').val()));
					aodcost = parseFloat(aodcost).toFixed(numberOfDecimal)+" "+"%";
					$(this).find('input[name=addonshow]').val(aodcost);
				}else{
					aodcost = parseFloat(aodcost).toFixed(numberOfDecimal)+" "+CURRENCYID;
					$(this).find('input[name=addonshow]').val(aodcost);
					var acost = (parseFloat($(this).find('input[name=aod]').val()));
					cost = parseFloat(Ucost)+parseFloat(AODCOST);
					cost = parseFloat(cost).toFixed(numberOfDecimal);
					$(this).find('input[name=cost]').val(cost);
				}
		}
