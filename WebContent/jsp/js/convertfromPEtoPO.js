...
//	getSupplierDetails();
	...
	$("#btnSalesOpenEmail").click(function(){
		...
		if(isValid){
			...
			$.ajax({
				type : "POST",
				url : "../purchaseorder/convertfromPEtoPO",
				data : data,
				dataType : "json",
				contentType: false,
				processData: false,
...
//		});
	...
	$("input[name=QTY]").focusout(function(){
		...
		if(plno != "0"){
			if(value < rcqry){
				alert("Quantity should not be less than estimate quantity, Purchase Estimate quantity = "+rcqry);
				return false;
			}
		}
	});
...
function checkqty(obj){
	...
	var estqry = $(obj).closest('tr').find("input[name=ESTQTY]").val();
	var plno = $(obj).closest('tr').find("input[name=POLNNO]").val();	
	var conqry= parseFloat(estqry) - parseFloat(rcqry);
	if(plno != "0"){
		if(parseFloat(value) > parseFloat(conqry)){			
			alert("Quantity should not be less than estimate quantity, Purchase Estimate Quantity = "+estqry);
			$(obj).val(addZeroes(parseFloat(conqry).toFixed(3)));
			return false;
		}
	}
	calculateAmount(obj);
	...
	$(".itemSearch").typeahead({
		...
		},
		{
		  ...
		  source: function (query, process,asyncProcess) {
			...
			async : true,
			data : {
				//ACTION : "GET_PRODUCT_LIST_FOR_SUGGESTION", //old
				//ACTION : "GET_PRODUCT_LIST_AUTO_SUGGESTION", //new in purchaseorder.js
				...
				success : function(data) {
					...
					if (data.status == "100") {
						...
						else
						{
							...
							$(obj).closest('tr').find("input[name=unitprice]").val(covcalAmount);
...
function setLineNo(){
	...
	$(".po-table tbody tr td:first-child").each(function() {
		...
		$(this).find('input[name=chkdPONO]').val(i);
		i++;
	});
}
...
function productCallbackpotable(productData){
	if(productData.STATUS="SUCCESS"){
		...
		body += '<button type="button" style="position: absolute;margin-left: -18px;z-index: 2;vertical-align: middle;font-size: 20px;opacity: 0.5;" onclick="changeitem(this)"><i class="glyphicon glyphicon-menu-down" style="font-size: 8px;"></i></button>';
		body += '<input class="form-control"  name="ITEMDES" value="'+productData.ITEM_DESC+'" readonly style="height: 23px;background-color: #fff;" readonly>';
		body += '</td>';
		body += '<td class="bill-item">';
...
if(data.quantity>0) {
			...
			body += '</td>';
			body += '<td class="bill-item">';
			body += '<input type="text" name="item" class="form-control" onchange="checkitems(this.value,this)" value="'+data.ITEM+'" style="width:87%" placeholder="Type or click to select an item." READONLY>';
			//body += '<button type="button" style="position: absolute;margin-left: -18px;z-index: 2;vertical-align: middle;font-size: 20px;opacity: 0.5;" onclick="changeitem(this)"><i class="glyphicon glyphicon-menu-down" style="font-size: 8px;"></i></button>';
			...
			body += '</td>';
			body += '<td class="bill-item">';
			body += '<input type="text" name="UOM" class="form-control uomSearch" onchange="checkprduom(this.value,this)" value="'+data.UOM+'" placeholder="UOM" READONLY>';
			body += '</td>';
			...
			body += '<td class="item-qty text-right"><input type="text" name="QTY" value="'+data.quantity+'" class="form-control text-right" data-rl="'+data.minstkqty+'" data-msq="'+data.maxstkqty+'" data-soh="'+data.stockonhand+'" data-eq="'+data.EstQty+'" data-aq="'+data.AvlbQty+'" onchange="checkqty(this)" onkeypress="return isNumberKey(event,this,4)"></td>';
			body += '<td>';
