	...
	$(".itemSearch").typeahead({
		...
		},
		{
		  ...
		  source: function (query, process,asyncProcess) {
			...
			data : {
				PLANT : plant,
				/*Submit : "GET_SALES_ORDER_DETAILS_FOR_INVOICE_USING_INVOICENO",
				INVOICENO : invno*/
...
function calculateTotal(){
	...
	$(".bill-table tbody tr td:last-child").each(function() {
	 ...
	 if(discounType == "%"){
		 discountValue = parseFloat(fsubtotal*(discount/100)).toFixed(3);
	 }else{
...
//		var curency = document.form.curency.value;
		...
		body += '</td>';
		body += '<td class="bill-item">';
		body += '<input type="text" name="item" style="width:90%" class="form-control itemSearch" value="'+productData.ITEM+'" placeholder="Type or click to select an item." onchange="calculateAmount(this)">';
		body += '<button type="button" style="position: absolute;margin-left: -18px;z-index: 2;vertical-align: middle;font-size: 20px;opacity: 0.5;" onclick="changeitem(this)"><i class="glyphicon glyphicon-menu-down" style="font-size: 8px;"></i></button>';
		...
		body += '<input type="text" name="tax" class="form-control taxSearch" placeholder="" value="'+taxdisplay+'" readonly>';
		body += '</td>';*/
		body += '<td class="item-amount text-right grey-bg" style="position:relative;">';
		body += '<span class="glyphicon glyphicon-remove-circle bill-action" aria-hidden="true"></span>';
...
function loadBillTable(orders){
	...
	$.each(orders, function( key, data ) {	
		...
		body += '<td class="item-discount text-right">';		
		body += "<input name=\"item_discount\" type=\"text\" class=\"form-control text-right\" value="+szero+" onchange=\"calculateAmount(this)\" readonly=\"readonly\" onkeypress=\"return isNumberKey(event,this,4)\">";
