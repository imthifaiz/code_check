...
//				addr += '<p>'+selection.SHIPZIP+'</p>';
	...
	$("#btnSalesOpenEmail").click(function(){
		...
		var isValid = validateConsignmentOrder();
		if(isValid){
			var data = new FormData($("#salesOrderForm")[0]); //$("#purchaseOrderForm").serialize();
			data.append("action_button", "btnSalesOpenEmail");
			$.ajax({
				type : "POST",
				url : "../consignment/edit",
				data : data,
...
function addRow(event){
	...
	body += '<input type="hidden" name="itemprice" value="">';
...
function onAdd(){
	...
	/*var fromlocation =  document.form.FROMWAREHOUSE.value; 
	var tolocation =  document.form.TOWAREHOUSE.value; */
	...
	/* if(fromlocation == "" || fromlocation == null) {
		 ...
		 document.form.FROMWAREHOUSE.focus();
		 return false; 
		 }
	 
	 if(tolocation == "" || tolocation == null) {
		 alert("Please Select ToLocation"); 
		 document.form.TOWAREHOUSE.focus();
		 return false; 
		 }
...
{
	 var urlStrAttach = "/track/consignment/downloadAttachmentById?attachid="+id;
...
{
	var urlStrAttach = "/track/consignment/removeAttachmentById?removeid="+id;	
	$.ajax( {
		type : "POST",
		url : urlStrAttach,
		success : function(data) {
