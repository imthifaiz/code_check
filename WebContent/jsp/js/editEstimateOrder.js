	...
	$("#CURRENCY").typeahead({
		...
		},
		{
		  ...
		  empty: [
			...
			suggestion: function(data) {
			return '<div onclick="setCurrencyid(\''+data.CURRENCYID+'\',\''+data.CURRENCYUSEQT+'\')">'+data.CURRENCY+'</div>';
			}
		  }
	}).on('typeahead:render',function(event,selection){
...
function addRow(event){
	...
	body += '<input type="text" name="item" class="form-control itemSearch" onchange="checkitems(this.value,this)" placeholder="Type or click to select an item." onchange="calculateAmount(this)">';
...
//	   END
//	 <!-- Author Name:Resviya ,Date:13/07/21 -->
...
{
	 var urlStrAttach = "/track/salesestimate/downloadAttachmentById?attachid="+id;
...
{
	var urlStrAttach = "/track/salesestimate/removeAttachmentById?removeid="+id;	
	$.ajax( {
		type : "POST",
		url : urlStrAttach,
		success : function(data) {
