$(document).ready(function(){
	 /*getLocalStorageValue('salesOrderSummary_FROMDATE', $('#FROM_DATE').val(), 'FROM_DATE');
	 getLocalStorageValue('salesOrderSummary_TODATE', '', 'TO_DATE');
	 getLocalStorageValue('salesOrderSummary_CUSTOMER', '', 'CUSTOMER');
	 getLocalStorageValue('salesOrderSummary_ORDERNO', '', 'ORDERNO');
	 getLocalStorageValue('salesOrderSummary_ORDERTYPE', '', 'ORDERTYPE');
	 getLocalStorageValue('salesOrderSummary_REFERENCE', '', 'Reference');
	 getLocalStorageValue('salesOrderSummary_STATUS', '', 'status');*/
 onGo();
 ...
 var COMP_INDUSTRY = document.form1.COMP_INDUSTRY.value;
...
//		    	return '<p onclick="getcustname(\''+data.TAXTREATMENT+'\',\''+data.CUSTNO+'\')">' + data.CNAME + '</p>';
	...
	/* To get the suggestion data for Approve Status */
	if(COMP_INDUSTRY=="Centralised Kitchen"){
	$("#approvestatus").typeahead({
	  hint: true,
	  minLength:0,  
	  searchOnFocus: true
	},
	{
	  name: 'postat',
	  display: 'value',  
	  source: substringMatcher(postat),
	  limit: 9999,
	  templates: {
	  empty: [
		  '<div style="padding:3px 20px">',
	...
	},
	{
	  name: 'postatapp',
	  display: 'value',  
	  source: substringMatcher(postatapp),
	  limit: 9999,
	  templates: {
	  empty: [
		  '<div style="padding:3px 20px">',
...
var postat =   [{
  ...
  {
	    "year": "Waiting For Manager Approval",
	    "value": "Waiting For Manager Approval",
	    "tokens": [
	  ...
	  },
	  {
		  ...
		  {
			    "year": "Waiting For HQ Manager Approval",
			    "value": "Waiting For HQ Manager Approval",
			    "tokens": [
			      "Waiting For HQ Manager Approval"
			    ]
			  }];
	
	var postatapp =   [{
    "year": "NEW_WAITING FOR OWNER APPROVAL",
    "value": "NEW_WAITING FOR OWNER APPROVAL",
    "tokens": [
      "NEW_WAITING FOR OWNER APPROVAL"
    ]
  },{
    "year": "EDIT_WAITING FOR OWNER APPROVAL",
    "value": "EDIT_WAITING FOR OWNER APPROVAL",
    "tokens": [
      "EDIT_WAITING FOR OWNER APPROVAL"
    ]
  },
	  {
		    "year": "OWNER APPROVED",
