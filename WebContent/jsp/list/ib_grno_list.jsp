...
<html>
<head>
<title>Purchase GRNO List</title>
...
<h3 class="panel-title">Purchase GRNO List</h3> 
</div>
</div>
...
mLogger.setLoggerConstans(loggerDetailsHasMap);
   ...
   // StrUtils strUtils = new StrUtils();
    ...
    try{
     ...
     ht.put("PLANT",plant);
     extCond=extCond  + " and ISNULL(recvdate,'')<>'' and plant='"+plant+"' and pono like '"+pono+"%' and grno like '" + grno + "%' and pono in(select pono from [" + plant + "_" + "POHDR" + "] where status like '"+status+"%' and status<>'N') and tran_type='IB'";
     extCond=extCond+" order by GRNO desc";
     ArrayList listQry = itemUtil.getIBRecvList(" distinct grno,cname",ht,extCond);
     for(int i =0; i<listQry.size(); i++) {
      sBGColor = ((i == 0) || (i % 2 == 0)) ? "#FFFFFF" : "#dddddd";
      Map m=(Map)listQry.get(i);
      pono     = (String)m.get("grno");
      String custName    = (String)m.get("cname");
    %>
    
    <TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			onClick="window.opener.form1.GRNO.value='<%=pono%>';
