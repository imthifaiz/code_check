...
<h3 class="panel-title">Sales GINO List</h3> 
</div>
</div>
 ...
 <thead style="background: #eaeafa"> 
   ...
   try{
     ...
     */
     ...
     if(pickissue.equalsIgnoreCase("Y")){
     		extCond=extCond  + " and plant='"+plant+"' and dono like '"+dono+"%' and dono in(select dono from [" + plant + "_" + "DOHDR" + "] where status like '"+STATUS+"%' ) ";
     }
     else{
    	 extCond=extCond  + " and plant='"+plant+"' and dono like '"+dono+"%' and status like '"+STATUS+"%' ";
     }
     
     extCond=extCond+" order by dono desc";
     
     if(pickissue.equalsIgnoreCase("Y")){
      		listQry = _DOUtil.getOBIssueList(" distinct invoiceno, dono,cname as custName",ht,extCond);
     }
     else{
      		listQry = _DOUtil.getDoHdrDetails("invoiceno, dono,custName",ht,extCond);
     }
     for(int i =0; i<listQry.size(); i++) {
        sBGColor = ((i == 0) || (i % 2 == 0)) ? "#FFFFFF" : "#dddddd";
        Map m=(Map)listQry.get(i);
        dono     = (String)m.get("invoiceno");
        String custName    = (String)m.get("custName");
        
    %>
    <TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			onClick="window.opener.form1.INVOICENO.value='<%=dono%>';
