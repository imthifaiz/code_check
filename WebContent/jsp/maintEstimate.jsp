...
<%
String title = "Edit Sales Estimate Order";
%>
<%@include file="sessionCheck.jsp" %>
<jsp:include page="header2.jsp" flush="true">
	<jsp:param name="title" value="<%=title %>" />
	<jsp:param name="mainmenu" value="<%=IConstants.SALES_ESTIMATE%>"/>
...
function popWin(vname){
    window.open('../jsp/vendSumm.jsp?VENDNAME='+vname+'&P=Y');
}
...
function onDelete(form)
{
	  if (form.ESTNO.value.length < 1)
	    ...
	    return false;
		     ...
		     var mes=confirm("Do you want to Delete the Sales Estimate order ?");
		      if(mes==true)
		      {
		      
		      document.form.action="/track/EstimateServlet?Submit=REMOVE_EST";
		      document.form.submit();
		      }
		      else
		      {  
...
function onUpdate(form)
{
     ...
     if(!IsNumeric(form.GST.value))
   {
     ...
     var mes=confirm("Do you want to update the Sales Estimate Order ?");
      if(mes==true)
      {
    	  add_attachments();
   /*    document.form.action="/track/EstimateServlet?Submit=Update";
...
String DIRTYPE ="";
    ...
    String vend = "",deldate="",jobNum = "",custName = "",custCode="",personIncharge = "",
    		contactNum = "",remark1 = "",remark2 = "",remarks="",address="",address2="",address3="",
    		collectionDate="",collectionTime="",contactname="",telno="",email="",add1="",currencyid="",
    		add2="",add3="",add4="",country="",zip="",gst="",ordertype="",
            deliverydate="",customerstatusid="",customerstatusdesc="",customertypeid="",
            customertypedesc="",shippingid="",shippingcustomer="",orderdiscount="",shippingcost="",
            incoterms="",statusID="",EMP_NAME = "",EXPIREDATE="",STATUS="",paymenttype="",dateformat="",sTAXTREATMENT="";
    ...
    String levelname = (String)session.getAttribute("USERLEVEL");
    boolean convertob = false;
     convertob = ub.isconvertob(levelname, sUserId, plant);
       ...
       if(m.size()>0){
    	   currencyid = (String) m.get("currencyid");
			...
			float gstVatVal ="".equals(gst) ? 0.0f :  Float.parseFloat(gst);
		 	float shipingCostVal="".equals(shippingcost) ? 0.0f :  Float.parseFloat(shippingcost);
			...
			supplierattachlist = estimateAttachDAO.getsalesAttachByESTNO(plant, estno);
        ...
        sSaveEnb  = "enabled";
        com.track.dao.EstHdrDAO _CustMstDAO=new   com.track.dao.EstHdrDAO();
        _CustMstDAO.setmLogger(mLogger);
        estno=_CustMstDAO.getNextOrder(plant);
        deldate=du.getDate();
        collectionTime=du.getTimeHHmm();
      
     
  }
     double dShippingCost = Double.parseDouble(shippingcost);
     shippingcost = StrUtils.addZeroes(dShippingCost, numberOfDecimal);
%>
...
<form class="form-horizontal" id="Estimateform" name="form" method="post"  action="/track/EstimateServlet?">
   
       <%
  String statusValue = su.fString(request.getParameter("statusValue"));
  if(statusValue.equals("100")) {
	  out.println("<div><b><font color='green'><center>Sales Estimate Order "+estno+" has sucessfully Deleted!</center></font></b></div>");
  }
  if(statusValue.equals("99")) {
	  out.println("<div><b><font color='red'><center>Sales Estimate Order "+estno+"  does not exist</center></font></b></div>");
  }
  if(statusValue.equals("98")) {
	  out.println("<div'><b><font color='red'><center>Sales Estimate Order "+estno+" is in use. Cannot Deleted!</center></font></b></div>");
  }
  if(statusValue.equals("97")) {
	  out.println("<div'><b><font color='red'><center>Unable to remove Sales Estimate Order "+estno+". Some Error Occured!</center></font></b></div>");
  }
  	  %>
...
<div class="form-group">
       <label class="control-label col-sm-2" for="create transfer Order"><a href="#" data-placement="left">
   		 	<i class="glyphicon glyphicon-star" style="font-size: 10px; top:-6px; color: #e50000"></i></a>&nbsp;Order Number:</label>
       <div class="col-sm-3">
      	    <div class="input-group">
    		<INPUT class="form-control" type="TEXT" size="20" MAXLENGTH="20" name="ESTNO" value="<%=estno%>"  onKeyPress ="submitToView()" />
   		 	<span class="input-group-addon"  onClick="javascript:popUpWin('../jsp/est_list.jsp?ESTNO='+form.ESTNO.value);">
   		 	<a href="#" data-toggle="tooltip" data-placement="top" title="Sales Estimate Details">
   		 	<i class="glyphicon glyphicon-log-in" style="font-size: 20px;"></i></a></span>
  		</div>
  		</div>
  		<div class="form-inline">
  		<div class=col-sm-1>
  		<button type="button" class="Submit btn btn-default" onClick="return validateEST(document.form)"><b>View</b></button>&nbsp;&nbsp;
  		</div>
  		</div>
  		<div class="form-inline">
  		<label class="control-label col-sm-2" for="Customer name"><a href="#" data-placement="left">
        ...
        <div class="col-sm-3">
    		...
    		<span class="input-group-addon"  onClick="javascript:popUpWin('../jsp/customer_list_order.jsp?CUST_NAME='+form.CUST_NAME.value);">
...
<div class="form-group">
        ...
        <INPUT name="PAYMENTTYPE" value="<%=paymenttype%>" type="TEXT" value="" class="form-control" style="width: 100%" MAXLENGTH=100>
 		<span class="input-group-addon"  onClick="javascript:popUpWin('../jsp/list/paymenttypelist_save.jsp?paymenttype='+form.PAYMENTTYPE.value+'&EDIT=EDIT');">
...
<div class="form-group">
        ...
        <div class="col-sm-3">
        <div class="input-group">          
       <INPUT  type = "TEXT" size="30"  MAXLENGTH="20" id="DELIVERYDATE" <%if(dateformat.equals("1")) {%>readonly class="form-control datepicker"<%} else {%> class=form-control <%}%> name="DELIVERYDATE" value="<%=deliverydate%>">
      	</div>
    	</div>
    	<div class="form-inline">
        ...
        <span class="input-group-addon"  onClick="javascript:popUpWin('../jsp/shippingdetails_list.jsp?SHIPPINGCUSTOMER='+form.SHIPPINGCUSTOMER.value);">
...
<div class="form-group">
        ...
        <span class="input-group-addon"  onClick="javascript:popUpWin('../jsp/OrderType_list.jsp?ORDERTYPE='+form.ORDERTYPE.value+'&FORMTYPE='+form.formtype.value);">
    ...
    <!-- 	<div class="form-inline">
        ...
        <span class="input-group-addon"  onClick="javascript:popUpWin('../jsp/incoterms_list.jsp?INCOTERMS='+form.INCOTERMS.value);">
...
<div class="form-group">
        ...
        <span class="input-group-addon"  onClick="javascript:popUpWin('../jsp/list/ordcurencyLst.jsp?DISPLAY='+form.DISPLAY.value);">
        ...
        <span class="input-group-addon"  onClick="javascript:popUpWin('../jsp/remarks_list.jsp?REMARKS='+form.REMARK1.value+'&TYPE=REMARKS1');">
...
<div class="form-group">
        ...
        <span class="input-group-addon"  onClick="javascript:popUpWin('../jsp/remarks_list.jsp?REMARKS='+form.REMARK3.value+'&TYPE=REMARKS2');">
   		 	<a href="#" data-toggle="tooltip" data-placement="top" title="Remarks Details">
   		 	<i class="glyphicon glyphicon-log-in" style="font-size: 20px;"></i></a></span>
        </div>
        </div>
...
<div class="form-group">
        ...
        <span class="input-group-addon"  onClick="javascript:popUpWin('../jsp/employee_list.jsp?EMP_NAME='+form.EMP_NAME.value+'&TYPE=ESTIMATE&FORM=form');">
...
<div class="form-group">
        ...
        <div class="col-sm-3">           	
  	    <SELECT class="form-control"  data-toggle="dropdown" data-placement="right" NAME="STATUS" style="width: 100%">
		 ...
		 <input type="hidden" name="CUSTOMERSTATUSDESC" value="">
  		 ...
  		 <TABLE style="font-size:15px" class="table"  align="center">
       ...
       <%=fieldDesc%>
        ...
        <!-- <button type="button" class="Submit btn btn-default" onClick="{backNavigation('orderManagement.jsp','EO');}"><b>Back</b></button>&nbsp;&nbsp; -->
      	...
      	<button type="button" class="Submit btn btn-default" onClick="submitFormPrint('Print Estimate Order With Price');"><b>Print Sales Estimate Order With Price</b></button>&nbsp;&nbsp;
        <button type="button" class="Submit btn btn-default" onClick="submitForm('Export To Excel');"><b>Export to Excel</b></button>&nbsp;&nbsp;
        <%if(convertob==true){ %>
        <button type="button" class="Submit btn btn-default" onClick="if(validateform(document.form)) {submitForm('Convert OB');}"><b>Convert To Sales</b></button>&nbsp;
        <!-- <button type="button" class="Submit btn btn-default" onClick="submitForm('Convert OB');"><b>Convert To Sales</b></button>&nbsp;&nbsp; -->
        <% }%>	        
        </div>
        </div>
                                 <div class="form-group">  
...
{
	 var urlStrAttach = "/track/EstimateAttachmentServlet?Submit=downloadAttachmentById&attachid="+id;
...
{
	var urlStrAttach = "/track/EstimateAttachmentServlet?Submit=removeAttachmentById&removeid="+id;	
	$.ajax( {
		type : "POST",
		url : urlStrAttach,
...
function add_attachments(){
    var formData = new FormData($('#Estimateform')[0]);
    var userId= form.ESTNO.value;
	if(userId){
    $.ajax({
        type: 'post',
        url: "/track/EstimateAttachmentServlet?Submit=add_attachments",
