...
function onDelete(form)
{
	  if (form.DONO.value.length < 1)
	    ...
	    return false;
		     ...
		     var mes=confirm("Do you want to Delete the Sales order ?");
		      if(mes==true)
		      {
...
function onUpdate(form)
{
    ...
    if(!IsNumeric(form.GST.value))
   {
    ...
    if (form.DELDATE.value.length > 1){
     ...
     var mes=confirm("Do you want to update the Sales Order ?");
      if(mes==true)
      {
      add_attachments();
...
function onClear(){
            ...
            form.STATUS_ID.value=""; //orderstatus
...
function CopyOBtoIBsubmitForm(actionvalue)
{
...
<%
    ...
    System.out.println(" session.setAttribute"+ session.getAttribute("RFLAG"));
    ...
    String contactname="",telno="",email="",add1="",currencyid="",add2="",add3="",add4="",country="",zip="",remarks="",gst="";
    String deliverydate="",customerstatusid="",customerstatusdesc="",customertypeid="",customertypedesc="";
    String shippingid="",shippingcustomer="",orderdiscount="",shippingcost="",incoterms="",paymenttype="";
    String sSaveEnb    = "disabled",statusID="",ordertype="",EMP_NAME="",dateformat="",sTAXTREATMENT="",sSALES_LOC="";
    String fieldDesc="<tr><td colspan=\"9\" align=\"center\">Please enter any search criteria</td></tr>" ;/* "<tr><td></td><td></td><td></td><td>Please enter any search criteria</td></tr>"; */
    if(DIRTYPE.length()<=0){
      	 DIRTYPE = "OUTBOUND";
      	 }
    ...
    /* String PRD_CLS_ID =  su.fString(request.getParameter("PRD_CLS_ID"));
    String PRD_TYPE_ID =  su.fString(request.getParameter("PRD_TYPE_ID"));
    String PRD_BRAND_ID =  su.fString(request.getParameter("PRD_BRAND_ID"));
   if(PRINT.length()<=0)
    {
    	PRINT = "LNNO";
    }*/
     if(action.equalsIgnoreCase("View")){
      ...
      fieldDesc=(String)request.getSession().getAttribute("RESULT");
       ...
       if(m.size()>0){
           ...
           supplierattachlist = salesAttachDAO.getsalesAttachByDONO(plant, dono);	
		   ...
		   float gstVatVal ="".equals(gst) ? 0.0f :  Float.parseFloat(gst);
			float orderdiscountVal ="".equals(gst) ? 0.0f :  Float.parseFloat(orderdiscount);
			/* float shippingcostVal ="".equals(gst) ? 0.0f :  Float.parseFloat(shippingcost); */
        ...
        dono=_CustMstDAO.getNextOrder(plant);
        deldate=du.getDate();
...
<form class="form-horizontal" id= "salesform" name="form" method="post" action="/track/deleveryorderservlet?" >
 ...
 <%
  ...
  if(statusValue.equals("100")) {
	  out.println("<div><b><font color='green'><center>Sales Order "+dono+" has sucessfully Deleted!</center></font></b></div>");
  }
  if(statusValue.equals("99")) {
	  out.println("<div><b><font color='red'><center>Sales Order "+dono+"  does not exist</center></font></b></div>");
  }
  if(statusValue.equals("98")) {
	  out.println("<div'><b><font color='red'><center>Sales Order "+dono+" is in use. Cannot Deleted!</center></font></b></div>");
  }
  if(statusValue.equals("97")) {
	  out.println("<div'><b><font color='red'><center>Unable to remove Sales Order "+dono+". Some Error Occured!</center></font></b></div>");
  }
  if(statusValue.equals("96")) {
  out.println("<div'><b><font color='red'><center>Payment is processed for Sales order "+dono+". Please void payment before Deleting the Order!</center></font></b></div>");
  }
		  %>
   	  
       
<div class="form-group">
       <label class="control-label col-sm-2" for="create outbound Order">
   		 	<i class="glyphicon glyphicon-star" style="font-size: 10px; top:-6px; color: #e50000"></i>&nbsp;Order Number:</label>
       <div class="col-sm-3">
       <div class="input-group">
       <INPUT type="TEXT" size="20" MAXLENGTH="20" name="DONO" value="<%=dono%>"  onKeyPress ="submitToView()" class="form-control" /> 
   		 	<span class="input-group-addon"  onClick="javascript:popUpWin('do_list_do.jsp?DONO='+form.DONO.value);">
  		...
  		<div class="col-sm-1 ">   
  		<button type="button" class="Submit btn btn-default" value="View"  onClick="return validateDO(document.form)"><b>View</b></button>
  		</div>
  		</div>
  		<div class="form-inline">
  		<label class="control-label col-sm-2" for="customer name">
...
<div class="form-group">
        ...
        <div class="col-sm-3">
        <INPUT type = "TEXT" style="width: 100%" class = "form-control" MAXLENGTH=20  name="PERSON_INCHARGE" value="<%=su.forHTMLTag(personIncharge)%>" readonly>
        </div>
 		</div>
 		</div>
...
<div class="form-group">
        ...
        <!-- </div>
 		...
 		<div class="form-inline">
    	<label class="control-label col-sm-3" for="Customer Type">Customer Type:</label>
        <div class="col-sm-3">
...
<label class="control-label col-sm-2" for="Sales Location">Sales Location:</label>
       ...
       <%=fieldDesc%>
        ...
        <button type="button" class="Submit btn btn-default" onClick="if(validateform(document.form)) {submitForm('Copy OB');}"><b>Convert To Sales</b></button>&nbsp;
        <button type="button" class="Submit btn btn-default" onClick="if(validateform(document.form)) {CopyOBtoIBsubmitForm('Convert IB');}"><b>Convert To Purchase</b></button>
        </div>
        </div>
                               <div class="form-group">  
                				<div class="form-inline">
...
function advPayment()
{
	...
	if(dono == ""){
		...
		window.location.href = "invoicePayment.jsp?type=ADVANCE&DONO="+dono+"&CUST_CODE="+form.CUST_CODE1.value+"&CUST_NAME="+form.CUST_NAME.value;		
	}
	}
function downloadFile(id,fileName)
{
	 var urlStrAttach = "/track/SalesAttachmentServlet?Submit=downloadAttachmentById&attachid="+id;
...
{
	var urlStrAttach = "/track/SalesAttachmentServlet?Submit=removeAttachmentById&removeid="+id;	
	$.ajax( {
		type : "POST",
		url : urlStrAttach,
