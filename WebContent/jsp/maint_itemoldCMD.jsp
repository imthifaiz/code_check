...
<%
	...
	String DYNAMIC_ALTERNATE_SIZE = "",ISPARENTCHILD="",prddepid="",nonstk="",nonstkid="",loc="",maxstkqty="",CUSTOMER_TYPE_ID="", DYNAMIC_CUSTOMERDISCOUNT_SIZE="",FLAGSUCCESS="",
		   SUPPLIER="",DYNAMIC_SUPPLIERDISCOUNT_SIZE="",IBDISCOUNT="",NETWEIGHT="",DESCRIPTION_SIZE="",PRD_SIZE="",ISNEWARRIVAL="",ISTOPSELLING="",
		   GROSSWEIGHT="",HSCODE="",COO="",VINNO="",MODEL="",RENTALPRICE="",SERVICEPRICE="",deptdisplay="",PURCHASEUOM="",SALESUOM="",RENTALUOM="",SERVICEUOM="",INVENTORYUOM="",ISCHILDCAL="",ISPOSDISCOUNT="",ISBASICUOM="",sSAVE_RED="",sSAVE_REDELETE="",iscompro="",cppi="",incprice="",incpriceunit="";
					        			  ...
					        			  String childplant = (String) map.get("CHILD_PLANT");
					 			        	...
					 			        	if(StrUtils.fString(request.getParameter("DYNAMIC_CUSTOMER_DISCOUNT_"+nameCount))==""){
					 			    			   ...
					 			    			   //IMTHI ADD child company to insert in customer type
								        			  ArrayList arrCust = new CustomerBeanDAO().getCustTypeDetails(StrUtils.fString(request.getParameter("CUSTOMER_TYPE_ID_"+nameCount)),plant);
								        			  if (arrCust.size() > 0) {
									        			...
									        			htcust.put(IDBConstants.CUSTOMERTYPEID,StrUtils.fString(request.getParameter("CUSTOMER_TYPE_ID_"+nameCount)));
					        			  ...
					        			  String childplant = (String) map.get("CHILD_PLANT");
					 		        	   	for(int nameCount = 0; nameCount<=DYNAMIC_SUPPLIERDISCOUNT_SIZE_INT;nameCount++){
					 				        	if(StrUtils.fString(request.getParameter("DYNAMIC_SUPPLIER_DISCOUNT_"+nameCount))==""){
					 				    			 ...
					 				    			 //SUPPLIER START 
					 			        				ArrayList arrCust = new CustUtil().getVendorDetails(StrUtils.fString(request.getParameter("SUPPLIER_"+nameCount)),plant);
					 			        				if (arrCust.size() > 0) {
					 			        				String sCustCode = (String) arrCust.get(0);
					 			        				String sCustName = (String) arrCust.get(1);
					 			        				if (!new CustUtil().isExistVendor(StrUtils.fString(request.getParameter("SUPPLIER_"+nameCount)), childplant) && !new CustUtil().isExistVendorName(sCustName, childplant)) // if the Customer exists already 
					 			        				{
					 			        					String sAddr1 = (String) arrCust.get(2);
					 			        					String sAddr2 = (String) arrCust.get(3);
					 			        					String sAddr3 = (String) arrCust.get(4);
...
//					 		 	        			          htsup.put(IDBConstants.BANKNAME,sBANKNAME);
					 				    			...
					 				    			/*  //IMTHI ADD child company to insert in supplier type
									        			  ArrayList arrCust = new CustomerBeanDAO().getSupplierTypeDetails(StrUtils.fString(request.getParameter("SUPPLIER_"+nameCount)),plant);
									        			  	String sSupTypeId = (String) arrCust.get(1);
									        			  	String sSupTypeDesc = (String) arrCust.get(2);
									        			  	String sIsactive = (String) arrCust.get(7);
										        			Hashtable htSup = new Hashtable();
										        			htSup.put(IDBConstants.PLANT,childplant);
										        			htSup.put(IDBConstants.SUPPLIERTYPEID,StrUtils.fString(request.getParameter("SUPPLIER_"+nameCount)));
						 		        	   	...
						 		        	   	for(int nameCount = 0; nameCount<=DYNAMIC_SUPPLIERDISCOUNT_SIZE_INT;nameCount++){
						 				        	if(StrUtils.fString(request.getParameter("DYNAMIC_SUPPLIER_DISCOUNT_"+nameCount))==""){
						 				    			   ...
						 				    			   //SUPPLIER START 
						 			        				...
						 			        				String sCustCode = (String) arrCust.get(0);
						 			        				String sCustName = (String) arrCust.get(1);
						 			        				if (!new CustUtil().isExistVendor(StrUtils.fString(request.getParameter("SUPPLIER_"+nameCount)), parentplant) && !new CustUtil().isExistVendorName(sCustName, parentplant)) // if the Customer exists already 
						 			        				{
						 			        					String sAddr1 = (String) arrCust.get(2);
						 			        					String sAddr2 = (String) arrCust.get(3);
						 			        					String sAddr3 = (String) arrCust.get(4);
...
<div class="row">
    ...
    <div class="form-group">
					<label class="control-label col-form-label col-sm-3" for="supplier name">Supplier Name/ID:</label>
        			<div class="col-sm-4">
        					<input type="hidden" name="vendno" value="<%=vendno%>" >
        					<input type = "hidden" name="custModal">
    		 				<INPUT class=" form-control" id="vendname" value="<%=vendname%>" name="vendname" type="TEXT" size="30" MAXLENGTH=100 placeholder="Select Supplier Name/ID">
    ...
    <div class="form-group">
      <label class="control-label col-form-label col-sm-3" for="Alternate Product" >Alternate Product</label>
      <div class="col-sm-4">                
					...
					<div class="col-sm-3"></div>
						<div class="col-sm-6">
							<a class="add-line"
								...
								onclick="addRow('alternateItemDynamicTable','');return false;">+ Add New Alternate Product</a>
						</div>
					</div>
				</div>
...
function addRowDescCost(tableID,desc,addDesc) {
	...
	itemCellText = itemCellText+ " id=\"DESCRIPTION"+rowCount+"\" type = \"TEXT\" size=\"20\" placeholder=\"Max 1000 Characters\"  value=\""+addDesc+"\" MAXLENGTH=\"1000\"></div></div>&nbsp;";
	itemCell.innerHTML = itemCellText;
	}
	else
		{
