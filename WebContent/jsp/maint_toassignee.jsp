...
<%@ page import="com.track.dao.ToHdrDAO"%>
...
<%
String title = "Edit Consignment Order Customer";
%>
...
function onNew(){
   ...
   document.form.action  = "maint_toassignee.jsp?action=Clear";
...
function onUpdate(){
   ...
   if(chkmsg){
   document.form.action  = "maint_toassignee.jsp?action=UPDATE";
   document.form.submit();}
   else
   {
	   return false;
...
function onDelete(){
	   var CUST_CODE   = document.form.CUST_CODE.value;
	   if(CUST_CODE == "" || CUST_CODE == null) {alert("Please Enter Consignment Order Customer ID");  return false; }
	var chkmsg=confirm("Are you sure you would like to delete? ");
	    if(chkmsg){
	   document.form.action  = "maint_toassignee.jsp?action=DELETE";
	   document.form.submit();}
	   else
	   { return false;
	   }
...
function onView(){
   ...
   document.form.action  = "maint_toassignee.jsp?action=VIEW";
   document.form.submit();
}
...
//      VendMstDAO _CustMstDAO=new VendMstDAO(plant);
//     sCustCode=_CustMstDAO.getNextVendCode(plant);
   ...
   if(custUtil.isExistToAssignee(sCustCode,plant))
    {
          ...
          htCondition.put(IConstants.PLANT,plant);
          	...
          	httohdr.put("CustCode", sCustCode);
          	httohdr.put(IConstants.PLANT, plant);
			
			StringBuffer updatetohdr = new StringBuffer("set ");
			updatetohdr.append(IConstants.IN_CUST_CODE + " = '" + sCustCode + "'");
			updatetohdr.append("," +IConstants.IN_CUST_NAME + " = '" + sCustName + "'");
			updatetohdr.append("," + IConstants.IN_PERSON_IN_CHARGE + " = '"+ sContactName + "'");
			updatetohdr.append(",Address = '"+ sAddr1 + "'");
			updatetohdr.append(",Address2 = '"+ sAddr2 + "'");
			updatetohdr.append(",Address3 = '"+ sAddr3 + "'");
          
			Hashtable httodet = new Hashtable();
			httodet.put(IConstants.PLANT, plant); 
			
			String todetquery = "select Count(*) from "+plant+"_TODET where TONO in(select TONO from "+plant+"_TOHDR where plant='"+plant+"' and  CustCode='"+sCustCode+"' )";
			String todetupdate = "set userfld3='"+sCustName+"' ";

			String topickquery = "select Count(*) from "+plant+"_TO_PICK where TONO in(select TONO from "+plant+"_TOHDR where  plant='"+plant+"' and CustCode='"+sCustCode+"' )";
			String topickupdate = "set cname='"+sCustName+"' ";

			String recvdetquery = "select Count(*) from "+plant+"_RECVDET where PONO in(select TONO from "+plant+"_TOHDR where  plant='"+plant+"' and CustCode='"+sCustCode+"' )";
         ...
         htm.put("PLANT",plant);
         htm.put("DIRTYPE",TransactionConstants.UPD_TO_ASSIGNEE);
         htm.put("RECID",""); 
         ...
         boolean flag = false;
			if(custUpdated){
				 flag = ToHdrDAO.isExisit(httohdr);
				 if(flag){
				 	flag = ToHdrDAO.update(updatetohdr.toString(), httohdr, " ");
				 }
				 if(flag){
					 flag = ToDetDAO.isExisit(todetquery);
					 if(flag)
					 {
						 flag = ToDetDAO.update(todetupdate, httodet, " and TONO in(select TONO from "+plant+"_TOHDR where plant='"+plant+"' and CustCode='"+sCustCode+"' )");
					 }
				 }
				 if(flag){
					 flag = ToDetDAO.isExisit(topickquery);
					 if(flag)
					 {
						 flag = ToDetDAO.updateToPickTable(topickupdate, httodet, " and TONO in(select TONO from "+plant+"_TOHDR where plant='"+plant+"' and CustCode='"+sCustCode+"' )");
					 }
				 }
				 if(flag){
					 ...
					 if(flag)
					 {
						 flag = RecvDetDAO.update(recvdetupdate, httodet, " and PONO in(select TONO from "+plant+"_TOHDR where plant='"+plant+"' and CustCode='"+sCustCode+"' )", plant);
					 }
				 }
	...
	{	
			res = "<font class = " + IDBConstants.FAILED_COLOR
					+ " >Consignment Order Customer Exists In Transactions</font>";
		
	}else{
		
    if(custUtil.isExistToAssignee(strUtils.InsertQuotes(sCustCode),plant))
    {
          boolean custDeleted = custUtil.deleteToAssignee(strUtils.InsertQuotes(sCustCode),plant);
          
          MovHisDAO mdao = new MovHisDAO(plant);
          mdao.setmLogger(mLogger);
        	 Hashtable htm = new Hashtable();
          htm.put("PLANT",plant);
          htm.put("DIRTYPE",TransactionConstants.DEL_TO_ASSIGNEE);
          htm.put("RECID",""); 
          htm.put("ITEM",sCustCode);
          htm.put("UPBY",username);   
          htm.put("CRBY",username);
...
try{
  ...
  sState    = (String)arrCust.get(17);
   ...
   {
       res="no details found for Consignment Order Customer id : "+  sCustCode;
   }
 ...
 <div class="box-body">
  ...
  <form class="form-horizontal" name="form" method="post">
    <div class="form-group">
      <label class="control-label col-sm-4" for="Maintain Consignment Assignee ID">
    ...
    <div class="form-group">
      <label class="control-label col-sm-4" for="Consignment Assignee Name">
