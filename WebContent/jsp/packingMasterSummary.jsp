...
function popUpWin(URL) {
 		...
 		subWin = window.open(URL, 'Packing', 'toolbar=0,scrollbars=yes,location=0,statusbar=0,menubar=0,dependant=1,resizable=1,width=800,height=400,left = 200,top = 184');
...
String res= "",action="",packing="",fieldDesc="",allChecked="";
...
{
displaySummaryDelete = ub.isCheckValAcc("removepackingMaster", plant,USERID);
displaySummaryNew = ub.isCheckValAcc("newpackingMaster", plant,USERID);

}
if(systatus.equalsIgnoreCase("INVENTORY"))
{
	displaySummaryDelete = ub.isCheckValinv("removepackingMaster", plant,USERID);
displaySummaryNew = ub.isCheckValinv("newpackingMaster", plant,USERID);
}
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Packing Summary</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->     
            <div class="box-header menu-drop">
              ...
              <button type="button" class="btn btn-default" onclick="window.location.href='../packinglist/new'" style="background-color: rgb(214, 72, 48);color: rgb(255, 255, 255);" >+ New</button>&nbsp;
             <%} %>
              </div>
		</div>
...
<form class="form-horizontal" name="form" method="post" action="/track/PackingServlet?">
...
<button type="button" class="Submit btn btn-default" value="Remove From Packing" onClick="onDelete()">Remove From Packing</button>&nbsp;&nbsp;
	<%} %>
	</div>
	 </div>
 ...
 function checkAll(isChk)
{
  ...
  var urlStr = "/track/PackingServlet";
  
  // Call the method of JQuery Ajax provided
  $.ajax({type: "POST",url: urlStr, data: {PLANT:"<%=plant%>",action: "VIEW_PACKING_DETAILS",PLANT:"<%=plant%>"},dataType: "json", success: callback });
...
function onDelete()
{
	 ...
	 for (var i = 0; i < len ; i++)
   {
	 ...
	 var chkmsg = confirm("Are you sure you would like to delete?");
		if (chkmsg) {
	  	  document.form.action="/track/PackingServlet?action=DELETE_PACKING";
...
function callback(data){
	...
	if(!errorBoo){
        ...
        $.each(data.packingmaster, function(i,item){
                   
        	outPutdata = outPutdata+item.PACKINGMASTERDATA;
                    	ii++;
