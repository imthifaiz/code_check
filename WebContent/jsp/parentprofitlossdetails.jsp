...
<%
	String title = "Profit Loss Transactions";
...
<div class="container-fluid m-t-20">
	...
	<div class="box">
		...
		<div class="box-header menu-drop">
			 <h1 style="font-size: 18px;cursor:pointer;" class="box-title pull-right" onclick="window.location.href='../businessoverview/parentprofitloss'">
			...
			<div class="row">
				...
				<div class="col-sm-4" style="text-align:center">
					...
					<div style="font-size:18px"><%=ledgerDetails.get(0).getACCOUNT()%></div>
    ...
    <tbody>
     <tr>
    	 ...
    	 <td></td>
    	 <% if(openingBalance>=0){ %>
        <td><%=curency%> <%=String.format("%."+numberOfDecimal+"f", openingBalance)%></td>
        <%}else{%>
        <td></td>
        <%} %>
        <% if(openingBalance<0){ %>
         <td><%=curency%> <%=String.format("%."+numberOfDecimal+"f", Math.abs(openingBalance)) %></td>
        <%}else{%>
        <td></td>
        <%} %>
    	 <td></td>
      ...
      <tr>
        ...
        <td><%=String.format("%."+numberOfDecimal+"f", ledgerDet.getDEBIT()) %></td>
        <td><%=String.format("%."+numberOfDecimal+"f", ledgerDet.getCREDIT()) %></td>
...
{     
	...
	var plant = "<%=plant%>";
	window.location = "/track/TrialBalanceServlet?action=getCTrialBalanceTransactionDetails&fromDate="+fromdate+"&toDate="+todate+"&account="+accountSelected+"&accname="+accname+"&plant="+plant;
}
...
function generatePdf(dataUrl){	
	...
	doc.setFontSize(12);
	doc.text('<%=ledgerDetails.get(0).getACCOUNT()%>',89,50);
	doc.text('From ',83,57);
