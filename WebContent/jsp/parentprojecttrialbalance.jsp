...
<%
	String title = "Consolidated Project Trial Balance";
	String title2 = "Project Trial Balance";
...
<div class="container-fluid m-t-20">
	...
	<!-- Thanzith Modified on 28.02.2022 -->
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Consolidated Project Trial Balance</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 28.02.2022 --> 
...
function postDatePickerInit(){
	if (period == 'CUSTOM'){
		var parentprojecttrialbalance_fromdate = getLocalStorageValue('parentprojecttrialbalance_fromdate', '', 'fromdate');
		$('#fromdate').datepicker("setDate", 
		        moment(parentprojecttrialbalance_fromdate, 'DD/MM/YYYY').toDate());
		var parentprojecttrialbalance_todate = getLocalStorageValue('parentprojecttrialbalance_todate', '', 'todate');
		$('#todate').datepicker("setDate", 
				moment(parentprojecttrialbalance_todate, 'DD/MM/YYYY').toDate());
	}		
	$('#srbtn').click();
	
}
	$(document).ready(function() {
		var periodFromSession = getLocalStorageValue('parentprojecttrialbalance_reportperiod', 'THISYEARTODATE', 'reportperiod');
		...
		<%for (ParentChildCmpDet plist : plantlist) {
			String plant=plist.getCHILD_PLANT(); %>
			 $('#<%=plant %>drop').change(function() {
			        ...
			        updateSelectedCompaniesInLocalStorage('parentprojecttrialbalance_companieslist'); 
			    });
		<%}%>
		var selectedCompanies = getLocalStorageValue('parentprojecttrialbalance_companieslist', '', '');
		$(".companyname").each(function( index  ) {
			console.log($( this ).val() + ' : ' + selectedCompanies.search($( this ).val()));
	...
	{
/* 		storeInLocalStorage('parentprojecttrialbalance_reportperiod', $('#reportperiod').val());
		storeInLocalStorage('parentprojecttrialbalance_project', $('#project').val());
		...
		storeInLocalStorage('parentprojecttrialbalance_companieslist', selectedCompanies);
		if ($("#reportperiod").val().toLowerCase() == 'custom'){
			storeInLocalStorage('parentprojecttrialbalance_fromdate', $('#fromdate').val());
		}
		if ($("#reportperiod").val().toLowerCase() == 'custom'){
			storeInLocalStorage('parentprojecttrialbalance_todate', $('#todate').val());
		} */
		//
		storeInLocalStorage('parentprojecttrialbalance_reportperiod', $('#reportperiod').val());
		storeInLocalStorage('parentprojecttrialbalance_project', $('#project').val());
	...
	{
		//alert("Called");
		window.location.href="../projecttrial/parentprojtrialbaldetail?account="+account+"&accname="+accountName+"&fromDate="+fromDateFormatted+"&toDate="+toDateFormatted+"&plant=<%=plant%>";
		}
	...
	function downTBExcel()
	{
		...
		window.location = "/track/ParentTrialBalanceServlet?action=getProjectTrialBalanceExcel&fromDate="+fromDateFormatted+"&toDate="+toDateFormatted+"&projectId="+finalProjId+"&plist="+plist;
	}
