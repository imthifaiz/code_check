...
<!-- END -->
<%
String title = "Product Category Summary";
%>
...
// } 
 function onView(){
	   document.form1.action  = "../jsp/prdclsSummary.jsp?action=VIEW";
	   document.form1.submit();
	}
</script>
...
/* RSN     = strUtils.fString(request.getParameter("PRD_CLS_ID")); */
...
{
displaySummaryLink = ub.isCheckValAcc("summarylnkprdCls", PLANT,USERID);
displaySummaryEdit = ub.isCheckValAcc("editprdCls", PLANT,USERID);
displaySummaryNew = ub.isCheckValAcc("newprdCls", PLANT,USERID);


}
if(systatus.equalsIgnoreCase("INVENTORY"))
{
displaySummaryLink = ub.isCheckValinv("summarylnkprdCls", PLANT,USERID);
displaySummaryEdit = ub.isCheckValinv("editprdCls", PLANT,USERID);
displaySummaryNew = ub.isCheckValinv("newprdCls", PLANT,USERID);
...
if(PGaction.equalsIgnoreCase("View")){
 try{
      ...
      if(strUtils.fString(RSN).length() > 0)  RSN=RSN;
    	  ...
    	  RSN= strUtils.fString(request.getParameter("PRD_CLS_ID1"));
      if(fieldDesc.equalsIgnoreCase("Product Class Deleted Successfully")){
    	  locQryList= prdclsutil.getPrdTypeList(RSN="",PLANT,"");
      }
      else{
              locQryList= prdclsutil.getPrdTypeList(RSN,PLANT,"");}
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Product Category Summary</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->     
            <div class="box-header menu-drop">
              ...
              <button type="button" class="btn btn-default" onclick="window.location.href='../productclass/new'" style="background-color: rgb(214, 72, 48);color: rgb(255, 255, 255);" >+ New</button>&nbsp;
             <%}%>
              </div>
		</div>
...
<form class="form-horizontal" name="form1" method="post" action="prdclsSummary.jsp">
		...
		<div class="form-group">
			<label class="control-label col-sm-2" for="Product Class ID">Product Category ID</label>
			<div class="col-sm-4" >
			<div class="input-group">
				<INPUT name="PRD_CLS_ID" id="PRD_CLS_ID"  type = "TEXT" value="<%=RSN%>" size="50" MAXLENGTH=100  class="form-control"> 
					<span class="select-icon" onclick="$(this).parent().find('input[name=\'PRD_CLS_ID\']').focus()"><i class="glyphicon glyphicon-menu-down"></i></span>
				<!-- <span class="btn-danger input-group-addon"  onClick="javascript:popUpWin('../jsp/PrdClsList.jsp?ITEM_ID='+form1.PRD_CLS_ID.value);">
   ...
   <thead> 
       ...
       %>  
          <TR>
            ...
            <TD align="center"> <a href ="prdClsDetail.jsp?action=View&PRD_CLS_ID=<%=(String)lineArr.get("prd_cls_id")%>&PRD_CLS_DESC=<%=strUtils.replaceCharacters2Send((String)lineArr.get("prd_cls_desc"))%>&ISACTIVE=<%=(String)lineArr.get("ISACTIVE")%>"%><%=(String)lineArr.get("prd_cls_id")%></a></TD>
            <% } else { %>    
            <TD align="center"> <%=(String)lineArr.get("prd_cls_id")%></TD>
            <% }%>
    	    <TD align="center">&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; <%=strUtils.fString((String)lineArr.get("prd_cls_desc"))%></TD>
            <TD align="center">&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; <%=strUtils.fString((String)lineArr.get("ISACTIVE"))%></TD>
            <% if (displaySummaryEdit) { %>
            <TD align="center"> <a href ="maint_prdcls.jsp?action=View&PRD_CLS_ID=<%=(String)lineArr.get("prd_cls_id")%>&PRD_CLS_DESC=<%=strUtils.replaceCharacters2Send((String)lineArr.get("prd_cls_desc"))%>&ACTIVE=<%=(String)lineArr.get("ISACTIVE")%>"><i class="fa fa-pencil-square-o"></i></a></TD>
            <% } else { %> 
...
<script LANGUAGE="JavaScript">
		...
		var tabletype;
		var PRODUCTCLASSID, groupRowColSpan = 6;
		...
		function onGo(){
			PRODUCTCLASSID=document.form1.PRD_CLS_ID.value;
			    	...
			    	tabletype = $('#tabletype').DataTable({
						...
						"ajax": {
					        ...
					        "dataSrc": function(data){
						        	...
						        	if(typeof data.CUSTOMERTYPELIST[0].prd_cls_id === 'undefined'){
						        		return [];
						        	}else {				        		
						        			for(var dataIndex = 0; dataIndex < data.CUSTOMERTYPELIST.length; dataIndex ++){
						        				var lineno = data.CUSTOMERTYPELIST[dataIndex].prd_cls_id;
						        				var catlog = data.CUSTOMERTYPELIST[dataIndex].CATALOG;
						        				var sno=dataIndex+1;
						        				data.CUSTOMERTYPELIST[dataIndex]['SNO'] = sno;
												<%if(displaySummaryEdit){ %>
						        				data.CUSTOMERTYPELIST[dataIndex]['EDIT'] = '<a href="../productclass/edit?action=View&PRD_CLS_ID='+lineno+'&PRD_CLS_DESC=' +data.CUSTOMERTYPELIST[dataIndex].prd_cls_desc+'&ACTIVE=' +data.CUSTOMERTYPELIST[dataIndex].ISACTIVE+'"><i class="fa fa-pencil-square-o"></i></a>';
												<%}else{ %>
												...
												<%} %>
												<%if(displaySummaryLink){ %>
						        				data.CUSTOMERTYPELIST[dataIndex]['prd_cls_id'] = '<a href="../productclass/detail?action=View&PRD_CLS_ID='+lineno+'&PRD_CLS_DESC=' +data.CUSTOMERTYPELIST[dataIndex].prd_cls_desc+'&ISACTIVE=' +data.CUSTOMERTYPELIST[dataIndex].ISACTIVE+'">'+data.CUSTOMERTYPELIST[dataIndex].prd_cls_id+'</a>';
												<%} %>
