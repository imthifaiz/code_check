...
function onRePrint(){
    ...
    document.form1.action="/track/DynamicFileServlet?action=printDOWITHBATCH&Submit=Print Outbound Order";
...
function onRePrintWithContainer(){
    ...
    document.form1.action="/track/DynamicFileServlet?action=printDOWITHBATCHANDCONTAINER&Submit=Print Outbound Order";
    document.form1.submit();
}
function onRePrintWithOutBatch(){
    ...
    document.form1.action="/track/DynamicFileServlet?action=printDOWITHOUTBATCH&Submit=Print Outbound Order";
    document.form1.submit();
}
var postatus =   [{
...
String CUSTOMERID="",statusID="",EMP_NAME="",CUSTOMERTYPE="",GINO="",issuests="";;
...
{
	displaySummaryLink = ub.isCheckValAcc("summarylnkdonoprice", plant,LOGIN_USER);
	displaySummaryPrintBatch = ub.isCheckValAcc("printdobatch", plant,LOGIN_USER);
	displaySummaryPrintNoBatch = ub.isCheckValAcc("printdonobatch", plant,LOGIN_USER);
	displaySummaryPrintContainer = ub.isCheckValAcc("printdocontainer", plant,LOGIN_USER);
...
{
	displaySummaryLink = ub.isCheckValinv("summarylnkdonoprice", plant,LOGIN_USER);
	displaySummaryPrintBatch = ub.isCheckValinv("printdobatch", plant,LOGIN_USER);
	displaySummaryPrintNoBatch = ub.isCheckValinv("printdonobatch", plant,LOGIN_USER);
	displaySummaryPrintContainer = ub.isCheckValinv("printdocontainer", plant,LOGIN_USER);
	displaySummaryLink=true;
	displaySummaryPrintBatch=true;
	displaySummaryPrintNoBatch=true;
	displaySummaryPrintContainer=true;
...
<div class="row" style="padding:3px">
 ...
 <input type="text" name="PICKSTATUS" id="PICKSTATUS" class="ac-selected form-control"  placeholder="STATUS" value="<%=StrUtils.forHTMLTag(PICKSTATUS)%>">
...
<!--   		<span class="select-search btn-danger input-group-addon " onClick="javascript:popUpWin('employee_list.jsp?EMP_NAME='+form1.EMP_NAME.value+'&TYPE=ESTIMATE&FORM=form1');"><span class="glyphicon glyphicon-search" aria-hidden="true" style="left: -4px;"></span></span> -->
  		...
  		<div class="col-sm-2">
  			...
  			<div class="col-sm-4 ac-box">
  		<div class=""> 
  		<input type="text" name="INVOICENO" id="INVOICENO" class="ac-selected form-control status" placeholder="GINO" value="<%=StrUtils.forHTMLTag(GINO)%>">
...
<!--   		<span class="select-search btn-danger input-group-addon " onClick="javascript:popUpWin('list/ob_invoiceno_list.jsp?DONO='+form1.DONO.value+'&INVOICENO='+form1.INVOICENO.value+'&STATUS='+form1.PICKSTATUS.value+'&PICKISSUE=Y'+'&FROMDATE='+form1.FROM_DATE.value+'&TODATE='+form1.TO_DATE.value);"><span class="glyphicon glyphicon-search" aria-hidden="true" style="left: -4px;"></span></span> -->
	 ...
	 <INPUT type="Hidden" name="DIRTYPE" value="OB_PRINT">
  	...
  	<div class="row">
  		<div class="col-12 col-sm-12"><INPUT Type=Checkbox class="form-check-input"  style="border:0;" name = "select" value="select" <%if(allChecked.equalsIgnoreCase("true")){%>checked <%}%>onclick="return checkAll(this.checked);">
                       &nbsp; Select/Unselect All </strong>&nbsp;<input type="checkbox" class="form-check-input" name="printwithigino" value="printwithigino" />&nbsp;Print with GINO</div>
  </div>
 ...
 function getParameters(){
 	return {
 			...
 			FDATE:FROM_DATE,TDATE:TO_DATE,DTYPE:DIRTYPE,CNAME:USER,ORDERNO:ORDERNO,JOBNO:JOBNO,PICKSTATUS:PICKSTATUS,ORDERTYPE:ORDERTYPE,STATUS_ID:STATUS_ID,TYPE:"PICKISSUE",INVOICE:"N",EMPNO:EMPNO,CUSTOMERTYPE:CUSTOMERTYPE,INVOICENO:INVOICENO,ACTION: "VIEW_OUTBOUND_DETAILS_PRINT",PLANT:"<%=plant%>",LOGIN_USER:"<%=USERID%>"
 		        ...
 		        "dataSrc": function(data){
 		        	if(typeof data.items[0].dono === 'undefined'){
 		        		...
 		        		for(var dataIndex = 0; dataIndex < data.items.length; dataIndex ++){
 		        			...
 		        			data.items[dataIndex]['dono'] = '<a href="../salesorder/salespdfdetails?DONO=' +data.items[dataIndex]['dono']+ '&INVOICE='+'INVOICE'+'&TYPE='+ORDERTYPE+'&FROMDATE='+FROM_DATE+'&TODATE='+TO_DATE+'">' + data.items[dataIndex]['dono'] + '</a>';
 		        			<%}else{ %>
