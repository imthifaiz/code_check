...
function onRePrint(){
    ...
    document.form1.action="/track/DynamicFileServlet?action=printPOWITHBATCH";
   // document.form1.action="/track/DynamicFileServlet?action=printPOWITHBATCH&PONO="+DONO;
...
function onRePrintWithOutBatch(){
    ...
    document.form1.action="/track/DynamicFileServlet?action=printPOWITHOUTBATCH";
    document.form1.submit();
}
var postatus =   [{
...
String USERID ="",plant="";
String FROM_DATE ="",  TO_DATE = "", RECEIVESTATUS="",ORDERTYPE="",DIRTYPE ="",BATCH ="",USER="",fdate="",
tdate="",JOBNO="",PONO="",CUSTOMER="",PGaction="",SUPPLIERTYPE="";
String SUPPLIERID="",statusID="",GRNO="",newstatus="";
...
if(systatus.equalsIgnoreCase("ACCOUNTING"))
{
	...
	displaySummaryPrintNoBatch = ub.isCheckValAcc("printponobatch", plant,LOGIN_USER);
...
if(systatus.equalsIgnoreCase("INVENTORY"))
{
	...
	displaySummaryPrintNoBatch = ub.isCheckValinv("printponobatch", plant,LOGIN_USER);
	displaySummaryLink=true;
	displaySummaryPrintBatch=true;
	displaySummaryPrintNoBatch=true;
}
...
<div class="container-fluid m-t-20">
	...
	<!-- Muruganantham Modified on 16.02.2022 -->
            <ol class="breadcrumb" style="background-color: rgb(255, 255, 255);">      	
                ...
                <li><label>Purchase Order Details</label></li>                             
            </ol>             
    <!-- Muruganantham Modified on 16.02.2022 -->
...
<div class="row" style="padding:3px">
  		...
  		<div class="col-sm-4 ac-box">                       
        <input type="text" name="RECEIVESTATUS" id="RECEIVESTATUS" class="ac-selected form-control"  placeholder="STATUS" value="<%=StrUtils.forHTMLTag(newstatus)%>">
...
<!--   		<span class="select-search btn-danger input-group-addon " onClick="javascript:popUpWin('../jsp/list/ib_grno_list.jsp?PONO='+form1.PONO.value+'&GRNO='+form1.GRNO.value+'&STATUS='+form1.RECEIVESTATUS.value+'&FROMDATE='+form1.FROM_DATE.value+'&TODATE='+form1.TO_DATE.value);"><span class="glyphicon glyphicon-search" aria-hidden="true" style="left: -4px;"></span></span> -->
  		</div>
  		</div>
  		</div>
    ...
    <INPUT type="Hidden" name="DIRTYPE" value="PO_PRINT">
     ...
     <div class="row">
  		<div class="col-12 col-sm-12"><INPUT Type=Checkbox class="form-check-input"  style="border:0;" name = "select" value="select" <%if(allChecked.equalsIgnoreCase("true")){%>checked <%}%>onclick="return checkAll(this.checked);">
                        &nbsp; Select/Unselect All &nbsp;<input type="checkbox" class="form-check-input" name="printwithgrno" value="printwithgrno" />&nbsp;Print with GRNO</div>
  </div>
...
var tablePOList;
var FROM_DATE, TO_DATE, DIRTYPE, USER, ORDERNO, JOBNO, RECEIVESTATUS, ORDERTYPE, STATUS_ID,SUPPLIERTYPE,GRNO;
function getParameters(){
	return { 
		FDATE:FROM_DATE,TDATE:TO_DATE,DTYPE:DIRTYPE,SUPPLIERNAME:USER,ORDERNO:ORDERNO,JOBNO:JOBNO,RECEIVESTATUS:RECEIVESTATUS,ORDERTYPE:ORDERTYPE,STATUS_ID:STATUS_ID,TYPE:"IB",INVOICE:"N","SUPPLIERTYPE":SUPPLIERTYPE,GRNO:GRNO,ACTION: "VIEW_INBOUND_DETAILS_PRINT",PLANT:"<%=plant%>",LOGIN_USER:"<%=USERID%>"
	}
}  
  function onGo(){
//debugger;
...
//imti end
   ...
   //storeInLocalStorage('printPO_FROMDATE', FROM_DATE);
	//storeInLocalStorage('printPO_TODATE', TO_DATE);
	storeInLocalStorage('printPO_CUSTOMER', USER);
	storeInLocalStorage('printPO_SUPPLIER_TYPE_ID',SUPPLIERTYPE);
	storeInLocalStorage('printPO_ORDERNO', ORDERNO);
	storeInLocalStorage('printPO_GRNO', GRNO);
	storeInLocalStorage('printPO_JOBNO', JOBNO);
	storeInLocalStorage('printPO_ORDERTYPE',ORDERTYPE);
	storeInLocalStorage('printPO_STATUS', RECEIVESTATUS);
   }
...
// 			"lengthMenu": [[100, 500, 1000, 2500], [100, 500, 1000, "All"]],
		        ...
		        "dataSrc": function(data){
		        	if(typeof data.items[0].PONO === 'undefined'){
		        		...
		        		for(var dataIndex = 0; dataIndex < data.items.length; dataIndex ++){
		        			...
		        			data.items[dataIndex]['PONO'] = '<a href="../purchaseorder/printpdfdetails?PONO=' +data.items[dataIndex]['PONO']+ '&INVOICE='+data.items[dataIndex]['INVOICE']+'&FROMDATE='+data.items[dataIndex]['FROMDATE']+'&TODATE='+data.items[dataIndex]['TODATE']+'">' + data.items[dataIndex]['PONO'] + '</a>';
		        			<%}else{ %>
   ...
   <%-- document.getElementById('VIEW_RESULT_HERE').innerHTML = '';
    ...
    var urlStr = "/track/InboundOrderHandlerServlet";
    $.ajax({type: "POST",url: urlStr, data: {FDATE:,TDATE:TO_DATE,DTYPE:DIRTYPE,SUPPLIERNAME:USER,ORDERNO:ORDERNO,JOBNO:JOBNO,RECEIVESTATUS:RECEIVESTATUS,ORDERTYPE:ORDERTYPE,STATUS_ID:STATUS_ID,TYPE:"IB",INVOICE:"N",ACTION: "VIEW_INBOUND_DETAILS_PRINT",PLANT:"<%=plant%>",LOGIN_USER:"<%=USERID%>"},dataType: "json", success: callback });
...
$(document).ready(function(){
 ...
 $(document).ready(function(){
	 var ALLOWCATCH_ADVANCE_SEARCH = '<%=ALLOWCATCH_ADVANCE_SEARCH%>';
	    if(ALLOWCATCH_ADVANCE_SEARCH==='1'){
	 //getLocalStorageValue('printPO_FROMDATE', $('#FROM_DATE').val() , 'FROM_DATE');
	// getLocalStorageValue('printPO_TODATE', '', 'TO_DATE');
	 getLocalStorageValue('printPO_CUSTOMER', '', 'CUSTOMER');
	 getLocalStorageValue('printPO_SUPPLIER_TYPE_ID', '', 'SUPPLIER_TYPE_ID');
	 getLocalStorageValue('printPO_ORDERNO', '', 'PONO');
	 getLocalStorageValue('printPO_JOBNO', '', 'JOBNO');
	 getLocalStorageValue('printPO_GRNO', '', 'GRNO');
	 getLocalStorageValue('printPO_ORDERTYPE', '', 'ORDERTYPE');
	 getLocalStorageValue('printPO_STATUS', '', 'RECEIVESTATUS');
	    }
 onGo();
    $('.Show').click(function() {
	    $('#target').show(500);
