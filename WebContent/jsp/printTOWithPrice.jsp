...
<%
String title = "Consignment Order with price";
%>
...
//       document.form1.action="printTOWithPriceWithPrice.jsp";
...
function onRePrint(){
    ...
    document.form1.action="/track/DynamicFileServlet?action=printTOWITHBATCHWITHPRICE";
    document.form1.submit();
}
...
function onRePrintWithOutBatch(){
	...
	form1.setAttribute("target", "_blank");
    document.form1.action="/track/DynamicFileServlet?action=printTOWITHOUTBATCHWITHPRICE";
    document.form1.submit();
}
...
%>
	...
	<!-- Muruganantham Modified on 16.02.2022 -->
            <ol class="breadcrumb backpageul">      	
                ...
                <li><label>Consignment Order with price</label></li>                                   
            </ol>             
    <!-- Muruganantham Modified on 16.02.2022 -->
...
<div class="box-body">
    <FORM class="form-horizontal" name="form1" method="post" action="../consignment/consignorderwithprice">
...
<!--   		<span class="select-search btn-danger input-group-addon " onClick="javascript:popUpWin('../jsp/employee_list.jsp?EMP_NAME='+form1.EMP_NAME.value+'&TYPE=ESTIMATE&FORM=form1');"><span class="glyphicon glyphicon-search" aria-hidden="true" style="left: -4px;"></span></span> -->
 ...
 function getParameters(){
 	return {
 			...
 			FDATE:FROM_DATE,TDATE:TO_DATE,DTYPE:DIRTYPE,CUSTOMER:USER,ORDERNO:ORDERNO,JOBNO:JOBNO,PICKSTATUS:PICKSTATUS,ORDERTYPE:ORDERTYPE,TYPE:"PICKISSUE",EMPNO:EMPNO,CUSTOMERTYPE:CUSTOMERTYPE,Submit: "VIEW_CONSIGNMENT_DETAILS_PRINT_WITH_PRICE",
 ...
 function storeUserPreferences(){
	   //storeInLocalStorage('printTOWithPrice_FROMDATE', $('#FROM_DATE').val());
		//storeInLocalStorage('printTOWithPrice_TODATE', $('#TO_DATE').val());
		storeInLocalStorage('printTOWithPrice_CUSTOMER', $('#CUSTOMER').val());
		storeInLocalStorage('printTOWithPrice_CUSTOMER_TYPE_ID', $('#CUSTOMER_TYPE_ID').val());
		storeInLocalStorage('printTOWithPrice_TONO', $('#TONO').val());
		storeInLocalStorage('printTOWithPrice_JOBNO', $('#JOBNO').val());
		storeInLocalStorage('printTOWithPrice_ORDERTYPE', $('#ORDERTYPE').val());
		storeInLocalStorage('printTOWithPrice_PICKSTATUS',$('#PICKSTATUS').val());
		storeInLocalStorage('printTOWithPrice_EMP_NAME',$('#EMP_NAME').val());
}
...
$(document).ready(function(){
	...
	if (document.form1.FROM_DATE.value == ''){
		   getLocalStorageValue('printTOWithPrice_FROMDATE', '','FROM_DATE');
	 }
	   if (document.form1.TO_DATE.value == ''){
		   getLocalStorageValue('printTOWithPrice_TODATE', '','TO_DATE');
		} 
	   if (document.form1.CUSTOMER.value == ''){
			getLocalStorageValue('printTOWithPrice_CUSTOMER', '','CUSTOMER');
		} 	
	   if (document.form1.CUSTOMER_TYPE_ID.value == ''){
			getLocalStorageValue('printTOWithPrice_CUSTOMER_TYPE_ID', '','CUSTOMER_TYPE_ID');
		} 
	   if (document.form1.TONO.value == ''){
		   getLocalStorageValue('printTOWithPrice_TONO','','TONO');
		}  
	   if (document.form1.JOBNO.value == ''){
		   getLocalStorageValue('printTOWithPrice_JOBNO','', 'JOBNO');
		} 
	   if (document.form1.ORDERTYPE.value == ''){
			getLocalStorageValue('printTOWithPrice_ORDERTYPE','','ORDERTYPE');
		}
	   if (document.form1.PICKSTATUS.value == ''){
			getLocalStorageValue('printTOWithPrice_PICKSTATUS','','PICKSTATUS');
		}
	   if (document.form1.EMP_NAME.value == ''){
		   getLocalStorageValue('printTOWithPrice_EMP_NAME','','EMP_NAME');
		}
    }
	onGo();
