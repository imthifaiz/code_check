...
<%
	String title = "Profit And Loss Account Transactions";
...
<div class="container-fluid m-t-20">
	...
	<div class="box">
		...
		<div class="box-header menu-drop">
			 <h1 style="font-size: 18px;cursor:pointer;" class="box-title pull-right" onclick="window.location.href='../businessoverview/profitloss'">
    ...
    <tbody>
     <tr>
    	 ...
    	 <td></td>
    	 <% if(openingBalance>=0){ %>
        <td class="text-right"><%=curency%> <%=Numbers.toMillionFormat(openingBalance, Integer.parseInt(numberOfDecimal))%></td>
        <%}else{%>
        <td></td>
        <%} %>
        <% if(openingBalance<0){ %>
         <td class="text-right"><%=curency%> <%=Numbers.toMillionFormat(Math.abs(openingBalance), Integer.parseInt(numberOfDecimal))%></td>
        <%}else{%>
        <td></td>
        <%} %>
    	 <td></td>
      ...
      <tr>
        ...
        <td class="text-right"><%=Numbers.toMillionFormat(ledgerDet.getDEBIT(), Integer.parseInt(numberOfDecimal))%></td>
        <td class="text-right"><%=Numbers.toMillionFormat(ledgerDet.getCREDIT(), Integer.parseInt(numberOfDecimal)) %></td>
...
function generatePdf(dataUrl){	
	...
	doc.setFontSize(12);
	doc.text('<%=ledgerDetails.get(0).getACCOUNT()%>',cwidth,50,'center');
...
function generatePdfPrint(dataUrl){	
	...
	doc.setFontSize(12);
	doc.text('<%=ledgerDetails.get(0).getACCOUNT()%>',cwidth,50,'center');
	doc.text('From '+'<%=fromdateString%>'+' to '+'<%=todateString%>',cwidth,57,'center');
	<%-- doc.text('From ',83,57);
	doc.text('<%=fromdateString%>',95,57);
	doc.text('to ',120,57);
	doc.text('<%=todateString%>',125,57); --%>
