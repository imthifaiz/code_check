...
<%
	String title = "Project Profit and Loss";
...
<div class="container-fluid m-t-20">
	...
	<!-- Thanzith Modified on 28.02.2022 -->
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Project Profit and Loss</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 28.02.2022 --> 
...
function postDatePickerInit(){
	if (period == 'CUSTOM'){
		var projectprofitloss_fromdate = getLocalStorageValue('projectprofitloss_fromdate', '', 'fromdate');
		$('#fromdate').datepicker("setDate", 
		        moment(projectprofitloss_fromdate, 'DD/MM/YYYY').toDate());
		var projectprofitloss_todate = getLocalStorageValue('projectprofitloss_todate', '', 'todate');
		$('#todate').datepicker("setDate", 
				moment(projectprofitloss_todate, 'DD/MM/YYYY').toDate());
	}		
	$('#srbtn').click();
}
	$(document).ready(function() {
		getLocalStorageValue('projectprofitloss_project', '', 'project');
		var periodFromSession = getLocalStorageValue('projectprofitloss_reportperiod', 'TODAY', 'reportperiod');
		...
		$('#project').typeahead({
			...
			},
			{
			  ...
			  empty: [
				...
				finalProjId = selection.ID;
				var selectedVal = $("#reportperiod").val();
				pldatechanged(selectedVal);
	...
	{
		window.location = "/track/ProfitLossServlet?action=getProjectProfitLossExcel&fromDate="+fromDateFormatted+"&toDate="+toDateFormatted+"&projectId="+finalProjId;
	}

	
	function loadPLSection1Data(from,to)
	{
		storeInLocalStorage('projectprofitloss_reportperiod', $('#reportperiod').val());
		storeInLocalStorage('projectprofitloss_project', $('#project').val());
		if ($("#reportperiod").val().toLowerCase() == 'custom'){
			storeInLocalStorage('projectprofitloss_fromdate', $('#fromdate').val());
		}
		if ($("#reportperiod").val().toLowerCase() == 'custom'){
			storeInLocalStorage('projectprofitloss_todate', $('#todate').val());
		}
		var acctypes= "('10','8')";
		$.ajax({
			type : "POST",
		...
		var node1;
		    ...
		    initialSort:[
 		    ...
 		    groupHeader: function(value, count, data, group){
 		        ...
 		        return value + "<a style='float:right;'>" + getNumberInMillionFormat(TotalValue.toFixed(numberOfDecimal)) +"</a>";
 		    },
		    ...
		    {title:"Total", field:"total",align:"right",formatter:function(cell, formatterParams, onRendered){
		    	var value=cell.getValue();
		    	return getNumberInMillionFormat(value.toFixed(numberOfDecimal));
	...
	function grossprofit(){
		...
		gross=getNumberInMillionFormat(gross.toFixed(numberOfDecimal));
	...
	function netprofit(){
		...
		net=grossGlobal+net;
		net=getNumberInMillionFormat(net.toFixed(numberOfDecimal));
		$('#netprofit').html(net);
	}
...
{
	//alert("Called");
	window.location.href="../businessoverview/Projectprofitlossdetail?account="+account+"&fromDate="+fromDateFormatted+"&toDate="+toDateFormatted+"";
}
...
function generatePdf(dataUrl){	
	...
	groups_1.forEach(group =>{
		...
		grouprows.forEach(element => {    
	       ...
	       // var temp1 = [element.id,element.name];
	        rows.push(temp);
	        //rows1.push(temp1);
	        htmlTable+="<tr><td>"+element.getData().account_name+"</td><td>"+getNumberInMillionFormat(total)+"</td></tr>";
	    }); 
	});
	var grossprofit=$('#grossprofit').html();
	htmlTable+="<tr><td class='calculated'>Gross Profit:</td><td class='calculated'>"+getNumberInMillionFormat(grossprofit)+"</td></tr>";
	...
	htmlTable+="<tr><td class='calculated'>Net Profit:</td><td class='calculated'>"+getNumberInMillionFormat(netprofit)+"</td></tr>";
	htmlTable+="</tbody>";
