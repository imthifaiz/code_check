...
<%
	String title = "Project Trial Balance Account Transactions";
...
<div class="container-fluid m-t-20">
	...
	<div class="box">
		...
		<div class="box-header menu-drop">
			 <h1 style="font-size: 18px;cursor:pointer;" class="box-title pull-right" onclick="window.location.href='../projecttrial/trialbalance'">
     ...
     <tr>
      ...
      <tr>
       ...
       <%
        ...
        <td align="right"> <%=Numbers.toMillionFormat(trialBalDet.getDEBIT(), Integer.parseInt(numberOfDecimal))%>
        </td>
        <td align="right"><%=Numbers.toMillionFormat(trialBalDet.getCREDIT(), Integer.parseInt(numberOfDecimal))%>
        </td>
        <%Double amount=trialBalDet.getDEBIT()-trialBalDet.getCREDIT();
        if(amount>0){ %>
        <td align="right"><%=Numbers.toMillionFormat(amount, Integer.parseInt(numberOfDecimal))%>
        Dr</td>
        <%}else{%>
         <td align="right"><%=Numbers.toMillionFormat(Math.abs(amount), Integer.parseInt(numberOfDecimal))%>
         Cr</td>
        <%} %>
      </tr>
      <%} %>
      <tr>
    	 ...
    	 <td></td>
    	 <td></td>
    	 <td align="right"><%=Numbers.toMillionFormat(totalDebit, Integer.parseInt(numberOfDecimal))%>
    	 </td>
    	 <td align="right"><%=Numbers.toMillionFormat(totalCredit, Integer.parseInt(numberOfDecimal))%>
    	 </td>
    	 <td></td>
    	 </tr>
      <tr>
      <% totalClosingBalance=(totalDebit-totalCredit);%>
        ...
        <td align="right"><%=curency%> <%=Numbers.toMillionFormat(totalClosingBalance, Integer.parseInt(numberOfDecimal))%> 
        </td>
        <%}else{%>
        <td></td>
        <%} %>
        <% if(totalClosingBalance<0){ %>
         <td align="right"><%=curency%>  <%=Numbers.toMillionFormat(Math.abs(totalClosingBalance), Integer.parseInt(numberOfDecimal))%> 
         </td>
        <%}else{%>
        <td></td>
        <%} %>
...
function generatePdf(dataUrl){	
	...
	doc.setFontSize(12);
	doc.text('<%=actcode+" "+actnme%>',cwidth,50,'center');
