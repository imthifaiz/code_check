			...
			<div class=" pull-right">
				...
				<%if(displaySummaryEdit){ %>
					<button type="button" class="btn btn-default" data-toggle="tooltip"  
						...
						<a href="../salesestimate/edit1?estno=<%=esthdr.getESTNO()%>"><i class="fa fa-pencil" aria-hidden="true"></i></a>
					</button>
					<%}%>
				</div>
				<div class="btn-group" role="group">
				...
				<%if(displayMore){ %>
					...
					<ul class="dropdown-menu" style="min-width: 0px;">
						<li id=""><a href="../salesestimate/copyPro?estno=<%=esthdr.getESTNO()%>">Copy</a></li>
						<%if(!esthdr.getORDER_STATUS().equalsIgnoreCase("PROCESSED") && !esthdr.getORDER_STATUS().equalsIgnoreCase("Draft")){%>
						<li id=""><a href="../salesestimate/convertToSalesPro?estno=<%=esthdr.getESTNO()%>">Convert To Sales</a></li>
						<%} %>
						...
						<table id="table3" class="table">
							<tbody style="text-align:right;">
								...
								<%if(esthdr.getORDERDISCOUNTTYPE().equalsIgnoreCase("%")){%>
									<tr>
										<td colspan="7">
										Order Discount (<%=StrUtils.addZeroes(esthdr.getORDERDISCOUNT(), "3")%>%)
										<br>
										<%if(esthdr.getISORDERDISCOUNTTAX() == 1){ %>
										(Tax Inclusive)
										<%}else{%>
								...
								<%if(esthdr.getDISCOUNT_TYPE().equalsIgnoreCase("%")){%>
									<tr>
										<td colspan="7">
										Discount (<%=StrUtils.addZeroes(esthdr.getDISCOUNT(), "3")%>%)
										<br>
										<%if(esthdr.getISDISCOUNTTAX() == 1){ %>
										(Tax Inclusive)
										<%}else{%>
								...
								<tr>
									...
									<td><%=StrUtils.addZeroes(esthdr.getSHIPPINGCOST(), numberOfDecimal)%></td>
								</tr>
								...
								if(fintaxtype.getSHOWTAX() == 1){%>
										<tr>
											...
											<td><%=StrUtils.addZeroes(totalTax, numberOfDecimal)%></td>
										</tr>
								<%}
								}%>
								...
								<tr>
									...
									<td><%=StrUtils.addZeroes(esthdr.getADJUSTMENT(), numberOfDecimal)%></td>
								</tr>
								<tr>
									<td colspan="7"><b><%=lblTotal%></b></td>
