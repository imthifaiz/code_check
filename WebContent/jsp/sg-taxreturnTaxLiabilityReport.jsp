...
<%
	String title = "Tax Liability Report";
...
<!-- Muruganantham Modified on 16.02.2022 -->
            <ul class="breadcrumb backpageul">      	
                ...
                <li><label>Tax Liability Report </label></li>                                   
            </ul>             
    <!-- Muruganantham Modified on 16.02.2022 --> 
...
var tableDataSummary;
	...
	{
		var asofdate=moment(toDateFormatted).format('MMM DD,YYYY');
		window.location = "/track/TaxReturnFiling?action=getTaxLiabilityReportExcel&fromDate="+fromDateFormatted+"&toDate="+toDateFormatted+"&asOfDate="+asofdate;
	}
...
function generatePdf(dataUrl){	
	...
	doc.text('<%=PLNTDESC%>', 100, 50,'center');
	doc.setFontSize(18);
	doc.setFontStyle("bold");
	doc.text('<%=title%>',100,60,'center');
	doc.setFontSize(14);
	doc.setFontStyle("normal");
	var asofdate=moment(toDateFormatted).format('MMM DD,YYYY');
	doc.text('As of '+asofdate,100,70, 'center');
	doc.setFontStyle("normal");
	/* **** */
	var totalPagesExp = "{total_pages_cont_string}";
	doc.setFontSize(14);
	var htmlTable="<thead><tr><th></th><th  style='text-align:right'>NET AMOUNT</th><th  style='text-align:right'>GST AMOUNT</th></tr></thead><tbody>";
	var col = ['', 'NET AMOUNT', 'GST AMOUNT'];
	var rows = [];
	...
	groups_1.forEach(group =>{
		...
		htmlTable+="<tr><td class='innerHeader' colspan='3'><strong>"+key+"</strong></td></tr>";
		...
		grouprows.forEach(element => {    
	        ...
	        //rows.push(temp);
	        htmlTable+="<tr><td>"+element.getData().taxname;
	        ...
	        htmlTable+="</td><td>"+parseFloat(element.getData().amount).toFixed(numberOfDecimal)+"</td></tr>";
	    }); 
	});
	htmlTable+="</tbody>";
	...
	if (typeof doc.putTotalPages === 'function') {
	  ...
	  doc.save('TaxLiabilityReport.pdf')
}
...
function loadTaxFillTable_1()
{
	...
	var node1;
	    ...
	    columns:[
	    	{title:"", field:"taxname",headerSort:false},
	    	...
	    	{title:"GST AMOUNT", field:"amount",align:"right",headerSort:false,width:150,formatter:function(cell, formatterParams, onRendered){
	    		var value=cell.getValue();
	    		return parseFloat(value).toFixed(numberOfDecimal);
