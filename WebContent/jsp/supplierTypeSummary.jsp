...
<!-- ifend -->
<%
String title = "Supplier Group Summary";
%>
...
var subWin = null;
  function popWin(URL) {
    subWin = window.open(URL, 'SupplierTypeSummary', 'toolbar=0,scrollbars=yes,location=0,statusbar=0,menubar=0,dependant=1,resizable=1,width=800,height=400,left = 200,top = 184');
  }
/*  function onGo(){
 ...
 function onView(){
	   document.form1.action  = "../jsp/supplierTypeSummary.jsp?action=VIEW";
	   document.form1.submit();
	}
</script>
...
{
displaySummaryEdit = ub.isCheckValAcc("editsupplierType", PLANT,USERID);
displaySummaryNew = ub.isCheckValAcc("newsupplierType", PLANT,USERID);
}
if(systatus.equalsIgnoreCase("INVENTORY"))
{
displaySummaryEdit = ub.isCheckValinv("editsupplierType", PLANT,USERID);
displaySummaryNew = ub.isCheckValinv("newsupplierType", PLANT,USERID);
}
...
if(PGaction.equalsIgnoreCase("View")){
 try{
      ...
      if(strUtils.fString(PLANT).length() > 0)  ht.put("PLANT",PLANT);
         ...
         QryList =  _CustUtil.getSupplierTypeSummary(SUPPLIERTYPEID,PLANT,"");
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Supplier Group Summary</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->     
            <div class="box-header menu-drop">
              ...
              <button type="button" class="btn btn-default" onclick="window.location.href='../suppliertype/new'"  style="background-color: rgb(214, 72, 48);color: rgb(255, 255, 255);" >+ New</button>&nbsp;
              <%} %>
              </div>
              <h1
...
<FORM class="form-horizontal" name="form1" method="post" action="supplierTypeSummary.jsp">
...
<label class="control-label col-sm-2" for="Customer Group Desc or ID">Supplier Group Desc or ID</label>
<div class="col-sm-4" >
<div class="input-group">
<INPUT name="SUPPLIER_TYPE_ID"  id="SUPPLIER_TYPE_ID"type = "TEXT" value="<%=SUPPLIERTYPEID%>" size="20"  MAXLENGTH=100 class="form-control">
<span class="select-icon" onclick="$(this).parent().find('input[name=\'SUPPLIER_TYPE_ID\']').focus()"><i class="glyphicon glyphicon-menu-down"></i></span>
  <!-- <span class="btn-danger input-group-addon" onClick="javascript:popWin('../jsp/SupplierTypeList.jsp?SUPPLIER_TYPE_ID='+form1.SUPPLIER_TYPE_ID.value);">
 ...
 <table id="table" class="table table-bordred table-striped" > 
   ...
   <thead>  
          <tr>  
            <th >S/N</th>  
            <th>Supplier Group ID</th>
            <th>Supplier Group Description</th>
            <th>IsActive</th>
            <th>Edit</th>      
          </tr>  
        </thead>
       ...
       %>
          ...
          <TR>
           ...
           <TD align="center">&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; <%=strUtils.fString((String)lineArr.get("SUPPLIER_TYPE_ID"))%></TD>
    	     <TD align="center">&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; <%=strUtils.fString((String)lineArr.get("SUPPLIER_TYPE_DESC"))%></TD>
              <TD align="center">&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; <%=strUtils.fString((String)lineArr.get("ISACTIVE"))%></TD>
              <%if(displaySummaryEdit){ %>
               <TD align="center"> <a href ="maintSupplierType.jsp?action=View&SUPPLIER_TYPE_ID=<%=(String)lineArr.get("SUPPLIER_TYPE_ID")%>&SUPPLIER_TYPE_DESC=<%=strUtils.replaceCharacters2Send((String)lineArr.get("SUPPLIER_TYPE_DESC"))%>&ISACTIVE=<%=(String)lineArr.get("ISACTIVE")%>"><i class="fa fa-pencil-square-o"></i></a></TD>
               <%}else{ %>
...
<script LANGUAGE="JavaScript">
		...
		var tabletype;
		var SUPPLIERTYPE, groupRowColSpan = 6;
		
		function getParameters(){
			return {
				"SUPPLIERTYPE":SUPPLIERTYPE,
				"PLANT":plant,
				"action":"GET_SUPPLIERTYPE_FOR_SUMMARY"
			}
		}
		function onGo(){
			    	...
			    	tabletype = $('#table').DataTable({
						...
						"ajax": {
					        ...
					        "dataSrc": function(data){
						        	...
						        	if(typeof data.SUPPLIERTYPELIST[0].SUPPLIER_TYPE_ID === 'undefined'){
						        		return [];
						        	}else {				        		
						        			for(var dataIndex = 0; dataIndex < data.SUPPLIERTYPELIST.length; dataIndex ++){
						        				var lineno = data.SUPPLIERTYPELIST[dataIndex].SUPPLIER_TYPE_ID;
						        				var sno=dataIndex+1;
						        				data.SUPPLIERTYPELIST[dataIndex]['SNO'] = sno;
												<%if(displaySummaryEdit){ %>
						        				data.SUPPLIERTYPELIST[dataIndex]['EDIT'] = '<a href="../suppliertype/edit?SUPPLIER_TYPE_ID=' +lineno+'&SUPPLIER_TYPE_DESC=' +data.SUPPLIERTYPELIST[dataIndex].SUPPLIER_TYPE_DESC+'&ACTIVE=' +data.SUPPLIERTYPELIST[dataIndex].ISACTIVE+'"><i class="fa fa-pencil-square-o"></i></a>';
						        				<%}else{ %>
				        ...
				        "columns": [
				        	...
				        	{"data": 'SUPPLIER_TYPE_ID', "orderable": true},
				        	{"data": 'SUPPLIER_TYPE_DESC', "orderable": true},
