...
<%
String title = "Supplier Discount Summary";
%>
...
{
  document.form1.action = "/track/ReportServlet?action=Export_SupplierReport_Excel";
	document.form1.submit();
}

// NAVAS COMMENTS
/*  function onGo(){
 document.form1.action="SupplierReport.jsp";
 document.form1.submit();
 } */
// ENDS
</script>
...
String fieldDesc="";
String USERID ="",PLANT="",LOC ="",  ITEM = "",CUSTOMER="",STOCK_SALE="",STOCK_CLAIM="",BATCH="",PRD_TYPE_ID="",PRD_DESCRIP="",QTY ="",QTYALLOC ="", fdate="",tdate="";
String html = "";
int Total=0,STOCK_EXPIRE_INT=0,STOCK_CLAIM_INT=0, STOCK_FLOATING_INT=0,STOCK_SALE_INT=0;
String SumColor="",PRD_CLS_ID="",PRD_CLS_ID1="",sSupplierTypeId="";
boolean flag=false;
...
{
displaySummaryExport = ub.isCheckValAcc("exportsupplierdiscount", PLANT,USERID);
displaySummaryImport = ub.isCheckValAcc("importsupplierdiscount", PLANT,USERID);
}
if(systatus.equalsIgnoreCase("INVENTORY"))
{
displaySummaryExport = ub.isCheckValinv("exportsupplierdiscount", PLANT,USERID);
displaySummaryImport = ub.isCheckValinv("importsupplierdiscount", PLANT,USERID);
}
...
if(PGaction.equalsIgnoreCase("View")){
 try{
	 ...
	 ITEM = itemMstUtil.isValidInvAlternateItemInItemmst( PLANT, ITEM);
      ...
      invQryList = vendmstdao.getSupplierCount(ht,PRD_DESCRIP);
     ...
     invQryList = vendmstdao.getSupplierReport(ht,PRD_DESCRIP,start,end);
		 ...
		 Hashtable ht = new Hashtable();
	      ...
	      /*if(strUtils.fString(sSupplierTypeId).length() > 0)      
	      {
	        ht.put("V.VEND_TYPE_ID",sSupplierTypeId);
		      ...
		      //locQryList= itemUtil.queryItemMstForSearchCriteriaNew(ITEM,ITEM_DESC,PRD_CLS_ID,PRD_TYPE_ID,PRD_BRAND_ID,PLANT,"",start,end);
		      invQryList = vendmstdao.getSupplierReport(ht,PRD_DESCRIP,start,end);
...
<SCRIPT LANGUAGE="JavaScript">
	...
	document.form1.cur_page.value = cur_page;
      ...
      document.form1.action = "../jsp/SupplierReport.jsp";
	document.form1.submit();
    }
    //////////////////////////////////////////////////////////////////////////////////////////////////
...
<div class="container-fluid m-t-20">
	...
	<!-- Thanzith Modified on 23.02.2022 -->
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Supplier Discount Summary</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->     
            <div class="box-header menu-drop">
              ...
              <button type="button" class="btn btn-default"
					onclick="window.location.href='../supplier/discountimport'" >
						Import Supplier Discount</button>
					&nbsp;
					<%} %>
...
<div class="box-body">
<FORM class="form-horizontal" name="form1" method="post" action="SupplierReport.jsp">
...
<!--    		 	<span class="btn-danger input-group-addon"  onClick="javascript:popUpWin('../jsp/supplierlist.jsp?CUSTOMER='+convertCharToString(form1.CUSTOMER.value));">
   		 	<span class="glyphicon glyphicon-search" aria-hidden="true"></span></a></span> -->
  		</div>
  		</div>
 		</div>
              	...
              	<table id="tableMovementHistory"
					...
					<thead>
		                <tr role="row">
		                	...
		                	<th style="font-size: smaller;">Cost Wit Discount</th>
       ...
       <%
          ...
          int total_stock_claim = 0;
              for (int iCnt =0; iCnt<invQryList.size(); iCnt++){
               ...
               // double discountedcost=(discount/cost)*100;
              
          %>
          
          var rowData = [];
          rowData[rowData.length] = '<%=(String)lineArr.get("ITEM") %>';
          rowData[rowData.length] = '<%=(String)lineArr.get("ITEMDESCRIPTION")%>';
          rowData[rowData.length] = '<%=(String)lineArr.get("SUPPLIERID")%>';
          rowData[rowData.length] = '<%=(String)lineArr.get("SUPPLIERNAME")%>';
          rowData[rowData.length] = '<%=StrUtils.formatNum(String.format("%.2f",cost))%>';
          rowData[rowData.length] = '<%=(String)lineArr.get("DISCOUNT")%>';
        <%if(discountType.equalsIgnoreCase("BYPERCENTAGE")){ %>
          rowData[rowData.length] = '<%=StrUtils.formatNum(String.format("%.2f",discountedper))%>';
          <%}else{ %>
          rowData[rowData.length] = '<%=StrUtils.formatNum(String.format("%.2f",discount1))%>';
...
<!--     NAVAS START -->
     ...
     var tabletype;
     var ITEM,PRD_DESCRIP,CUSTOMER, groupRowColSpan = 6;
     function getParameters(){
    		return {
    			...
    			"CUSTOMER":CUSTOMER,
    		   	"PLANT":plant,
    			"action":"GET_SUPPLIERDISCOUNT_FOR_SUMMARY"
    		}
    	}
     
     function onGo(){
    	 ITEM=document.form1.ITEM.value;
    	 PRD_DESCRIP=document.form1.PRD_DESCRIP.value;
    		    ...
    		    if (tabletype){
    		    	...
    		    	tabletype = $('#tableMovementHistory').DataTable({
    			        ...
    			        "columns": [
    			        	...
    			        	{"data": 'SUPPLIERID', "orderable": true},
