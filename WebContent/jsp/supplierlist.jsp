...
<%@page import="com.track.dao.CurrencyDAO"%>
<html>
<head>
...
<%   
    ...
    String CUSTOMER = StrUtils.replaceStr2Char(strUtils.fString(request.getParameter("CUSTOMER")));
    ...
    listQry = _PoHdrDAO.getSupplierList(PLANT,CUSTOMER);
    for(int i =0; i<listQry.size(); i++) {
   
     	sBGColor = ((i == 0) || (i % 2 == 0)) ? "#FFFFFF" : "#FFFFFF";
     	Map m=(Map)listQry.get(i);
     	String sCustomerid    = (String)m.get("custcode");
     	String sCustomername   = (String)m.get("custname");
     
if(TYPE.equalsIgnoreCase("MULTIPLEIB")){%>

<TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			onClick=" window.opener.form.CUSTOMER.value='<%=StrUtils.forHTMLTag(sCustomername)%>';
      			window.close();"><%=sCustomerid%></a></td>
		<td align="left" class="main2">&nbsp;<%=strUtils.replaceCharacters2Recv(sCustomername)%></td>
	</TR>
		
<%} else{ %>
<TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			onClick=" window.opener.form1.CUSTOMER.value='<%=StrUtils.forHTMLTag(sCustomername)%>';
      	  	 	window.close();"><%=sCustomerid  %></a></td>
		<td align="left" class="main2">&nbsp;<%=strUtils.replaceCharacters2Recv(sCustomername)%></td>
	</TR>
