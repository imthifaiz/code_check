...
<html>
<head>
<title>Supplier Type List</title>
<meta charset="utf-8">
...
function onAdd(){
	   var SUPPLIER_TYPE_ID   = document.form1.SUPPLIER_TYPE_ID1.value;
	   var SUPPLIER_TYPE_DESC = document.form1.SUPPLIER_TYPE_DESC.value;
	    if(SUPPLIER_TYPE_ID == "" || SUPPLIER_TYPE_ID == null) {alert("Please Enter Supplier Type ID"); return false; }
	    if(SUPPLIER_TYPE_DESC == "" || SUPPLIER_TYPE_DESC == null) {alert("Please Enter Supplier Type Description"); return false; }
	   document.form1.action  = "suppliertypelistsave.jsp?action=ADD";
	   document.form1.submit();
	}
function onNew(){
	   document.form1.action  = "suppliertypelistsave.jsp?action=Clear";
	   document.form1.submit();
	}
	
function onGenID(){
    document.form1.action  = "suppliertypelistsave.jsp?action=Auto_ID";
	   document.form1.submit();
	}
...
String res        = "",action     =   "",supplierTypeID="",supplierTypeDesc="";
...
supplierTypeID  = strUtils.fString(request.getParameter("SUPPLIER_TYPE_ID1"));
supplierTypeDesc  = strUtils.fString(request.getParameter("SUPPLIER_TYPE_DESC"));

//User Check 	
	boolean al=false;
	al = ub.isCheckVal("suppliertypepopup", plant,sUserId);
	if(al==true)
	{		
		System.out.println("suppliertypepopup");
	}
            ...
            supplierTypeID="ST"+sZero+updatedSeq;
            ...
            } catch(Exception e)
             {
         	   mLogger.exception(true,
    					"ERROR IN JSP PAGE -suppliertypelistsave.jsp ", e);
             }
 }
 //2. >> Add
    ...
    ht.put(IDBConstants.SUPPLIERTYPEID,supplierTypeID);
     if(!(custutil.isExistsSupplierType(ht))) // if the Item  exists already
     {
           ...
           if(itemInserted&&inserted) {
                     ...
                     res = "<font class = "+IDBConstants.FAILED_COLOR +">Failed to add New Supplier Type </font>";
           }
     }else{
            res = "<font class = "+IDBConstants.FAILED_COLOR +">Supplier Type  Exists already. Try again</font>";
...
<h3 class="panel-title">Supplier Type List</h3> 
</div>
</div>
  
<form class="form-horizontal" method="post" name="form1" method="post">
...
<div class="form-group">
      <label class="control-label col-sm-2" for="Supplier Type ID">
      <i class="glyphicon glyphicon-star" style="font-size: 10px; top:-6px; color: #e50000"></i>&nbsp;Supplier Type ID:</label>
      <div class="col-sm-3">
      	<div class="input-group">
        <input type="hidden" value="hide" name="search_criteria_status" id="search_criteria_status" />  		
    		<input name="SUPPLIER_TYPE_ID1" type="TEXT" value="<%=supplierTypeID%>"
      ...
      <label class="control-label col-sm-3" for="Product Type Description">Supplier Type Description:</label>
      <div class="col-sm-3">          
        <INPUT  class="form-control" name="SUPPLIER_TYPE_DESC" type="TEXT" value="<%=supplierTypeDesc%>" style="width:100%"
    ...
    <div class="form-group">        
     ...
     <button type="button" class="Submit btn btn-default" data-toggle="modal" data-target="#myModal" onClick="onAdd();" <%=sAddEnb%>><b>Save</b></button>&nbsp;&nbsp;
      	 <% } else { %>
      	<button type="button" disabled="disabled" class="Submit btn btn-default" data-toggle="modal" data-target="#myModal" onClick="onAdd();" <%=sAddEnb%>><b>Save</b></button>&nbsp;&nbsp;
      	      <% } %> 
    ...
    <div class="form-group">
      <div class="col-sm-4">
      <a href="#" id="Show" style="font-size: 15px; color: #0059b3; text-decoration:none;">Show Create Supplier Type</a>
      <a href="#" id="Hide" style="font-size: 15px; color: #0059b3; text-decoration:none; display:none;">Hide Create Supplier Type</a>
      </div>
       	  </div>
    </div>
...
CustUtil.setmLogger(mLogger);
  ...
  String supplier_type_id = strUtils.fString(request.getParameter("SUPPLIER_TYPE_ID"));
  String type  = strUtils.fString(request.getParameter("TYPE"));
 // String plant = (String)session.getAttribute("PLANT");
  String sBGColor = "";
 try{
  List listQry = CustUtil.getSupplierTypeList(supplier_type_id,plant,"");
  for (int i =0; i<listQry.size(); i++){
  Map map = (Map) listQry.get(i);
  sBGColor = ((i == 0) || (i % 2 == 0)) ? "#FFFFFF" : "#dddddd";
   String suppliertypeid        = strUtils.fString((String) map.get("SUPPLIER_TYPE_ID"));
   String desc             = strUtils.fString((String) map.get("SUPPLIER_TYPE_DESC"));
    String isactive             = strUtils.fString((String) map.get("ISACTIVE"));
if(type.equalsIgnoreCase("suppliersummary")){
	%>
	   <TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			onClick="window.opener.form.SUPPLIER_TYPE_ID.value='<%=suppliertypeid%>';window.close();"><%=suppliertypeid%></a></td>
...
%>
	...
	<%}else if(type.equalsIgnoreCase("PURCHASEWITHCOST")){
%>
    <TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			onClick="window.opener.form1.SUPPLIER_TYPE_ID.value='<%=suppliertypeid%>';window.close();"><%=suppliertypeid%></a></td>
		<td align="left" class="main2">&nbsp;<%=desc%></td>
		<td align="left" class="main2">&nbsp;<%=isactive%></td>

	</TR>
	<%}else if(type.equalsIgnoreCase("SUPMODALUOM")){
		%>
	    <TR>
			<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
				onClick="window.opener.supplierForm.SUPPLIER_TYPE_ID.value='<%=suppliertypeid%>';window.close();"><%=suppliertypeid%></a></td>
			<td align="left" class="main2">&nbsp;<%=desc%></td>
			<td align="left" class="main2">&nbsp;<%=isactive%></td>

		</TR>
	<%}else if(type.equalsIgnoreCase("POPUP1")){
		%>
	    <TR>
			<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
				onClick="window.opener.form1.SUPPLIER_TYPE_ID.value='<%=suppliertypeid%>';window.close();"><%=suppliertypeid%></a></td>
	...
	<%}else if(type.equalsIgnoreCase("POPUP2")){
	    ...
	    <TR>
			<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
				onClick="window.opener.formsupplier.SUPPLIER_TYPE_ID.value='<%=suppliertypeid%>';window.close();"><%=suppliertypeid%></a></td>
			<td align="left" class="main2">&nbsp;<%=desc%></td>
			<td align="left" class="main2">&nbsp;<%=isactive%></td>
...
%>
    <TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			onClick="window.opener.form.SUPPLIER_TYPE_ID.value='<%=suppliertypeid%>';window.opener.form.SUPPLIER_TYPE_DESC.value='<%=desc%>';  window.close();"><%=suppliertypeid%></a></td>
		<td align="left" class="main2">&nbsp;<%=desc%></td>
		<td align="left" class="main2">&nbsp;<%=isactive%></td>
