...
mLogger.setLoggerConstans(loggerDetailsHasMap);
    ...
    String issue = strUtils.fString(request.getParameter("TYPE"));
   ...
   try{
     	...
     	if(issue.equalsIgnoreCase("issue")){
     		extCond=extCond+" AND (status = 'O' or status = 'N')";
     	}
     	else if(issue.equalsIgnoreCase("view")){
    		extCond=extCond+" AND (status = 'O' or status = 'C')";
    	}else{
    		...
    		ArrayList listQry = _TOUtil.getToHdrDetails("tono,custName,custCode,status,collectiondate,jobNum",ht,extCond);
   	...
   	<TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;<a href="#"
			onClick="window.opener.form.TONO.value='<%=tono%>';
                     window.opener.form.CUST_NAME.value='<%=custName%>';
