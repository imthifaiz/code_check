...
<html>
<head>
<title>Transfer Order Customer List</title>
<meta charset="utf-8">
...
<SCRIPT LANGUAGE="JavaScript">
	...
	function onNew(){
   		document.form.action  = "toassignee_list_order.jsp?action=NEW";
	...
	function onAdd(){
   		...
   		if(CUST_CODE == "" || CUST_CODE == null) {alert("Please Enter Transfer Order Customer ID");document.form.CUST_CODE.focus(); return false; }
  		if(CUST_NAME == "" || CUST_NAME == null) 
  	  		{
  				alert("Please Enter Transfer Order Customer Name"); 
   				document.form.CUST_NAME1.focus();
   				return false; 
   			}
   		document.form.action  = "toassignee_list_order.jsp?action=ADD";
   		document.form.submit();
	}
	
	function onIDGen()
	{
   		document.form.action  = "toassignee_list_order.jsp?action=Auto-ID";
   		document.form.submit();
	}
           ...
           //--if exitflag is not false than we updated nxtseq batch number based on plant,currentmonth
           ...
           } catch(Exception e)
            {
        	   mLogger.exception(true,
   					"ERROR IN JSP PAGE - toassignee_list_order.jsp ", e);
            }
  ...
  if(!custUtil.isExistToAssignee(sCustCode,plant)) // if the Customer exists already
    {
        ...
        if(custInserted&&inserted) {
               res = "<font class = "+IConstants.SUCCESS_COLOR+">Transfer Order Customer Added Successfully</font>";
             //clear data automatically when click save button
               ...
               res = "<font class = "+IConstants.FAILED_COLOR+">Failed to add New Transfer Order Customer</font>";

          }
    }else{
           res = "<font class = "+IConstants.FAILED_COLOR+">Transfer Order Customer Exists already. Try again</font>";
...
<h3 class="panel-title">Transfer Order Customer List</h3> 
</div>
</div>
...
<div class="box-body">
   ...
   <div class="form-group">
      <label class="control-label col-sm-3" for="Transfer Assignee ID">
      ...
      <label class="control-label col-sm-2" for="Transfer Assignee Name">
...
<div class="form-group">
      <div class="col-sm-5">
      <a href="#" id="Show" style="font-size: 15px; color: #0059b3; text-decoration:none;">Show Create Transfer Order Customer</a>
      <a href="#" id="Hide" style="font-size: 15px; color: #0059b3; text-decoration:none; display:none;">Hide Create Transfer Order Customer</a>
      </div>
       	  </div>
</div>
<div>
...
mLogger.setLoggerConstans(loggerDetailsHasMap);
    ...
    try{
    	...
    	ArrayList arrCust = custUtils.getOutGoingToAssigneeDetails(plant,sCustName," AND ISACTIVE='Y'");
    	...
    	for(int i =0; i<arrCust.size(); i++) {
            ...
            sContactName     = strUtils.removeQuotes((String)arrCustLine.get(2));
        	sTelNo     = strUtils.removeQuotes((String)arrCustLine.get(3));
        	sEmail     = strUtils.removeQuotes((String)arrCustLine.get(4));
        	String sAdd1     = strUtils.removeQuotes((String)arrCustLine.get(5));
        	String sAdd2     = strUtils.removeQuotes((String)arrCustLine.get(6));
          	String sAdd3     = strUtils.removeQuotes((String)arrCustLine.get(7));
        	String sAdd4     = strUtils.removeQuotes((String)arrCustLine.get(8));
        	sCountry  = strUtils.removeQuotes((String)arrCustLine.get(9));
        	sRemarks  = strUtils.removeQuotes((String)arrCustLine.get(10));
        	sZip     = strUtils.removeQuotes((String)arrCustLine.get(11));
