...
boolean displaySummaryLink=true,displaySummaryEdit=true,displaySummaryNew=true,displaySummaryDelete=true;
/* boolean displaySummaryDelete=false,displaySummaryNew=false;
if(systatus.equalsIgnoreCase("ACCOUNTING"))
{
displaySummaryDelete = ub.isCheckValAcc("removetransportmode", plant,USERID);
displaySummaryNew = ub.isCheckValAcc("newtransportmode", plant,USERID);

}
if(systatus.equalsIgnoreCase("INVENTORY"))
{
	displaySummaryDelete = ub.isCheckValinv("removetransportmode", plant,USERID);
displaySummaryNew = ub.isCheckValinv("newtransportmode", plant,USERID);
}
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Edit Transport Mode</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->     
            <div class="box-header menu-drop">
              ...
              <button type="button" class="btn btn-default" onclick="window.location.href='../transportmode/new'" style="background-color: rgb(214, 72, 48);color: rgb(255, 255, 255);" >+ New</button>&nbsp;
               <%} %>
              </div>
		</div>
...
<button type="button" class="Submit btn btn-default" value="Remove From Transport Mode" onClick="onDelete()">Remove From Transport Mode</button>&nbsp;&nbsp;
 <%} %>	
	</div>
	 </div>
 ...
 function checkAll(isChk)
{
  ...
  $.ajax({type: "POST",url: urlStr, data: {PLANT:"<%=plant%>",action: "VIEW_TRANSPORT_MODE_EDIT",PLANT:"<%=plant%>"},dataType: "json", success: callback });
...
function onDelete()
{
	 ...
	 for (var i = 0; i < len ; i++)
   {
	 ...
	 var chkmsg = confirm("Are you sure you would like to delete?");
		if (chkmsg) {
	  	  document.form.action="/track/MasterServlet?action=DELETE_TRANSPORT_MODE";
