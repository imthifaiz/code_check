...
<%
	String title = "Trial Balance Account Transactions";
	...
	if(fromDate != ""){
		...
		fromdateString=fromdatetime.format(DateTimeFormatter.ofPattern("dd-MMM-yyyy"));
	}
	if(toDate != ""){
		LocalDate todatetime=LocalDate.parse(toDate);
		todateString=todatetime.format(DateTimeFormatter.ofPattern("dd-MMM-yyyy"));
	}
	...
	CoaDAO coaDAO=new CoaDAO();
	 net.sf.json.JSONObject coaFullDetail = coaDAO.CoaFullRecord(plant,String.valueOf(account));
	 net.sf.json.JSONObject coaJson = ( net.sf.json.JSONObject) coaFullDetail.get("results");
	String actnme= coaJson.getString("account_name");
	String actcode = coaDAO.GetAccountCodeByName(actnme, plant);
%>
...
<div class="container-fluid m-t-20">
    ...
    <thead>
      <tr>
        ...
        <th class="text-center">TRANSACTION_DETAILS</th>
    ...
    <tbody>
     <tr>
    	 ...
    	 <td></td>
    	 <td></td>
    	 <td class="text-right"><%=curency%> <%=Numbers.toMillionFormat(0.00, Integer.parseInt(numberOfDecimal))%></td>
    	 <td></td>
    	 </tr>
    	<%for(TrialBalanceDetails trialBalDet:trialBalanceDetails) 
    	{
    		...
    		String accountcode = "";
    		if(trialBalDet.getACCOUNT().length() > 0){
    			accountcode = coaDAO.GetAccountCodeByName(trialBalDet.getACCOUNT(), plant);
    		}
    	%>
      <tr>
       ...
       <%
        ...
        <td class="text-right"><%=Numbers.toMillionFormat(trialBalDet.getDEBIT(), Integer.parseInt(numberOfDecimal))%></td>        
        <td class="text-right"><%=Numbers.toMillionFormat(trialBalDet.getCREDIT(), Integer.parseInt(numberOfDecimal))%></td>
        <%Double amount=trialBalDet.getDEBIT()-trialBalDet.getCREDIT();
        if(amount>0){ %>        
        <td class="text-right"><%=Numbers.toMillionFormat(amount, Integer.parseInt(numberOfDecimal))%>Dr</td>
        <%}else{%>
         <td class="text-right"><%=Numbers.toMillionFormat(Math.abs(amount), Integer.parseInt(numberOfDecimal))%>Cr</td>
        <%} %>
      </tr>
      <%} %>
      <tr>
    	 ...
    	 <td></td>
    	 <td></td>
    	 <td class="text-right"><%=Numbers.toMillionFormat(totalDebit, Integer.parseInt(numberOfDecimal))%></td>
    	 <td class="text-right"><%=Numbers.toMillionFormat(totalCredit, Integer.parseInt(numberOfDecimal))%></td>
    	 <td></td>
    	 </tr>
      <tr>
      <% totalClosingBalance=(totalDebit-totalCredit);%>
        ...
        <td class="text-right"><%=curency%><%=Numbers.toMillionFormat(totalClosingBalance, Integer.parseInt(numberOfDecimal))%></td>
        <%}else{%>
        <td></td>
        <%} %>
        <% if(totalClosingBalance<0){ %>
         <td class="text-right"><%=curency%> <%=Numbers.toMillionFormat(Math.abs(totalClosingBalance), Integer.parseInt(numberOfDecimal))%></td>
        <%}else{%>
        <td></td>
        <%} %>
    	 <td></td>
...
{     
	...
	var accNameSelected = "<%=actnme %>";

	window.location = "/track/TrialBalanceServlet?action=getTrialBalanceTransactionDetails&fromDate="+fromdate+"&toDate="+todate+"&account="+accountSelected+"&accname="+accNameSelected;
}
function generate() {
...
function generatePdf(dataUrl){	
	...
	doc.setFontSize(12);
	doc.text('<%=accountName%>',cwidth,50,'center');
	
	var ftdate = 'From '+'<%=fromdateString%>'+' to '+'<%=todateString%>';
	doc.text(ftdate,cwidth,57,'center');
	<%-- doc.text('From ',83,57);
