	...
	String pdaInHouse[] = request.getParameterValues("PdaInHouse");
 	...
 	if (action.equalsIgnoreCase("delete")) {
 		...
 		if (n == -2) //  User available for selected level
 		{
 			...
 			response.sendRedirect("GroupSummaryAccounting.jsp?RESULT=The '"+ level_name+ "' User Level has been successfully deleted from database");
 		}
 			...
 			&& (!action.equalsIgnoreCase("update"))) //  Checking if the level name exists already
 	{
		...
		if(pdaInHouse!=null)
 		...
 		if (b) {
 			 response.sendRedirect("GroupSummaryAccounting.jsp?RESULT=The User Level "+level_name+" has been Updated Sucessfully");
