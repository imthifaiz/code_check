...
<h3><%!String okBtn, nextPage;%> <%
	...
	String ProductReturnReceive[] = request.getParameterValues("ProductReturnReceive");
 	...
 	String rentalTransaction[] = request.getParameterValues("RentalTransaction");//used old code imthi
 	...
 	String integrationsShoppingCart[] = request.getParameterValues("integrations_shopping_cart");
 	String integrationsECommerce[] = request.getParameterValues("integrations_ecommerce");
 	String activityLogs[] = request.getParameterValues("ActivityLogs");
 	String OwnerApp[] = request.getParameterValues("OwnerApp");
	...
	String pdaInHouse[] = request.getParameterValues("PdaInHouse");
 	...
 	if (action.equalsIgnoreCase("delete")) {
 		int n = ub.deletePreviousRecords(level_name, "1", company);
 		...
 		result = " <font class=mainred> No levels selected to add/update the database..."
 				+ nextPage;
 	} */ else if ((ub.isAlreadyAvail(level_name, 1, company))
 			&& (!action.equalsIgnoreCase("update"))) //  Checking if the level name exists already
 	{
		...
		if(integrationsECommerce!=null)
 		{
 		 for (int i = 0; i < integrationsECommerce.length; i++) {
 			url_name = integrationsECommerce[i];
 			createdOn = gn.getDateTime();
 			updatedOn = createdOn;
		...
		if(integrationsShoppingCart!=null)
 		{
 		 for (int i = 0; i < integrationsShoppingCart.length; i++) {

 			url_name = integrationsShoppingCart[i];
			 System.out.println("integrationsShoppingCart : " + url_name);
			 createdOn = gn.getDateTime();
 			updatedOn = createdOn;
		...
		if(OwnerApp!=null)
 		{
 		 for (int i = 0; i < OwnerApp.length; i++) {
 			url_name = OwnerApp[i];
 			createdOn = gn.getDateTime();
 			updatedOn = createdOn;
		...
		if(pdaInHouse!=null)
 		...
 		if (action.equalsIgnoreCase("update")) {
 			...
 			int n = ub.deletePreviousRecords(level_name, "0", company);
 			if (n > 0) {
 		...
 		if (b) {
 			 response.sendRedirect("../useraccess/invsummary?RESULT=The User Level "+level_name+" has been Updated Sucessfully");
