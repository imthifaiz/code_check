...
<%
    	...
    	for(int i =0; i<arrCust.size(); i++) {
            ...
            String sCustName1     = strUtils.replaceCharacters2Send((String)arrCustLine.get("VNAME"));
            String isactive    = strUtils.removeQuotes((String)arrCustLine.get("ISACTIVE"));
            String sTAXTREATMENT=strUtils.removeQuotes((String)arrCustLine.get("TAXTREATMENT"));
%>
   	<TR>
		<td class="main2" style="font-weight: bold" align="left">&nbsp;
			<a href="#" onClick="setSupplierData('<%=sCustCode%>','<%=sCustName1%>','<%=sTAXTREATMENT%>');"><%=sCustCode%>
           	</a>
        </td>
		<td align="left" class="main2">&nbsp;<%=strUtils.replaceCharacters2Recv(sCustName1)%></td>
		<td align="left" class="main2">&nbsp;<%=isactive%></td>
...
function setSupplierData(vendno, vendname, sTAXTREATMENT){
	var suppierData = new Object();
	suppierData.vendno = vendno;
	suppierData.vendname = vendname;
	suppierData.sTAXTREATMENT = sTAXTREATMENT;
	window.opener.setSupplierData(suppierData);
	window.close();
}
</script>