...
function onNew(){
   ...
   /* document.form.action  = "vendor_view.jsp?action=Clear";
...
/* function onUpdate(){
 ...
 return false;
   ...
   if(TAXTREATMENT =="GCC VAT Registered"||TAXTREATMENT=="VAT Registered")
   {
   	if(RCBNO == "" || RCBNO == null) {
   		var  d = document.getElementById("TaxByLabel").value;
	   alert("Please Enter "+d+" No."); 
   ...
   document.form.action  = "vendor_view.jsp?action=UPDATE";
   document.form.submit();
}
function onDelete(){
   var CUST_CODE   = document.form.CUST_CODE.value;
   if(CUST_CODE == "" || CUST_CODE == null) {alert("Please Enter Supplier ID");  return false; }
confirm("Are you sure to delete Supplier permanently ");
   document.form.action  = "vendor_view.jsp?action=DELETE";
   document.form.submit();
} 
function onView(){
   var CUST_CODE   = document.form.CUST_CODE.value;
   ...
   document.form.action  = "vendor_view.jsp?action=VIEW";
   document.form.submit();
} */
function onIDGen()
{
	...
	$.ajax({
		type: "GET",
		url: "../supplier/Auto-ID",
		dataType: "json",
		beforeSend: function(){
			showLoader();
		}, 
		success: function(data) {
			$("#CUST_CODE").val(data.SUPPLIER);
		},
		error: function(data) {
			alert('Unable to generate Supplier ID. Please try again later.');
		},
		complete: function(){
			hideLoader();
		}
...
TblControlDAO _TblControlDAO = new TblControlDAO();
	...
	String action = "";
	String sCustCode = "", sCustName = "", sCustNameL = "",companyregnumber="",sAddr1 = "", sAddr2 = "", sAddr3 = "", sAddr4 = "", sCountry = "", sZip = "", sCons = "Y";
	String sContactName = "", sDesgination = "", sTelNo = "", sHpNo = "", sFax = "", sEmail = "",sRcbno="", 
			sRemarks = "",sPayTerms="",sPayMentTerms="",sPayInDays="",suppliertypeid="",supplier_type_id="",transport="",desc="",sState="",sTAXTREATMENT="";
	String CUSTOMEREMAIL="",WEBSITE="",FACEBOOK="",TWITTER="",CURRENCY="",LINKEDIN="",SKYPE="",OPENINGBALANCE="",WORKPHONE="",sSAVE_RED,PEPPOL="",PEPPOL_ID="";
	...
	transport=strUtils.fString(request.getParameter("TRANSPORTID"));//Resvi
...
/* 	else if (action.equalsIgnoreCase("Auto-ID")) {
		...
		try {
			...
			mLogger.exception(true,
					"ERROR IN JSP PAGE - vender_view.jsp ", e);
		}
	} */
	//2. >> Add
		...
		boolean updateFlag;
			...
			if (custInserted) {
				...
				sSAVE_RED="Supplier Added Successfully";
			//	res = "<font class = " + IConstants.SUCCESS_COLOR+ ">Supplier Added Successfully</font>";
				//                    sAddEnb  = "disabled";
				sCustEnb = "disabled";
			} else {
				sSAVE_RED="Failed to add New Supplier";
				/* res = "<font class = " + IConstants.FAILED_COLOR+ ">Failed to add New Supplier</font>"; */
				//                    sAddEnb  = "enabled";
				sCustEnb = "enabled";
			}
		} else {
			sSAVE_RED="Supplier ID Or Name Exists already. Try again with diffrent Supplier ID Or Name";
	...
	//3. >> Update
		...
		if (custUtil.isExistVendor(sCustCode, plant)) {
			...
			htUpdate.put(IConstants.payment_terms, sPayMentTerms);
			...
			boolean inserted = mdao.insertIntoMovHis(htm);
			if (custUpdated) {
				sSAVE_RED="Supplier Updated Successfully";
...
<div class="container-fluid m-t-20">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Create Supplier</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->     
 ...
 <%--  <input name="SUPPLIER_TYPE_DESC" type="hidden" value="<%=desc%>"> --%>
   <input type="number" id="numberOfDecimal" style="display:none;" value=<%=numberOfDecimal%>>
    <div class="form-group">
      <label class="control-label col-form-label col-sm-2 required" for="Create Supplier ID">Supplier Id</label>
      <div class="col-sm-4">
      <div class="input-group">   
      	  	 <INPUT class="form-control" name="CUST_CODE" id="CUST_CODE" type="TEXT" value="<%=sCustCode%>" onchange="checkitem(this.value)" size="50" MAXLENGTH=50 onkeypress="return blockSpecialChar(event)" width="50"<%=sCustEnb%>> 
    ...
    <div class="form-group" id="UEN">
	 ...
	 <div class="form-group">
      <label class="control-label col-form-label col-sm-2" for="supplier Grou">Supplier Group</label>
      <div class="col-sm-4">           	
  	      	<div class="input-group">    
			...
			size="50" MAXLENGTH=50 class="form-control">
   		 	<span class="select-icon"  onclick="$(this).parent().find('input[name=\'SUPPLIER_TYPE_DESC\']').focus()"> 	
    ...
    <div class="form-group">
      ...
      <div class="col-sm-4">
    	...
    	<div class="form-group">
      	...
      	<div class="col-sm-4">  
        <INPUT  name="WORKPHONE" type="TEXT" value="<%=WORKPHONE%>" onkeypress="return validateInput(event)"	size="50" MAXLENGTH=30 class="form-control">
      	</div>
    	</div>
...
<!-- 				<OPTION style="display:none;">Select Bank</OPTION> -->
	...
	<i class="glyphicon glyphicon-menu-down"></i></span>
		...
		<div class="form-group">
      	...
      	<div class="col-sm-4">  
        <INPUT name="BRANCH" id="BRANCH" type="TEXT" value="<%=sBRANCH%>"	size="50" MAXLENGTH=100 class="form-control" readonly>
      	</div>
    	</div>
...
<%@include file="../jsp/newTransportModeModal.jsp"%> <!-- Resvi Add for add transport -->
...
<input type="hidden" name="Basecurrency" id="Basecurrency" style="display: none;" value=<%=curency%>>
 ...
 $(document).ready(function(){
	 /* Bank Name Auto Suggestion */
		...
		{
		document.form.action  = "../supplier/summary?PGaction=View&result=Supplier Added Successfully";
		document.form.submit();
		}
    ...
    function add_attachments(){
        ...
        $.ajax({
            ...
            //	window.location.reload();
            	document.form.action  = "../jsp/vendor_view.jsp?action=ADD";
             document.form.submit();
            },
            error: function (data) {
