...
<!-- END -->
<%
String title = "Inventory Summary With Total Quantity With PCS";
%>
...
<div class="container-fluid m-t-20">
	<div class="box">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Inventory Summary With Total Quantity With PCS</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->    
...
<div class="box-body">
<FORM class="form-horizontal" name="form1" method="post" action="viewInventoryGroupByProd.jsp">
...
<!--   		<div class="form-inline"> -->
  		<!-- <label class="control-label col-sm-2" for="supplier name"> </label> -->
        <div class="col-sm-4 ac-box">
...
<!-- 				<span class="btn-danger input-group-addon" onClick="javascript:popUpWin('../jsp/LocType2List.jsp?LOC_TYPE_ID2='+form1.LOC_TYPE_ID2.value);"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></span>  -->
  	 ...
  	 <div class="form-group">
     <label class="control-label col-sm-2" for="Loc">  </label>
        <div class="col-sm-4 ac-box">
...
<!-- 									role="grid" aria-describedby="tableInventorySummary_info"> -->
					<thead>
		                <tr role="row">
		                	...
		                	<th style="background-color: #eaeafa; color:#333; text-align: center;">PCS/EA/PKT/KG/LT UOM</th>
  ...
  var productId, desc, loc, prdBrand, prdDep,prdClass, prdType, locType,locType2,locType3,model, groupRowColSpan = 6;
function getParameters(){
	return { 
		...
		"MODEL": model,
		"ACTION": "VIEW_INV_SUMMARY_GROUPBY_PRD",
		"PLANT":"<%=PLANT%>"
	}
}  

function storeUserPreferences(){
	storeInLocalStorage('viewInventoryGroupByProd_ITEM', $('#ITEM').val());
	storeInLocalStorage('viewInventoryGroupByProd_PRD_DEPT_ID', $('#PRD_DEPT_ID').val());
	storeInLocalStorage('viewInventoryGroupByProd_PRD_CLS_ID', $('#PRD_DEPT_ID').val());
	storeInLocalStorage('viewInventoryGroupByProd_PRD_TYPE_ID', $('#PRD_TYPE_ID').val());
	storeInLocalStorage('viewInventoryGroupByProd_PRD_BRAND_ID', $('#PRD_BRAND_ID').val());
	storeInLocalStorage('viewInventoryGroupByProd_LOC', $('#LOC').val());
	storeInLocalStorage('viewInventoryGroupByProd_LOC_TYPE_ID', $('#LOC_TYPE_ID').val());
	storeInLocalStorage('viewInventoryGroupByProd_LOC_TYPE_ID2', $('#LOC_TYPE_ID2').val());
	storeInLocalStorage('viewInventoryGroupByProd_LOC_TYPE_ID3', $('#LOC_TYPE_ID3').val());
	storeInLocalStorage('viewInventoryGroupByProd_MODEL', $('#MODEL').val());
}
function onGo(){
...
// 			"lengthMenu": [[100, 500, 1000, 2500], [100, 500, 1000, "All"]],
	        ...
	        "footerCallback": function(row, data, start, end, display) {
	          ...
	          $(api.column(7).footer()).html(parseFloat(totalQty).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>));
	          }/**/
		});
...
$(document).ready(function(){
    ...
    if(ALLOWCATCH_ADVANCE_SEARCH==='1'){
	   if (document.form1.ITEM.value == ''){
		   getLocalStorageValue('viewInventoryGroupByProd_ITEM','', 'ITEM');
		 } 	
	   if (document.form1.PRD_CLS_ID.value == ''){
		   getLocalStorageValue('viewInventoryGroupByProd_PRD_CLS_ID','', 'PRD_CLS_ID');
		} 	
	   if (document.form1.PRD_DEPT_ID.value == ''){
		   getLocalStorageValue('viewInventoryGroupByProd_PRD_DEPT_ID','', 'PRD_DEPT_ID');
		} 	
	   if (document.form1.PRD_TYPE_ID.value == ''){
		   getLocalStorageValue('viewInventoryGroupByProd_PRD_TYPE_ID','', 'PRD_TYPE_ID');
		}	
	   if (document.form1.PRD_BRAND_ID.value == ''){
		   getLocalStorageValue('viewInventoryGroupByProd_PRD_BRAND_ID','', 'PRD_BRAND_ID');
		 }	
	   if (document.form1.LOC.value == ''){
			getLocalStorageValue('viewInventoryGroupByProd_LOC','','LOC');
		}
	   if (document.form1.LOC_TYPE_ID.value == ''){
			getLocalStorageValue('viewInventoryGroupByProd_LOC_TYPE_ID','','LOC_TYPE_ID');
		}
	   if (document.form1.LOC_TYPE_ID2.value == ''){
			getLocalStorageValue('viewInventoryGroupByProd_LOC_TYPE_ID2','','LOC_TYPE_ID2');
		}
	   if (document.form1.LOC_TYPE_ID3.value == ''){
		   getLocalStorageValue('viewInventoryGroupByProd_LOC_TYPE_ID3','','LOC_TYPE_ID3');
		}
	   if (document.form1.MODEL.value == ''){
		   getLocalStorageValue('viewInventoryGroupByProd_MODEL','','MODEL');
		}
    }
// 	$('#table').dataTable({
