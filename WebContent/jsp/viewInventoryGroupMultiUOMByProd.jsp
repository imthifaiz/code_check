...
<!-- END -->
<%
String title = "Inventory Summary With Total Quantity (group by product)";
%>
...
<div class="container-fluid m-t-20">
	<div class="box">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Inventory Summary With Total Quantity (group by product)</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 -->    
...
<!-- 								<th style="text-align: left !important">Total</th> -->
  ...
  var productId, desc, loc, prdBrand, prdClass,prdDep,prdType, locType,locType2,locType3,model,uom,SORT, groupRowColSpan = 5;
...
// 			"lengthMenu": [[100, 500, 1000, 2500], [100, 500, 1000, "All"]],
			...
			"drawCallback": function ( settings ) {
	            ...
	            api.column(groupColumn, {page:'current'} ).data().each( function ( group, i ) {
	                ...
	                groupTotalQty += parseFloat($(rows).eq( i ).find('td:eq(6)').html());
	                totalQty += parseFloat($(rows).eq( i ).find('td:eq(6)').html());
	                groupTotalQty1 += parseFloat($(rows).eq( i ).find('td:eq(8)').html());
	                totalQty1 += parseFloat($(rows).eq( i ).find('td:eq(8)').html());
	                currentRow = i;
	            } );
	            if (groupEnd > 0 || rows.length == (currentRow + 1)){
	            	 $(rows).eq( currentRow ).after(
	                        '<tr class="group"><td colspan=' + groupRowColSpan + '></td><td>Grand Total</td><td class="t-right">' + parseFloat(totalQty).toFixed(3) + '</td><td></td><td class="t-right">' + parseFloat(totalQty1).toFixed(3) + '</td></tr>'
                    ); 
                	$(rows).eq( currentRow ).after(
	                       // '<tr class="group"><td colspan=' + groupRowColSpan + '></td><td></<td><td></<td><td>Total</td><td class="t-right">' + parseFloat(groupTotalQty1).toFixed(3) + '</td><td></td><td class="t-right">' + parseFloat(groupTotalQty).toFixed(3) + '</td></tr>'
                    );
	        ...
	        "footerCallback": function(row, data, start, end, display) {
	            ...
	            // Update footer
	          <%--   $(api.column(6).footer()).html(parseFloat(totalOrdVal).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>));
	            $(api.column(8).footer()).html(parseFloat(totalPicVal).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>)); --%>
	            $(api.column(6).footer()).html(parseFloat(totalQty1).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>));
	            $(api.column(8).footer()).html(parseFloat(groupTotalQty1).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>));
