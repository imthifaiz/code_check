...
<!-- END -->
<%
String title = "Inventory Summary With Min/Max/Zero Quantity";
%>
...
String USERID="",PLANT="",LOC ="",  ITEM = "", BATCH="",PRD_TYPE_ID="",PRD_BRAND_ID="",PRD_DESCRIP="", STATUS ="",QTY ="",PRD_CLS_ID="",PRD_CLS_ID1="",
        LOC_TYPE_ID="", LOC_TYPE_ID2="",PRD_DEPT_ID="",LOC_TYPE_ID3="",VIEWSTATUS="",TOPUPBY="";
double Total=0;
...
<!-- 									role="grid" aria-describedby="tableInventorySummary_info"> -->
					<thead>
		                <tr role="row">		               
            				...
            				<th style="font-size: smaller;">INV.MAX QTY</th>
...
<!--           <TH style="background-color: #eaeafa; color:#333; text-align: right;">Inventory Quantity</TH> -->
  ...
  var productId, desc, loc, prdBrand,prdDept, prdClass, prdType, loctype,loctype2,loctype3,viewstatus,uom,TOPUPBY, groupRowColSpan = 8;
...
// 			"lengthMenu": [[100, 500, 1000, 2500], [100, 500, 1000, "All"]],
			...
			"drawCallback": function ( settings ) {
	            ...
	            api.column(groupColumn, {page:'current'} ).data().each( function ( group, i ) {
	                if ( last !== group ) {
	                	if (i > 0) {
	                		if (parseFloat(groupTotalQty) < parseFloat(groupMinQty) / parseFloat(groupRowCount)){
		                		$(rows).eq( i ).before(
				                        '<tr class="group"><td colspan="' + groupRowColSpan + '"></td><td></<td><td></<td><td>Total</td><td class="t-right" style="color:red;">' + parseFloat(groupTotalPCSQty).toFixed(3) + '</td><td></td><td></td><td></td><td class="t-right" style="color:red;">' + parseFloat(groupTotalQty).toFixed(3) + '</td><td></td></tr>'
				                    );
	                		}else{
		                		$(rows).eq( i ).before(
				                        '<tr class="group"><td colspan="' + groupRowColSpan + '"></td><td></<td><td></<td><td>Total</td><td class="t-right">' + parseFloat(groupTotalPCSQty).toFixed(3) + '</td><td></td><td></td><td></td><td class="t-right">' + parseFloat(groupTotalQty).toFixed(3) + '</td><td></td></tr>'
				                    );
	                		}
	                	}
	                    last = group;
	                ...
	                groupMinPCSQty += parseFloat($(rows).eq( i ).find('td:eq(9)').html());
	                groupTotalQty += parseFloat($(rows).eq( i ).find('td:eq(15)').html());
	                groupMinQty += parseFloat($(rows).eq( i ).find('td:eq(13)').html());
	                groupRowCount += 1;
	                totalPCSQty+= parseFloat($(rows).eq( i ).find('td:eq(11)').html());
	                totalQty += parseFloat($(rows).eq( i ).find('td:eq(15)').html());
	                currentRow = i;
	            } );
	            if (groupEnd > 0 || rows.length == (currentRow + 1)){
	            	$(rows).eq( currentRow ).after(
	                        '<tr class="group"><td colspan=' + groupRowColSpan + '></td><td></<td><td></<td><td>Grand Total</td><td class="t-right">' + parseFloat(totalPCSQty).toFixed(3) + '</td><td></td><td></td><td></td>d><td class="t-right">' + parseFloat(totalQty).toFixed(3) + '</td><td></td></tr>'
                    );
	            	if (parseFloat(groupTotalQty) < parseFloat(groupMinQty) / parseFloat(groupRowCount)){
	            		$(rows).eq( currentRow ).after(
		                        '<tr class="group"><td colspan="' + groupRowColSpan + '"></td><td></<td><td></<td><td>Total</td><td class="t-right" style="color:red;">' + parseFloat(groupTotalPCSQty).toFixed(3) + '</td><td></td><td></td><td></td><td class="t-right" style="color:red;">' + parseFloat(groupTotalQty).toFixed(3) + '</td><td></td></tr>'
	        ...
	        "footerCallback": function(row, data, start, end, display) {
	           ...
	           <%--  $(api.column(11).footer()).html(parseFloat(totalQty).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>));
	            $(api.column(15).footer()).html(parseFloat(totalCostVal).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>)); --%>
	            
	            $(api.column(11).footer()).html(parseFloat(groupTotalPCSQty).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>));
	            $(api.column(15).footer()).html(parseFloat(groupTotalQty).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>));
	          } 
		});
