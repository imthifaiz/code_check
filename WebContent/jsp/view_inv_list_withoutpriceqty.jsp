...
<head> -->
<%
String title = "Inventory Summary With Batch/Sno With PCS";
%>
...
function ExportReport(){
	document.form1.action = "/track/ReportServlet?action=Export_Inv_Reports&INV_REP_TYPE=invWithBatch";
...
<div class="container-fluid m-t-20">
	<div class="box">
	 ...
	 <ul class="breadcrumb backpageul">    
                ...
                <li><label>Inventory Summary with Batch/Sno With PCS</label></li>                                   
            </ul>
        <!-- Thanzith Modified on 23.02.2022 --> 
...
<!-- 		<span class="btn-danger input-group-addon" onClick="javascript:popUpWin('../jsp/activeproductTypeList.jsp?ITEM_ID='+form1.PRD_TYPE_ID.value);"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></span>  -->
    	...
    	<!-- <span class="input-group-addon"  onClick="javascript:popUpWin('activeproductTypeList.jsp?ITEM_ID='+form1.PRD_TYPE_ID.value);">
  		...
  		<!-- <label class="control-label col-sm-2" for="PRODUCT SUB CATEGORY ID">PRODUCT SUB CATEGORY ID:</label> -->
    <div class="col-sm-3 ac-box">
...
<!-- 		<span class="btn-danger input-group-addon" onClick="javascript:popUpWin('../jsp/loc_list_inventory.jsp?LOC='+form1.LOC.value);"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></span>  -->
       ...
       <div class="col-sm-3 ac-box">
        <INPUT class="form-control" name="LOC_TYPE_ID" ID="LOC_TYPE_ID" type="TEXT" value="<%=LOC_TYPE_ID%>"	placeholder="LOCATION TYPE ONE" size="35" MAXLENGTH=20>
...
<!-- 		<span class="btn-danger input-group-addon" onClick="javascript:popUpWin('../jsp/LocTypeList.jsp?LOC_TYPE_ID='+form1.LOC_TYPE_ID.value);"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></span>  -->
    	...
    	<!-- <span class="input-group-addon"  onClick="javascript:popUpWin('LocTypeList.jsp?LOC_TYPE_ID='+form1.LOC_TYPE_ID.value);">
 		   ...
 		   <div class="col-sm-3 ac-box">
        <INPUT class="form-control" name="LOC_TYPE_ID2" ID="LOC_TYPE_ID2" type="TEXT" value="<%=LOC_TYPE_ID2%>" placeholder="LOCATION TYPE TWO" size="35" MAXLENGTH=20>
...
<!-- 		<span class="btn-danger input-group-addon" onClick="javascript:popUpWin('../jsp/LocType2List.jsp?LOC_TYPE_ID2='+form1.LOC_TYPE_ID2.value);"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></span>  -->
  	...
  	<!-- <div class="form-inline"> -->
       ...
       <div class="col-sm-3 ac-box">
        <INPUT class="form-control" name="LOC_TYPE_ID3" ID="LOC_TYPE_ID3" type="TEXT" value="<%=LOC_TYPE_ID3%>" placeholder="LOCATION TYPE THREE" size="35" MAXLENGTH=20>
...
<!-- 		<span class="btn-danger input-group-addon" onClick="javascript:popUpWin('../jsp/LocType2List.jsp?LOC_TYPE_ID2='+form1.LOC_TYPE_ID2.value);"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></span>  -->
	...
	<div class="form-inline"> -->
 		...
 		<div class="form-group">
  		...
  		<div class="col-sm-3 ac-box">
    		<INPUT class="form-control" name="BATCH" id="BATCH" type = "TEXT" value="<%=BATCH%>" placeholder="BATCH" size="30"  MAXLENGTH=40>
...
<!--    		 	<span class="btn-danger input-group-addon" onClick="javascript:popUpWin('../jsp/batch_list_inventory.jsp?BATCH='+form1.BATCH.value);"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></span>  -->
  		...
  		<div class="col-sm-3 ac-box">
        <INPUT class="form-control" name="MODEL" id="MODEL" type="TEXT" value="<%=MODEL%>" placeholder="MODEL"	size="35" MAXLENGTH=20>
    	</div>
...
<!--           <TH style="background-color: #eaeafa; color:#333; text-align: center;">Quantity</TH> -->
  ...
  var productId, desc, loc, prdBrand,prdDept, prdClass, prdType, loctype,loctype2,batch,model, groupRowColSpan = 7;
function getParameters(){
	return { 
		...
		"MODEL":model,
		"ACTION": "VIEW_INV_SUMMARY_WO_PRICE",
		"PLANT":"<%=PLANT%>"
	}
}  

function storeUserPreferences(){
	storeInLocalStorage('view_inv_list_withoutpriceqty_ITEM', $('#ITEM').val());
	storeInLocalStorage('view_inv_list_withoutpriceqty_PRD_CLS_ID', $('#PRD_CLS_ID').val());
	storeInLocalStorage('view_inv_list_withoutpriceqty_PRD_DEPT_ID', $('#PRD_DEPT_ID').val());
	storeInLocalStorage('view_inv_list_withoutpriceqty_PRD_TYPE_ID', $('#PRD_TYPE_ID').val());
	storeInLocalStorage('view_inv_list_withoutpriceqty_PRD_BRAND_ID', $('#PRD_BRAND_ID').val());
	storeInLocalStorage('view_inv_list_withoutpriceqty_LOC', $('#LOC').val());
	storeInLocalStorage('view_inv_list_withoutpriceqty_LOC_TYPE_ID', $('#LOC_TYPE_ID').val());
	storeInLocalStorage('view_inv_list_withoutpriceqty_LOC_TYPE_ID2', $('#LOC_TYPE_ID2').val());
	storeInLocalStorage('view_inv_list_withoutpriceqty_LOC_TYPE_ID3', $('#LOC_TYPE_ID3').val());
	storeInLocalStorage('view_inv_list_withoutpriceqty_BATCH', $('#BATCH').val());
	storeInLocalStorage('view_inv_list_withoutpriceqty_MODEL', $('#MODEL').val());
}
...
// 			"lengthMenu": [[100, 500, 1000, 2500], [100, 500, 1000, "All"]],
	        ...
	        "footerCallback": function(row, data, start, end, display) {
	            ...
	            // Update footer
	            <%-- $(api.column(8).footer()).html(parseFloat(totalOrdVal).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>)); --%>
	            $(api.column(8).footer()).html(parseFloat(groupTotalQty).toFixed(<%=DbBean.NOOFDECIMALPTSFORWEIGHT%>));
	          }
		});
...
$(document).ready(function(){
    ...
    if(ALLOWCATCH_ADVANCE_SEARCH==='1'){
	   if (document.form1.ITEM.value == ''){
		   getLocalStorageValue('view_inv_list_withoutpriceqty_ITEM','', 'ITEM');
		 } 	
	   if (document.form1.PRD_CLS_ID.value == ''){
		   getLocalStorageValue('view_inv_list_withoutpriceqty_PRD_CLS_ID','', 'PRD_CLS_ID');
		} 	

	   if (document.form1.PRD_DEPT_ID.value == ''){
		   getLocalStorageValue('view_inv_list_withoutpriceqty_PRD_DEPT_ID','', 'PRD_DEPT_ID');
		} 	
	   if (document.form1.PRD_TYPE_ID.value == ''){
		   getLocalStorageValue('view_inv_list_withoutpriceqty_PRD_TYPE_ID','', 'PRD_TYPE_ID');
		}	
	   if (document.form1.PRD_BRAND_ID.value == ''){
		   getLocalStorageValue('view_inv_list_withoutpriceqty_PRD_BRAND_ID','', 'PRD_BRAND_ID');
		 }	
	   if (document.form1.LOC.value == ''){
			getLocalStorageValue('view_inv_list_withoutpriceqty_LOC','','LOC');
		}
	   if (document.form1.LOC_TYPE_ID.value == ''){
			getLocalStorageValue('view_inv_list_withoutpriceqty_LOC_TYPE_ID','','LOC_TYPE_ID');
		}
	   if (document.form1.LOC_TYPE_ID2.value == ''){
			getLocalStorageValue('view_inv_list_withoutpriceqty_LOC_TYPE_ID2','','LOC_TYPE_ID2');
		}
	   if (document.form1.LOC_TYPE_ID3.value == ''){
		   getLocalStorageValue('view_inv_list_withoutpriceqty_LOC_TYPE_ID3','','LOC_TYPE_ID3');
		}
	   if (document.form1.MODEL.value == ''){
		   getLocalStorageValue('view_inv_list_withoutpriceqty_MODEL','','MODEL');
		}
	   if (document.form1.BATCH.value == ''){
		   getLocalStorageValue('view_inv_list_withoutpriceqty_BATCH','','BATCH');
		}
    }
    onGo();
