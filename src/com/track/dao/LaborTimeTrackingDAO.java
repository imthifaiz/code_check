...
public class LaborTimeTrackingDAO extends BaseDAO {
	private MLogger mLogger = new MLogger();
	private boolean printQuery = MLoggerConstant.LaborTimeTrackingDAO_PRINTPLANTMASTERQUERY;
	private boolean printLog = MLoggerConstant.LaborTimeTrackingDAO_PRINTPLANTMASTERLOG;
	...
	public LaborTimeTrackingDAO() {

	}

	public boolean insertIntoLaborTimeTracking(Hashtable ht) throws Exception {
	...
	public boolean isExists(Hashtable ht) throws Exception {
		...
		try {
			...
			sql.append(" 1 ");
			sql.append(" ");
			sql.append(" FROM " + "[" + ht.get("PLANT") + "_" + "LABOR_TIME_TRACKING"
					+ "]");
	...
	public boolean updateLaborTimeTracking(Hashtable htUpdate, Hashtable htCondition)
			throws Exception {
		...
		try {
			...
			String stmt = "UPDATE " + "[" + htCondition.get("PLANT") + "_"
					+ "LABOR_TIME_TRACKING" + "]" + sUpdate + sCondition;
	...
	public boolean deleteLaborTimeTracking(java.util.Hashtable ht) throws Exception {
	...
	public ArrayList getLaborTimeTrackingDetails(String event, String plant,
		...
		try {
			...
			String sQry = "select distinct EVENT_ID,ISNULL(EVENT_DESC,'')EVENT_DESC,ISNULL(TYPE,'')TYPE,ISNULL(REMARKS,'')REMARKS from "
					...
					+ plant
					+ "_"
					+ "LABOR_TIME_TRACKING] where EVENT_ID like '"
					+ event
					+ "%'   " + extraCon
					+ " ORDER BY EVENT_ID ";
