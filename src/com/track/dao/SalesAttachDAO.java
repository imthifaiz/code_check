...
public class SalesAttachDAO  extends BaseDAO {
	private MLogger mLogger = new MLogger();
	private boolean printQuery = MLoggerConstant.SalesAttachDAO_PRINTPLANTMASTERQUERY;
	private boolean printLog = MLoggerConstant.SalesAttachDAO_PRINTPLANTMASTERLOG;
	...
	public boolean addsalesAttachments(List<Hashtable<String, String>>  salesAttachmentList, String plant)throws Exception {
		...
		try {		
			...
			for (Hashtable<String, String> empDetInfo : salesAttachmentList) {
				...
				query = "INSERT INTO ["+ plant +"_SALESORDERATTACHMENTS]  ("
	...
	public List getsalesAttachByDONO(String plant, String eid) throws Exception {
		...
		try {
			...
			query = "SELECT * FROM"
					+ "[" + plant + "_SALESORDERATTACHMENTS] WHERE DONO = ? AND PLANT =? ";
	...
	public List getsalesAttachById(String plant, String eid) throws Exception {
		...
		try {
			...
			con = com.track.gates.DbBean.getConnection();
			query = "SELECT * FROM"
					+ "[" + plant + "_SALESORDERATTACHMENTS] WHERE ID = ? AND PLANT =? ";
			PreparedStatement ps = con.prepareStatement(query);
	 ...
	 public int deletesalesAttachByPrimId(String plant, String eid) throws Exception {
			...
			try {			
				...
				con = com.track.gates.DbBean.getConnection();
				query="DELETE FROM " + "[" + plant+ "_SALESORDERATTACHMENTS] "
						+ "WHERE ID = ? AND PLANT = ?";
				PreparedStatement ps = con.prepareStatement(query);  
