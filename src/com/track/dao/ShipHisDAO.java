...
public ArrayList getproductcustomersaleswithzeroqty1(String plant,String item,String itemDesc,String custname,String loc,
...
//		   				sql.append("SELECT A.ITEM AS ID,A.ITEMDESC AS NAME,A.UNITPRICE,isnull(A.COST,0) as COST,isnull(sum(TOTAL_UNITCOST/QTYOR),0) as COST,ISNULL(SUM(PO.QTYOR), 0) AS TOTQTY, ");
		   				sql.append("SELECT A.ITEM AS ID,A.ITEMDESC AS NAME,A.UNITPRICE,isnull(sum(TOTAL_UNITCOST/QTYOR),0) as COST,ISNULL(SUM(PO.QTYOR), 0) AS TOTQTY, ");
		   				if(radiosearch.equalsIgnoreCase("3")) {
...
//		   				sql.append("ISNULL(SUM(PO.TOTAL_UNITPRICE - PO.TOTAL_DISCOUNT) - ISNULL(E.EXPRICE, 0), 0) AS TOTAMT, ");
		   				...
		   				//imti tot amt calc
		   				sql.append("ISNULL(CASE WHEN ISNULL(PO.ITEM_RATES, 1) = 0 THEN ((SUM(PO.TOTAL_UNITPRICE) / (100 + PO.OUTBOUND_GST)) * 100) ELSE SUM((PO.TOTAL_UNITPRICE - PO.TOTAL_DISCOUNT) - ISNULL(E.EXPRICE, 0) )+SUM(PO.TOTAL_UNITPRICE) - ((SUM(PO.TOTAL_UNITPRICE) / (100 + PO.OUTBOUND_GST)) * 100) END ,  0) AS TOTAMT, ");
//		   				ISNULL(CASE WHEN ISNULL(PO.ITEM_RATES, 1) = 1 THEN SUM((PO.TOTAL_UNITPRICE - PO.TOTAL_DISCOUNT) - ISNULL(E.EXPRICE, 0) ) -(SUM(PO.TOTAL_UNITPRICE) / 100) * PO.OUTBOUND_GST  
//		   						ELSE SUM((PO.TOTAL_UNITPRICE - PO.TOTAL_DISCOUNT) - ISNULL(E.EXPRICE, 0) )+SUM(PO.TOTAL_UNITPRICE) - ((SUM(PO.TOTAL_UNITPRICE) / (100 + PO.OUTBOUND_GST)) * 100) END ,  0) AS TOTAMT,
...
//		   				sql.append("ISNULL(CASE WHEN ISNULL(PO.ITEM_RATES, 1) = 1 THEN SUM(0)  ELSE (SUM(PO.TOTAL_UNITPRICE) / 100) * PO.OUTBOUND_GST END , 0) AS TOTTAX, ");
		   				sql.append("ISNULL(SUM(PO.TOTAL_UNITPRICE - PO.TOTAL_DISCOUNT) + CASE WHEN ISNULL(PO.ITEM_RATES, 1) = 1 THEN 0 ELSE (SUM(PO.TOTAL_UNITPRICE) / 100) * PO.OUTBOUND_GST END , 0) AS TOTPRICE,  ");
		   	...
		   	/*	sql.append(" select ITEM as ID ,ITEMDESC as NAME,UNITPRICE,COST, ");  
		   		...
		   		if(ispos.equalsIgnoreCase("3"))
			   		...
			   		posext =  " AND A.ORDERTYPE != 'POS' " ;
		   				...
		   				sql.append("WHERE A.ORDER_STATUS = 'PROCESSED' "+posext+" AND A.PLANT = '"+ plant +"' "+bcondition1+" ");
		   				sql.append("GROUP BY PE.ITEM) ");
		   				//imt//sql.append("SELECT A.ITEM AS ID,A.ITEMDESC AS NAME,A.UNITPRICE,isnull(A.COST,0) as COST,ISNULL(SUM(PO.QTYOR), 0) AS TOTQTY, ");
		   				sql.append("SELECT A.ITEM AS ID,A.ITEMDESC AS NAME,A.UNITPRICE,isnull(sum(TOTAL_UNITCOST/QTYOR),0) as COST,ISNULL(SUM(PO.QTYOR), 0) AS TOTQTY, ");
		   				if(radiosearch.equalsIgnoreCase("3")) {
		   					sql.append("ISNULL(SUM(BPO.QTYOR), 0) AS BTOTQTY, ");
		   				}else {
...
//		   			imt//	sql.append("ISNULL(CASE WHEN ISNULL(PO.ITEM_RATES, 1) = 1 THEN SUM(PO.TOTAL_UNITPRICE) - ((SUM(PO.TOTAL_UNITPRICE) / (100 + PO.OUTBOUND_GST)) * 100) ELSE (SUM(PO.TOTAL_UNITPRICE) / 100) * PO.OUTBOUND_GST END , 0) AS TOTTAX, ");
//		   				sql.append("ISNULL(CASE WHEN ISNULL(PO.ITEM_RATES, 1) = 1 THEN SUM(0)  ELSE (SUM(PO.TOTAL_UNITPRICE) / 100) * PO.OUTBOUND_GST END , 0) AS TOTTAX, ");//imt
//		   				sql.append("ISNULL(SUM(PO.TOTAL_UNITPRICE - PO.TOTAL_DISCOUNT) ");
//		   				sql.append("+ CASE WHEN ISNULL(PO.ITEM_RATES, 1) = 1 ");		   				
//		   				sql.append("THEN SUM(PO.TOTAL_UNITPRICE) - ((SUM(PO.TOTAL_UNITPRICE) / (100 + PO.OUTBOUND_GST)) * 100) ");
//		   				sql.append("ELSE (SUM(PO.TOTAL_UNITPRICE) / 100) * PO.OUTBOUND_GST END , 0) AS TOTPRICE, "); 
		   				sql.append("ISNULL(CASE WHEN ISNULL(PO.ITEM_RATES, 1) = 1 THEN SUM(PO.TOTAL_UNITPRICE) - ((SUM(PO.TOTAL_UNITPRICE) / (100 + PO.OUTBOUND_GST)) * 100) ELSE (SUM(PO.TOTAL_UNITPRICE) / 100) * PO.OUTBOUND_GST END , 0) AS TOTTAX,  ");
		   				sql.append("ISNULL( CASE WHEN ISNULL(PO.ITEM_RATES, 1) = 1 THEN SUM(PO.TOTAL_UNITPRICE - PO.TOTAL_DISCOUNT) - ISNULL(E.EXPRICE, 0) ELSE SUM(PO.TOTAL_UNITPRICE - PO.TOTAL_DISCOUNT) +(SUM(PO.TOTAL_UNITPRICE) / 100) * PO.OUTBOUND_GST  END , 0) AS TOTPRICE, ");
		   				//sql.append("- ISNULL(E.EXPRICE, 0), 0) AS TOTPRICE, ");
		   				sql.append("ISNULL((SELECT SUM(QTY) FROM ["+ plant +"_Invmst] WHERE PLANT = '"+ plant +"' AND LOC LIKE '%"+loc+"%' AND ITEM = A.ITEM), 0) AS QTY ");
//		   				sql.append(",ISNULL((SELECT TOP 1 IV.UPAT FROM ["+plant+"_Invmst] IV WHERE PLANT = '"+plant+"' AND LOC LIKE '%%' AND ITEM = A.ITEM), '') AS UPAT ");
		   				sql.append(", CASE WHEN ISNULL((SELECT TOP 1 IV.UPAT FROM ["+plant+"_Invmst] IV WHERE PLANT = '"+plant+"' AND LOC LIKE '%%' AND ITEM = A.ITEM), '') = '' THEN ISNULL((SELECT TOP 1 IV.CRAT FROM ["+plant+"_Invmst] IV WHERE PLANT = '"+plant+"' AND LOC LIKE '%%' AND ITEM = A.ITEM), '') ELSE (SELECT TOP 1 IV.UPAT FROM ["+plant+"_Invmst] IV WHERE PLANT = '"+plant+"' AND LOC LIKE '%%' AND ITEM = A.ITEM) END AS UPAT ");
...
public ArrayList getproductcustomersaleswithzeroqty2(String plant,String item,String itemDesc,String custname,String loc,
		String sort, String fromdt,String todt,String ispos,String radiosearch) throws Exception {
	ArrayList alData = new ArrayList();
	java.sql.Connection con = null;
    String extCond = "";
...
public ArrayList getproductcustomersaleswithzeroqtyAVGCHANGES(String plant,String item,String itemDesc,String custname,String loc,
		String sort, String fromdt,String todt,String ispos,String radiosearch) throws Exception {
	ArrayList alData = new ArrayList();
	java.sql.Connection con = null;
    String extCond = "";
		   	...
		   	/*	sql.append(" select ITEM as ID ,ITEMDESC as NAME,UNITPRICE,COST, ");  
		   		...
		   		if(ispos.equalsIgnoreCase("3"))
			   		...
			   		posext =  " AND A.ORDERTYPE != 'POS' " ;
		   				...
		   				sql.append("WITH ProcessedOrders AS ( ");
		   				sql.append("SELECT A.DONO,isnull (A.TAXID,0)TAXID, B.ITEM, SUM(B.QTYOR) AS QTYOR, SUM(B.UNITCOST * B.QTYOR) AS TOTAL_UNITCOST, "); 
		   				sql.append("SUM(B.UNITPRICE * B.QTYOR) AS TOTAL_UNITPRICE,SUM(CASE WHEN ISNULL(B.DISCOUNT_TYPE, '%') = '%' ");
		   				sql.append("THEN (B.UNITPRICE * B.QTYOR) * (B.DISCOUNT / 100) ELSE B.DISCOUNT END) AS TOTAL_DISCOUNT, ");
		   				sql.append("A.ITEM_RATES, A.OUTBOUND_GST ");
		   				sql.append("FROM ["+ plant +"_DOHDR] A JOIN ["+ plant +"_DODET] B ON A.DONO = B.DONO JOIN ["+ plant +"_SHIPHIS] S ON B.DONO = S.DONO AND B.ITEM=S.ITEM AND B.DOLNNO=S.DOLNO ");
		   				sql.append("WHERE S.STATUS='C' "+posext+" AND A.PLANT = '"+ plant +"' AND S.LOC LIKE '%"+loc+"%' "+bcondition1+" ");
		   				sql.append("GROUP BY A.DONO,A.TAXID, B.ITEM, A.ITEM_RATES, A.OUTBOUND_GST), ");
		   				if(radiosearch.equalsIgnoreCase("3")) {
		   				...
		   				sql.append("WHERE A.ORDER_STATUS = 'PROCESSED' "+posext+" AND A.PLANT = '"+ plant +"' "+bcondition1+" ");
		   				sql.append("GROUP BY PE.ITEM) ");
//		   				sql.append("SELECT A.ITEM AS ID,isnull(PO.TAXID,0)TAXID,A.ITEMDESC AS NAME,A.UNITPRICE,isnull(A.COST,0) as COST,ISNULL(SUM(PO.QTYOR), 0) AS TOTQTY, ");
		   				sql.append("SELECT A.ITEM AS ID,isnull(PO.TAXID,0)TAXID,A.ITEMDESC AS NAME,A.UNITPRICE,ISNULL(SUM(PO.QTYOR), 0) AS TOTQTY, ");
		   				if(radiosearch.equalsIgnoreCase("3")) {
		   					sql.append("ISNULL(SUM(BPO.QTYOR), 0) AS BTOTQTY, ");
		   				}else {
		   					sql.append("ISNULL('0', 0) AS BTOTQTY,  ");
		   				...
		   				sql.append("isnull(sum(TOTAL_UNITCOST/QTYOR),0) as COST, ");
		   				sql.append("ISNULL(CASE WHEN ISNULL(PO.TAXID, 1) in('8','27','47','55','59','63','67','71') THEN SUM((PO.TOTAL_UNITPRICE) /100 * PO.OUTBOUND_GST) ELSE 0 END , 0) AS TOTTAX, ");
		   				sql.append("ISNULL(SUM(PO.TOTAL_UNITPRICE - PO.TOTAL_DISCOUNT) + CASE WHEN ISNULL(PO.TAXID, 1) not in('8','27','47','55','59','63','67','71') THEN 0 ELSE (SUM(PO.TOTAL_UNITPRICE) / 100) * PO.OUTBOUND_GST END , 0) AS TOTPRICE, ");
...
//		   				sql.append(",ISNULL((SELECT TOP 1 IV.UPAT FROM ["+plant+"_Invmst] IV WHERE PLANT = '"+plant+"' AND LOC LIKE '%%' AND ITEM = A.ITEM), '') AS UPAT ");
		   				...
		   				sql.append("LEFT JOIN ExchangeOrders E ON A.ITEM = E.ITEM "+extCond);
		   				sql.append("GROUP BY A.ITEM,PO.TAXID,A.ITEMDESC,A.CATLOGPATH,A.UNITPRICE,A.COST,PO.ITEM_RATES,PO.OUTBOUND_GST,E.EXCOST,E.EXPRICE; ");
