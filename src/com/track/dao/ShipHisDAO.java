...
//		   				sql.append("SELECT A.ITEM AS ID,A.ITEMDESC AS NAME,A.UNITPRICE,isnull(A.COST,0) as COST,ISNULL(SUM(PO.QTYOR), 0) AS TOTQTY, ");//imt
		   				sql.append("SELECT A.ITEM AS ID,A.ITEMDESC AS NAME,A.UNITPRICE,ISNULL(CASE WHEN ISNULL(A.AVERAGECOST, '') = '' THEN ROUND((A.COST),2) ELSE ROUND((A.AVERAGECOST),2) END , 0) AS COST,ISNULL(SUM(PO.QTYOR), 0) AS TOTQTY, ");
...
//		   				sql.append("ELSE (SUM(PO.TOTAL_UNITPRICE) / 100) * PO.OUTBOUND_GST END , 0) AS TOTPRICE, "); //imt 
...
//		   				sql.append(",ISNULL((SELECT TOP 1 IV.UPAT FROM ["+plant+"_Invmst] IV WHERE PLANT = '"+plant+"' AND LOC LIKE '%%' AND ITEM = A.ITEM), '') AS UPAT ");
		   				...
		   				sql.append("GROUP BY A.ITEM,A.ITEMDESC,A.CATLOGPATH,A.UNITPRICE,A.AVERAGECOST,A.COST,PO.ITEM_RATES,PO.OUTBOUND_GST,E.EXCOST,E.EXPRICE; ");
