...
import com.track.dao.BrandPromotionDetDAO;
import com.track.dao.BrandPromotionHdrDAO;
import com.track.dao.ItemMstDAO;
import com.track.db.object.PosPrd_BrandPromotionDet;
...
@WebServlet("/brandpromotion/*")
public class BrandPromotionServlet extends HttpServlet implements IMLogger {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		BrandPromotionHdrDAO PosItemHdrDAO= new BrandPromotionHdrDAO();
		BrandPromotionDetDAO poDetDAO = new BrandPromotionDetDAO();
		...
		if (action.equalsIgnoreCase("save") || action.equalsIgnoreCase("edit")) {
			...
			try{		
				...
				PosPrd_BrandPromotionHdr promotionHdr = new PosPrd_BrandPromotionHdr();				

				List Lnno = new ArrayList(), buyitem_brand = new ArrayList(), buyqty = new ArrayList(), getitem = new ArrayList(), getqty = new ArrayList(),
...
//							}
							...
							if (fileItem.getFieldName().equalsIgnoreCase("item_brand")) {
								buyitem_brand.add(StrUtils.fString(fileItem.getString()).trim());
							}
					...
					if (action.equalsIgnoreCase("edit")) {
					   ...
					   poDetDAO.DeletePromotionDet(plant, promoid);
						...
						if(issadded) {
							...
							List<PosPrd_BrandPromotionDet> podetlist = new ArrayList<PosPrd_BrandPromotionDet>();
							
								for (int i = 0; i < lnnocount; i++) {
									int index = Integer.parseInt((String) Lnno.get(i)) - 1;
									int lnno = i + 1;
									PosPrd_BrandPromotionDet poDet = new PosPrd_BrandPromotionDet();
									...
									poDet.setBUY_PRD_BRAND_ID((String) buyitem_brand.get(index));
...
//							htRecvHis.put(IDBConstants.POHDR_JOB_NUM, pohdr.getJobNum());
						...
						response.sendRedirect("../brandpromotion/summary?result=" + message);
					}
					if(issadded) {
...
//						htRecvHis.put(IDBConstants.POHDR_JOB_NUM, pohdr.getJobNum());
						...
						/* String message = ""; */
						if (action.equalsIgnoreCase("edit")) 
						 message = "Unable To Update Brand Promotion.";
						else
						 message = "Unable To Add Brand Promotion.";
						if (ajax) {
				...
				if (ajax) {
					...
					response.sendRedirect("../brandpromotion/summary?result=" + ThrowableUtil.getMessage(e));
				}
			}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/createBrandPromotion.jsp");
		...
		if(action.equalsIgnoreCase("summary")) {
			try {
				String msg = StrUtils.fString(request.getParameter("msg"));
				request.setAttribute("Msg", msg);
				RequestDispatcher rd = request.getRequestDispatcher("/jsp/BrandPromotionSummary.jsp");
				rd.forward(request, response);
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		if(action.equalsIgnoreCase("edit")) {
			try {
				...
				String msg = StrUtils.fString(request.getParameter("msg"));
				request.setAttribute("Msg", msg);
				RequestDispatcher rd = request.getRequestDispatcher("/jsp/EditBrandPromotion.jsp");
				rd.forward(request, response);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private JSONObject getEditPrdPromotionDetails(HttpServletRequest request) {
		...
		BrandPromotionDetDAO posPromotionDetDAO = new BrandPromotionDetDAO();
		BrandPromotionHdrDAO posPromotionHdrDAO = new BrandPromotionHdrDAO();
		...
		try {
			...
			if (podetail.size() > 0) {
				for (int i=0;i<podetail.size();i++) {
			        ...
			        buyitem = StrUtils.fString((String)map.get("BUY_PRD_BRAND_ID"));
					...
					resultJsonInt.put("BUY_PRD_BRAND_ID", buyitem);
