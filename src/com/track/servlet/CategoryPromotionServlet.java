...
import com.track.dao.CategoryPromotionDetDAO;
import com.track.dao.CategoryPromotionHdrDAO;
...
@WebServlet("/categorypromotion/*")
public class CategoryPromotionServlet extends HttpServlet implements IMLogger {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		CategoryPromotionHdrDAO PosItemHdrDAO= new CategoryPromotionHdrDAO();
		CategoryPromotionDetDAO poDetDAO = new CategoryPromotionDetDAO();
		...
		if (action.equalsIgnoreCase("save") || action.equalsIgnoreCase("edit")) {
			...
			try{
				...
				PosPrd_ClassPromotionHdr promotionHdr = new PosPrd_ClassPromotionHdr();

				List Lnno = new ArrayList(), buyitem_category = new ArrayList(), buyqty = new ArrayList(), getitem = new ArrayList(), getqty = new ArrayList(),
					 promotiontype = new ArrayList(), promotionvalue = new ArrayList(),limitusage = new ArrayList(),chkdDoNo = new ArrayList();
...
//							}
							...
							if (fileItem.getFieldName().equalsIgnoreCase("item_category")) {
								buyitem_category.add(StrUtils.fString(fileItem.getString()).trim());
							}
					...
					if (action.equalsIgnoreCase("edit")) {
					   issadded = PosItemHdrDAO.updateProductClsHdr(promotionHdr,promoid);
					...
					if(issadded) {
						List<PosPrd_ClassPromotionDet> podetlist = new ArrayList<PosPrd_ClassPromotionDet>();
						
							for (int i = 0; i < lnnocount; i++) {
								int index = Integer.parseInt((String) Lnno.get(i)) - 1;
								int lnno = i + 1;
								PosPrd_ClassPromotionDet poDet = new PosPrd_ClassPromotionDet();
								...
								poDet.setBUY_PRD_CLS_ID((String) buyitem_category.get(index));
						...
						if (action.equalsIgnoreCase("edit")) {
							htRecvHis.put(IDBConstants.DIRTYPE, TransactionConstants.UPDATE_PRDCLS_PROMOTION);
						}else {
							htRecvHis.put(IDBConstants.DIRTYPE, TransactionConstants.CREATE_PRDCLS_PROMOTION);
						}
//						htRecvHis.put(IDBConstants.CUSTOMER_CODE, pohdr.getCustCode());
//						htRecvHis.put(IDBConstants.POHDR_JOB_NUM, pohdr.getJobNum());
						htRecvHis.put(IDBConstants.MOVHIS_ORDNUM, promotionHdr.getPROMOTION_NAME());
						...
						response.sendRedirect("../categorypromotion/summary?result=" + message);
					} 
					if(issadded) {
...
//						htRecvHis.put(IDBConstants.POHDR_JOB_NUM, pohdr.getJobNum());
						...
						/*String message = "";*/
						if (action.equalsIgnoreCase("edit")) 
						 message = "Unable To Update Category Promotion.";
						else
						 message = "Unable To Add Category Promotion.";
						if (ajax) {
				...
				if (ajax) {
					...
					response.sendRedirect("../categorypromotion/summary?result=" + ThrowableUtil.getMessage(e));
				}
			}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/createCategoryPromotion.jsp");
		...
		if(action.equalsIgnoreCase("summary")) {
			try {
				String msg = StrUtils.fString(request.getParameter("msg"));
				request.setAttribute("Msg", msg);
				RequestDispatcher rd = request.getRequestDispatcher("/jsp/CategoryPromotionSummary.jsp");
				rd.forward(request, response);
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		if(action.equalsIgnoreCase("edit")) {
			try {
				...
				String msg = StrUtils.fString(request.getParameter("msg"));
				request.setAttribute("Msg", msg);
				RequestDispatcher rd = request.getRequestDispatcher("/jsp/EditCategoryPromotion.jsp");
				rd.forward(request, response);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private JSONObject getEditPrdPromotionDetails(HttpServletRequest request) {
		...
		CategoryPromotionDetDAO posPromotionDetDAO = new CategoryPromotionDetDAO();
		CategoryPromotionHdrDAO posPromotionHdrDAO = new CategoryPromotionHdrDAO();
		...
		try {
			...
			if (podetail.size() > 0) {
				for (int i=0;i<podetail.size();i++) {
			        ...
			        buyitem = StrUtils.fString((String)map.get("BUY_PRD_CLS_ID"));
					...
					resultJsonInt.put("BUY_PRD_CLS_ID", buyitem);
					...
					resultJsonInt.put("PROMOTION", StrUtils.addZeroes(promo, com.track.gates.DbBean.NOOFDECIMALPTSFORWEIGHT));
