...
@WebServlet("/clearagent/*")
public class ClearingAgentServlet extends HttpServlet implements IMLogger {
	...
	if(action.equalsIgnoreCase("summary")) {
			...
			try {
				...
				RequestDispatcher rd = request.getRequestDispatcher("/jsp/ClearAgentSummary.jsp");
	...
	if(action.equalsIgnoreCase("new")) {
		...
		try {
			...
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/CreateClearAgent.jsp");
		rd.forward(request, response);
	} catch (Exception e) {
		e.printStackTrace();
	}
		...
		ht.put(IDBConstants.TBL_FUNCTION, "CLEARING_AGENT");
		try {
			...
			if (exitFlag == false) {
				...
				htTblCntUpdate.put(IDBConstants.TBL_FUNCTION,"CLEARING_AGENT");
				htTblCntUpdate.put(IDBConstants.TBL_PREFIX1, "CA");
				StringBuffer updateQyery = new StringBuffer("set ");
				updateQyery.append(IDBConstants.TBL_NEXT_SEQ + " = '"+ (String) updatedSeq.toString() + "'");

				sItemId = "CA" + sZero + updatedSeq;
			}
			json.put("AGENTID", sItemId);
	...
	if(action.equalsIgnoreCase("detail")) {
		...
		try {
			String msg = StrUtils.fString(request.getParameter("msg"));
			request.setAttribute("Msg", msg);
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/clearAgentdetail.jsp");
		rd.forward(request, response);
	} catch (Exception e) {
		e.printStackTrace();
	}
	...
	if(action.equalsIgnoreCase("edit")) {
		...
		try {
			...
			request.setAttribute("Msg", msg);
			request.setAttribute("result", result);
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/MaintClearAgent.jsp");
		rd.forward(request, response);
	} catch (Exception e) {
		e.printStackTrace();
	}
