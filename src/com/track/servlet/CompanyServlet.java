...
@WebServlet("/company/*")
public class CompanyServlet extends HttpServlet implements IMLogger {
	...
	if(action.equalsIgnoreCase("editcompany")) {
			
			
			try {
				String msg = StrUtils.fString(request.getParameter("msg"));
				request.setAttribute("Msg", msg);
				RequestDispatcher rd = request.getRequestDispatcher("/jsp/customer_maintPlant.jsp?action=ADD");
...
if(action.equalsIgnoreCase("editpassword")) {
		...
		try {
			String msg = StrUtils.fString(request.getParameter("msg"));
			request.setAttribute("Msg", msg);
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/chngPwd.jsp");
		rd.forward(request, response);
	} catch (Exception e) {
		e.printStackTrace();
	}
...
if(action.equalsIgnoreCase("banner")) {
	...
	try {
		String msg = StrUtils.fString(request.getParameter("msg"));
		request.setAttribute("Msg", msg);
		RequestDispatcher rd = request.getRequestDispatcher("/jsp/BannerMst.jsp");
		rd.forward(request, response);
	} catch (Exception e) {
		e.printStackTrace();
	}
	...
	if(action.equalsIgnoreCase("editcompany")) {
			...
			try {
				String msg = StrUtils.fString(request.getParameter("msg"));
				request.setAttribute("Msg", msg);
				RequestDispatcher rd = request.getRequestDispatcher("/jsp/customer_maintPlant.jsp");
			rd.forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
		}
