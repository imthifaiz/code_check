...
public class DOMultiTransferServlet extends HttpServlet implements IMLogger {
			...
			if (this.printInfo) {
			 ...
			 if (action.equalsIgnoreCase("View")) {
				 ...
				 if (rflag.equals("2")) {
					...
					response.sendRedirect("jsp/DOMultipleTransferSummary.jsp?DONO="
							+ dono + "&action=View");
				} 
			}
	...
	private String DoTransferPickingDataByWMS(HttpServletRequest request,
		...
		try {
			...
			response
					.sendRedirect("jsp/DOMultipleTransferSummary.jsp?action=View&PLANT="
							+ PLANT + "&DONO=" + TO_NUM);
			throw e;
		}
		if (!flag) {
			...
			response.sendRedirect("jsp/DOMultiTransferPicking.jsp?ORDERNO="
	...
	private String process_DoTranferOrderPickingByWMS(HttpServletRequest request,
		...
		try {
			...
			ISSUED_QTY=StrUtils.fString(request.getParameter("ISSUEDQTY"));
			PICKED_QTY = StrUtils.fString(request.getParameter("PICKEDQTY"));
			PICKING_QTY = StrUtils.fString(request.getParameter("PICKINGQTY"));
			...
			int orderqty = Integer.parseInt(((String) ORDER_QTY.trim()));
			int invqty = Integer.parseInt(((String) ITEM_QTY.trim()));
			int issuedqty = Integer.parseInt(((String) ISSUED_QTY.trim()));
			int pickedqty = Integer.parseInt(((String) PICKED_QTY.trim()));
			int pickingQty = Integer.parseInt(((String) PICKING_QTY.trim().toString()));
		
			boolean qtyFlag = false;
			
			if (pickingQty > (orderqty-issuedqty) 
					|| (pickingQty + pickedqty) > (orderqty-issuedqty)|| pickingQty > invqty  ) {
				qtyFlag = true;
