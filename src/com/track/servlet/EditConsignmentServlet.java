...
@WebServlet("/editconsignment/*")
public class EditConsignmentServlet extends HttpServlet implements IMLogger {
...
if(action.equalsIgnoreCase("consignorderemail")) {
	
	
	try {
		String msg = StrUtils.fString(request.getParameter("msg"));
		request.setAttribute("Msg", msg);
		RequestDispatcher rd = request.getRequestDispatcher("/jsp/editTransferOrderEmailMsg.jsp");
...
if(action.equalsIgnoreCase("consignmentprintout")) {
		
		
		try {
			String msg = StrUtils.fString(request.getParameter("msg"));
			request.setAttribute("Msg", msg);
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/editTransfer.jsp");
		rd.forward(request, response);
	} catch (Exception e) {
		e.printStackTrace();
	}
...
if(action.equalsIgnoreCase("consignprintoutwithprice")) {
	
	
	try {
		String msg = StrUtils.fString(request.getParameter("msg"));
		request.setAttribute("Msg", msg);
		RequestDispatcher rd = request.getRequestDispatcher("/jsp/editTransferTO.jsp");
	rd.forward(request, response);
} catch (Exception e) {
	e.printStackTrace();
}
