...
@WebServlet("/goodsissued/*")
public class GoodsIssuedServlet extends HttpServlet implements IMLogger {
	...
	if(action.equalsIgnoreCase("summary")) {
			...
			try {
				...
				RequestDispatcher rd = request.getRequestDispatcher("/jsp/ginotoinvoiceSummary.jsp");
	...
	if(action.equalsIgnoreCase("detail")) {
		...
		try {
			String msg = StrUtils.fString(request.getParameter("msg"));
			request.setAttribute("Msg", msg);
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/ginotoinvoiceDetail.jsp");
			rd.forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
		}
