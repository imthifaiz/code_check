...
@WebServlet("/goods/*")
public class GoodsReceiptServlet extends HttpServlet implements IMLogger {
...
//		String region = StrUtils.fString((String) request.getSession().getAttribute("REGION"));
	...
	if(action.equalsIgnoreCase("summary")) {
			...
			try {
				...
				RequestDispatcher rd = request.getRequestDispatcher("/jsp/grntobillSummary.jsp");
	...
	if(action.equalsIgnoreCase("detail")) {
		...
		try {
			String msg = StrUtils.fString(request.getParameter("msg"));
			request.setAttribute("Msg", msg);
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/grntobillDetail.jsp");
			rd.forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
		}
