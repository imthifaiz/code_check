...
@WebServlet("/outlets/*")
public class OutletServelt extends HttpServlet implements IMLogger {
...
if(action.equalsIgnoreCase("summary")) {
			...
			try {
				...
				RequestDispatcher rd = request.getRequestDispatcher("/jsp/OutletSummary.jsp");
...
if(action.equalsIgnoreCase("summary")) {
	...
	if(action.equalsIgnoreCase("new")) {
		...
		try {
			String msg = StrUtils.fString(request.getParameter("msg"));
			request.setAttribute("Msg", msg);
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/createOutlet.jsp");
		rd.forward(request, response);
	} catch (Exception e) {
		e.printStackTrace();
	}
		...
		ht.put(IDBConstants.TBL_FUNCTION, "OUTLET");
		try {
			...
			if (exitFlag == false) {
				...
				htTblCntUpdate.put(IDBConstants.TBL_FUNCTION,"OUTLET");
				htTblCntUpdate.put(IDBConstants.TBL_PREFIX1, "OT");
				...
				sCustCode = "OT" + sZero + updatedSeq;
			}
			json.put("OUTLET", sCustCode);
