...
@WebServlet("/outletterminal/*")
public class OutletTerminalServelt extends HttpServlet implements IMLogger {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String plant = StrUtils.fString((String) request.getSession().getAttribute("PLANT")).trim();
			...
			try {
				...
				RequestDispatcher rd = request.getRequestDispatcher("/jsp/OutletTerminalSummary.jsp");
		...
		String region = StrUtils.fString((String) request.getSession().getAttribute("REGION"));
			...
			if(action.equalsIgnoreCase("new")) {
				try {
					String msg = StrUtils.fString(request.getParameter("msg"));
					request.setAttribute("Msg", msg);
					RequestDispatcher rd = request.getRequestDispatcher("/jsp/createOutletTerminal.jsp");
					rd.forward(request, response);
				} catch (Exception e) {
					e.printStackTrace();
				}
			...
			ht.put(IDBConstants.TBL_FUNCTION, "OUTLETTERMINAL");
			try {
				...
				if (exitFlag == false) {
					...
					htTblCntUpdate.put(IDBConstants.TBL_FUNCTION,"OUTLETTERMINAL");
					htTblCntUpdate.put(IDBConstants.TBL_PREFIX1, "TR");
					StringBuffer updateQyery = new StringBuffer("set ");
					updateQyery.append(IDBConstants.TBL_NEXT_SEQ + " = '"+ (String) updatedSeq.toString() + "'");
	
					sCustCode = "TR" + sZero + updatedSeq;
				}
				json.put("OUTLETTERMINAL", sCustCode);
			...
			if(action.equalsIgnoreCase("edit")) {
				try {
					String msg = StrUtils.fString(request.getParameter("msg"));
					request.setAttribute("Msg", msg);
					RequestDispatcher rd = request.getRequestDispatcher("/jsp/editOutletTerminal.jsp");
					rd.forward(request, response);
				} catch (Exception e) {
					e.printStackTrace();
				}
