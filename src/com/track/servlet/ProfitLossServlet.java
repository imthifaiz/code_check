...
@WebServlet("/ProfitLossServlet")
public class ProfitLossServlet extends HttpServlet implements IMLogger {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (action.equalsIgnoreCase("getProfitLoss")) {
			...
			try {
				...
				for (JournalDetail journalDet : journalDetails) {
					if (!AccountId.contains(journalDet.getACCOUNT_ID())) {
						...
						if (coaJson != null) {
							...
							profitLoss.setAccount_name(coaJson.getString("account_name")+" - "+accountcode);
				...
				profitLossArr.forEach((k, v) -> {
			      ...
			      Collections.sort(list, new Comparator() {
			          private static final String KEY_NAME = "account_type";
				...
				/*
				 * List<JSONObject> jsonValues = new ArrayList<JSONObject>(); for (int i = 0; i
				 * < jarray.size(); i++) { jsonValues.add(jarray.getJSONObject(i)); }
				 * Collections.sort( jsonValues, new Comparator<JSONObject>() { //You can change
				 * "Name" with "ID" if you want to sort by ID private static final String
				 * KEY_NAME = "Name";
				 * 
				 * @Override public int compare(JSONObject a, JSONObject b) { String valA = new
				 * String(); String valB = new String();
				 * 
				 * valA = (String) a.get(KEY_NAME); valB = (String) b.get(KEY_NAME);
				 * 
				 * return valA.compareTo(valB); //if you want to change the sort order, simply
				 * use the following: //return -valA.compareTo(valB); } });
				 * 
				 * for (int i = 0; i < jarray.size(); i++) {
			...
			//resp.getWriter().write(sortedJsonArray.toString());
			resp.getWriter().flush();
			resp.getWriter().close();
		}else if (action.equalsIgnoreCase("getProjectProfitLoss")) {
			String fromDate = StrUtils.fString(req.getParameter("fromDate")).trim();
			String toDate = StrUtils.fString(req.getParameter("toDate")).trim();
			String accounttypes = StrUtils.fString(req.getParameter("accounttypes")).trim();
			...
			try {
				...
				// String accounttypes= "('8','9')";
				List<JournalDetail> journalDetails = journalDAO.getJournalDetailsByAccountTypeAndDATEproid(plant,
						fromDate, toDate, accounttypes,projectid);
				...
				for (JournalDetail journalDet : journalDetails) {
					if (!AccountId.contains(journalDet.getACCOUNT_ID())) {
						...
						if (coaJson != null) {
							...
							profitLoss.setAccount_id(journalDet.getACCOUNT_ID());
							String accountcode = coaDAO.GetAccountCodeByName(coaJson.getString("account_name"), plant);
							profitLoss.setAccount_name(accountcode+" "+coaJson.getString("account_name"));
							profitLoss.setAccount_type(coaJson.getString("account_type_name"));
							profitLoss.setExtended_type(coaJson.getString("account_det_type"));
							profitLoss.setMain_account(coaJson.getString("mainaccname"));
							profitLoss.setNet_credit(journalDet.getCREDITS());
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (action.equalsIgnoreCase("getProfitLossExcel")) {
			...
			String toDate = StrUtils.fString(req.getParameter("toDate")).trim();
			// String accounttypes =
			// StrUtils.fString(req.getParameter("accounttypes")).trim();
			// List<Journal> journalList = null;
			...
			if (!fromDate.isEmpty()) {
				...
				try {
					...
					workbook = populateExcel("Profit&Loss", toDate, excelDataArray, numberOfDecimal);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
		...
		if (action.equalsIgnoreCase("getProjectProfitLossExcel")) {
			...
			for (String account : list) {
				...
				try {
					...
					List<JournalDetail> journalDetails = journalDAO.getJournalDetailsByAccountTypeAndDATEproid(plant,
							fromDate, toDate, account,projectid);
			...
			if (!fromDate.isEmpty()) {
				...
				try {
					String numberOfDecimal = _PlantMstDAO.getNumberOfDecimal(plant);
					workbook = populateExcel("project Profit&Loss", toDate, excelDataArray, numberOfDecimal);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				...
				resp.setContentLength(outArray.length);
				resp.setHeader("Expires:", "0"); // eliminates browser caching
				resp.setHeader("Content-Disposition", "attachment; filename=ProjectProfit&Loss.xls");
				OutputStream outStream = resp.getOutputStream();
				outStream.write(outArray);
				outStream.flush();
				outStream.close();
	...
	public HSSFWorkbook populateExcel(String headerTitle, String excelHeaderDate, List<JSONArray> excelDatas,
			String noOfDecimal) throws JSONException {

		String salesString = excelDatas.get(0).toString();
		String incomeString = excelDatas.get(1).toString();
		...
		HSSFSheet spreadsheet = workbook.createSheet("Profit&Loss");
		// Create row object
		...
		my_style2.setAlignment(HSSFCellStyle.ALIGN_RIGHT);
		...
		HSSFRow rt1 = spreadsheet.createRow(dataRow);
		HSSFCell rtotal1 = rt1.createCell(0);
		rtotal1.setCellValue("Total");
		rtotal1.setCellStyle(my_style);
		HSSFCell ramt1 = rt1.createCell(1);
		ramt1.setCellValue(StrUtils.addZeroes(totalSales, noOfDecimal));
		ramt1.setCellStyle(rightAligned);
		...
		HSSFRow rt2 = spreadsheet.createRow(dataRow);
		HSSFCell rtotal2 = rt2.createCell(0);
		rtotal2.setCellValue("Total");
		rtotal2.setCellStyle(my_style);
		HSSFCell ramt2 = rt2.createCell(1);
		ramt2.setCellValue(StrUtils.addZeroes(totalCostOfSales, noOfDecimal));
		ramt2.setCellStyle(rightAligned);
		...
		HSSFRow rt3 = spreadsheet.createRow(dataRow);
		HSSFCell rtotal3 = rt3.createCell(0);
		rtotal3.setCellValue("Total");
		rtotal3.setCellStyle(my_style);
		HSSFCell ramt3= rt3.createCell(1);
		ramt3.setCellValue(StrUtils.addZeroes(totalIncome, noOfDecimal));
		ramt3.setCellStyle(rightAligned);
		...
		double totalGrossProfit = (totalSales+totalIncome) - totalCostOfSales;
		...
		cellTotal1.setCellStyle(my_style2);
		...
		HSSFRow rt4 = spreadsheet.createRow(dataRow);
		HSSFCell rtotal4 = rt4.createCell(0);
		rtotal4.setCellValue("Total");
		rtotal4.setCellStyle(my_style);
		HSSFCell ramt4= rt4.createCell(1);
		ramt4.setCellValue(StrUtils.addZeroes(totalExpense, noOfDecimal));
		ramt4.setCellStyle(rightAligned);
		...
		double totalnetProfit = totalExpense;
