...
@WebServlet("/salesreturn/*")
public class SalesReturnServlet extends HttpServlet implements IMLogger {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (action.equalsIgnoreCase("CheckOrderno")) {
			...
			try {
				...
				ht.put(IDBConstants.SORETURN, orderno);
				if (new InvoiceDAO().isExisitSo(ht)) {
	...
	if(action.equalsIgnoreCase("summary")) {
		...
		try {
			...
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/SOReturnSummary.jsp");
	...
	if(action.equalsIgnoreCase("new")) {
		...
		try {
			String msg = StrUtils.fString(request.getParameter("msg"));
			request.setAttribute("Msg", msg);
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/SalesOrderReturn.jsp");
			rd.forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	if(action.equalsIgnoreCase("detail")) {
		...
		try {
			String msg = StrUtils.fString(request.getParameter("msg"));
			request.setAttribute("Msg", msg);
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/soReturnDetail.jsp");
			rd.forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	if(action.equalsIgnoreCase("creditnote")) {
		
		try {
			String msg = StrUtils.fString(request.getParameter("msg"));
			request.setAttribute("Msg", msg);
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/SalesReturnToCreditNotes.jsp");
			rd.forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
		}
