...
@WebServlet("/shipper/*")
public class ShipperServlet extends HttpServlet implements IMLogger {
...
if(action.equalsIgnoreCase("summary")) {
			...
			try {
				...
				RequestDispatcher rd = request.getRequestDispatcher("/jsp/ShipperSummary.jsp");
...
if(action.equalsIgnoreCase("summary")) {
	...
	if(action.equalsIgnoreCase("new")) {
		...
		try {
			String msg = StrUtils.fString(request.getParameter("msg"));
			request.setAttribute("Msg", msg);
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/CreateShipper.jsp");
		rd.forward(request, response);
	} catch (Exception e) {
		e.printStackTrace();
	}
		...
		ht.put(IDBConstants.TBL_FUNCTION, "FREIGHT_FORWARDER");
		try {
			...
			if (exitFlag == false) {
				...
				htTblCntUpdate.put(IDBConstants.TBL_FUNCTION,"FREIGHT_FORWARDER");
				htTblCntUpdate.put(IDBConstants.TBL_PREFIX1, "FF");
				...
				sCustCode = "FF" + sZero + updatedSeq;
			}
			json.put("FREIGHT_FORWARDER", sCustCode);
	...
	if(action.equalsIgnoreCase("edit")) {
		...
		try {
			String msg = StrUtils.fString(request.getParameter("msg"));
			request.setAttribute("Msg", msg);
			RequestDispatcher rd = request.getRequestDispatcher("/jsp/editShipper.jsp");
		rd.forward(request, response);
	} catch (Exception e) {
		e.printStackTrace();
	}
