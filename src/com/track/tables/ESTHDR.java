...
public class ESTHDR {
	private boolean printQuery = MLoggerConstant.ESTHDR_PRINTPLANTMASTERQUERY;
	private boolean printLog = MLoggerConstant.ESTHDR_PRINTPLANTMASTERLOG;
	sqlBean sb;
	Generator gn;
	 ...
	 */
	public ESTHDR() throws Exception {
		sb = new sqlBean();
		gn = new Generator();
	}
	 ...
	 * 
	 */
	public int insertESTHDR() throws Exception {
		String sql;
		...
		try {
			...
			sql = "INSERT INTO ESTHDR values('COM', '" + doNo + "','" + soNo
	...
	public void selectESTHDR() throws Exception {
		String q = "";
		Connection con = getDBConnection();
		try {
			Statement stmt = con.createStatement();
			q = "select * from ESTHDR where PLANT = 'SUS' and DONO = '" + doNo
					+ "'";
	...
	/**
	 * Delete Function -- ESTHDR
	 * 
	 * 
	 */
	public int deleteESTHDR() throws Exception {
		String sql;
		Connection con = getDBConnection();
		ResultSet rs = null;
		int returnCode = 1;
	 ...
	 * 
	 */
	public int deleteESTHDR(String aDONO) throws Exception {
		String sql;
		Connection con = getDBConnection();
		ResultSet rs = null;
		int returnCode = 1;
		...
		try {
			sql = "delete from ESTHDR where PLANT= 'COM' and DONO= '" + aDONO
					+ "'";
			sb.setmLogger(mLogger);
			returnCode = sb.insertRecords(sql);
		} catch (Exception e) {
	...
	public int countESTHDR(String adoNo) throws Exception {
		int res = 0;
		String q = "";
		Connection con = getDBConnection();
		try {
			Statement stmt = con.createStatement();
			q = "select count(*) from ESTHDR where PLANT= 'COM' and DONO= '"
					+ adoNo + "'";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			int n = 1;
	...
	public String getCustomerCode(String aDoNo) throws Exception {
		...
		try {
			Statement stmt = con.createStatement();
			q = "select custno from ESTHDR where PLANT= 'COM' and DONO= '"
					+ aDoNo + "'";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			int n = 1;
	...
	public String getDateRequired(String aDONO) throws Exception {
		...
		try {
			Statement stmt = con.createStatement();
			q = "select ORDDATE from ESTHDR where PLANT= 'COM' and DONO= '"
					+ aDONO + "'";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			int n = 1;
	...
	public String listSO(String aDONO) throws Exception {
		...
		try {
			...
			where = (aDONO.equalsIgnoreCase("")) ? "" : " WHERE DONO LIKE '%"
					+ aDONO + "%'";
			q = "select dono,custno,orddate,deldate from ESTHDR" + where;
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			while (rs.next()) {
