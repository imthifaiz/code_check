...
public class PLTMST {
	...
	/**
	 * constructor method PLTMST sb - SQL Bean gn - Utilites
	 */
	public PLTMST() throws Exception {
		sb = new sqlBean();
		gn = new Generator();
	}
	...
	/**
	 * Insert Command - PLTMST
	 * 
	 * 
	 */
	public int insertPLTMST() throws Exception {
		String sql;
		...
		try {
			sql = "Insert into PLTMST values ('" + sPLANT + "','" + sPALLET
					+ "','" + sITEM + "','" + sBATCH + "'," + fQTY + ",'"
	...
	public int addItem(String plant, String item, String category,
			String batch, int pallet, float qty, String crby) throws Exception {
		...
		try {
			sql = "Insert into PLTMST (PLANT,ITEM,CATEGORY,BATCH,PALLET,QTY,CRBY) values ('"
					+ plant
					+ "','"
					+ item
					+ "','"
	...
	/**
	 * Update Function - PLTMST
	 * 
	 * 
	 */
	public int updatePLTMST() throws Exception {
		String sql;
		Connection con = getDBConnection();
		ResultSet rs = null;
		int returnCode = 1;
		...
		try {
			sql = "Update PLTMST set PLANT= '" + sPLANT + "',PALLET= '"
					+ sPALLET + "',ITEM= '" + sITEM + "',BATCH= '" + sBATCH
					+ "',QTY = " + fQTY + ",CRAT= '" + sCRAT + "',CRBY= '"
					...
					+ fUSERDBL5 + ",USERDBL6 = " + fUSERDBL6 + " where";
			sb.setmLogger(mLogger);
			returnCode = sb.insertRecords(sql);
		} catch (Exception e) {
			this.mLogger.exception(this.printLog, "", e);
	...
	/**
	 * Select FunctionPLTMST
	 * 
	 * 
	 */
	public void selectPLTMST() throws Exception {
		String q = "";
		Connection con = getDBConnection();
		try {
			Statement stmt = con.createStatement();
			this.mLogger.query(this.printQuery, q);
			q = "select * from PLTMST where ";
			ResultSet rs = stmt.executeQuery(q);
			int n = 1;
			while (rs.next()) {
				sPLANT = rs.getString("PLANT");
				sPALLET = rs.getString("PALLET");
	...
	public float getPalletQty(String pallet, String item, String batch)
			throws Exception {
		String q = "";
		Connection con = getDBConnection();
		try {
			Statement stmt = con.createStatement();
			q = "select QTY from PLTMST where PLANT = " + sPLANT
					+ "' and PALLET = '" + pallet + "' and ITEM = '" + item
					+ "' and BATCH = '" + batch + "'";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			int n = 1;
	...
	/**
	 * Delete Function -- PLTMST
	 * 
	 * 
	 */
	public int deletePLTMST() throws Exception {
		String sql;
		Connection con = getDBConnection();
		ResultSet rs = null;
		int returnCode = 1;
	...
	public boolean isAvaliable(String pallet, String item, String batch)
			throws Exception {
		...
		try {
			...
			q = "select count(*) from PLTMST where PLANT= 'PLNT' ";
			q += (pallet.equalsIgnoreCase("")) ? "" : " and PALLET='" + pallet
					+ "'";
			q += (item.equalsIgnoreCase("")) ? "" : " and ITEM='" + item + "'";
			q += (batch.equalsIgnoreCase("")) ? "" : " and BATCH='" + batch
					+ "'";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			int n = 1;
	...
	public boolean isAvaliable(int pallet) throws Exception {
		boolean result = false;
		String q = "";
		Connection con = getDBConnection();
		try {
			...
			q = "select count(*) from PLTMST where PLANT= 'PLNT' and PALLET="
					+ pallet + "";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			int n = 1;
	...
	public String getNextPalletNo() throws Exception {
		String result = "";
		String q = "";
		Connection con = getDBConnection();
		try {
			...
			q = "select max(PALLET)+1 as nextID from PLTMST where PLANT= 'PLNT' ";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			int n = 1;
			while (rs.next()) {
				int m = rs.getInt(1);
				if (m > 0) {
					result = String.valueOf(rs.getInt("nextID"));
				}
			}
		} catch (Exception e) {
			this.mLogger.exception(this.printLog, "", e);
