...
public class PODET {
	private boolean printQuery = MLoggerConstant.PODET_PRINTPLANTMASTERQUERY;
	private boolean printLog = MLoggerConstant.PODET_PRINTPLANTMASTERLOG;
	sqlBean sb;
	Generator gn;
	String sPLANT = "SIS";
	String sPONO = "";
	...
	public void setPOLNNO(String aPOLNNO) {
		sPOLNNO = aPOLNNO;
	}
	...
	public int insertPODET() throws Exception {
		...
		try {
			sql = "Insert into PODET values ('" + sPLANT + "','" + sPONO
					+ "','" + sPOLNNO + "','" + sLNSTAT + "','" + sITEM + "','"
	...
	public int updatePODET() throws Exception {
		...
		try {
			sql = "Update PODET set LNSTAT= '" + sLNSTAT + "',ITEM= '" + sITEM
					...
					+ " where PLANT= '" + sPLANT + "' and PONO= '" + sPONO
					+ "' and POLNNO= '" + sPOLNNO + "'";
			sb.setmLogger(mLogger);
			returnCode = sb.insertRecords(sql);
		} catch (Exception e) {
	...
	public void selectPODET() throws Exception {
		...
		try {
			...
			q = "select * from PODET where PLANT= '" + IConstants.PLANT_VAL
					+ "' and PONO= '" + sPONO + "' and POLNNO= '" + sPOLNNO
					+ "'";
	...
	public void selectPODET(String plant) throws Exception {
		...
		try {
			Statement stmt = con.createStatement();
			q = "select * from " + "[" + plant + "_" + "PODET" + "]"
					+ " where PLANT= '" + plant + "' and PONO= '" + sPONO
					+ "' and POLNNO= '" + sPOLNNO + "'";
...
//			int n = 1;
	...
	public String listPODET(String aPONO) throws Exception {
		String q = "";
		String link = "";
		String result = "";
		String where = "";
		...
		try {
			...
			q = "select pono,polnno,lnstat,item,userfld1 from podet " + where;
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			while (rs.next()) {
				String pono = rs.getString("PONO").trim();
				String polnno = rs.getString("POLNNO").trim();
				...
				link = "<a href=\"" + "modiPODET.jsp?PONO=" + pono + "&POLNNO="
						+ polnno + "\">";
				result += "<tr valign=\"middle\">"
						+ "<td align=\"center\" width=\"20%\">" + link
						+ "<FONT COLOR=\"blue\">" + polnno + "</a></font>"
	...
	//
	public String listPODET1(String aPONO) throws Exception {
		String q = "";
		String link = "";
		String result = "";
		String where = "";
		...
		try {
			...
			q = "select pono,polnno,item,qtyor,qtyrc,qtyac,unitmo from podet "
					+ where;
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			while (rs.next()) {
	 ...
	 * 
	 */
	public int deletePODET(String aPONO, String aPOLNNO) throws Exception {
		String sql;
		Connection con = getDBConnection();
		ResultSet rs = null;
		int returnCode = 1;
		...
		try {
			sql = "delete from PODET where PLANT= '" + sPLANT + "' and PONO= '"
					+ aPONO + "' and POLNNO= '" + aPOLNNO + "'";
	...
	public int deletePODETML(String aPONO, String aPOLNNO, String plant)
			throws Exception {
		String sql;
		Connection con = getDBConnection();
		ResultSet rs = null;
		...
		try {
			sql = "delete from " + "[" + plant + "_" + "PODET" + "]"
					+ " where PLANT= '" + plant + "' and PONO= '" + aPONO
					+ "' and POLNNO= '" + aPOLNNO + "'";

			sb.setmLogger(mLogger);
			returnCode = sb.insertRecords(sql);
            sql = "update  " + "[" + plant + "_" + "PODET" + "]" 
            + "SET POLNNO =POLNNO-1 where PLANT= '" + plant + "' and PONO= '" + aPONO+ "'  and POLNNO > '" + aPOLNNO + "' ";
            sb.insertRecords(sql);
            
            sql = "update  " + "[" + plant + "_" + "RECVDET" + "]" 
            + "SET LNNO =LNNO-1 where PLANT= '" + plant + "' and PONO= '" + aPONO+ "'  and CAST(LNNO as int) > '" + aPOLNNO + "' ";
            sb.insertRecords(sql);
	...
	public int deletePODET(String aPONO) throws Exception {
		String sql;
		Connection con = getDBConnection();
		ResultSet rs = null;
		int returnCode = 1;
	...
	public int deletePODETML(String aPONO, String plant) throws Exception {
		String sql;
		Connection con = getDBConnection();
		ResultSet rs = null;
		int returnCode = 1;
	...
	public void processLoading(String filepath) throws Exception {
		...
		while ((line = reader.readLine()) != null) {
			...
			sPONO = vecStr.elementAt(1).toString();
			sPOLNNO = vecStr.elementAt(3).toString();
	...
	public String listPODET4REC(String aPONO) throws Exception {
		String q = "";
		String link = "";
		String result = "";
		String where = "";
		...
		try {
			...
			q = "select pono,polnno,lnstat,item,qtyor,qtyrc,qtyac,unitmo from podet "
					+ where;
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			while (rs.next()) {
				...
				String desc = itm.getItemDesc(item);
				link = "<a href=\"" + "recvQty.jsp?PONO=" + pono + "&POLNNO="
						+ polnno + "\">";
				if (lnstat.equalsIgnoreCase("C")) {
					result += "<tr valign=\"middle\">"
							+ "<td align=\"center\" width=\"15%\">" + polnno
							+ "</td>" + "<td width=\"15%\">"
	...
	public String listPdaPODET(String aPONO) throws Exception {
		String q = "";
		String link = "";
		String result = "";
		String where = "";
		...
		try {
			...
			while (rs.next()) {
				...
				float bal = Float.parseFloat(qtyor) - Float.parseFloat(qtyrc);
				String desc = itm.getItemDesc(item);
				link = "<a href=\"" + "pdaRecvQty.jsp?PONO=" + pono
						+ "&POLNNO=" + polnno + "\">";
				if (lnstat.equalsIgnoreCase("C")) {
					result += "<tr valign=\"middle\">"
							+ "<td align=\"center\" width=\"20%\">" + polnno
							+ "</td>" + "<td width=\"20%\">"
							+ sb.formatHTML(item) + "</td>"
	...
	public String listPOReport(String aFRDT, String aTODT, String aSUPP,
		...
		try {
			...
			int cnt = vecWhr.size();
			if (cnt > 0) {
				where = "where  a.pono = c.pono and a.item = b.item and ";
				for (int i = 0; i < cnt; i++) {
					where = where + vecWhr.elementAt(i).toString() + " ";
					if (i < cnt - 1) {
						where = where + " and ";
			...
			q = "select a.pono,a.item,b.itemdesc,a.qtyor,a.qtyrc,c.vendno,c.deldate from podet a , itemmst b, "
					+ " pohdr c " + where;
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			while (rs.next()) {
				...
				link = "<a href=\"" + "createPO.jsp?PONO=" + pono + "\">";
				if (bal > 0) {
					result += "<tr valign=\"middle\">"
	...
	public String RptlistPOReport(String aFRDT, String aTODT, String aSUPP,
		...
		try {
			...
			q = "select a.pono,a.item,b.itemdesc,a.qtyor,a.qtyrc,c.vendno,c.orddate from podet a , itemmst b, "
					+ " pohdr c " + where;
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			while (rs.next()) {
	...
	public String getNextLineNo(String aPONO) throws Exception {
		String q = "";
		String result = "";
		Connection con = getDBConnection();
		try {
			Statement stmt = con.createStatement();
			q = "select max(cast(polnno as int))+ 1 from podet where pono = '"
					+ aPONO + "'";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			while (rs.next()) {
	...
	public String getNextLineNo(String aPONO, String plant) throws Exception {
		String q = "";
		String result = "";
		Connection con = getDBConnection();
		try {
			Statement stmt = con.createStatement();
			q = "select max(cast(polnno as int))+ 1 from " + "[" + plant + "_"
					+ "podet" + "]" + " where pono = '" + aPONO + "'";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			while (rs.next()) {
				result = rs.getString(1);
	...
	public String listPOAllReport(String aFRDT, String aTODT, String aSUPP,
		...
		try {
			...
			q = "select a.PONO,a.VENDNO,b.ITEM,b.QTYOR,a.ORDDATE from pohdr a, podet b "
					+ where;
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			while (rs.next()) {
				...
				result += "<tr valign=\"middle\">"
						...
						+ "</td>"
						+ "<td width=\"20%\">"
						+ vm.getVendDesc(ph.getVendorCode(pono))
						+ "</td>"
						+ "<td width=\"15%\">"
						+ sb.formatHTML(item)
						+ "</td>"
	 ...
	 * 
	 */
	public int updatePODET(String aPONO, String aPOLNNO, float qty)
			throws Exception {
		String sql;
		Connection con = getDBConnection();
		ResultSet rs = null;
		...
		try {
			sql = "UPDATE PODET SET QTYOR = " + qty + " WHERE PLANT= '"
					+ sPLANT + "' and PONO= '" + aPONO + "' and POLNNO= '"
					+ aPOLNNO + "'";
			sb.setmLogger(mLogger);
			returnCode = sb.insertRecords(sql);
		} catch (Exception e) {
	...
	public String listItems4MR(String pono) throws Exception {
		String recStr = "";
		String link = "";
		String itm = "", itemdesc = "", uom = "", status = "";
		float qty = 0;
		String q = "select ITEM,UNITMO,LNSTAT,QTYOR from PODET where  PONO='"
				+ pono + "'";
