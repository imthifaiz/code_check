...
public class POHDR {
	private boolean printQuery = MLoggerConstant.POHDR_PRINTPLANTMASTERQUERY;
	private boolean printLog = MLoggerConstant.POHDR_PRINTPLANTMASTERLOG;
	sqlBean sb;
	Generator gn;
	String sPLANT = "COM";
	String sPONO = "";
	...
	public int insertPOHDR() throws Exception {
		...
		try {
			sql = "Insert into POHDR values ('" + sPLANT + "','" + sPONO
	...
	public void selectPOHDR() throws Exception {
		...
		try {
			...
			q = "select * from POHDR where PLANT= '" + IConstants.PLANT_VAL
					+ "' and PONO= '" + sPONO + "'";
	...
	public int countPOHDR(String aPONO) throws Exception {
		...
		try {
			Statement stmt = con.createStatement();
			q = "select count(*) from POHDR where PLANT= '"
					+ IConstants.PLANT_VAL + "' and PONO= '" + aPONO + "'";
			...
			DbBean.writeError("POHDR", "countPOHDR()", e);
		} finally {
			DbBean.closeConnection(con);
		}
		return res;
	 ...
	 * 
	 */
	public int deletePOHDR(String aPONO) throws Exception {
		String sql;
		Connection con = getDBConnection();
		ResultSet rs = null;
		int returnCode = 1;
		...
		try {
			sql = "delete from POHDR where PLANT= 'COM' and PONO= '" + aPONO
					+ "'";
			sb.setmLogger(mLogger);
			returnCode = sb.insertRecords(sql);
		} catch (Exception e) {
	...
	public int deletePOHDRML(String aPONO, String plant) throws Exception {
		String sql;
		Connection con = getDBConnection();
		ResultSet rs = null;
		int returnCode = 1;
		...
		try {
			sql = "delete from " + "[" + plant + "_" + "POHDR" + "]"
					+ " where PLANT= 'COM' and PONO= '" + aPONO + "'";
			sb.setmLogger(mLogger);
			returnCode = sb.insertRecords(sql);
		} catch (Exception e) {
			this.mLogger.exception(this.printLog, "", e);
	...
	public String listPOHDR(String aPONO, String aVENDNO) throws Exception {
		String q = "";
		String link = "";
		String result = "";
		String where = "";
		...
		try {
			Statement stmt = con.createStatement();
			if ((aPONO.length() > 0)) {
				vecWhr.addElement("PONO like '" + aPONO + "%'");
			}
			if ((aVENDNO.length() > 0)) {
			...
			while (rs.next()) {
				...
				link = "<a href=\"" + "recvDet.jsp?PONO=" + pono + "\">";
	...
	public int processPO(String aPONO) throws Exception {
		String sql1, sql2;
		Connection con = getDBConnection();
		int returnCode = 1;
		int ret1 = 1;
		...
		try {
			sql1 = "INSERT POHDR SELECT * FROM POHDR  WHERE PONO = '" + aPONO
					+ "'";
			sql2 = "INSERT PODET SELECT * FROM PODET  WHERE PONO = '" + aPONO
					+ "'";
			sb.setmLogger(mLogger);
	...
	public boolean isAvaliable(String aPONO) throws Exception {
		boolean result = false;
		String q = "";
		Connection con = getDBConnection();
		try {
			Statement stmt = con.createStatement();
			q = "select count(*) from POHDR where PLANT= 'COM' and PONO '"
					+ aPONO + "'";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			int n = 1;
	...
	public String getVendorCode(String aPONO) throws Exception {
		String q = "";
		String vendCode = "";
		Connection con = getDBConnection();
		try {
			Statement stmt = con.createStatement();
			q = "select vendno from POHDR where PLANT= 'COM' and PONO= '"
					+ aPONO + "'";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			int n = 1;
	...
	public String getDateRequired(String aPONO) throws Exception {
		String q = "";
		String delDate = "";
		Connection con = getDBConnection();
		try {
			Statement stmt = con.createStatement();
			q = "select DELDATE from POHDR where PLANT= 'COM' and PONO= '"
					+ aPONO + "'";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			int n = 1;
