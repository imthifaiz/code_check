...
public class PO_MULTI_ESTHDR {
	private boolean printQuery = MLoggerConstant.PO_MULTI_ESTHDR_PRINTPLANTMASTERQUERY;
	private boolean printLog = MLoggerConstant.PO_MULTI_ESTHDR_PRINTPLANTMASTERLOG;
	sqlBean sb;
	Generator gn;
	String sPLANT = "COM";
	...
	/**
	 * constructor method POHDR$ sb - SQL Bean gn - Utilites
	 */
	public PO_MULTI_ESTHDR() throws Exception {
		sb = new sqlBean();
		gn = new Generator();
	}
	...
	public int insertPOHDR() throws Exception {
		...
		try {
			sql = "Insert into PO_MULTI_ESTHDR values ('" + sPLANT + "','" + sPOMULTIESTNO
	...
	public int updatePOHDR() throws Exception {
		...
		try {
			sql = "Update POHDR set VENDNO= '" + sVENDNO + "',ORDDATE= '"
	...
	public void selectPOHDR() throws Exception {
		...
		try {
			...
			q = "select * from PO_MULTI_ESTHDR where PLANT= '" + IConstants.PLANT_VAL
					+ "' and POMULTIESTNO= '" + sPOMULTIESTNO + "'";
	...
	public int countPOHDR(String aPOMULTIESTNO) throws Exception {
		...
		try {
			Statement stmt = con.createStatement();
			q = "select count(*) from PO_MULTI_ESTHDR where PLANT= '"
					+ IConstants.PLANT_VAL + "' and POMULTIESTNO= '" + aPOMULTIESTNO + "'";
			...
			DbBean.writeError("PO_MULTI_ESTHDR", "countPO_MULTI_ESTHDR()", e);
		} finally {
			DbBean.closeConnection(con);
		}
		return res;
	 ...
	 * 
	 */
	public int deletePOHDR(String aPOMULTIESTNO) throws Exception {
		String sql;
		Connection con = getDBConnection();
		ResultSet rs = null;
		int returnCode = 1;
		...
		try {
			sql = "delete from PO_MULTI_ESTHDR where PLANT= 'COM' and POMULTIESTNO= '" + aPOMULTIESTNO
					+ "'";
			sb.setmLogger(mLogger);
			returnCode = sb.insertRecords(sql);
		} catch (Exception e) {
	...
	public int deletePOHDRML(String aPOMULTIESTNO, String plant) throws Exception {
		String sql;
		Connection con = getDBConnection();
		ResultSet rs = null;
		int returnCode = 1;
		...
		try {
			sql = "delete from " + "[" + plant + "_" + "PO_MULTI_ESTHDR" + "]"
					+ " where PLANT= 'COM' and POMULTIESTNO= '" + aPOMULTIESTNO + "'";
			sb.setmLogger(mLogger);
			returnCode = sb.insertRecords(sql);
		} catch (Exception e) {
			this.mLogger.exception(this.printLog, "", e);
	...
	public void processLoading(String filepath) throws Exception {
		...
		while ((line = reader.readLine()) != null) {
			...
			sPOMULTIESTNO = vecStr.elementAt(1).toString();
	...
	public String listPOHDR(String aPOMULTIESTNO, String aVENDNO) throws Exception {
		String q = "";
		String link = "";
		String result = "";
		String where = "";
		...
		try {
			...
			if ((aPOMULTIESTNO.length() > 0)) {
				vecWhr.addElement("POMULTIESTNO like '" + aPOMULTIESTNO + "%'");
			}
			if ((aVENDNO.length() > 0)) {
			...
			q = "select pono,vendno,orddate,crby,deldate from pohdr " + where;
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			while (rs.next()) {
				String POMULTIESTNO = rs.getString("POMULTIESTNO").trim();
				link = "<a href=\"" + "recvDet.jsp?POMULTIESTNO=" + aPOMULTIESTNO + "\">";
	...
	public int processPO(String aPOMULTIESTNO) throws Exception {
		String sql1, sql2;
		Connection con = getDBConnection();
		int returnCode = 1;
		int ret1 = 1;
		...
		try {
			sql1 = "INSERT PO_MULTI_ESTHDR SELECT * FROM PO_MULTI_ESTHDR  WHERE POMULTIESTNO = '" + aPOMULTIESTNO
					+ "'";
			sql2 = "INSERT PO_MULTI_ESTDET SELECT * FROM PO_MULTI_ESTDET  WHERE POMULTIESTNO = '" + aPOMULTIESTNO
					+ "'";
			sb.setmLogger(mLogger);
	...
	public boolean isAvaliable(String aPOMULTIESTNO) throws Exception {
		boolean result = false;
		String q = "";
		Connection con = getDBConnection();
		try {
			Statement stmt = con.createStatement();
			q = "select count(*) from PO_MULTI_ESTHDR where PLANT= 'COM' and POMULTIESTNO '"
					+ aPOMULTIESTNO + "'";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			int n = 1;
	...
	public String getVendorCode(String aPOMULTIESTNO) throws Exception {
		String q = "";
		String vendCode = "";
		Connection con = getDBConnection();
		try {
			Statement stmt = con.createStatement();
			q = "select vendno from PO_MULTI_ESTHDR where PLANT= 'COM' and POMULTIESTNO= '"
					+ aPOMULTIESTNO+ "'";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			int n = 1;
	...
	public String getDateRequired(String aPOMULTIESTNO) throws Exception {
		String q = "";
		String delDate = "";
		Connection con = getDBConnection();
		try {
			Statement stmt = con.createStatement();
			q = "select DELDATE from PO_MULTI_ESTHDR where PLANT= 'COM' and POMULTIESTNO= '"
					+ aPOMULTIESTNO + "'";
			this.mLogger.query(this.printQuery, q);
			ResultSet rs = stmt.executeQuery(q);
			int n = 1;
