...
public class WmsDoTransferPickingRandom implements WmsTran, IMLogger {
	...
	public  WmsDoTransferPickingRandom() {
	...
	public boolean processDoTransferdetForPick(Map map) throws Exception {
		...
		try {
			...
			alResult = _DOTransferUtil.getDOTransferDetDetailsRandom((String)map.get("PLANT"),(String) map.get("DONO"),(String) map.get("ITEM"));
			 ...
			 if(alResult.size()>=1){
				  ...
				  for (int i = 0; i < alResult.size(); i++) {           		  	
	          		  Map mapLn = (Map) alResult.get(i);
		            	   ...
		            	   String orderqty = ((String)mapLn.get("qtyor"));			 
		 		           pickedqty = Double.parseDouble((String)mapLn.get("qtyPick"));
		 		           
		 		           ordqty = Double.parseDouble((String)mapLn.get("qtyor"));	
		 		          
		 		          DOLNO =  StrUtils.fString((String)mapLn.get("dolnno"));
		 		          
		 		         pickqty =_DoTransferDetDAO.getDoTransferPickQty((String)map.get("PLANT"), (String) map.get("DONO"), (String) map.get("ITEM"),DOLNO);
	 		         	 
		 		         pickedqty = Double.parseDouble(pickqty);
		 		         balQty = ordqty - pickedqty;
		 		          // DOLNO =  StrUtils.fString((String)mapLn.get("dolnno"));
		 		           map.put(IConstants.DODET_DOLNNO,DOLNO);
		            	   map.put(IConstants.ORD_QTY,orderqty);
		...
		/*	query.append("isnull(QtyOr,0) as QtyOr");
			...
			*/
		    ...
		    htCondiPoDet.put("PLANT", map.get("PLANT"));
		    htCondiPoDet.put("dono", map.get("DONO"));
			...
			if (flag) {
				...
				htCondiPoHdr.put("dono", map.get("DONO"));
			...
			if( insertFlag==false){	
				...
				if(!flagpick)
				{
					...
					htPickCalc.put("DONO", map.get(IConstants.DODET_DONUM));
					htPickCalc.put("DOLNO", DOLNO);
