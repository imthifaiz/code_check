...
public class WmsIssuingByPDA implements WmsTran, IMLogger {
	...
	public boolean processInvMst(Map map) throws Exception {
		...
		try {
			...
			ht.put(IDBConstants.ITEM, map.get(IConstants.ITEM));
				...
				sLocShipHis = (String) map.get(IConstants.LOC);
				
				sBatch = (String) map.get(IConstants.BATCH);
				sIssueQty = (String) map.get(IConstants.QTY);
				sIssuedate= (String) map.get(IConstants.ISSUEDATE);
