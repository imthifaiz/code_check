...
public class WmsMobileRegistration implements WmsTran, IMLogger {
	...
	private boolean processdodet(Map m) throws Exception {
		...
		try {
			...
			for (Iterator iterator = catloglist.iterator(); iterator.hasNext();) {
				...
				htdodet.put(IDBConstants.DODET_ITEM_DESC, _StrUtils
						.InsertQuotes(itemDesc));

				htdodet.put(IDBConstants.DODET_JOB_NUM, "");
				htdodet.put(IDBConstants.DODET_CUST_NAME, "");// add customer
				// name
				htdodet.put(IDBConstants.DODET_QTYOR, "1");
				...
				htdodet.put(IDBConstants.TRAN_DATE, _dateutils
		...
		try {
			...
			List cutomerdetlst = _custmst
					.selectCustMst(
							...
							ht, " CUSTNO='"+map
							.get(IDBConstants.USER_ID)+"'"+" OR "+ "HPNO='"+map
							.get(IDBConstants.USER_ID)+ "'");
			_doutil.setmLogger(mLogger);
			_DOTransferUtil.setmLogger(mLogger);
			...
			ht.put(IDBConstants.ORDERTYPE,  map.get(IDBConstants.ORDERTYPE));
			...
			ht.put("Remark1",(String) map.get(IDBConstants.REMARKS) );
			...
			ht.put(IDBConstants.CURRENCYID, "");
	...
	public boolean processMovHis(Map map) throws Exception {
		...
		try {
			...
			htRecvHis.put("DIRTYPE", TransactionConstants.CREATE_REGISTRATION);
			htRecvHis.put(IDBConstants.ITEM, map.get(IConstants.PRODUCTID));
			htRecvHis.put("MOVTID", "");
			htRecvHis.put("RECID", "");
			htRecvHis.put(IDBConstants.LOC, "");
			htRecvHis.put("ORDNUM", (String) map.get(IDBConstants.DODET_DONUM));
			htRecvHis.put("BATNO", "");
			htRecvHis.put("QTY", "1");
			htRecvHis.put(IDBConstants.CREATED_BY, map.get(IConstants.LOGIN_USER));
			htRecvHis.put(IDBConstants.REMARKS, "");

			htRecvHis.put(IDBConstants.TRAN_DATE, _dateutils.getDateinyyyy_mm_dd(_dateutils.getDate()));
