...
public class WmsPickingByPDA implements WmsTran, IMLogger {
		...
		try {
			...
			if (flag ==true && insertFlag==false) {
				...
				if (flag)
					...
					queryPoHdr = "set PickStaus='C' , STATUS='O'"; //, STATUS='C'
				...
				String nonstocktype= StrUtils.fString((String) map.get("NONSTKFLAG"));
			        if(!nonstocktype.equals("Y"))	
				    {
						...
						htInvMstExpDate.put(IDBConstants.INVID, map.get(IConstants.INVID));
				...
				ArrayList shalStock = _InvMstDAO.selectInvMst("ID, CRAT, QTY", htInvMst, extCond);
				if (!alStock.isEmpty()) {
				...
				//process bom
					...
					if(flag)
					{
						...
						StringBuffer sql1 = new StringBuffer(" SET ");
						sql1.append(IDBConstants.PARENT_LOC + " ='"	+ map.get(IConstants.LOC2) + "'");
						sql1.append(" , "+IDBConstants.CHILD_LOC+"='"+ map.get(IConstants.LOC2) + "'");
			    ...
			    //process bom end
				 ...
				 //MovHis_IN
					...
					empno=(String)map.get(IConstants.EMPNO);
					if(empno.length()>0) {
						htRecvHis.put(IDBConstants.REMARKS, su.InsertQuotes((String)map.get(IConstants.OUT_REMARK1))+","+empno);
		          	}
		            else
	...
	private boolean processInvRemove(Map map) throws Exception {
		...
		try {
			...
			if (flag) {
				...
				String extCond = "AND QTY >='" + map.get(IConstants.QTY) + "' ";
