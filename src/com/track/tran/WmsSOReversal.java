...
public class WmsSOReversal implements WmsTran, IMLogger{
	...
	public boolean processDodetForReversal(Map map) throws Exception {
		...
		try {
			...
			if (flag) {
				...
				if(!map.get("DONO").toString().equalsIgnoreCase("")) {	
					String updateDoDet = "";
			...
			if(!map.get("DONO").toString().equalsIgnoreCase("")) {	
				...
				if (flag)
					updateDoHdr = "set  STATUS='O',PickStaus='O',ORDER_STATUS='PARTIALLY PROCESSED' ";
				else
					updateDoHdr = "set STATUS='N',PickStaus='N',ORDER_STATUS='Open' ";
	        	...
	        	if (ALIssueddetails.size() > 0) {
					for (int i = 0; i < ALIssueddetails.size(); i++) {
						...
						if(reverseqty == 0) break;
				          ...
				          String updateissuedet = "";
				          if(reverseqty >= issueqty)
				          updateissuedet = "set REVERSEQTY = ISNULL(REVERSEQTY,0) +" + issueqty +",UPAT='" +dateUtils.getDateTime()+"',UPBY='"+map.get(IConstants.LOGIN_USER)+"'";
				          else
				          {
				        	  updateissuedet = "set REVERSEQTY = ISNULL(REVERSEQTY,0) +" + reverseqty +",UPAT='" +dateUtils.getDateTime()+"',UPBY='"+map.get(IConstants.LOGIN_USER)+"'";
				        	  issueqty=reverseqty;
				          }
				          htCondiShipHis.put("CRAT", crat);
				          if(!ID.equalsIgnoreCase("0"))
			...
			if (flag) {
				...
				htRoRet.put("DONO", map.get("DONO"));
				htRoRet.put("LNNO", map.get("DOLNNO"));
				htRoRet.put("INVOICE", map.get("INVOICE"));
				htRoRet.put("GINO", map.get("GINO"));
				htRoRet.put("ITEM", map.get("ITEM"));
				htRoRet.put("CUSTNO", map.get("CUSTNO"));
				...
				htRoRet.put("SORETURN", (String) map.get("SORETURN"));
				
				flag = returnOrderDao.insertSOReturnDetails(htRoRet, (String)map.get("PLANT"));
			}
...
private boolean processInvAdd(Map map) throws Exception {
		...
		try {
			...
			//sql1.append(IDBConstants.QTY + " = QTY +'" + map.get(IConstants.QTY) + "'");
			 ...
			 {
				 double inqty = Double.valueOf(reverseqty) * Double.valueOf((String)map.get("UOMQTY"));
				 sql1.append(IDBConstants.QTY + " = QTY +'" + inqty + "'");
				 issueqty=reverseqty;
			 }
...
public boolean processMovHis_IN(Map map) throws Exception {
	...
	try {
		...
		if((String)map.get(IConstants.UNITMO)!=null)
			htMovHis.put(IDBConstants.UOM, map.get(IConstants.UNITMO));
