...
public class WmsTOPickReceive implements WmsTran, IMLogger {
	private boolean printLog = MLoggerConstant.WmsTOPickReceive_PRINTPLANTMASTERLOG;
	private boolean printInfo = MLoggerConstant.WmsTOPickReceive_PRINTPLANTMASTERINFO;
	...
	*/
	public boolean processTodetForPick(Map map) throws Exception {
		...
		try {
			...
			if (flag) {
				...
				htInvMst.put(IDBConstants.LOC, map.get(IConstants.FROMLOC));
				...
				if (!alStock.isEmpty()) {
					...
					while(quantityToAdjust > 0.0 && iterStock.hasNext()) {
						...
						htPickDet.put(IDBConstants.LOC, map.get(IConstants.FROMLOC));
						...
						htPickDet.put("GINO",  map.get(IConstants.GRNO));
	...
	public boolean processTodetForReceive(Map map) throws Exception {
		...
		try {
			...
			StringBuffer query = new StringBuffer("");
			htCondiPoDet.put("PLANT", map.get("PLANT"));
			htCondiPoDet.put("tono", map.get(IConstants.TODET_TONUM));
			htCondiPoDet.put("tolnno", map.get(IConstants.TODET_TOLNNO));
			query.append("isnull(QtyOr,0) as QtyOr");
			query.append(",isnull(qtyPick,0) as qtyPick");
			...
			// Added By Samatha extracond to Controll the Receive Qty Excced the PickQty Aug 24 1010
			...
			if (flag) {
				...
				if (flag)
					queryPoHdr = "set STATUS='O',ORDER_STATUS='PARTIALLY PROCESSED' ";
				else
					queryPoHdr = "set STATUS='C',ORDER_STATUS='PROCESSED' ";
		    ...
		    if (flag) {
		            ...
		            htCondiToPick.put("tono", map.get(IConstants.TODET_TONUM));
		            htCondiToPick.put("tolno", map.get(IConstants.TODET_TOLNNO));
		            htCondiToPick.put("batch", map.get(IConstants.BATCH));
		            ...
		            if(type!= null && type.equalsIgnoreCase("BULK")){
		            	...
		            	double recievingQty =Double.parseDouble((String)map.get(IConstants.QTY));
		            	double totalQty = rcQty + recievingQty;
		            	sqlQry.append("STATUS" + " =  CASE WHEN " +totalQty +"< "+ pickQty +" THEN '' ELSE 'C' END " );
		            }else{
		            sqlQry.append("STATUS" + " =  CASE WHEN " + map.get(IConstants.QTY) + "< PICKQTY THEN '' ELSE 'C' END " );
		            }
			...
			htInvMst.put(IDBConstants.CREATED_AT, (map.get(IConstants.ISSUEDATE) != null ? map.get(IConstants.ISSUEDATE).toString().replaceAll("-", "").replaceAll("/", "") : map.get(IConstants.TRAN_DATE).toString().replaceAll("-", "").replaceAll("/", "")) + "000000");
			String extCond = "";
			//if (map.get(IConstants.BATCH_ID) != null && !"".equals(StrUtils.fString(map.get(IConstants.BATCH_ID).toString())) && !"-1".equals(map.get(IConstants.BATCH_ID).toString())) {
				//htInvMst.put(IDBConstants.INVID, map.get(IConstants.BATCH_ID));
			...
			htRecvDet.put(IDBConstants.RECVDATE,  map.get(IConstants.ISSUEDATE));
			htRecvDet.put("UNITCOST",  map.get("UNITPRICE"));			
			 //Start added by Bruhan for Bulk receiving on 3 Aug 2012.
			if (type!=null && type.equalsIgnoreCase("BULK")) {
				//double qtypick = Double.parseDouble((String)map.get(IDBConstants.PICKQTY));	
				//double recvdQty = Double.parseDouble((String)map.get(IConstants.TR_RECVQTY ));		           
...
//				Get details in ascending order of CRAT for that batch
				...
				if (!alStock.isEmpty()) {
					...
					while(quantityToAdjust > 0.0 && iterStock.hasNext()) {
						...
						double currRecordQuantity = Double.parseDouble("" + mapIterStock.get(IConstants.QTY));
						double adjustedQuantity = quantityToAdjust > currRecordQuantity ? currRecordQuantity : quantityToAdjust;
						double inqty = adjustedQuantity * Double.valueOf((String)map.get("UOMQTY"));
						...
						htInvMstReduce.put(IDBConstants.LOC, map.get(IConstants.FROMLOC));
...
//			htInvMst.put(IDBConstants.LOC, map.get(IConstants.FROMLOC));
			...
			//Code removed due to BATCH_ID used for from location
			/*if (map.get(IConstants.BATCH_ID) != null && !"".equals(StrUtils.fString(map.get(IConstants.BATCH_ID).toString())) && !"-1".equals(map.get(IConstants.BATCH_ID).toString())) {
				htInvMst.put(IDBConstants.INVID, map.get(IConstants.BATCH_ID));
			}*/
...
//				htInvMst.put(IDBConstants.LOC, map.get(IConstants.LOC));
//				htInvMst.put(IDBConstants.USERFLD4, map.get(IConstants.BATCH));
//				htInvMst.put(IDBConstants.CREATED_AT, mapIterStock.get(IDBConstants.CREATED_AT));
//			}
//			else
//			{
//			htInvMst.put(IDBConstants.LOC, map.get(IConstants.LOC));
...
//	}
	...
	private boolean processBOM(Map map) throws Exception {
		...
		{*/
			...
			htBomMst.put("PARENT_PRODUCT_LOC", map.get(IConstants.FROMLOC));
			...
			if(flag)
			{
				...
				htBomUpdateMst.put("PARENT_PRODUCT_LOC", map.get(IConstants.FROMLOC));
				...
				{   
					...
					htInvParentBom.put("USERFLD4", map.get(IConstants.BATCH));
						...
						if (!invMstDAO.isExisitBomQty(htInvParentBom,"")) {
							...
							htInsertInvParentBom.put("QTY",  map.get(IConstants.QTY));
	...
	*/
	public boolean processMovHis_OUT(Map map) throws Exception {
		...
		try {
			...
			htRecvHis.put(IDBConstants.REMARKS , map.get(IConstants.REMARKS)+","+map.get(IConstants.GRNO));
	...
	*/
	public boolean processMovHis_IN(Map map) throws Exception {
		...
		try {
			...
			htRecvHis.clear();
			htRecvHis.put(IDBConstants.PLANT, map.get(IConstants.PLANT));
			htRecvHis.put("DIRTYPE", TransactionConstants.TO_RECV);
			htRecvHis.put(IDBConstants.ITEM, map.get(IConstants.ITEM));
			htRecvHis.put("BATNO", map.get(IConstants.BATCH));
			htRecvHis.put(IDBConstants.QTY, map.get(IConstants.QTY));
			htRecvHis.put(IDBConstants.CUSTOMER_CODE, map.get(IConstants.CUSTOMER_CODE));
